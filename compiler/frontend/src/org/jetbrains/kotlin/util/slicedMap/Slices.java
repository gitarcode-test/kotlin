...
public class Slices {
  ...
  // Rewrite is allowed for equal objects and for signed constant values that were converted to
  // unsigned ones
  // This is needed to avoid making `CompileTimeConstant` mutable
  public static final RewritePolicy COMPILE_TIME_VALUE_REWRITE_POLICY =
      new RewritePolicy() {
        @Override
        public <K> boolean rewriteProcessingNeeded(K key) {
          return true;
        ...
        public <K, V> boolean processRewrite(
            WritableSlice<K, V> slice, K key, V oldValue, V newValue) {
          if ((oldValue == null && newValue == null)
              || (oldValue != null && oldValue.equals(newValue))) return true;

          if (oldValue instanceof IntegerValueTypeConstant
              && newValue instanceof IntegerValueTypeConstant) {
            ...
            if (oldConstant.getParameters().isPure()
                && newConstant.getParameters().isUnsignedNumberLiteral()) {
              ...
              if (oldConstantValue == newConstantValue.longValue()
                  || oldConstantValue
                      == ConstantValueFactoryKt.fromUIntToLong(newConstantValue.intValue())
                  || oldConstantValue
                      == ConstantValueFactoryKt.fromUByteToLong(newConstantValue.byteValue())
                  || oldConstantValue
                      == ConstantValueFactoryKt.fromUShortToLong(newConstantValue.shortValue())) {
                return true;
              }
            }
          }
  ...
  public static class SliceBuilder<K, V> {
    ...
    @SafeVarargs
    public final SliceBuilder<K, V> setFurtherLookupSlices(
        ReadOnlySlice<K, V>... furtherLookupSlices) {
  ...
  private static <K, V> void logErrorAboutRewritingNonEqualObjects(
      WritableSlice<K, V> slice, K key, V oldValue, V newValue) {
    ...
    LOG.error(
        "Rewrite at slice "
            ...
            + oldValue
            + '@'
            + System.identityHashCode(oldValue)
            + " new value: "
            + newValue
            + '@'
            + System.identityHashCode(newValue)
