...
public class FunctionCodegen {
  ...
  public void gen(@NotNull KtNamedFunction function) {
    ...
    if (bindingContext.get(CodegenBinding.SUSPEND_FUNCTION_TO_JVM_VIEW, functionDescriptor)
        != null) {
      functionDescriptor =
          (SimpleFunctionDescriptor)
              bindingContext.get(CodegenBinding.SUSPEND_FUNCTION_TO_JVM_VIEW, functionDescriptor);
    }

    if (functionDescriptor == null) {
      throw ExceptionLogger.logDescriptorNotFound(
          "No descriptor for function " + function.getName(), function);
    }
    ...
    generateDefaultIfNeeded(
        owner.intoFunction(functionDescriptor, true),
        functionDescriptor,
  ...
  public void generateMethod(
    ...
    OwnerKind contextKind =
        TypeMapperUtilsKt.isInlineClassConstructorAccessor(functionDescriptor)
            ? OwnerKind.ERASED_INLINE_CLASS
            : methodContext.getContextKind();
    DeclarationDescriptor containingDeclaration = functionDescriptor.getContainingDeclaration();
    if (isInterface(containingDeclaration)
        && !processInterfaceMethod(
            functionDescriptor, contextKind, false, false, state.getJvmDefaultMode())) {
      return;
    }

    if (!shouldGenerateMethodInsideInlineClass(
        origin, functionDescriptor, contextKind, containingDeclaration)) {
      return;
    }

    boolean hasSpecialBridge = hasSpecialBridgeMethod(functionDescriptor);
    JvmMethodGenericSignature jvmSignature =
        strategy.mapMethodSignature(functionDescriptor, typeMapper, contextKind, hasSpecialBridge);
    ...
    boolean isCompatibilityStubInDefaultImpls =
        isCompatibilityStubInDefaultImpls(
            functionDescriptor, methodContext, state.getJvmDefaultMode());
    ...
    GenerateJava8ParameterNamesKt.generateParameterNames(
        functionDescriptor, mv, jvmSignature, state, (flags & ACC_SYNTHETIC) != 0);
    ...
    boolean staticInCompanionObject =
        CodegenUtilKt.isJvmStaticInCompanionObject(functionDescriptor);
    ...
    } else if (canDelegateMethodBodyToInlineClass(
        origin, functionDescriptor, contextKind, containingDeclaration)) {
      generateMethodInsideInlineClassWrapper(
          origin, functionDescriptor, (ClassDescriptor) containingDeclaration, mv, typeMapper);
    } else {
      generateMethodBody(
          origin,
          functionDescriptor,
  ...
  private void recordMethodForFunctionIfAppropriate(
      @NotNull FunctionDescriptor functionDescriptor, Method asmMethod) {
    if (functionDescriptor instanceof AccessorForConstructorDescriptor) {
      ConstructorDescriptor originalConstructor =
          ((AccessorForConstructorDescriptor) functionDescriptor).getCalleeDescriptor();
  ...
  private static boolean shouldGenerateMethodInsideInlineClass(
    ...
    return !canDelegateMethodBodyToInlineClass(
            origin, functionDescriptor, contextKind, containingDeclaration)
  ...
  public static void generateMethodInsideInlineClassWrapper(
    ...
    Method erasedMethodImpl =
        typeMapper.mapAsmMethod(functionDescriptor.getOriginal(), OwnerKind.ERASED_INLINE_CLASS);
  ...
  private void generateOpenMethodInSuspendClass(
    ...
    Method asmMethodForOpenSuspendImpl =
        CoroutineCodegenUtilKt.getImplForOpenMethod(asmMethod, v.getThisName());
    ...
    JvmMethodSignature jvmSignatureForOpenSuspendImpl =
        new JvmMethodGenericSignature(
            asmMethodForOpenSuspendImpl, jvmSignature.getValueParameters(), null);
    ...
    MethodVisitor mvForOpenSuspendImpl =
        strategy.wrapMethodVisitor(
            v.newMethod(
                origin,
    ...
    generateMethodBody(
        ...
        methodContext,
        strategy,
        mvForOpenSuspendImpl,
        jvmSignatureForOpenSuspendImpl,
        staticInCompanionObject);
  }
  ...
  private void generateMethodBody(
    ...
    if (!functionDescriptor.isExternal()) {
      ...
      FunctionDescriptor staticFunctionDescriptor =
          JvmStaticInCompanionObjectGenerator.createStaticFunctionDescriptor(functionDescriptor);
      ...
      generateDelegateToStaticMethodBody(
          false, mv, accessorMethod, owningType.getInternalName(), false);
    }
  ...
  public static void generateParameterAnnotations(
    ...
    int syntheticParameterCount =
        CollectionsKt.count(
            kotlinParameterTypes, signature -> signature.getKind().isSkippedInGenericSignature());

    Asm7UtilKt.visitAnnotableParameterCount(
        mv, kotlinParameterTypes.size() - syntheticParameterCount);
    ...
    for (int i = 0; i < kotlinParameterTypes.size(); i++) {
      ...
      if (annotated != null) {
        ...
        AnnotationCodegen.forParameter(
                parameterIndex, mv, memberCodegen, state, skipNullabilityAnnotations)
            .genAnnotations(
                annotated,
                parameterSignature.getAsmType(),
                annotated.getReturnType(),
                functionDescriptor,
                Collections.emptyList());
      }
    }
  ...
  private static Type getThisTypeForFunction(
    ...
    // all functions inside erased version of inline class are static, so they don't have `this` as
    // is,
  ...
  public static void generateMethodBody(
    ...
    if (context.getParentContext() instanceof MultifileClassFacadeContext) {
      ...
      FunctionDescriptor compatibility =
          ((DefaultImplsClassContext) context.getParentContext())
              .getInterfaceContext()
              .getAccessorForJvmDefaultCompatibility(functionDescriptor);
      int flags =
          DescriptorAsmUtil.getMethodAsmFlags(
              functionDescriptor, OwnerKind.DEFAULT_IMPLS, context.getState());
      assert (flags & Opcodes.ACC_ABSTRACT) == 0
          : "Interface method with body should be non-abstract" + functionDescriptor;
      ...
      // Manually add `continuation` parameter to frame map, thus it will not overlap with fake
      // indices
      if (functionDescriptor.isSuspend()
          && CoroutineCodegenUtilKt.isSuspendFunctionNotSuspensionView(functionDescriptor)) {
        FunctionDescriptor view =
            CoroutineCodegenUtilKt.getOrCreateJvmSuspendFunctionView(
                functionDescriptor, parentCodegen.state);
        ValueParameterDescriptor continuationValueDescriptor =
            view.getValueParameters().get(view.getValueParameters().size() - 1);
        frameMap.enter(
            continuationValueDescriptor, typeMapper.mapType(continuationValueDescriptor));
      }
    ...
    if (functionDescriptor instanceof AnonymousFunctionDescriptor
        && functionDescriptor.isSuspend()) {
      functionDescriptor =
          CoroutineCodegenUtilKt.getOrCreateJvmSuspendFunctionView(
              functionDescriptor, parentCodegen.state);
    ...
    if (context.isInlineMethodContext() && functionFakeIndex != -1) {
      ...
      mv.visitLocalVariable(
          JvmAbi.LOCAL_VARIABLE_NAME_PREFIX_INLINE_FUNCTION
              + typeMapper.mapAsmMethod(functionDescriptor).getName(),
          Type.INT_TYPE.getDescriptor(),
          null,
          methodEntry,
          methodEnd,
          functionFakeIndex);
    } else if (context instanceof InlineLambdaContext
        && thisType != null
        && functionFakeIndex != -1) {
      ...
      if (functionArgument instanceof KtFunction) {
        ValueParameterDescriptor inlineArgumentDescriptor =
            InlineUtil.getInlineArgumentDescriptor(
                (KtFunction) functionArgument, parentCodegen.bindingContext);
  ...
  private static boolean isLambdaPassedToInlineOnly(
      KtFunction lambda, BindingContext bindingContext) {
    ValueParameterDescriptor parameterDescriptor =
        InlineUtil.getInlineArgumentDescriptor(lambda, bindingContext);
  ...
  private static boolean isCompatibilityStubInDefaultImpls(
    ...
    return OwnerKind.DEFAULT_IMPLS == context.getContextKind()
        ...
        && JvmAnnotationUtilKt.checkIsImplementationCompiledToJvmDefault(
            functionDescriptor, jvmDefaultMode);
  }

  private static boolean isCompiledInCompatibilityMode(
      JvmDefaultMode mode, CallableMemberDescriptor descriptor) {
    return mode == JvmDefaultMode.ALL_COMPATIBILITY
        || (mode == JvmDefaultMode.ALL
            && JvmAnnotationUtilKt.hasJvmDefaultWithCompatibilityAnnotation(
  ...
  private static void generateLocalVariableTable(
    ...
    if (functionDescriptor.isSuspend()
        && !(functionDescriptor instanceof AnonymousFunctionDescriptor)) {
      ...
      if (unwrapped != functionDescriptor) {
        generateLocalVariableTable(
            ...
            new JvmMethodSignature(
                ...
                jvmMethodSignature
                    .getValueParameters()
                    .subList(0, jvmMethodSignature.getValueParameters().size() - 1)),
            unwrapped,
            thisType,
            methodBegin,
            methodEnd,
    ...
    generateLocalVariablesForParameters(
        ...
        methodEnd,
        functionDescriptor.getValueParameters(),
        DescriptorAsmUtil.isStaticMethod(ownerKind, functionDescriptor),
        state);
  }
    ...
    for (int i = 0; i < params.size(); i++) {
      ...
      switch (kind) {
        case VALUE:
          ...
          String nameForDestructuredParameter =
              VariableAsmNameManglingUtils.getNameForDestructuredParameterOrNull(parameter);
        ...
        case OUTER:
          parameterName =
              CAPTURED_THIS_FIELD; // should be 'this$X' depending on inner class nesting
        ...
        default:
          ...
          parameterName =
              needIndexForVar(kind) ? "$" + lowercaseKind + "$" + i : "$" + lowercaseKind;
          break;
      }
  ...
  private static String computeParameterName(int i, ValueParameterDescriptor parameter) {
    ...
    if (element instanceof KtParameter
        && UnderscoreUtilKt.isSingleUnderscore((KtParameter) element)) {
      return "$noName_" + i;
    }
  ...
  public static void generateFacadeDelegateMethodBody(
    ...
    generateDelegateToStaticMethodBody(
        true, mv, asmMethod, context.getFilePartType().getInternalName(), false);
  }
  ...
  private static void generateDelegateToStaticErasedVersion(
    ...
    iv.invokestatic(
        internalName,
        erasedStaticAsmMethod.getName(),
        erasedStaticAsmMethod.getDescriptor(),
        false);
  ...
  public static void endVisit(
      MethodVisitor mv, @Nullable String description, @Nullable KtPureElement method) {
    endVisit(mv, description, (PsiElement) (method == null ? null : method.getPsiOrParent()));
  }

  public static void endVisit(
      MethodVisitor mv, @Nullable String description, @NotNull KtElement method) {
  ...
  private boolean hasSpecialBridgeMethod(@NotNull FunctionDescriptor descriptor) {
    if (SpecialBuiltinMembers.getOverriddenBuiltinReflectingJvmDescriptor(descriptor) == null)
      return false;
  ...
  public void generateBridges(@NotNull FunctionDescriptor descriptor) {
    ...
    if (JvmBridgesImplKt.isAbstractOnJvmIgnoringActualModality(
        descriptor, state.getJvmDefaultMode())) return;
    ...
    if (!isSpecial) {
      bridgesToGenerate =
          JvmBridgesImplKt.generateBridgesForFunctionDescriptorForJvm(
              descriptor, typeMapper::mapAsmMethod, state);
      ...
      if (!descriptor.getKind().isReal()
          && isAbstractMethod(descriptor, OwnerKind.IMPLEMENTATION, state.getJvmDefaultMode())) {
        ...
        if (!isThereOverriddenInKotlinClass(descriptor)) {
          ...
          v.newMethod(
              JvmDeclarationOriginKt.AugmentedBuiltInApi(overridden),
              flags,
              method.getName(),
  ...
  private KotlinType getBridgeReturnType(
      Bridge<Method, DescriptorBasedFunctionHandleForJvm> bridge) {
    ...
    if (state.getClassBuilderMode().mightBeIncorrectCode) {
      ...
      throw new AssertionError(
          "No overridden functions for the bridge method '" + bridge.getTo() + "'");
    } else {
      throw new AssertionError(
          "No return type for the bridge method '"
              + bridge.getTo()
              + "' found in the following overridden functions:\n"
  ...
  public static boolean isThereOverriddenInKotlinClass(
      @NotNull CallableMemberDescriptor descriptor) {
    return CollectionsKt.any(
        getAllOverriddenDescriptors(descriptor),
        overridden ->
            !(overridden.getContainingDeclaration() instanceof JavaClassDescriptor)
                && isClass(overridden.getContainingDeclaration()));
  }

  @NotNull
  public static String[] getThrownExceptions(
      @NotNull FunctionDescriptor function, @NotNull KotlinTypeMapper typeMapper) {
  ...
  public static List<ClassDescriptor> getThrownExceptions(
    ...
    if (function.getKind() == DELEGATION
        && languageVersionSettings.supportsFeature(
            LanguageFeature.DoNotGenerateThrowsForDelegatedKotlinMembers)) {
      return Collections.emptyList();
    }

    AnnotationDescriptor annotation =
        function.getAnnotations().findAnnotation(ThrowUtilKt.JVM_THROWS_ANNOTATION_FQ_NAME);
  ...
  void generateDefaultIfNeeded(
    ...
    if (isInterface(contextClass)
        && !processInterfaceMethod(
            functionDescriptor, kind, true, false, state.getJvmDefaultMode())) {
      return;
    }
    ...
    int visibilityFlag =
        (!isInterface(functionDescriptor.getContainingDeclaration())
                ...
                && (DescriptorVisibilities.isPrivate(functionDescriptor.getVisibility())
                    || isInlineOnlyPrivateInBytecode(functionDescriptor))
    ...
    if (!state.getClassBuilderMode().generateBodies) {
      ...
      else endVisit(mv, "default method", getSourceFromDescriptor(functionDescriptor));
      return;
    }
    ...
    if (this.owner instanceof MultifileClassFacadeContext) {
      ...
      endVisit(mv, "default method delegation", getSourceFromDescriptor(functionDescriptor));
    } else if (isCompatibilityStubInDefaultImpls(
        functionDescriptor, owner, state.getJvmDefaultMode())) {
      mv.visitCode();
      Method interfaceDefaultMethod =
          typeMapper.mapDefaultMethod(functionDescriptor, OwnerKind.IMPLEMENTATION);
      generateDelegateToStaticMethodBody(
          true,
          mv,
      ...
      endVisit(
          mv,
          "default method delegation to interface one",
          getSourceFromDescriptor(functionDescriptor));
    } else {
      mv.visitCode();
  ...
  public static void generateDefaultImplBody(
    ...
    JvmMethodSignature signature =
        state
            .getTypeMapper()
            .mapSignatureWithGeneric(functionDescriptor, methodContext.getContextKind());

    // 'null' because the "could not find expected declaration" error has been already reported in
    // isDefaultNeeded earlier
    ...
    assert valueParameters.size() > 0
        : "Expecting value parameters to generate default function " + functionDescriptor;
    ...
    for (int index = 0; index < valueParameters.size(); index++) {
      ...
      if (parameterDescriptor.declaresDefaultValue()) {
        ...
        codegen.runWithShouldMarkLineNumbers(
            !(containingDeclaration instanceof MemberDescriptor)
                || !((MemberDescriptor) containingDeclaration).isExpect(),
            () -> {
    ...
    for (int index = 0; index < valueParameters.size(); index++) {
      ...
      Type type = mappedParameters.get(capturedArgumentsCount + index).getAsmType();
      int parameterIndex = frameMap.getIndex(parameterDescriptor);
      generator.putValueIfNeeded(
          new JvmKotlinType(type, null), StackValue.local(parameterIndex, type));
    }

    CallableMethod method =
        state
            .getTypeMapper()
            .mapToCallableMethod(functionDescriptor, false, methodContext.getContextKind());
  ...
  private static void genDefaultSuperCallCheckIfNeeded(
    ...
    Label end = new Label();
    int handleIndex =
        InlineCodegenUtilsKt.argumentsSize(defaultMethod.getDescriptor(), true)
            - 1; // last argument
  ...
  private static FrameMap createFrameMap(
    ...
    for (JvmMethodParameterSignature parameter : signature.getValueParameters()) {
      if (parameter.getKind() == JvmMethodParameterKind.RECEIVER
          || parameter.getKind() == JvmMethodParameterKind.CONTEXT_RECEIVER) {
        if (extensionReceiverParameter != null) {
          frameMap.enter(
              extensionReceiverParameter,
              state.getTypeMapper().mapType(extensionReceiverParameter));
        } else {
  ...
  private static void loadExplicitArgumentsOnStack(
    ...
    if (!isStatic) {
      callGenerator.putValueIfNeeded(
          new JvmKotlinType(ownerType, null), StackValue.local(var, ownerType));
  ...
  private void generateBridge(
    ...
    String bridgeSignature =
        isSpecialBridge
            ? typeMapper
                .mapSignatureWithGeneric(descriptor, OwnerKind.IMPLEMENTATION)
    ...
    if (descriptor.getExtensionReceiverParameter() != null)
      allKotlinParameters.add(descriptor.getExtensionReceiverParameter());
    ...
    if (delegateTo.getArgumentTypes().length > 0 && isSpecialBridge) {
      generateTypeCheckBarrierIfNeeded(
          iv, descriptor, bridge.getReturnType(), delegateTo.getArgumentTypes(), typeMapper);
    }
      ...
      if (isVarargInvoke) {
        value =
            StackValue.arrayElement(
                OBJECT_TYPE, null, StackValue.local(1, argTypes[0]), StackValue.constant(i));
      } else {
    ...
    StackValue.coerce(
        delegateTo.getReturnType(), returnValueType, bridge.getReturnType(), bridgeReturnType, iv);
  ...
  private static void generateTypeCheckBarrierIfNeeded(
    ...
    assert overriddenBuiltin != null
        : "Overridden built-in method should not be null for " + descriptor;
    ...
    for (int i = 0; i < descriptor.getValueParameters().size(); i++) {
      ...
      boolean isCheckForAny =
          delegateParameterTypes == null || OBJECT_TYPE.equals(delegateParameterTypes[i]);
    ...
    if (typeSafeBarrierDescription.equals(
  ...
  public void genSamDelegate(
    ...
    // Skip writing generic signature for the SAM wrapper class method because it may reference type
    // parameters from the SAM interface
    // which would make little sense outside of that interface and may break Java reflection.
    // E.g. functionDescriptor for a SAM wrapper for java.util.function.Predicate is the method
    // `test` with signature "(T) -> Boolean"
    genDelegate(
        functionDescriptor,
        delegatedTo,
        declarationOrigin,
        ...
        new FunctionGenerationStrategy() {
          ...
          public void generateBody(
            ...
            if (toClass.getKind() == ClassKind.INTERFACE) {
              ...
              iv.invokestatic(
                  internalName,
                  delegateToMethod.getName(),
                  delegateToMethod.getDescriptor(),
                  false);
            } else {
              iv.invokevirtual(
                  internalName,
            ...
            StackValue stackValue =
                DescriptorAsmUtil.genNotNullAssertions(
                    state,
                    StackValue.onStack(
                        delegateToMethod.getReturnType(), delegatedTo.getReturnType()),
          ...
          private List<KotlinType> getKotlinTypesForJvmParameters(
              @NotNull FunctionDescriptor functionDescriptor) {
  ...
  public static boolean processInterfaceMethod(
    ...
    assert isInterface(containingDeclaration)
        : "'processInterfaceMethod' method should be called only for interfaces, but: "
            + containingDeclaration;

    // Fake overrides in interfaces should be expanded to implementation to make proper default
    // check
      ...
      return (kind != OwnerKind.DEFAULT_IMPLS && !isSyntheticInCompatibilityOrJvmDefault)
          || (kind == OwnerKind.DEFAULT_IMPLS
              && (isSyntheticInCompatibilityOrJvmDefault
                  || (isCompatibilityMode
                      && !JvmAnnotationUtilKt.hasJvmDefaultNoCompatibilityAnnotation(
                          containingDeclaration)))
              && !DescriptorVisibilities.isPrivate(memberDescriptor.getVisibility()));
    } else {
      switch (kind) {
        case DEFAULT_IMPLS:
          return true;
        case IMPLEMENTATION:
          return !DescriptorVisibilities.isPrivate(memberDescriptor.getVisibility())
              && !isDefault
              && !isSynthetic;
        default:
          return false;
