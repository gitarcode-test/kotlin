  ...
  enum LocalLookupCase {
    VAR {
      ...
      public StackValue.StackValueWithSimpleReceiver innerValue(
        ...
        KotlinType delegateType =
            vd instanceof VariableDescriptorWithAccessors
                ? JvmCodegenUtil.getPropertyDelegateType(
                    (VariableDescriptorWithAccessors) vd, state.getBindingContext())
                : null;
        ...
        if (sharedVarType != null) {
          StackValue.Field wrapperValue =
              StackValue.receiverWithRefWrapper(localType, classType, fieldName, thiz, vd);
          innerValue =
              StackValue.fieldForSharedVar(localType, classType, fieldName, wrapperValue, vd);
          enclosedValueDescriptor =
              new EnclosedValueDescriptor(fieldName, d, innerValue, wrapperValue, type, kotlinType);
        } else {
          innerValue = StackValue.field(type, kotlinType, classType, fieldName, false, thiz, vd);
          enclosedValueDescriptor =
              new EnclosedValueDescriptor(fieldName, d, innerValue, type, kotlinType);
        }
      ...
      public StackValue.StackValueWithSimpleReceiver innerValue(
        ...
        if (localFunClosure != null && JvmCodegenUtil.isConst(localFunClosure)) {
          ...
          return StackValue.field(
              localType, null, localType, JvmAbi.INSTANCE_FIELD, true, StackValue.LOCAL_0, vd);
        }
        ...
        String localFunSuffix =
            localClassIndexStart >= 0 ? simpleName.substring(localClassIndexStart) : "";

        String fieldName = AsmUtil.getCapturedFieldName(vd.getName().asString()) + localFunSuffix;
        StackValue.StackValueWithSimpleReceiver innerValue =
            StackValue.field(localType, null, classType, fieldName, false, StackValue.LOCAL_0, vd);

        closure.captureVariable(
            new EnclosedValueDescriptor(fieldName, d, innerValue, localType, null));
    ...
    RECEIVER {
      ...
      public StackValue.StackValueWithSimpleReceiver innerValue(
          ...
          MutableClosure closure,
          Type classType) {
        CallableDescriptor enclosingCallableWithReceiver =
            closure.getEnclosingCallableDescriptorWithReceiver();
        ReceiverParameterDescriptor enclosingReceiverDescriptor =
            enclosingCallableWithReceiver != null
                ? enclosingCallableWithReceiver.getExtensionReceiverParameter()
                : null;
        ...
        String fieldName =
            closure.getCapturedReceiverFieldName(
                state.getBindingContext(), state.getLanguageVersionSettings());
        StackValue.StackValueWithSimpleReceiver innerValue =
            StackValue.field(
      ...
      public StackValue outerValue(
        ...
        return StackValue.local(
            descriptor.getDispatchReceiverParameter() != null ? 1 : 0, d.getType());
      }
    };
