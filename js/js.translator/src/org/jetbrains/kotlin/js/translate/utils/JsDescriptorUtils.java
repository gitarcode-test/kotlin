...
public final class JsDescriptorUtils {
  ...
  private static final Set<String> FAKE_CLASSES =
      Collections.singleton(StandardNames.FqNames.any.asString());
    ...
    return ContainerUtil.filter(
        ...
        type -> {
          ...
          return !FAKE_CLASSES.contains(getFqNameSafe(classDescriptor).asString())
              && !(classDescriptor.getKind() == ClassKind.INTERFACE
                  && isNativeObject(classDescriptor));
        });
  }
  ...
  private static DeclarationDescriptor getDeclarationDescriptorForReceiver(
    ...
    if (receiverParameter instanceof ImplicitReceiver) {
      DeclarationDescriptor declarationDescriptor =
          ((ImplicitReceiver) receiverParameter).getDeclarationDescriptor();
  ...
  public static ReceiverParameterDescriptor getReceiverParameterForDeclaration(
    ...
    if (declarationDescriptor instanceof ClassDescriptor) {
      ...
      ReceiverParameterDescriptor receiverDescriptor =
          ((CallableMemberDescriptor) declarationDescriptor).getExtensionReceiverParameter();
  ...
  private static boolean isDefaultAccessor(
    ...
    return accessorDescriptor == null
        || accessorDescriptor.isDefault()
            && !(accessorDescriptor instanceof PropertySetterDescriptor
                && accessorDescriptor.getCorrespondingProperty().isLateInit());
  }

  public static boolean sideEffectsPossibleOnRead(@NotNull PropertyDescriptor property) {
    return DynamicCallsKt.isDynamic(property)
        || !isDefaultAccessor(property.getGetter())
        || ModalityUtilsKt.isOverridableOrOverrides(property)
  ...
  public static String getModuleName(@NotNull DeclarationDescriptor descriptor) {
    return getModuleName(
        DescriptorUtils.getContainingModule(findRealInlineDeclaration(descriptor)));
  }
  ...
  @NotNull
  public static DeclarationDescriptor findRealInlineDeclaration(
      @NotNull DeclarationDescriptor descriptor) {
  ...
  @Nullable
  public static CallableMemberDescriptor findRealDeclaration(
  ...
  public static boolean isImmediateSubtypeOfError(@NotNull ClassDescriptor descriptor) {
    ...
    return TypeUtilsKt.isNotNullThrowable(superClass.getDefaultType())
        || AnnotationsUtils.isNativeObject(superClass);
  }

  public static boolean isExceptionClass(@NotNull ClassDescriptor descriptor) {
    ModuleDescriptor module = DescriptorUtils.getContainingModule(descriptor);
    return TypeUtilsKt.isSubtypeOf(
        descriptor.getDefaultType(), module.getBuiltIns().getThrowable().getDefaultType());
  }
}
