...
public class ExpressionTypingUtils {
  ...
  public static KotlinType safeGetType(@NotNull KotlinTypeInfo typeInfo) {
    ...
    assert type != null
        : "safeGetType should be invoked on safe KotlinTypeInfo; safeGetTypeInfo should return"
              + " @NotNull type";
    return type;
  }
  ...
  public static LexicalWritableScope newWritableScopeImpl(
    ...
    return new LexicalWritableScope(
        context.scope,
        context.scope.getOwnerDescriptor(),
        false,
        new TraceBasedLocalRedeclarationChecker(context.trace, overloadChecker),
        scopeKind);
  }
  ...
  public static void checkVariableShadowing(
    ...
    VariableDescriptor oldDescriptor =
        ScopeUtilsKt.findLocalVariable(scope, variableDescriptor.getName());
    ...
    if (variableDescriptor instanceof ParameterDescriptor) {
      ...
      // parameter of lambda
      if (variableContainingDeclaration.getContainingDeclaration()
          != oldDescriptor.getContainingDeclaration()) {
        return;
      }
    }
    ...
    if (declaration != null) {
      if (declaration instanceof KtDestructuringDeclarationEntry
          && declaration.getParent().getParent() instanceof KtParameter) {
        // foo { a, (a, b) -> } -- do not report NAME_SHADOWING on the second 'a', because
        // REDECLARATION must be reported here
        PsiElement oldElement = DescriptorToSourceUtils.descriptorToDeclaration(oldDescriptor);

        if (oldElement != null
            && oldElement.getParent().equals(declaration.getParent().getParent().getParent()))
          return;
      }
  ...
  public static ObservableBindingTrace makeTraceInterceptingTypeMismatch(
    ...
    return new ObservableBindingTrace(trace) {
      ...
      public void report(@NotNull Diagnostic diagnostic) {
        ...
        if (Errors.TYPE_MISMATCH_ERRORS.contains(factory)
  ...
  public static boolean isBinaryExpressionDependentOnExpectedType(
      @NotNull KtBinaryExpression expression) {
    IElementType operationType = expression.getOperationReference().getReferencedNameElementType();
    return (operationType == KtTokens.IDENTIFIER
        || OperatorConventions.BINARY_OPERATION_NAMES.containsKey(operationType)
        || operationType == KtTokens.ELVIS);
  }

  public static boolean isUnaryExpressionDependentOnExpectedType(
  ...
  public static boolean isExclExclExpression(@Nullable KtExpression expression) {
    return expression instanceof KtUnaryExpression
        && ((KtUnaryExpression) expression).getOperationReference().getReferencedNameElementType()
            == KtTokens.EXCLEXCL;
  }
   ...
   *
   * <p>I had a locally declared extension function or a local variable of function type called foo
   * And I called it on my x Now, someone added function foo() to the class of x My code should not
   * change
   *
   * <p>thus
   *
   * <p>local extension prevail over members (and members prevail over all non-local extensions)
   */
  public static boolean isLocal(
      DeclarationDescriptor containerOfTheCurrentLocality, DeclarationDescriptor candidate) {
  ...
  public static boolean isLocalFunction(@Nullable DeclarationDescriptor descriptor) {
    if (descriptor != null && descriptor.getClass() == SimpleFunctionDescriptorImpl.class) {
      return ((SimpleFunctionDescriptorImpl) descriptor).getVisibility()
