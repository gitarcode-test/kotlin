...
public class TranslationContext {
  @NotNull private final DynamicContext dynamicContext;
  @NotNull private final StaticContext staticContext;
  @NotNull private final AliasingContext aliasingContext;
  @Nullable private final UsageTracker usageTracker;
  @Nullable private final TranslationContext parent;
  @Nullable private final DeclarationDescriptor declarationDescriptor;
  @Nullable private final ClassDescriptor classDescriptor;
  @Nullable private final ValueParameterDescriptor continuationParameterDescriptor;

  @Nullable private InlineFunctionContext inlineFunctionContext;
  ...
  public static TranslationContext rootContext(@NotNull StaticContext staticContext) {
    DynamicContext rootDynamicContext =
        DynamicContext.rootContext(
            staticContext.getFragment().getScope(),
            staticContext.getFragment().getInitializerBlock());
    AliasingContext rootAliasingContext = AliasingContext.getCleanContext();
    return new TranslationContext(
        null, staticContext, rootDynamicContext, rootAliasingContext, null, null);
  }
  ...
  private TranslationContext newFunctionBody(
    ...
    return new TranslationContext(
        this, this.staticContext, dynamicContext, aliasingContext, this.usageTracker, descriptor);
  }

  @NotNull
  public TranslationContext newFunctionBodyWithUsageTracker(
      @NotNull JsFunction fun, @NotNull MemberDescriptor descriptor) {
    DynamicContext dynamicContext = DynamicContext.newContext(fun.getScope(), fun.getBody());
  ...
  public TranslationContext innerWithUsageTracker(@NotNull MemberDescriptor descriptor) {
    UsageTracker usageTracker = new UsageTracker(this.usageTracker, descriptor, bindingContext());
    return new TranslationContext(
        this, staticContext, dynamicContext, aliasingContext.inner(), usageTracker, descriptor);
  }
  ...
  public TranslationContext innerBlock(@NotNull JsBlock block) {
    return new TranslationContext(
        this,
        staticContext,
        dynamicContext.innerBlock(block),
        aliasingContext,
        usageTracker,
  ...
  public TranslationContext newDeclaration(@NotNull DeclarationDescriptor descriptor) {
    ...
    DynamicContext dynamicContext =
        DynamicContext.newContext(getScopeForDescriptor(descriptor), innerBlock);
    return new TranslationContext(
        this, staticContext, dynamicContext, aliasingContext, usageTracker, descriptor);
  }

  @NotNull
  private TranslationContext innerWithAliasingContext(AliasingContext aliasingContext) {
    return new TranslationContext(
        this, staticContext, dynamicContext, aliasingContext, usageTracker, declarationDescriptor);
  }

  @NotNull
  public TranslationContext innerContextWithAliased(
      @NotNull DeclarationDescriptor correspondingDescriptor, @NotNull JsExpression alias) {
  ...
  public TranslationContext innerContextWithAliasesForExpressions(
      @NotNull Map<KtExpression, JsExpression> aliases) {
  ...
  public TranslationContext innerContextWithDescriptorsAliased(
      @NotNull Map<DeclarationDescriptor, JsExpression> aliases) {
    if (aliases.isEmpty()) return this;
  ...
  public JsName getInlineableInnerNameForDescriptor(@NotNull DeclarationDescriptor descriptor) {
    ...
    if (inlineFunctionContext == null
        ...
        || !shouldBeExported(descriptor, getConfig())
  ...
  private JsName createInlineableInnerNameForDescriptor(@NotNull DeclarationDescriptor descriptor) {
    ...
    if (imported instanceof JsNameRef) {
      ...
      if (importedNameRef.getQualifier() == null
          && importedNameRef.getIdent().equals(Namer.getRootPackageName())
          && (descriptor instanceof PackageFragmentDescriptor
              || descriptor instanceof ModuleDescriptor)) {
    ...
    if (isFromCurrentModule(descriptor) && !AnnotationsUtils.isNativeObject(descriptor)) {
      MetadataProperties.setLocalAlias(
          result,
          new LocalAlias(getInnerNameForDescriptor(descriptor), staticContext.getTag(descriptor)));
    }
          ...
          inlineFunctionContext
              ...
              .computeIfAbsent(
                  ...
                  t -> {
                    ...
                    JsName name =
                        JsScope.declareTemporaryName(NameSuggestion.sanitizeName(intrinsicName));
                    MetadataProperties.setImported(name, true);
                    inlineFunctionContext
                        .getImportBlock()
                        .getStatements()
                        .add(JsAstUtils.newVar(name, imported));
                    return name;
                  });
    }
    ...
    if (expression instanceof JsNameRef) {
      ...
      if (nameRef.getQualifier() == null) {
        ...
        JsExpression newQualifier =
            replaceModuleReference(nameRef.getQualifier(), expectedModuleName, reexportExpr);
        if (newQualifier == nameRef.getQualifier()) {
          return expression;
        }
        JsExpression result =
            nameRef.getName() != null
  ...
  @NotNull
  private JsExpression createInlineLocalImportExpression(
    ...
    if (suggested != null
        && getConfig().getModuleKind() != ModuleKind.PLAIN
        && isPublicInlineFunction()) {
      String moduleId = AnnotationsUtils.getModuleName(suggested.getDescriptor());
      if (moduleId != null) {
        JsExpression replacement =
            staticContext.exportModuleForInline(
                moduleId, new JsImportedModule(moduleId, name, null));
        result = replaceModuleReference(result, name, replacement);
      } else if (isNativeObject(suggested.getDescriptor())
          && DescriptorUtils.isTopLevelDeclaration(suggested.getDescriptor())) {
        String fileModuleId =
            AnnotationsUtils.getFileModuleName(bindingContext(), suggested.getDescriptor());
        if (fileModuleId != null) {
          JsImportedModule fileModuleName = staticContext.getImportedModule(fileModuleId, null);
          JsExpression replacement =
              staticContext.exportModuleForInline(fileModuleId, fileModuleName);
          result =
              replaceModuleReference(
                  staticContext.getQualifiedReference(descriptor),
                  fileModuleName.getInternalName(),
                  replacement);
        }
      }
    }
  ...
  public TemporaryVariable declareTemporary(
      @Nullable JsExpression initExpression, @Nullable Object source) {
  ...
  public JsExpression defineTemporary(@NotNull JsExpression initExpression) {
    TemporaryVariable var =
        dynamicContext.declareTemporary(initExpression, initExpression.getSource());
  ...
  @NotNull
  public TemporaryConstVariable getOrDeclareTemporaryConstVariable(
      @NotNull JsExpression expression) {
  ...
  public JsExpression getDispatchReceiver(@NotNull ReceiverParameterDescriptor descriptor) {
    ...
    ClassifierDescriptor classifier =
        descriptor.getValue().getType().getConstructor().getDeclarationDescriptor();
  ...
  private boolean isConstructorOrDirectScope(DeclarationDescriptor descriptor) {
    return descriptor
        == DescriptorUtils.getParentOfType(declarationDescriptor, ClassDescriptor.class, false);
  }

  @NotNull
  private JsExpression getDispatchReceiverPath(
      @NotNull ClassDescriptor from, @NotNull ClassDescriptor to, @NotNull JsExpression qualifier) {
    while (true) {
      ...
      if (from == to
          || !from.isInner()
          || !(from.getContainingDeclaration() instanceof ClassDescriptor)) {
        break;
      }
  ...
  @Nullable
  public JsExpression captureTypeIfNeedAndGetCapturedName(
  ...
  private boolean shouldCaptureViaThis() {
    ...
    if (declarationDescriptor instanceof ConstructorDescriptor
        && DescriptorUtils.isDescriptorWithLocalVisibility(
            declarationDescriptor.getContainingDeclaration())) return false;
  ...
  public void putClassOrConstructorClosure(
      @NotNull MemberDescriptor descriptor, @NotNull List<DeclarationDescriptor> closure) {
   ...
   *
   * <p>``` fun a(x) { fun b(y) = x + y return b } ```
   *
   * <p>Here, `x` is a free variable of `b`. Transform `a` into the following form:
   *
   * <p>``` fun a(x) { fun b0(x0) = { y -> x0 * y } return b0(x) } ```
   *
   * <p>This function generates arguments passed to newly generated `b0` closure, as well as for the
   * similar case of local class and object expression.
   *
  ...
  @NotNull
  public JsExpression getTypeArgumentForClosureConstructor(
      @NotNull TypeParameterDescriptor descriptor) {
  ...
  public void startDeclaration() {
    ...
    if (classDescriptor != null
        && !(classDescriptor.getContainingDeclaration()
            instanceof ClassOrPackageFragmentDescriptor)) {
  ...
  private boolean isValWithWriterInDifferentScope(VariableDescriptor descriptor) {
    ...
    return (preliminaryVisitor == null
        || !hasNoWritersInClosures(
            ...
            preliminaryVisitor.writers(descriptor),
            bindingContext()));
  }
  ...
  public void deferConstructorCall(
      @NotNull ClassConstructorDescriptor constructor, @NotNull List<JsExpression> invocationArgs) {
    ClassDescriptor classDescriptor = constructor.getContainingDeclaration();
    ...
    if (callSites == null)
      throw new IllegalStateException(
          "This method should be call only when `shouldBeDeferred` method "
              + "reports true for given constructor: "
              + constructor);
  ...
  public void addClass(@NotNull ClassDescriptor classDescriptor) {
    if (inlineFunctionContext != null) {
      ...
      List<JsStatement> targetStatements =
          inlineFunctionContext.getPrototypeBlock().getStatements();
      ...
      return inlineFunctionContext
          ...
          .computeIfAbsent(
              ...
              t -> {
                ...
                MetadataProperties.setImported(result, true);
                if (descriptor != null
                    && isFromCurrentModule(descriptor)
                    && !AnnotationsUtils.isNativeObject(descriptor)) {
                  MetadataProperties.setLocalAlias(
                      result, new LocalAlias(getInnerNameForDescriptor(descriptor), tag));
                }
                inlineFunctionContext
                    .getImportBlock()
                    .getStatements()
                    .add(JsAstUtils.newVar(result, value));
                return result;
              })
          .makeRef();
    }
          ...
          .computeIfAbsent(
              ...
              t -> {
                ...
                inlineFunctionContext
                    .getImportBlock()
                    .getStatements()
                    .add(JsAstUtils.newVar(result, imported));
                return result;
              });
    }
  }
  ...
  public LanguageVersionSettings getLanguageVersionSettings() {
    return CommonConfigurationKeysKt.getLanguageVersionSettings(
        staticContext.getConfig().getConfiguration());
  }
