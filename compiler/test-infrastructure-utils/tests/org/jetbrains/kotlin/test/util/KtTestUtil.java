...
public class KtTestUtil {
  ...
  @NotNull
  public static File tmpDirForTest(@NotNull String testClassName, @NotNull String testName)
      throws IOException {
  ...
  public static File tmpDirForReusableFolder(String name) throws IOException {
    return normalizeFile(
        FileUtil.createTempDirectory(
            new File(System.getProperty("java.io.tmpdir")), name, "", true));
  }
  ...
  private static File normalizeFile(File file) throws IOException {
    ...
    // for example, on Windows, if a canonical path contains any space from
    // FileUtil.createTempDirectory we will get
    // a File with short names (8.3) in its path, and it will break some normalization passes in
    // tests.
  ...
  public static KtFile createFile(
      @NotNull @NonNls String name, @NotNull String text, @NotNull Project project) {
    String shortName = name.substring(name.lastIndexOf('/') + 1);
    shortName = shortName.substring(shortName.lastIndexOf('\\') + 1);
    LightVirtualFile virtualFile =
        new LightVirtualFile(
            shortName, KotlinLanguage.INSTANCE, StringUtilRt.convertLineSeparators(text));
  ...
  // Differs from common mock JDK only by one additional 'nonExistingMethod' in Collection and
  // constructor from Double in Throwable
  // It's needed to test the way we load additional built-ins members that neither in black nor
  // white lists
    ...
    if (!file.mkdirs()) {
      if (file.exists()) {
        throw new IllegalStateException(
            "Failed to create " + file + ": file exists and not a directory");
      }
  ...
  private static final String PLEASE_REGENERATE_TESTS =
      "Please regenerate tests (GenerateTests.kt)";
  ...
  public static void assertAllTestsPresentByMetadata(
    ...
    assertAllTestsPresentByMetadata(
        testCaseClass, testDataDir, filenamePattern, TargetBackend.ANY, recursive, excludeDirs);
  }
  ...
  public static void assertAllTestsPresentByMetadataWithExcluded(
    ...
    if (files != null) {
      for (File file : files) {
        if (file.isDirectory()) {
          if (recursive
              && containsTestData(file, filenamePattern, excludedPattern)
              && !exclude.contains(file.getName())) {
            assertTestClassPresentByMetadata(testCaseClass, file);
          }
        } else {
          boolean excluded =
              excludedPattern != null && excludedPattern.matcher(file.getName()).matches();
          if (!excluded
  ...
  public static void assertAllTestsPresentByMetadata(
      ...
      @NotNull String... excludeDirs) {
    assertAllTestsPresentByMetadataWithExcluded(
        testCaseClass, testDataDir, filenamePattern, null, targetBackend, recursive, excludeDirs);
  }
  ...
  public static void assertAllTestsPresentInSingleGeneratedClass(
    ...
    assertAllTestsPresentInSingleGeneratedClass(
        testCaseClass, testDataDir, filenamePattern, TargetBackend.ANY);
  }
  ...
  public static void assertAllTestsPresentInSingleGeneratedClass(
      ...
      @NotNull TargetBackend targetBackend) {
    assertAllTestsPresentInSingleGeneratedClass(
        testCaseClass, testDataDir, filenamePattern, null, targetBackend);
  }
  ...
  public static void assertAllTestsPresentInSingleGeneratedClass(
    ...
    FileUtil.processFilesRecursively(
        ...
        file -> {
          boolean excluded =
              excludePattern != null && excludePattern.matcher(file.getName()).matches();
          if (file.isFile()
              && !excluded
              && filenamePattern.matcher(file.getName()).matches()
              && isCompatibleTarget(targetBackend, file)) {
  ...
  private static void assertFilePathPresent(File file, File rootFile, Set<String> filePaths) {
    ...
    if (path != null) {
      ...
      if (!filePaths.contains(relativePath)) {
        KtAssert.fail(
            "Test data file missing from the generated test class: "
                + file
                + "\n"
  ...
  private static Set<String> collectPathsMetadata(Class<?> testCaseClass) {
    return new HashSet<>(
        ContainerUtil.map(collectMethodsMetadata(testCaseClass), KtTestUtil::nameToCompare));
  }
  ...
  private static boolean containsTestData(
      File dir, Pattern filenamePattern, @Nullable Pattern excludedPattern) {
