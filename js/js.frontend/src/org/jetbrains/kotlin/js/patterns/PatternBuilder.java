...
public final class PatternBuilder {
  ...
  @NotNull private static final NamePredicate KOTLIN_NAME_PREDICATE = new NamePredicate("kotlin");
  ...
  @NotNull
  public static DescriptorPredicate pattern(
      @NotNull NamePredicate checker, @NotNull String stringWithPattern) {
  ...
  @NotNull
  public static DescriptorPredicate pattern(
      @NotNull String stringWithPattern, @NotNull NamePredicate checker) {
  ...
  public static DescriptorPredicate pattern(@NotNull String stringWithPattern) {
    return pattern(
        parseFqNamesFromString(stringWithPattern), parseArgumentsFromString(stringWithPattern));
  }
  ...
  private static String getArgumentsPatternFromString(@NotNull String stringWithPattern) {
    ...
    if (left < 0) {
      ...
      assert right == stringWithPattern.length() - 1
          : "expected ')' at the end: " + stringWithPattern;
  ...
  @NotNull
  private static DescriptorPredicate pattern(
      @NotNull List<NamePredicate> checkers, @Nullable List<NamePredicate> arguments) {
  ...
  public static class DescriptorPredicateImpl implements DescriptorPredicate {
    ...
    private boolean matches(@NotNull CallableDescriptor callable) {
      ...
      while (true) {
        ...
        if (descriptor instanceof PackageFragmentDescriptor) {
          return nameIndex == 0
              && names[0].equals(((PackageFragmentDescriptor) descriptor).getFqName().asString());
        }
      }
    }
    ...
    public boolean test(FunctionDescriptor functionDescriptor) {
      ...
      for (CallableMemberDescriptor real :
          OverridingUtil.getOverriddenDeclarations(functionDescriptor)) {
        if (matches(real)) {
          return true;
        }
      }
      ...
      if (checkOverridden) {
        for (CallableDescriptor overridden :
            DescriptorUtils.getAllOverriddenDescriptors(functionDescriptor)) {
