...
public class JavaAnnotationImpl extends JavaElementImpl<PsiAnnotation> implements JavaAnnotation {
  ...
  public Collection<JavaAnnotationArgument> getArguments() {
    return namedAnnotationArguments(
        getPsi().getParameterList().getAttributes(), getSourceFactory());
  }
  ...
  private static ClassId computeClassId(@NotNull PsiClass psiClass) {
    ...
    if (container != null) {
      ...
      return parentClassId == null || name == null
          ? null
          : parentClassId.createNestedClassId(Name.identifier(name));
    }
  ...
  public boolean isIdeExternalAnnotation() {
    ...
    ExternalAnnotationsManager externalAnnotationManager =
        ExternalAnnotationsManager.getInstance(psi.getProject());
