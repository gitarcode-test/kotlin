...
public class CommonSupertypes {
  @Nullable
  public static KotlinType commonSupertypeForNonDenotableTypes(
      @NotNull Collection<KotlinType> types) {
  ...
  public static KotlinType commonSupertype(@NotNull Collection<KotlinType> types) {
    ...
    // It can be slightly deeper, though: e.g. when initial types are simple, but their supertypes
    // are complex
  ...
  @NotNull
  private static KotlinType findCommonSupertype(
      @NotNull Collection<KotlinType> types, int recursionDepth, int maxDepth) {
    assert recursionDepth <= maxDepth
        : "Recursion depth exceeded: " + recursionDepth + " > " + maxDepth + " for types " + types;
  ...
  @NotNull
  private static SimpleType commonSuperTypeForInflexible(
      @NotNull Collection<SimpleType> types, int recursionDepth, int maxDepth) {
    ...
    for (Iterator<SimpleType> iterator = typeSet.iterator(); iterator.hasNext(); ) {
      ...
      assert type != null;
      assert !FlexibleTypesKt.isFlexible(type)
          : "Flexible type " + type + " passed to commonSuperTypeForInflexible";
    ...
    if (commonSupertypes.isEmpty()) {
      ...
      for (SimpleType type : types) {
        String superTypes =
            TypeUtils.getAllSupertypes(type).stream()
                .map(t -> "-- " + renderTypeFully(t))
                .collect(Collectors.joining("\n"));

        info.append("Info about ")
            .append(renderTypeFully(type))
            .append(": ")
            .append('\n')
            .append("- Supertypes: ")
            .append('\n')
            ...
            .append(classOfDeclarationDescriptor(type))
            .append('\n')
            .append('\n');
      }
      throw new IllegalStateException(
          "[Report version 3] There is no common supertype for: "
              + types
              + " \n"
              + info.toString());
    }
    ...
    // constructor of the supertype -> all of its instantiations occurring as supertypes
    Map.Entry<TypeConstructor, Set<SimpleType>> entry =
        commonSupertypes.entrySet().iterator().next();

    // Reconstructing type arguments if possible
    SimpleType result =
        computeSupertypeProjections(entry.getKey(), entry.getValue(), recursionDepth, maxDepth);
  ...
  private static String renderTypeFully(@NotNull KotlinType type) {
    return DescriptorRenderer.FQ_NAMES_IN_TYPES.renderType(type)
        + ", typeConstructor debug: "
        + renderTypeConstructorVerboseDebugInformation(type.getConstructor());
  }

  @SuppressWarnings("deprecation")
  private static String renderTypeConstructorVerboseDebugInformation(
    ...
    DeclarationDescriptor declarationDescriptor =
        abstractTypeConstructor.getDeclarationDescriptor();
    String moduleDescriptorString =
    ...
    return "descriptor="
        ...
        + "@"
        + Integer.toHexString(Objects.hashCode(declarationDescriptor))
        + ", moduleDescriptor="
        + moduleDescriptorString
        + ", "
  ...
  @NotNull
  private static Map<TypeConstructor, Set<SimpleType>> computeCommonRawSupertypes(
      @NotNull Collection<SimpleType> types) {
    assert !types.isEmpty();
    ...
    for (SimpleType type : types) {
      ...
      order =
          topologicallySortSuperclassesAndRecordAllInstances(
              type, constructorToAllInstances, visited);
  ...
  // types - instantiations of constructor occurring as supertypes of classes we are trying to
  // intersect
  @NotNull
  private static SimpleType computeSupertypeProjections(
      @NotNull TypeConstructor constructor,
      @NotNull Set<SimpleType> types,
      int recursionDepth,
      int maxDepth) {
    ...
    for (TypeParameterDescriptor parameterDescriptor : parameters) {
      ...
      newProjections.add(
          computeSupertypeProjection(
              parameterDescriptor, typeProjections, recursionDepth, maxDepth));
    }
    ...
    return KotlinTypeFactory.simpleTypeWithNonTrivialMemberScope(
        TypeAttributes.Companion.getEmpty(), constructor, newProjections, nullable, newScope);
  }
  ...
  private static TypeProjection computeSupertypeProjection(
    ...
    if (outs != null) {
      assert !outs.isEmpty()
          : "Out projections is empty for parameter "
              + parameterDescriptor
              + ", type projections "
              + typeProjections;
    ...
    if (ins != null) {
      assert !ins.isEmpty()
          : "In projections is empty for parameter "
              + parameterDescriptor
              + ", type projections "
              + typeProjections;
  ...
  private static void markAll(
      @NotNull TypeConstructor typeConstructor, @NotNull Set<TypeConstructor> markerSet) {
    ...
    return DFS.dfs(
        ...
        current -> {
          ...
          for (KotlinType supertype : supertypes) {
            ...
            result.add(
                FlexibleTypesKt.lowerIfFlexible(
                    substitutor.safeSubstitute(supertype, Variance.INVARIANT)));
          }
          return result;
        },
