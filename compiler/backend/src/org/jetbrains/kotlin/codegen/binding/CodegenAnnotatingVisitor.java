...
class CodegenAnnotatingVisitor extends KtVisitorVoid {
  ...
  public CodegenAnnotatingVisitor(@NotNull GenerationState state) {
    ...
    this.samTypeApproximator =
        new SamTypeApproximator(
            state.getModule().getBuiltIns(), state.getLanguageVersionSettings());
  }
  ...
  private ClassDescriptor recordClassForCallable(
    ...
    ClassDescriptor classDescriptor =
        new SyntheticClassDescriptorForLambda(
            customContainer != null
                ? customContainer
                : correctContainerForLambda(callableDescriptor),
  ...
  private DeclarationDescriptor correctContainerForLambda(@NotNull CallableDescriptor descriptor) {
    ...
    // In almost all cases the function's direct container is the correct container to consider in
    // JVM back-end
    // (and subsequently to write to EnclosingMethod and InnerClasses attributes).
    // The only exceptional case is when a lambda is declared in the super call of an anonymous
    // object:
    // in this case it's constructed in the outer code, despite being located under the object PSI-
    // and descriptor-wise
  ...
  public void visitKtElement(@NotNull KtElement element) {
    ...
    if (!classBuilderMode.generateBodies) {
      ...
      if (element instanceof KtElementImplStub<?>
          && ((KtElementImplStub) element).getStub() != null) {
  ...
  public void visitKtFile(@NotNull KtFile file) {
    ...
    if (file instanceof KtCodeFragment) {
      ...
      name =
          info.getClassDescriptor().getName().asString()
              + "$"
              + info.getMethodDescriptor().getName().asString();
    } else {
  ...
  public void visitLambdaExpression(@NotNull KtLambdaExpression lambdaExpression) {
    ...
    ClassDescriptor classDescriptor =
        recordClassForCallable(functionLiteral, functionDescriptor, supertypes, name);
    ...
    if (CoroutineUtilKt.isSuspendLambda(functionDescriptor)) {
      createAndRecordSuspendFunctionView(
          closure, (SimpleFunctionDescriptor) functionDescriptor, true);
    }
  ...
  private boolean isAdaptedCallableReference(
    ...
    if (callableReferenceType != null) {
      KotlinType callableReferenceReturnType =
          CollectionsKt.last(callableReferenceType.getArguments()).getType();
      KotlinType functionReturnType = functionDescriptor.getReturnType();
      return functionReturnType != null
          && KotlinBuiltIns.isUnit(callableReferenceReturnType)
          && !KotlinBuiltIns.isUnit(functionReturnType);
    }
    return false;
  }
  ...
  public void visitCallableReferenceExpression(@NotNull KtCallableReferenceExpression expression) {
    ResolvedCall<?> referencedFunction =
        CallUtilKt.getResolvedCall(expression.getCallableReference(), bindingContext);
    ...
    // TransientReceiver corresponds to an unbound reference, other receiver values -- to bound
    // references
    KotlinType receiverType =
        dispatchReceiver != null && !(dispatchReceiver instanceof TransientReceiver)
            ? dispatchReceiver.getType()
            : extensionReceiver != null && !(extensionReceiver instanceof TransientReceiver)
    ...
    if (target instanceof FunctionDescriptor) {
      ...
      supertypes =
          runtimeTypes.getSupertypesForFunctionReference(
              targetFunction,
              (AnonymousFunctionDescriptor) callableDescriptor,
              receiverType != null,
      ...
      supertypes =
          Collections.singleton(
              runtimeTypes.getSupertypeForPropertyReference(
                  ...
                  ReflectionTypes.Companion.isNumberedKMutablePropertyType(
                      callableDescriptor.getReturnType()),
                  receiverType != null));
    } else {
      return;
    }
    ...
    String name = inventAnonymousClassName();
    ClassDescriptor classDescriptor =
        recordClassForCallable(expression, callableDescriptor, supertypes, name);
    MutableClosure closure = recordClosure(classDescriptor, name);
  ...
  private SimpleFunctionDescriptor createAndRecordSuspendFunctionView(
    ...
    SimpleFunctionDescriptor jvmSuspendFunctionView =
        CoroutineCodegenUtilKt.getOrCreateJvmSuspendFunctionView(
            functionDescriptor, this.bindingContext);

    bindingTrace.record(
        CodegenBinding.SUSPEND_FUNCTION_TO_JVM_VIEW, functionDescriptor, jvmSuspendFunctionView);
  ...
  private MutableClosure recordClosure(
      @NotNull ClassDescriptor classDescriptor, @NotNull String name) {
    Type type =
        Type.getObjectType(JvmCodegenUtil.sanitizeNameIfNeeded(name, languageVersionSettings));
    return CodegenBinding.recordClosure(
        bindingTrace, classDescriptor, getProperEnclosingClass(), type);
  }
  ...
  private void recordLocalVariablePropertyMetadata(LocalVariableDescriptor variableDescriptor) {
    ...
    LocalVariableDescriptor metadataVariableDescriptor =
        new LocalVariableDescriptor(
            ...
            ReflectionTypes.Companion.createKPropertyStarType(
                DescriptorUtilsKt.getModule(variableDescriptor)),
            SourceElement.NO_SOURCE);
    bindingTrace.record(
        LOCAL_VARIABLE_PROPERTY_METADATA, variableDescriptor, metadataVariableDescriptor);
  }
  ...
  public void visitProperty(@NotNull KtProperty property) {
    ...
    if (nameForClassOrPackageMember != null) {
      ...
      nameStack.push(
          peekFromStack(nameStack) + '$' + safeIdentifier(property.getNameAsSafeName()).asString());
    }
    ...
    if (delegate != null && descriptor instanceof VariableDescriptorWithAccessors) {
      ...
      KotlinType supertype =
          runtimeTypes.getSupertypeForPropertyReference(
              variableDescriptor, variableDescriptor.isVar(), /* bound= */ false);
      ClassDescriptor classDescriptor =
          recordClassForCallable(
              delegate, variableDescriptor, Collections.singleton(supertype), name);
      recordClosure(classDescriptor, name);

      if (delegatedPropertiesCodegenHelper.isDelegatedPropertyMetadataRequired(
          variableDescriptor)) {
        Type containerType = getMetadataOwner(property);
        List<VariableDescriptorWithAccessors> descriptors =
            bindingTrace.get(DELEGATED_PROPERTIES_WITH_METADATA, containerType);
        if (descriptors == null) {
  ...
  private void checkRuntimeAsserionsOnDeclarationBody(
      @NotNull KtDeclaration declaration, DeclarationDescriptor descriptor) {
    if (classBuilderMode.generateBodies) {
      // NB This is required only for bodies generation.
      // In light class generation can cause recursion in types resolution.
      RuntimeAssertionsOnDeclarationBodyChecker.check(
          declaration, descriptor, bindingTrace, languageVersionSettings);
    }
  }
  ...
  private Type getMetadataOwner(@NotNull KtProperty property) {
    for (int i = classStack.size() - 1; i >= 0; i--) {
      ...
      // The first "real" containing class (not a synthetic class for lambda) is the owner of the
      // delegated property metadata
    ...
    return Type.getObjectType(
        JvmFileClassUtil.getFileClassInternalName(property.getContainingKtFile()));
  }
  ...
  public void visitNamedFunction(@NotNull KtNamedFunction function) {
    ...
    if (functionDescriptor instanceof SimpleFunctionDescriptor
      ...
      SimpleFunctionDescriptor jvmSuspendFunctionView =
          createAndRecordSuspendFunctionView(
              closure, (SimpleFunctionDescriptor) functionDescriptor, false);
      ...
      // But the problem is that the function may contain resolved calls referencing original
      // parameters, that are recreated
      // in jvmSuspendFunctionView.
      // So we remember the relation between the old and the new parameter descriptors and use it
      // when looking for their indices
      // in ExpressionCodegen.
      for (Pair<ValueParameterDescriptor, ValueParameterDescriptor> parameterDescriptorPair :
          CollectionsKt.zip(
              functionDescriptor.getValueParameters(),
              jvmSuspendFunctionView.getValueParameters())) {
        bindingTrace.record(
            CodegenBinding.PARAMETER_SYNONYM,
            parameterDescriptorPair.getFirst(),
            parameterDescriptorPair.getSecond());
      }
    ...
    if (nameForClassOrPackageMember != null) {
      ...
      String name = inventAnonymousClassName();
      ClassDescriptor classDescriptor =
          recordClassForFunction(function, functionDescriptor, name, null);
      MutableClosure closure = recordClosure(classDescriptor, name);
    ...
    if (resolvedCall instanceof VariableAsFunctionResolvedCall) {
      ...
      if (variableDescriptor instanceof ValueParameterDescriptor
        ...
        for (int i = functionsStack.size() - 1; i >= 0; i--) {
          ...
          bindingTrace.record(
              CodegenBinding.CAPTURES_CROSSINLINE_LAMBDA, functionsStack.get(i), true);
  ...
  private void recordSamValuesForOldInference(ResolvedCall<?> call, CallableDescriptor descriptor) {
    FunctionDescriptor original =
        SamCodegenUtil.getOriginalIfSamAdapter((FunctionDescriptor) descriptor);
    if (original == null) return;

    // TODO we can just record SAM_VALUE on relevant value arguments as we do in
    // recordSamValuesForNewInference
    ...
    for (ValueParameterDescriptor valueParameter : original.getValueParameters()) {
      ValueParameterDescriptor adaptedParameter =
          descriptor.getValueParameters().get(valueParameter.getIndex());
      if (KotlinTypeChecker.DEFAULT.equalTypes(
          adaptedParameter.getType(), valueParameter.getType())) continue;
      valueParametersWithSAMConversion.add(valueParameter);
    }
    writeSamValueForValueParameters(
        valueParametersWithSAMConversion, call.getValueArgumentsByIndex());
  }

  private void recordSamValuesForNewInference(@NotNull ResolvedCall<?> call) {
    NewAbstractResolvedCall<?> newResolvedCall =
        getNewResolvedCallForCallWithPossibleSamConversions(call);
    if (!(newResolvedCall instanceof NewResolvedCallImpl<?>)) return;

    Map<ValueParameterDescriptor, ResolvedValueArgument> arguments =
        newResolvedCall.getValueArguments();
      ...
      if (argument instanceof ExpressionValueArgument) {
        ...
        if (valueArgument != null
            && ((NewResolvedCallImpl<?>) newResolvedCall)
                    .getExpectedTypeForSamConvertedArgument(valueArgument)
                != null) {
  ...
  @Nullable
  private static NewAbstractResolvedCall<?> getNewResolvedCallForCallWithPossibleSamConversions(
  ...
  private SamType createSamTypeByValueParameter(ValueParameterDescriptor valueParameterDescriptor) {
    KotlinType kotlinSamType =
        samTypeApproximator.getSamTypeForValueParameter(valueParameterDescriptor, false);
  ...
  private void writeSamValueForValueParameters(
    ...
    for (ValueParameterDescriptor valueParameter : valueParametersWithSAMConversion) {
      ...
      ValueArgument valueArgument =
          ((ExpressionValueArgument) resolvedValueArgument).getValueArgument();
      assert valueArgument != null;
      recordSamTypeOnArgumentExpression(samType, valueArgument);
    }
  }

  private void recordSamTypeOnArgumentExpression(
      ValueParameterDescriptor valueParameter, ValueArgument valueArgument) {
    SamType samType = createSamTypeByValueParameter(valueParameter);
  ...
  private void withinUninitializedClass(@NotNull KtElement element, @NotNull Runnable operation) {
    ...
    assert currentClass != null
        : element.getClass().getSimpleName() + " should be inside a class: " + element.getText();
  ...
  private void recordSamConstructorIfNeeded(
      @NotNull KtCallElement expression, @NotNull ResolvedCall<?> call) {
  ...
  public void visitBinaryExpression(@NotNull KtBinaryExpression expression) {
    ...
    DeclarationDescriptor operationDescriptor =
        bindingContext.get(BindingContext.REFERENCE_TARGET, expression.getOperationReference());
    ...
    FunctionDescriptor original =
        SamCodegenUtil.getOriginalIfSamAdapter((FunctionDescriptor) operationDescriptor);
    if (original == null) return;
    ...
    for (ValueParameterDescriptor valueParameter : parameters) {
      ...
      if (isSetter && valueParameter.getIndex() == parameters.size() - 1) {
        ...
        if (parent instanceof KtBinaryExpression
            && ((KtBinaryExpression) parent).getOperationToken() == EQ) {
  ...
  public void visitWhenExpression(@NotNull KtWhenExpression expression) {
    ...
    String currentClassName =
        getCurrentTopLevelClassOrPackagePartInternalName(expression.getContainingKtFile());

    if (bindingContext.get(MAPPINGS_FOR_WHENS_BY_ENUM_IN_CLASS_FILE, currentClassName) == null) {
      bindingTrace.record(
          MAPPINGS_FOR_WHENS_BY_ENUM_IN_CLASS_FILE, currentClassName, new ArrayList<>(1));
    }

    List<WhenByEnumsMapping> mappings =
        bindingContext.get(MAPPINGS_FOR_WHENS_BY_ENUM_IN_CLASS_FILE, currentClassName);
    assert mappings != null : "guaranteed by contract";

    int fieldNumber = mappings.size();

    assert expression.getSubjectExpression() != null
        : "subject expression should be not null in a valid when by enums";
    ...
    ClassDescriptor classDescriptor =
        (ClassDescriptor) type.getConstructor().getDeclarationDescriptor();
    ...
    WhenByEnumsMapping mapping =
        new WhenByEnumsMapping(classDescriptor, currentClassName, fieldNumber, isPublicAbi);
  ...
  private boolean isWhenWithEnums(@NotNull KtWhenExpression expression) {
    ...
    return switchCodegenProvider.checkAllItemsAreConstantsSatisfying(
        expression, constant -> isEnumEntryOrNull(enumClassId, constant));
  }
