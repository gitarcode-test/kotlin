...
public class ImplementationBodyCodegen extends ClassBodyCodegen {
  ...
  @NotNull private SuperClassInfo superClassInfo;
  ...
  public ImplementationBodyCodegen(
    ...
    JvmSerializerExtension extension =
        ...
        DescriptorSerializer.create(
            ...
            parentCodegen instanceof ImplementationBodyCodegen
                ...
                : DescriptorSerializer.createTopLevel(
                    extension, state.getLanguageVersionSettings(), null),
            state.getLanguageVersionSettings(),
            state.getProject());

    this.constructorCodegen =
        new ConstructorCodegen(
            descriptor,
            context,
            functionCodegen,
            this,
  ...
  protected void generateDeclaration() {
    ...
    if (modality != Modality.OPEN && !isAbstract) {
      isFinal =
          kind == ClassKind.OBJECT
              ||
              // Light-class mode: Do not make enum classes final since PsiClass corresponding to
              // enum is expected to be inheritable from
    ...
    if (state.getClassBuilderMode() == ClassBuilderMode.LIGHT_CLASSES
      ...
      // !ClassBuilderMode.generateBodies means we are generating light classes & looking at a
      // nested or inner class
      // Light class generation is implemented so that Cls-classes only read bare code of classes,
      // without knowing whether these classes are inner or not (see
      // ClassStubBuilder.EMPTY_STRATEGY)
    ...
    if (isInterface(descriptor) && !isLocal) {
      ...
      ClassBuilder defaultImplsBuilder =
          state
              .getFactory()
              .newVisitor(
                  JvmDeclarationOriginKt.DefaultImpls(myClass.getPsiOrParent(), descriptor),
                  defaultImplsType,
      ...
      ClassContext defaultImplsContext =
          parentContext.intoDefaultImplsClass(descriptor, (ClassContext) context, state);
      new InterfaceImplBodyCodegen(myClass, defaultImplsContext, defaultImplsBuilder, state, this)
          .generate();
    }
  }
  ...
  protected void generateErasedInlineClassIfNeeded() {
    ...
    ClassContext erasedInlineClassContext =
        context.intoWrapperForErasedInlineClass(descriptor, state);
    new ErasedInlineClassBodyCodegen((KtClass) myClass, erasedInlineClassContext, v, state, this)
        .generate();
  }
  ...
  protected void generateUnboxMethodForInlineClass() {
    ...
    InlineClassRepresentation<SimpleType> inlineClassRepresentation =
        getInlineClassRepresentation(descriptor);
    ...
    SimpleFunctionDescriptor functionDescriptor =
        InlineClassDescriptorResolver.createUnboxFunctionDescriptor(this.descriptor);

    functionCodegen.generateMethod(
        JvmDeclarationOriginKt.UnboxMethodOfInlineClass(functionDescriptor),
        functionDescriptor,
  ...
  protected void generateKotlinMetadataAnnotation() {
    ...
    WriteAnnotationUtilKt.writeKotlinMetadata(
        v,
        state.getConfig(),
  ...
  private void writeEnclosingMethod() {
    // Do not emit enclosing method in "light-classes mode" since currently we generate local light
    // classes as if they're top level
    if (!state.getClassBuilderMode().generateBodies) {
      return;
    }

    // JVMS7: A class must have an EnclosingMethod attribute if and only if it is a local class or
    // an anonymous class.
    if (isAnonymousObject(descriptor)
        || !(descriptor.getContainingDeclaration() instanceof ClassOrPackageFragmentDescriptor)) {
  ...
  static {
    for (PlatformMutabilityMapping platformMutabilityMapping :
        JavaToKotlinClassMap.INSTANCE.getMutabilityMappings()) {
      ...
      KOTLIN_MARKER_INTERFACES.put(
          ...
          "kotlin/jvm/internal/markers/K"
              + mutableClassId
                  .getRelativeClassName()
                  .asString()
                  .replace(
                      "MutableEntry",
                      "Entry") // kotlin.jvm.internal.markers.KMutableMap.Entry for some reason
                  .replace(".", "$"));
    }
  }
  ...
  public static JvmClassSignature signature(
    ...
    for (KotlinType supertype : descriptor.getTypeConstructor().getSupertypes()) {
      if (isJvmInterface(supertype.getConstructor().getDeclarationDescriptor())) {
        FqName kotlinInterfaceName =
            DescriptorUtils.getFqName(supertype.getConstructor().getDeclarationDescriptor())
                .toSafe();
        ...
        String kotlinMarkerInterfaceInternalName =
            KOTLIN_MARKER_INTERFACES.get(kotlinInterfaceName);
        if (kotlinMarkerInterfaceInternalName != null) {
          if (typeMapper.getClassBuilderMode() == ClassBuilderMode.LIGHT_CLASSES) {
            sw.writeInterface();
            Type kotlinCollectionType =
                typeMapper.mapType(
                    supertype, sw, TypeMappingMode.SUPER_TYPE_KOTLIN_COLLECTIONS_AS_IS);
            sw.writeInterfaceEnd();
  ...
  protected void generateSyntheticPartsAfterBody() {
    ...
    for (ExpressionCodegenExtension extension :
  ...
  protected void generateConstructors() {
    try {
      ...
      if (!InlineClassesUtilsKt.isInlineClass(descriptor)
          && !(descriptor instanceof SyntheticClassOrObjectDescriptor)) {
        // Synthetic classes does not have declarations for secondary constructors
        for (ClassConstructorDescriptor secondaryConstructor :
            DescriptorUtilsKt.getSecondaryConstructors(descriptor)) {
      ...
      throw new RuntimeException(
          "Error generating constructors of class " + myClass.getName() + " with kind " + kind, e);
    }
  }
  ...
  private void generateToArray() {
    ...
    if (CollectionsKt.any(
        DescriptorUtilsKt.getAllSuperclassesWithoutAny(descriptor),
        classDescriptor ->
            !(classDescriptor instanceof JavaClassDescriptor)
    ...
    Collection<? extends SimpleFunctionDescriptor> functions =
        descriptor
            .getDefaultType()
            .getMemberScope()
            .getContributedFunctions(Name.identifier("toArray"), NoLookupLocation.FROM_BACKEND);
    ...
    if (!hasNonGenericToArray) {
      MethodVisitor mv =
          v.newMethod(NO_ORIGIN, ACC_PUBLIC, "toArray", "()[Ljava/lang/Object;", null, null);
  ...
  public static JvmKotlinType genPropertyOnStack(
    ...
    if (couldUseDirectAccessToProperty(
        propertyDescriptor,
        /* forGetter= */ true,
        /* isDelegated= */ false,
        context,
      ...
      String fieldName =
          ((FieldOwnerContext) context.getParentContext()).getFieldName(propertyDescriptor, false);
      ...
      iv.invokevirtual(
          classAsmType.getInternalName(), method.getName(), method.getDescriptor(), false);
  ...
  private class DataClassMethodGeneratorImpl extends DataClassMethodGenerator {
    ...
    DataClassMethodGeneratorImpl(KtClassOrObject klass, BindingContext bindingContext) {
      ...
      this.functionsFromAnyGenerator =
          new FunctionsFromAnyGeneratorImpl(
              klass,
              bindingContext,
              descriptor,
              classAsmType,
              ImplementationBodyCodegen.this.context,
              v,
              state);
    }

    @Override
    public void generateEqualsMethod(
    ...
    @Override
    public void generateHashCodeMethod(
        @NotNull FunctionDescriptor function,
        @NotNull List<? extends PropertyDescriptor> properties) {
    ...
    @Override
    public void generateComponentFunction(
      ...
      functionCodegen.generateMethod(
          JvmDeclarationOriginKt.OtherOrigin(originalElement, function),
          function,
              ...
              if (!componentType.equals(Type.VOID_TYPE)) {
                PropertyDescriptor property =
                    bindingContext.get(
                        ...
                        descriptorToDeclaration(parameter));
                assert property != null
                    : "Property descriptor is not found for primary constructor parameter: "
                        + parameter;

                JvmKotlinType propertyType =
                    genPropertyOnStack(
                        iv,
                        context,
                        property,
                        ImplementationBodyCodegen.this.classAsmType,
                        0,
                        state);
    ...
    public void generateCopyFunction(
          ...
          new FunctionGenerationStrategy() {
            ...
            public void generateBody(
              ...
              assert function.getValueParameters().size() == constructor.getValueParameters().size()
                  : "Number of parameters of copy function and constructor are different. "
                      + "Copy: "
                      + function.getValueParameters().size()
                      + ", "
                      + "constructor: "
              ...
              iv.invokespecial(
                  thisDescriptorType.getInternalName(),
                  "<init>",
                  constructorAsmMethod.getDescriptor(),
                  false);
            ...
            private void pushCapturedFieldsOnStack(InstructionAdapter iv, MutableClosure closure) {
              ...
              if (captureThis != null) {
                ...
                iv.getfield(
                    classAsmType.getInternalName(), CAPTURED_THIS_FIELD, type.getDescriptor());
              }
              ...
              if (captureReceiver != null) {
                ...
                String fieldName =
                    closure.getCapturedReceiverFieldName(
                        bindingContext, state.getLanguageVersionSettings());
                iv.getfield(classAsmType.getInternalName(), fieldName, type.getDescriptor());
              }
      ...
      functionCodegen.generateDefaultIfNeeded(
          ...
          (valueParameter, codegen) -> {
            ...
            PropertyDescriptor property =
                bindingContext.get(BindingContext.VALUE_PARAMETER_AS_PROPERTY, valueParameter);
            assert property != null
                : "Copy function doesn't correspond to any property: " + function;
  ...
  @NotNull
  private static ConstructorDescriptor getPrimaryConstructorOfDataClass(
  ...
  private void generateEnumValuesMethod() {
    Type type =
        typeMapper.mapType(
            DescriptorUtilsKt.getBuiltIns(descriptor)
                .getArrayType(INVARIANT, descriptor.getDefaultType()));

    FunctionDescriptor valuesFunction =
        CollectionsKt.single(
            descriptor
                .getStaticScope()
                .getContributedFunctions(ENUM_VALUES, NoLookupLocation.FROM_BACKEND));
    MethodVisitor mv =
        v.newMethod(
            JvmDeclarationOriginKt.OtherOriginFromPure(myClass, valuesFunction),
            ACC_PUBLIC | ACC_STATIC,
            ENUM_VALUES.asString(),
            "()" + type.getDescriptor(),
            null,
            null);
    ...
    mv.visitCode();
    mv.visitFieldInsn(
        GETSTATIC, classAsmType.getInternalName(), ENUM_VALUES_FIELD_NAME, type.getDescriptor());
    mv.visitMethodInsn(
        INVOKEVIRTUAL, type.getInternalName(), "clone", "()Ljava/lang/Object;", false);
  ...
  private void generateEnumValueOfMethod() {
    FunctionDescriptor valueOfFunction =
        CollectionsKt.single(
            descriptor
                .getStaticScope()
                .getContributedFunctions(ENUM_VALUE_OF, NoLookupLocation.FROM_BACKEND),
            DescriptorUtilsKt::isEnumValueOfMethod);
    MethodVisitor mv =
        v.newMethod(
            JvmDeclarationOriginKt.OtherOriginFromPure(myClass, valueOfFunction),
            ACC_PUBLIC | ACC_STATIC,
            ENUM_VALUE_OF.asString(),
            "(Ljava/lang/String;)" + classAsmType.getDescriptor(),
            null,
            null);
    if (!state.getClassBuilderMode().generateBodies) return;
  ...
  private void generateFieldForSingleton() {
    if (isCompanionObjectInInterfaceNotIntrinsic(descriptor)) {
      StackValue.Field field =
          StackValue.createSingletonViaInstance(descriptor, typeMapper, HIDDEN_INSTANCE_FIELD);
      // hidden instance in interface companion
      v.newField(
          JvmDeclarationOriginKt.OtherOrigin(descriptor),
    ...
    if (isNonCompanionObject(descriptor)) {
      StackValue.Field field =
          StackValue.createSingletonViaInstance(descriptor, typeMapper, INSTANCE_FIELD);
      FieldVisitor fv =
          v.newField(
              ...
              null,
              null);
      AnnotationCodegen.forField(fv, this, state)
          .visitAnnotation(Type.getDescriptor(NotNull.class), false)
          .visitEnd();
      return;
    }
    ...
    @Nullable
    KtObjectDeclaration companionObject = CollectionsKt.firstOrNull(myClass.getCompanionObjects());
    ...
    boolean deprecatedFieldForInvisibleCompanionObject =
        state
            ...
            .supportsFeature(LanguageFeature.DeprecatedFieldForInvisibleCompanionObject);
    ...
    boolean hasPrivateOrProtectedProperVisibility =
        (properFieldVisibilityFlag & (ACC_PRIVATE | ACC_PROTECTED)) != 0;
    boolean hasPackagePrivateProperVisibility =
        (properFieldVisibilityFlag & (ACC_PUBLIC | ACC_PRIVATE | ACC_PROTECTED)) == 0;
    boolean fieldShouldBeDeprecated =
        deprecatedFieldForInvisibleCompanionObject
            && !properVisibilityForCompanionObjectInstanceField
            && (hasPrivateOrProtectedProperVisibility
                || hasPackagePrivateProperVisibility
    ...
    FieldVisitor fv =
        v.newField(
            JvmDeclarationOriginKt.OtherOrigin(
                companionObject == null ? myClass.getPsiOrParent() : companionObject),
            fieldAccessFlags,
            field.name,
            field.type.getDescriptor(),
            null,
    ...
    if (fieldShouldBeDeprecated) {
      AnnotationCodegen.forField(fv, this, state)
          .visitAnnotation(Type.getDescriptor(Deprecated.class), true)
          .visitEnd();
    }
  }
  ...
  private void initializeObjects() {
    ...
    boolean isInterfaceCompanionWithBackingFieldsInOuter =
        isInterfaceCompanionWithBackingFieldsInOuter(descriptor);
    boolean isMappedIntrinsicCompanionObject = isMappedIntrinsicCompanionObject(descriptor);
    boolean isClassCompanionWithBackingFieldsInOuter =
        isClassCompanionObjectWithBackingFieldsInOuter(descriptor);
    if (isNonCompanionObject
      ...
      assert local0Index == 0
          : "Local variable with index 0 in clInit should be used only for singleton instance"
                + " keeping";
      ...
      StackValue.Field singleton =
          StackValue.createSingletonViaInstance(
              descriptor,
              typeMapper,
              isInterfaceCompanion ? HIDDEN_INSTANCE_FIELD : INSTANCE_FIELD);
      ...
      if (isInterfaceCompanion) {
        ...
        StackValue.singleton(descriptor, typeMapper)
            .store(singleton, getParentCodegen().createOrGetClInitCodegen().v, true);
      }
    } else if (isClassCompanionWithBackingFieldsInOuter
        || isInterfaceCompanionWithBackingFieldsInOuter) {
      ...
      FunctionDescriptor constructor =
          (FunctionDescriptor)
              parentCodegen.context.accessibleDescriptor(
                  CollectionsKt.single(descriptor.getConstructors()),
                  /* superCallExpression= */ null);
      ...
      StackValue.singleton(descriptor, parentCodegen.typeMapper)
          .store(instance, parentVisitor, true);
  ...
  private static boolean isInterfaceCompanionWithBackingFieldsInOuter(
    ...
    return CollectionsKt.any(
        descriptors,
        d -> d instanceof PropertyDescriptor && hasJvmFieldAnnotation((PropertyDescriptor) d));
  }

  private void generateCompanionObjectBackingFieldCopies() {
    if (companionObjectPropertiesToCopy == null || companionObjectPropertiesToCopy.isEmpty())
      return;

    boolean isPrivateCompanion =
        DescriptorVisibilities.isPrivate(
            ((ClassDescriptor)
                    companionObjectPropertiesToCopy.get(0).descriptor.getContainingDeclaration())
                .getVisibility());
    ...
    List<String> additionalVisibleAnnotations =
        isPrivateCompanion
            ? Collections.singletonList(CodegenUtilKt.JAVA_LANG_DEPRECATED)
    ...
    for (PropertyAndDefaultValue info : companionObjectPropertiesToCopy) {
      ...
      FieldVisitor fv =
          v.newField(
              JvmDeclarationOriginKt.Synthetic(
                  DescriptorToSourceUtils.descriptorToDeclaration(property), property),
              modifiers,
              context.getFieldName(property, false),
              type.getDescriptor(),
              typeMapper.mapFieldSignature(property.getType(), property),
              info.defaultValue);

      AnnotationCodegen.forField(fv, this, state)
          .genAnnotations(property, type, null, null, additionalVisibleAnnotations);

      // This field are always static and final so if it has constant initializer don't do anything
      // in clinit,
  ...
  private int putCompanionObjectInLocalVar(ExpressionCodegen codegen) {
    ...
    if (companionObjectIndex == -1) {
      ...
      StackValue.local(companionObjectIndex, companionObject.type)
          .store(companionObject, codegen.v);
    }
  ...
  private void copyFieldFromCompanionObject(PropertyDescriptor propertyDescriptor) {
    ExpressionCodegen codegen = createOrGetClInitCodegen();
    StackValue property =
        codegen.intermediateValueForProperty(propertyDescriptor, false, null, StackValue.none());
    StackValue.Field field =
        StackValue.field(
            property.type,
            property.kotlinType,
        ...
        new KtVisitorVoid() {
          ...
          public void visitSimpleNameExpression(@NotNull KtSimpleNameExpression expr) {
            DeclarationDescriptor descriptor =
                bindingContext.get(BindingContext.REFERENCE_TARGET, expr);
            ...
            if (isLocalFunction(descriptor)) {
              ...
              ResolvedCall<? extends CallableDescriptor> call =
                  CallUtilKt.getResolvedCall(expr, bindingContext);
              if (call != null) {
              ...
              if (containingDeclaration instanceof ConstructorDescriptor) {
                ClassDescriptor classDescriptor =
                    ((ConstructorDescriptor) containingDeclaration).getConstructedClass();
                ...
                if (isDelegatedLocalVariable(descriptor)) {
                  VariableDescriptor metadata =
                      getDelegatedLocalVariableMetadata(
                          (VariableDescriptor) descriptor, bindingContext);
            ...
            if (value instanceof ImplicitReceiver) {
              if (value instanceof ExtensionReceiver) {
                ...
                assert parameter != null
                    : "Extension receiver should exist: "
                        + ((ExtensionReceiver) value).getDeclarationDescriptor();
          ...
          public void visitThisExpression(@NotNull KtThisExpression expression) {
            DeclarationDescriptor descriptor =
                bindingContext.get(
                    BindingContext.REFERENCE_TARGET, expression.getInstanceReference());
            assert descriptor instanceof CallableDescriptor || descriptor instanceof ClassDescriptor
                : "'This' reference target should be class or callable descriptor but was "
                    + descriptor;
            ...
            if (descriptor instanceof CallableDescriptor) {
              ReceiverParameterDescriptor parameter =
                  ((CallableDescriptor) descriptor).getExtensionReceiverParameter();
              if (parameter != null) {
                lookupInContext(parameter);
              }
            }
    ...
    for (KtSuperTypeListEntry specifier : myClass.getSuperTypeListEntries()) {
      if (specifier instanceof KtDelegatedSuperTypeEntry) {
        KtExpression delegateExpression =
            ((KtDelegatedSuperTypeEntry) specifier).getDelegateExpression();
  ...
  private void generateEnumEntries() {
    ...
    List<KtEnumEntry> enumEntries =
        CollectionsKt.filterIsInstance(element.getDeclarations(), KtEnumEntry.class);
    ...
    for (KtEnumEntry enumEntry : enumEntries) {
      ...
      FieldVisitor fv =
          v.newField(
              JvmDeclarationOriginKt.OtherOrigin(enumEntry, descriptor),
              ACC_PUBLIC | ACC_ENUM | ACC_STATIC | ACC_FINAL | isDeprecated,
              descriptor.getName().asString(),
              classAsmType.getDescriptor(),
              null,
              null);
  ...
  private void initializeEnumConstants(@NotNull List<KtEnumEntry> enumEntries) {
    ...
    iv.putstatic(
        classAsmType.getInternalName(), ENUM_VALUES_FIELD_NAME, arrayAsmType.getDescriptor());
  }
  ...
  private void initializeEnumConstant(@NotNull List<KtEnumEntry> enumEntries, int ordinal) {
    ...
    ResolvedCall<?> defaultArgumentsConstructorCall =
        CallUtilKt.getResolvedCall(enumEntry, bindingContext);
    boolean enumEntryHasSubclass =
        CodegenBinding.enumEntryNeedSubclass(bindingContext, classDescriptor);
    if (delegationSpecifiers.size() == 1 && !enumEntryNeedSubclass(bindingContext, enumEntry)) {
      ResolvedCall<?> resolvedCall =
          CallUtilKt.getResolvedCallWithAssert(delegationSpecifiers.get(0), bindingContext);

      CallableMethod method =
          typeMapper.mapToCallableMethod(
              (ConstructorDescriptor) resolvedCall.getResultingDescriptor(), false);
      ...
      codegen
          .invokeFunction(defaultArgumentsConstructorCall, StackValue.none())
          .put(Type.VOID_TYPE, iv);
    } else {
    ...
    for (KtSuperTypeListEntry specifier : myClass.getSuperTypeListEntries()) {
      if (specifier instanceof KtDelegatedSuperTypeEntry) {
        DelegationFieldsInfo.Field field =
            delegationFieldsInfo.getInfo((KtDelegatedSuperTypeEntry) specifier);
        ...
        KotlinType delegateExpressionType =
            delegateExpression != null ? bindingContext.getType(delegateExpression) : null;
  ...
  private void generateDelegates(
    ...
    for (Map.Entry<CallableMemberDescriptor, CallableMemberDescriptor> entry :
        DelegationResolver.Companion.getDelegates(descriptor, toInterface, delegateExpressionType)
            .entrySet()) {
      ...
      if (member instanceof PropertyDescriptor) {
        propertyCodegen.genDelegate(
            (PropertyDescriptor) member, (PropertyDescriptor) delegateTo, field.getStackValue());
      } else if (member instanceof FunctionDescriptor) {
        functionCodegen.genDelegate(
            (FunctionDescriptor) member, (FunctionDescriptor) delegateTo, field.getStackValue());
      }
    }
  }

  public void addCompanionObjectPropertyToCopy(
      @NotNull PropertyDescriptor descriptor, Object defaultValue) {
  ...
  public void addAdditionalTask(
      Function2<ImplementationBodyCodegen, ClassBuilder, Unit> additionalTask) {
