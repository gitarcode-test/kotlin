...
public class ExpressionTypingServices {
  ...
  public KotlinType safeGetType(
    ...
    KotlinType type =
        getType(scope, expression, expectedType, dataFlowInfo, inferenceSession, trace);

    return type != null
        ? type
        : ErrorUtils.createErrorType(ErrorTypeKind.NO_RECORDED_TYPE, expression.getText());
  }
  ...
  public KotlinTypeInfo getTypeInfo(
    ...
    if (contextExpression != expression) {
      context =
          context.replaceExpressionContextProvider(
              arg -> arg == expression ? contextExpression : null);
    }
  ...
  public KotlinType getType(
      ...
      @NotNull BindingTrace trace) {
    return getTypeInfo(
            scope, expression, expectedType, dataFlowInfo, inferenceSession, trace, false)
        .getType();
  }
  ...
  public KotlinTypeInfo getBlockReturnedType(
      KtBlockExpression expression, ExpressionTypingContext context, boolean isStatement) {
    return getBlockReturnedType(
        expression,
        isStatement ? CoercionStrategy.COERCION_TO_UNIT : CoercionStrategy.NO_COERCION,
        context);
  }
  ...
  public KotlinTypeInfo getBlockReturnedType(
    ...
    TraceBasedLocalRedeclarationChecker redeclarationChecker =
        new TraceBasedLocalRedeclarationChecker(
            context.trace, expressionTypingComponents.overloadChecker);
  ...
  public KotlinType getBodyExpressionType(
    ...
    KotlinResolutionCallbacksImpl.LambdaInfo lambdaInfo =
        getNewInferenceLambdaInfo(context, function);
  ...
  public static KotlinResolutionCallbacksImpl.LambdaInfo getNewInferenceLambdaInfo(
      @NotNull ExpressionTypingContext context, @NotNull KtElement function) {
      ...
      if (!iterator.hasNext()) {
        ...
        if (result.getType() != null
          ...
          DataFlowValue lastExpressionValue =
              expressionTypingComponents.dataFlowValueFactory.createDataFlowValue(
  ...
  private KotlinTypeInfo getTypeOfLastExpressionInBlock(
    ...
    boolean isUnitExpectedType =
        context.expectedType != NO_EXPECTED_TYPE
    ...
    // In NI, `::bar` shouldn't be eagerly resolved, but introduced as a postponed atom to the
    // general inference system.
    ...
    // (3) lastStatement.parent.parent [statement -> block -> function literal] is a lambda that is
    // handled by NI
    //
    // When the conditions are met, `createCallArgument` at
    // `org.jetbrains.kotlin.resolve.calls.tower.KotlinResolutionCallbacksImpl.analyzeAndGetLambdaReturnArguments`
    // we just pick that last statement in the lambda.
    //
    // But due to a bug in initial implementation `createDontCareTypeInfoForNILambda` (third
    // condition), it was checking that
    ...
    // }
    //
    // The problem with the `::baz` is that it should not be processed as a postponed atom for the
    // call, but it has been.
    // At the same time, it's being ignored by the NI (because it's not the last statement), thus
    // left unresolved.
    // The bugs are described at KT-52270 and KT-55931.
    //
    // During attempt to fix KT-52270, instead of fixing the third condition (that is actually
    // broken), the second one has been changed to
    // "expected type is NO_EXPECTED_TYPE".
    // That helped to the main use case from the issue, but still didn't help with KT-55931.
    //
    // But the actual problem is that brought another regression bug (KT-55729) because after that
    // we started
    // resolving last-statement callable references with expected type eagerly as regular top-level
    // references (without conversions).
    // So, conversions stopped working there since 1.8.0
    //
    // While it might be tempting to revert the incorrect fix for KT-52270, and fix the third
    // condition, we can't do it easily
    // because it should be a language feature (since it allows new green code), that can't be
    // released in 1.8.10.
    // So, we're just trying to _partially_ revert the change, by allowing skipping reference not
    // only in case of NO_EXPECTED_TYPE
    // but also in case we have non-trivial expected type AND lastStatement.parent.parent is
    // function literal.
    //
    // That would mean that everything will work just the same as in 1.8.0, but the single case of
    // `expectSomeSpecificFunctionTypeReturnedFromLambda { ::functionNeededAConversion }` (that was
    // been working in 1.7.20).
    //
    // On the KT-55931, currently we don't have plans to fix it until K2 (where it already seems to
    // be fixed).
    if (isCallableReferenceShouldBeAttemptedToBeProcessedByNI(
        statementExpression, context, isUnitExpectedType)) {
    ...
    if (KtPsiUtil.deparenthesize(statementExpression) instanceof KtLambdaExpression
        && context.contextDependency == ContextDependency.DEPENDENT) {
      KotlinTypeInfo typeInfo = createDontCareTypeInfoForNILambda(statementExpression, context);
      if (typeInfo != null) return typeInfo;
    }
    ...
    if (coercionStrategyForLastExpression == COERCION_TO_UNIT) {
      ...
      if (statementExpression instanceof KtDeclaration) {
        if (!(statementExpression instanceof KtNamedFunction)
            || statementExpression.getName() != null) {
          mightBeUnit = true;
        }
      }
  ...
  private static boolean isCallableReferenceShouldBeAttemptedToBeProcessedByNI(
    ...
    // This is condition we added after 1.8.0 to fix KT-55729 (see the comment above its single
    // usage)
    if (!isUnitExpectedType
        && statementExpression.getParent().getParent() instanceof KtFunctionLiteral) return true;
  ...
  private static KotlinTypeInfo createDontCareTypeInfoForNILambda(
      @NotNull KtExpression statementExpression, @NotNull ExpressionTypingContext context) {
    if (!context.languageVersionSettings.supportsFeature(LanguageFeature.NewInference)
        || context.inferenceSession instanceof BuilderInferenceSession) return null;
    KtFunctionLiteral functionLiteral =
        PsiUtilsKt.getNonStrictParentOfType(statementExpression, KtFunctionLiteral.class);
    if (functionLiteral != null) {
      KotlinResolutionCallbacksImpl.LambdaInfo info =
          context
              .trace
              .getBindingContext()
              .get(BindingContext.NEW_INFERENCE_LAMBDA_INFO, functionLiteral);
      if (info != null) {
  ...
  public LocalRedeclarationChecker createLocalRedeclarationChecker(BindingTrace trace) {
    return new TraceBasedLocalRedeclarationChecker(
        trace, expressionTypingComponents.overloadChecker);
  }
}
