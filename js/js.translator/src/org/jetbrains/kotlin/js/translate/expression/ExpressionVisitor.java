...
public final class ExpressionVisitor extends TranslatorVisitor<JsNode> {
  private static final FqName primitiveClassesFqName =
      new FqName("kotlin.reflect.js.internal.PrimitiveClasses");
  ...
  @Override
  @NotNull
  public JsNode visitConstantExpression(
      @NotNull KtConstantExpression expression, @NotNull TranslationContext context) {
    return translateConstantExpression(expression, context).source(expression);
  }

  @NotNull
  private static JsNode translateConstantExpression(
      @NotNull KtConstantExpression expression, @NotNull TranslationContext context) {
    CompileTimeConstant<?> compileTimeValue =
        ConstantExpressionEvaluator.getConstant(expression, context.bindingContext());
    assert compileTimeValue != null
        : message(
            expression, "Expression is not compile time value: " + expression.getText() + " ");

    JsNode result = Translation.translateConstant(compileTimeValue, expression, context);
    if (result == null) {
      throw new AssertionError(
          message(expression, "Unsupported constant expression: " + expression.getText() + " "));
    }
  ...
  @Override
  @NotNull
  public JsNode visitBlockExpression(
      @NotNull KtBlockExpression ktBlockExpression, @NotNull TranslationContext context) {
    ...
    if (statements.isEmpty()) {
      ...
      jsBlock
          .getStatements()
          .add(
              JsAstUtils.asSyntheticStatement(
  ...
  @Override
  public JsNode visitDestructuringDeclaration(
      @NotNull KtDestructuringDeclaration multiDeclaration, @NotNull TranslationContext context) {
    ...
    return DestructuringDeclarationTranslator.translate(
        multiDeclaration, JsAstUtils.pureFqn(parameterName, null), context);
  }

  @Override
  @NotNull
  public JsNode visitReturnExpression(
      @NotNull KtReturnExpression ktReturnExpression, @NotNull TranslationContext context) {
    KtExpression returned = ktReturnExpression.getReturnedExpression();

    // TODO: add related descriptor to context and use it here
    KtDeclarationWithBody parent =
        PsiTreeUtil.getParentOfType(ktReturnExpression, KtDeclarationWithBody.class);
    if (parent instanceof KtSecondaryConstructor) {
      ClassDescriptor classDescriptor = context.getClassDescriptor();
      assert classDescriptor != null
          : "Missing class descriptor in context while translating constructor: "
              + PsiUtilsKt.getTextWithLocation(ktReturnExpression);
      JsExpression ref =
          ReferenceTranslator.translateAsValueReference(
              classDescriptor.getThisAsReceiverParameter(), context);
    ...
    if (returned == null) {
      ...
      assert returnedType != null
          : "Resolved return expression is expected to have type: "
              + PsiUtilsKt.getTextWithLocation(ktReturnExpression);
  ...
  private static FunctionDescriptor getNonLocalReturnTarget(
      @NotNull KtReturnExpression expression, @NotNull TranslationContext context) {
    DeclarationDescriptor descriptor = context.getDeclarationDescriptor();
    assert descriptor instanceof CallableMemberDescriptor
        : "Return expression can only be inside callable declaration: "
    ...
    if (isFunctionLiteral(descriptor) || isFunctionExpression(descriptor)) {
      if (target == null) {
        if (isFunctionLiteral(descriptor)) {
          return BindingContextUtils.getContainingFunctionSkipFunctionLiterals(descriptor, true)
              .getFirst();
        }
      } else {
    ...
    assert descriptor == null || descriptor instanceof FunctionDescriptor
        : "Function descriptor expected to be target of return label: "
            + PsiUtilsKt.getTextWithLocation(expression);
  ...
  @Override
  @NotNull
  public JsNode visitParenthesizedExpression(
      @NotNull KtParenthesizedExpression expression, @NotNull TranslationContext context) {
  ...
  @Override
  @NotNull
  public JsNode visitBinaryExpression(
      @NotNull KtBinaryExpression expression, @NotNull TranslationContext context) {
  ...
  public JsNode visitProperty(@NotNull KtProperty expression, @NotNull TranslationContext context) {
    VariableDescriptor descriptor =
        BindingContextUtils.getNotNull(
            context.bindingContext(), BindingContext.VARIABLE, expression);
    ...
    if (delegateExpression != null) {
      initializer =
          PropertyTranslatorKt.translateDelegateOrInitializerExpression(context, expression);
      ...
      initializer =
          JsAstUtils.wrapValue(alias, initializer == null ? new JsNullLiteral() : initializer);
    }
  ...
  @Override
  @NotNull
  public JsNode visitCallableReferenceExpression(
      @NotNull KtCallableReferenceExpression expression, @NotNull TranslationContext context) {
  ...
  public JsNode visitClassLiteralExpression(
    ...
    if (lhs instanceof DoubleColonLHS.Expression
      ...
      receiver =
          TranslationUtils.coerce(
              context, receiver, context.getCurrentModule().getBuiltIns().getAnyType());
  ...
  @NotNull
  public static JsExpression getObjectKClass(
      @NotNull TranslationContext context, @Nullable ClassifierDescriptor descriptor) {
    JsExpression primitiveExpression = getPrimitiveClass(context, descriptor);
    if (primitiveExpression != null) return primitiveExpression;

    // getKClass should be imported as intrinsic when used outside of inline context, otherwise
    // bootstrap fails.
    // Inside an inline function it should however be marked as SpecialFunction to support T::class
    // when T -> Int (KT-32215)
    ...
    return new JsInvocation(
        kClassName.makeRef(), UtilsKt.getReferenceToJsClass(descriptor, context));
  }

  @Nullable
  public static JsExpression getPrimitiveClass(
      @NotNull TranslationContext context, @Nullable ClassifierDescriptor classifierDescriptor) {
    if (!context.getConfig().isAtLeast(LanguageVersion.KOTLIN_1_2)
        || findPrimitiveClassesObject(context) == null) return null;
    ...
    switch (fqName.asString()) {
      ...
      case "kotlin.DoubleArray":
        return getKotlinPrimitiveClassRef(
            context, StringUtil.decapitalize(fqName.shortName().asString()) + "Class");
      ...
      default:
        {
          if (descriptor instanceof FunctionClassDescriptor) {
            ...
            if (functionClassDescriptor.getFunctionTypeKind()
                == FunctionTypeKind.Function.INSTANCE) {
              ...
              FunctionDescriptor function =
                  DescriptorUtils.getFunctionByName(
                      primitivesObject.getUnsubstitutedMemberScope(),
                      Name.identifier("functionClass"));
              JsExpression functionRef =
                  pureFqn(context.getInlineableInnerNameForDescriptor(function), null);
              return new JsInvocation(
                  functionRef, new JsIntLiteral(functionClassDescriptor.getArity()));
            }
          }
          break;
        }
  ...
  @NotNull
  private static JsExpression getKotlinPrimitiveClassRef(
      @NotNull TranslationContext context, @NotNull String name) {
    ClassDescriptor primitivesObject = findPrimitiveClassesObject(context);
    assert primitivesObject != null;
    PropertyDescriptor property =
        DescriptorUtils.getPropertyByName(
  ...
  private static ClassDescriptor findPrimitiveClassesObject(@NotNull TranslationContext context) {
    return findClassAcrossModuleDependencies(
        context.getCurrentModule(), ClassId.topLevel(primitiveClassesFqName));
  }

  @Override
  @NotNull
  public JsNode visitCallExpression(
      @NotNull KtCallExpression expression, @NotNull TranslationContext context) {
    return CallExpressionTranslator.translate(expression, null, context).source(expression);
  }

  @Override
  @NotNull
  public JsNode visitIfExpression(
      @NotNull KtIfExpression expression, @NotNull TranslationContext context) {
    assert expression.getCondition() != null
        : "condition should not ne null: " + expression.getText();
    JsExpression testExpression =
        Translation.translateAsExpression(expression.getCondition(), context);
    KotlinType type = context.bindingContext().getType(expression);

    boolean isKotlinExpression =
        BindingContextUtilsKt.isUsedAsExpression(expression, context.bindingContext());
    ...
    JsStatement thenStatement =
        thenExpression != null
            ? Translation.translateAsStatementAndMergeInBlockIfNeeded(thenExpression, context)
            : null;
    JsStatement elseStatement =
        elseExpression != null
            ? Translation.translateAsStatementAndMergeInBlockIfNeeded(elseExpression, context)
            : null;
      ...
      if (thenStatement != null) {
        thenStatement =
            LastExpressionMutator.mutateLastExpression(
                thenStatement, new CoercionMutator(type, context));
      }
      if (elseStatement != null) {
        elseStatement =
            LastExpressionMutator.mutateLastExpression(
                elseStatement, new CoercionMutator(type, context));
      }
    }
    ...
    if (isKotlinExpression) {
      ...
      if (canBeJsExpression) {
        return new JsConditional(testExpression, jsThenExpression, jsElseExpression)
            .source(expression);
      }
    }
    if (thenStatement == null) {
  ...
  @Override
  @NotNull
  public JsExpression visitSimpleNameExpression(
  ...
  @Override
  @NotNull
  public JsNode visitWhileExpression(
      @NotNull KtWhileExpression expression, @NotNull TranslationContext context) {
    return LoopTranslator.createWhile(false, expression, context);
  }

  @Override
  @NotNull
  public JsNode visitDoWhileExpression(
      @NotNull KtDoWhileExpression expression, @NotNull TranslationContext context) {
    return LoopTranslator.createWhile(true, expression, context);
  }

  @Override
  @NotNull
  public JsNode visitStringTemplateExpression(
      @NotNull KtStringTemplateExpression expression, @NotNull TranslationContext context) {
  ...
  @NotNull
  private static JsNode resolveAsTemplate(
      @NotNull KtStringTemplateExpression expression, @NotNull TranslationContext context) {
    return StringTemplateTranslator.translate(expression, context);
  }

  @Nullable
  private static JsStringLiteral resolveAsStringConstant(
      @NotNull KtExpression expression, @NotNull TranslationContext context) {
  ...
  @Override
  @NotNull
  public JsNode visitDotQualifiedExpression(
      @NotNull KtDotQualifiedExpression expression, @NotNull TranslationContext context) {
    return QualifiedExpressionTranslator.translateQualifiedExpression(expression, context);
  }

  @Override
  public JsNode visitLabeledExpression(
      @NotNull KtLabeledExpression expression, @NotNull TranslationContext context) {
    ...
    assert scope instanceof JsFunctionScope
        : "Labeled statement is unexpected outside of function scope";
    JsFunctionScope functionScope = (JsFunctionScope) scope;

    String labelIdent = getReferencedName(expression.getTargetLabel());

    JsName labelName =
        functionScope.enterLabel(labelIdent, NameSuggestion.sanitizeName(labelIdent));
  ...
  public JsNode visitPrefixExpression(
      @NotNull KtPrefixExpression expression, @NotNull TranslationContext context) {
    return UnaryOperationTranslator.translate(expression, context).source(expression);
  }

  @Override
  @NotNull
  public JsNode visitPostfixExpression(
      @NotNull KtPostfixExpression expression, @NotNull TranslationContext context) {
    return UnaryOperationTranslator.translate(expression, context).source(expression);
  }

  @Override
  @NotNull
  public JsNode visitIsExpression(
      @NotNull KtIsExpression expression, @NotNull TranslationContext context) {
    return Translation.patternTranslator(context)
        .translateIsExpression(expression)
        .source(expression);
  }

  @Override
  @NotNull
  public JsNode visitSafeQualifiedExpression(
      @NotNull KtSafeQualifiedExpression expression, @NotNull TranslationContext context) {
    return QualifiedExpressionTranslator.translateQualifiedExpression(expression, context)
        .source(expression);
  }
  ...
  @Override
  @Nullable
  public JsNode visitWhenExpression(
  ...
  public JsNode visitBinaryWithTypeRHSExpression(
      @NotNull KtBinaryExpressionWithTypeRHS expression, @NotNull TranslationContext context) {
  ...
  private static JsNameRef getTargetLabel(
      KtExpressionWithLabel expression, TranslationContext context) {
  ...
  @Override
  @NotNull
  public JsNode visitBreakExpression(
      @NotNull KtBreakExpression expression, @NotNull TranslationContext context) {
    return new JsBreak(getTargetLabel(expression, context)).source(expression);
  }

  @Override
  @NotNull
  public JsNode visitContinueExpression(
      @NotNull KtContinueExpression expression, @NotNull TranslationContext context) {
    return new JsContinue(getTargetLabel(expression, context)).source(expression);
  }

  @Override
  @NotNull
  public JsNode visitLambdaExpression(
      @NotNull KtLambdaExpression expression, @NotNull TranslationContext context) {
    return new LiteralFunctionTranslator(context)
        .translate(expression.getFunctionLiteral())
        .source(expression);
  }

  @Override
  @NotNull
  public JsNode visitNamedFunction(
      @NotNull KtNamedFunction expression, @NotNull TranslationContext context) {
    JsExpression alias = new LiteralFunctionTranslator(context).translate(expression);

    FunctionDescriptor descriptor = getFunctionDescriptor(context.bindingContext(), expression);
    JsNameRef nameRef =
        (JsNameRef) ReferenceTranslator.translateAsValueReference(descriptor, context);
  ...
  @Override
  @NotNull
  public JsNode visitThisExpression(
      @NotNull KtThisExpression expression, @NotNull TranslationContext context) {
    DeclarationDescriptor thisExpression =
        getDescriptorForReferenceExpression(
            context.bindingContext(), expression.getInstanceReference());
    assert thisExpression != null
        : "This expression must reference a descriptor: " + expression.getText();

    return context
        .getDispatchReceiver(getReceiverParameterForDeclaration(thisExpression))
        .source(expression);
  }

  @Override
  @NotNull
  public JsNode visitArrayAccessExpression(
      @NotNull KtArrayAccessExpression expression, @NotNull TranslationContext context) {
    return AccessTranslationUtils.translateAsGet(expression, context);
  }

  @Override
  @NotNull
  public JsNode visitSuperExpression(
      @NotNull KtSuperExpression expression, @NotNull TranslationContext context) {
    ResolvedCall<? extends CallableDescriptor> resolvedCall =
        getResolvedCallWithAssert(expression, context.bindingContext());
    return context.getDispatchReceiver(
        (ReceiverParameterDescriptor) resolvedCall.getResultingDescriptor());
  }

  @Override
  @NotNull
  public JsNode visitForExpression(
      @NotNull KtForExpression expression, @NotNull TranslationContext context) {
  ...
  public JsNode visitTryExpression(
      @NotNull KtTryExpression expression, @NotNull TranslationContext context) {
    return new TryTranslator(expression, context).translate();
  }

  @Override
  @NotNull
  public JsNode visitThrowExpression(
      @NotNull KtThrowExpression expression, @NotNull TranslationContext context) {
  ...
  @Override
  @NotNull
  public JsNode visitObjectLiteralExpression(
      @NotNull KtObjectLiteralExpression expression, @NotNull TranslationContext context) {
    ClassDescriptor descriptor =
        BindingUtils.getClassDescriptor(
            context.bindingContext(), expression.getObjectDeclaration());
    ...
    if (closure != null) {
      for (DeclarationDescriptor capturedValue : closure) {
        ...
        if (capturedValue instanceof TypeParameterDescriptor) {
          closureArgs.add(
              context.getTypeArgumentForClosureConstructor(
                  (TypeParameterDescriptor) capturedValue));
        }
      }
    }
    ...
    // we may capture local variables in expressions A, B, etc. We don't want to generate local
    // fields for these variables.
    // Our ClassTranslator is capable of such thing, but case of object expression is a little
    // special.
    ...
    //      fun foo() { object : A(x) }
    //
    // By calling A(x) super constructor we capture `this` explicitly. However, we can't tell which
    // `A::this` we are mentioning,
    // either `this` of an object literal or `this` of enclosing `class A`.
    // Frontend treats it as `this` of enclosing class declaration, therefore it expects backend to
    // generate
    ...
    ResolvedCall<FunctionDescriptor> superCall =
        BindingUtils.getSuperCall(context.bindingContext(), expression.getObjectDeclaration());
    if (superCall != null) {
      closureArgs.addAll(
          CallArgumentTranslator.translate(superCall, null, context).getTranslateArguments());
    }
  ...
  @Override
  public JsNode visitAnnotatedExpression(
      @NotNull KtAnnotatedExpression expression, TranslationContext context) {
    for (KtAnnotationEntry entry : expression.getAnnotationEntries()) {
      AnnotationDescriptor descriptor =
          context.bindingContext().get(BindingContext.ANNOTATION, entry);
