...
public class TrackingSlicedMap extends SlicedMapImpl {
  ...
  private <K, V> SliceWithStackTrace<K, V> wrapSlice(ReadOnlySlice<K, V> slice) {
    return (SliceWithStackTrace)
        sliceTranslationMap.computeIfAbsent(slice, k -> new SliceWithStackTrace<>(slice));
  }
  ...
  public void forEach(@NotNull Function3<WritableSlice, Object, Object, Void> f) {
    super.forEach(
        (slice, key, value) -> {
          f.invoke(
              ((SliceWithStackTrace) slice).getWritableDelegate(),
              key,
  ...
  private class SliceWithStackTrace<K, V> extends AbstractWritableSlice<K, TrackableValue<V>>
    ...
    @Override
    public TrackableValue<V> computeValue(
        SlicedMap map, K key, TrackableValue<V> value, boolean valueNotFound) {
      return new TrackableValue<>(
          delegate.computeValue(map, key, value == null ? null : value.value, valueNotFound),
