...
public class OverridingUtil {
  ...
  public static OverridingUtil create(
    ...
    return new OverridingUtil(
        equalityAxioms, kotlinTypeRefiner, KotlinTypePreparator.Default.INSTANCE, null);
  }
  ...
  public static <D extends CallableDescriptor> Set<D> filterOutOverridden(
    ...
    boolean allowDescriptorCopies =
        !candidateSet.isEmpty()
            && DescriptorUtilsKt.isTypeRefinementEnabled(
                DescriptorUtilsKt.getModule(candidateSet.iterator().next()));

    return filterOverrides(
        candidateSet,
        allowDescriptorCopies,
        null,
        new Function2<D, D, Pair<CallableDescriptor, CallableDescriptor>>() {
          @Override
  ...
  public static <D> Set<D> filterOverrides(
      ...
      @NotNull
          Function2<? super D, ? super D, Pair<CallableDescriptor, CallableDescriptor>>
              transformFirst) {
    ...
    for (D meD : candidateSet) {
      ...
      for (Iterator<D> iterator = result.iterator(); iterator.hasNext(); ) {
        ...
        Pair<CallableDescriptor, CallableDescriptor> meAndOther =
            transformFirst.invoke(meD, otherD);
  ...
  public static <D extends CallableDescriptor> boolean overrides(
    ...
    for (D overriddenFunction : DescriptorUtils.getAllOverriddenDescriptors(f)) {
      if (DescriptorEquivalenceForOverrides.INSTANCE.areEquivalent(
          originalG, overriddenFunction, allowDeclarationCopies, distinguishExpectsAndNonExpects)) {
        return true;
      }
    }
    return false;
  ...
  public OverrideCompatibilityInfo isOverridableByWithoutExternalConditions(
    ...
    TypeCheckerState typeCheckerState =
        createTypeCheckerState(superTypeParameters, subTypeParameters);

    for (int i = 0; i < superTypeParameters.size(); i++) {
      if (!areTypeParametersEquivalent(
          superTypeParameters.get(i), subTypeParameters.get(i), typeCheckerState)) {
    ...
    for (int i = 0; i < superValueParameters.size(); i++) {
      if (!areTypesEquivalent(
          superValueParameters.get(i), subValueParameters.get(i), typeCheckerState)) {
    ...
    if (superDescriptor instanceof FunctionDescriptor
        ...
        && ((FunctionDescriptor) superDescriptor).isSuspend()
            != ((FunctionDescriptor) subDescriptor).isSuspend()) {
    ...
    if (checkReturnType) {
      ...
      if (superReturnType != null && subReturnType != null) {
        boolean bothErrors =
            KotlinTypeKt.isError(subReturnType) && KotlinTypeKt.isError(superReturnType);
        if (!bothErrors
            && !AbstractTypeChecker.INSTANCE.isSubtypeOf(
                typeCheckerState, subReturnType.unwrap(), superReturnType.unwrap())) {
  ...
  private TypeCheckerState createTypeCheckerState(
    ...
    Map<TypeConstructor, TypeConstructor> matchingTypeConstructors =
  ...
  private static boolean areTypesEquivalent(
    ...
    return AbstractTypeChecker.INSTANCE.equalTypes(
        typeCheckerState, typeInSuper.unwrap(), typeInSub.unwrap());
  }
  ...
  public static boolean isVisibleForOverride(
    ...
    return !DescriptorVisibilities.isPrivate(fromSuper.getVisibility())
        && DescriptorVisibilities.isVisibleIgnoringReceiver(
            fromSuper, overriding, useSpecialRulesForPrivateSealedConstructors);
  }
  ...
  private Collection<CallableMemberDescriptor> extractAndBindOverridesForMember(
    ...
    for (CallableMemberDescriptor fromSupertype : descriptorsFromSuper) {
      OverrideCompatibilityInfo.Result result =
          isOverridableBy(fromSupertype, fromCurrent, current).getResult();
  ...
  public static boolean isMoreSpecific(
    ...
    if (a instanceof PropertyDescriptor) {
      ...
      if (pa.isVar() && pb.isVar()) {
        ...
        return AbstractTypeChecker.INSTANCE.equalTypes(
            checkerState, aReturnType.unwrap(), bReturnType.unwrap());
      } else {
  ...
  private static boolean isReturnTypeMoreSpecific(
      ...
      @NotNull TypeCheckerState typeCheckerState) {
    return AbstractTypeChecker.INSTANCE.isSubtypeOf(
        typeCheckerState, aReturnType.unwrap(), bReturnType.unwrap());
  }
  ...
  private static void createAndBindFakeOverride(
    ...
    DescriptorVisibility visibility =
        allInvisible ? DescriptorVisibilities.INVISIBLE_FAKE : DescriptorVisibilities.INHERITED;
  ...
  private static Modality determineModalityForFakeOverride(
      @NotNull Collection<CallableMemberDescriptor> descriptors, @NotNull ClassDescriptor current) {
    ...
    // Fake overrides of abstract members in non-abstract expected classes should not be abstract,
    // because otherwise it would be
    // impossible to inherit a non-expected class from that expected class in common code.
    // We're making their modality that of the containing class, because this is the least confusing
    // behavior for the users.
    // However, it may cause problems if we reuse resolution results of common code when compiling
    // platform code (see KT-15220)
    boolean transformAbstractToClassModality =
        current.isExpect()
            && (current.getModality() != Modality.ABSTRACT
                && current.getModality() != Modality.SEALED);
  ...
  public static void resolveUnknownVisibilityForMember(
    ...
    if (memberDescriptor instanceof PropertyDescriptorImpl) {
      ...
      PropertyAccessorDescriptorImpl propertyAccessorDescriptor =
          (PropertyAccessorDescriptorImpl) memberDescriptor;
      propertyAccessorDescriptor.setVisibility(visibilityToInherit);
      if (visibilityToInherit
          != propertyAccessorDescriptor.getCorrespondingProperty().getVisibility()) {
  ...
  @Nullable
  private static DescriptorVisibility computeVisibilityToInherit(
      @NotNull CallableMemberDescriptor memberDescriptor) {
  ...
  @Nullable
  public static DescriptorVisibility findMaxVisibility(
    ...
    for (CallableMemberDescriptor descriptor : descriptors) {
      ...
      assert visibility != DescriptorVisibilities.INHERITED
    ...
    for (CallableMemberDescriptor descriptor : descriptors) {
      Integer compareResult =
          DescriptorVisibilities.compare(maxVisibility, descriptor.getVisibility());
