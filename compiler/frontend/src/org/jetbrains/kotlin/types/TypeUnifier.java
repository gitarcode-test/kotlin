...
public class TypeUnifier {
   ...
   *
   * <p>Example: known = List<String> withVariables = List<X> variables = {X}
   *
   * <p>result = X -> String
   *
   * <p>Only types accepted by {@code isVariable} are considered variables.
   */
  @NotNull
  ...
  private static void doUnify(
    ...
    if (knownProjectionKind == withVariablesProjectionKind
      ...
      doUnify(
          new TypeProjectionImpl(known), new TypeProjectionImpl(withVariables), isVariable, result);
      return;
    }
    ...
    if (known.isMarkedNullable() && withVariables.isMarkedNullable()) {
      doUnify(
          ...
          new TypeProjectionImpl(
              withVariablesProjectionKind, TypeUtils.makeNotNullable(withVariables)),
          isVariable,
          result);
      return;
    }
    ...
    boolean structuralMismatch =
        known.isMarkedNullable() != withVariables.isMarkedNullable()
  ...
  private static class UnificationResultImpl implements UnificationResult {
    ...
    private final Map<TypeConstructor, TypeProjection> substitution =
        CollectionsKt.newHashMapWithExpectedSize(1);
    private final Set<TypeConstructor> failedVariables =
        CollectionsKt.newHashSetWithExpectedSize(0);
