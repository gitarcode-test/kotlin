...
public class InterceptionInstrumenter {
    ...
    for (Field field : handlerClass.getFields()) {
      ...
      if ((field.getModifiers() & Modifier.STATIC) == 0) {
        throw new IllegalArgumentException(
            "Non-static field annotated @MethodInterceptor: " + field);
      }
      ...
      try {
        ...
        if (enterData.isEmpty() && normalReturnData.isEmpty() && exceptionData.isEmpty()) {
          dumpTasks.add(
              out ->
                  out.println(
                      "WARNING: No relevant methods found in "
                          + field
                          + " of type "
                          + interceptorClass.getCanonicalName()));
        }
        ...
        MethodInstrumenter instrumenter =
  ...
  private byte[] instrument(byte[] classData, List<MethodInstrumenter> instrumenters) {
    ...
    cr.accept(
        new ClassVisitor(API_VERSION, cw) {
          ...
          public MethodVisitor visitMethod(
            ...
            int maxStackDepth =
                getMaxStackDepth(name, desc, normalReturnData, enterData, exceptionData);
            ...
            return new MethodVisitor(API_VERSION, mv) {
              ...
              public void visitCode() {
                for (MethodData methodData : enterData) {
                  // At the very beginning of a constructor, i.e. before any super() call, 'this' is
                  // not available
                  // It's too hard to detect a place right after the super() call, so we just put
                  // null instead of 'this' in such cases
                  invokeMethod(
                      access,
                      cr.getClassName(),
                      name,
                      desc,
                      getInstructionAdapter(),
                      methodData,
                      isConstructor);
                }
                super.visitCode();
                ...
                switch (opcode) {
                  ...
                  case ATHROW:
                    for (MethodData methodData : exceptionData) {
                      // A constructor may throw before calling super(), 'this' is not available in
                      // this case
                      invokeMethod(
                          access,
                          cr.getClassName(),
          ...
          private int getMaxStackDepth(
            ...
            org.jetbrains.org.objectweb.asm.commons.Method methodBeingInstrumented =
                new org.jetbrains.org.objectweb.asm.commons.Method(name, desc);
          ...
          private int stackDepth(
            ...
            return argsSize
                + allArgsStackDepth
                + receiverSize
                + Math.max(returnValueSize, exceptionSize);
          }

          private void checkMultipleMatches(
              MethodInstrumenter instrumenter, String name, String desc) {
          ...
          private TraceMethodVisitor getDumpingVisitorWrapper(
              MethodVisitor mv, String methodName, String methodDesc) {
            return new TraceMethodVisitor(
                mv,
                new Textifier(API_VERSION) {
                  @Override
  ...
  private static org.jetbrains.org.objectweb.asm.commons.Method getAsmMethod(
      MethodData methodData) {
    return new org.jetbrains.org.objectweb.asm.commons.Method(
        methodData.getName(), methodData.getDesc());
  }
  ...
  private static void invokeMethod(
      ...
      for (int i = 0; i < parameterCount; i++) {
        if (i == methodData.getThisParameterIndex()) {
          ...
          for (int parameterIndex = 0;
              parameterIndex < instrumentedMethodParameterTypes.length;
              parameterIndex++) {
            ia.dup();
    ...
    ia.invokevirtual(
        methodData.getDeclaringClass(), methodData.getName(), methodData.getDesc(), false);
  ...
  private static void box(InstructionAdapter ia, Type from, Class<?> boxedClass) {
    ...
    ia.invokestatic(
        ...
        "valueOf",
        "(" + from.getDescriptor() + ")" + boxedType.getDescriptor(),
        false);
  }
