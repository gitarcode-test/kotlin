 ...
 * using the <a href="#enter()">enter()</a> method.
 *
 * <p>The behavior of the execution engine may be altered through methods such as <a
 * href="#setLanguageVersion>setLanguageVersion</a> and <a
...
public class Context {
   ...
   *
   * <p>The Context stores the execution state of the JavaScript engine, so it is required that the
   * context be entered before execution may begin. Once a thread has entered a Context, then
   * getCurrentContext() may be called to find the context that is associated with the current
   * thread.
   *
   * <p>Calling <code>enter()</code> will return either the Context currently associated with the
   * thread, or will create a new context and associate it with the current thread. Each call to
   * <code>enter()</code> must have a matching call to <code>exit()</code>. For example,
   *
   * <pre>
   ...
   *
   * <p>The same as <code>enter()</code> except that <code>cx</code> is associated with the current
   * thread and returned if the current thread has no associated context and <code>cx</code> is not
   ...
   * Context if the prior call to <code>enter()</code> on this thread newly associated a Context
   * with this thread. Once the current thread no longer has an associated Context, it cannot be
   * used to execute JavaScript until it is again associated with a Context.
   */
  ...
  public static void reportWarning(
      ...
      @NotNull CodePosition startPosition,
      @NotNull CodePosition endPosition) {
  ...
  /**
   * if hasFeature(FEATURE_NON_ECMA_GET_YEAR) returns true, Date.prototype.getYear subtructs 1900
   * only if 1900 <= date < 2000 in deviation with Ecma B.2.4
   */
  public static final int FEATURE_NON_ECMA_GET_YEAR = 1;

  /**
   * if hasFeature(FEATURE_MEMBER_EXPR_AS_FUNCTION_NAME) returns true, allow 'function
   * <MemberExpression>(...) { ... }' to be syntax sugar for '<MemberExpression> = function(...) {
   * ... }', when <MemberExpression> is not simply identifier. See Ecma-262, section 11.2 for
   * definition of <MemberExpression>
   */
  public static final int FEATURE_MEMBER_EXPR_AS_FUNCTION_NAME = 2;

  /**
   * if hasFeature(RESERVED_KEYWORD_AS_IDENTIFIER) returns true, treat future reserved keyword (see
   * Ecma-262, section 7.5.3) as ordinary identifiers but warn about this usage
   */
  public static final int FEATURE_RESERVED_KEYWORD_AS_IDENTIFIER = 3;

  /**
   * if hasFeature(FEATURE_TO_STRING_AS_SOURCE) returns true, calling toString on JS objects gives
   * JS source with code to create an object with all enumeratable fields of the original object
   * instead of printing "[object <object-type>]". By default {@link #hasFeature(int)} returns true
   * only if the current JS version is set to {@link #VERSION_1_2}.
   */
