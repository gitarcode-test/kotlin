...
public abstract class AbstractReceiverParameterDescriptor extends DeclarationDescriptorImpl
  ...
  public ReceiverParameterDescriptor substitute(@NotNull TypeSubstitutor substitutor) {
    ...
    if (getContainingDeclaration() instanceof ClassDescriptor) {
      ...
      // So to make receiver with projections be a subtype of parameter's type with captured type
      // arguments,
    ...
    return new ReceiverParameterDescriptorImpl(
        getContainingDeclaration(), new TransientReceiver(substitutedType), getAnnotations());
  }
