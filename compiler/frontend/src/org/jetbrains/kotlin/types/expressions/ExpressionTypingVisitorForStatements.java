...
public class ExpressionTypingVisitorForStatements extends ExpressionTypingVisitor {
  ...
  private KotlinType checkAssignmentType(
    ...
    if (assignmentType != null
        && !KotlinBuiltIns.isUnit(assignmentType)
        && !noExpectedType(context.expectedType)
  ...
  @Override
  public KotlinTypeInfo visitObjectDeclaration(
      @NotNull KtObjectDeclaration declaration, ExpressionTypingContext context) {
    components.localClassifierAnalyzer.processClassOrObject(
        scope,
        context.replaceScope(scope).replaceContextDependency(INDEPENDENT),
        scope.getOwnerDescriptor(),
        declaration);
    return TypeInfoFactoryKt.createTypeInfo(
        components.dataFlowAnalyzer.checkStatementType(declaration, context), context);
  }

  @Override
  public KotlinTypeInfo visitProperty(
      @NotNull KtProperty property, ExpressionTypingContext typingContext) {
    Pair<KotlinTypeInfo, VariableDescriptor> typeInfoAndVariableDescriptor =
        components.localVariableResolver.process(property, typingContext, scope, facade);
    scope.addVariableDescriptor(typeInfoAndVariableDescriptor.getSecond());
    return typeInfoAndVariableDescriptor.getFirst();
  }

  @Override
  public KotlinTypeInfo visitTypeAlias(
      @NotNull KtTypeAlias typeAlias, ExpressionTypingContext context) {
    TypeAliasDescriptor typeAliasDescriptor =
        components.descriptorResolver.resolveTypeAliasDescriptor(
    ...
    facade
        ...
        .checkLocalTypeAliasDeclaration(typeAlias, typeAliasDescriptor);

    return TypeInfoFactoryKt.createTypeInfo(
        components.dataFlowAnalyzer.checkStatementType(typeAlias, context), context);
  }

  @Override
  public KotlinTypeInfo visitDestructuringDeclaration(
      @NotNull KtDestructuringDeclaration multiDeclaration, ExpressionTypingContext context) {
    ...
    components
        .modifiersChecker
        .withTrace(context.trace)
        .checkModifiersForDestructuringDeclaration(multiDeclaration);
    ...
    if (expressionReceiver == null) {
      ...
      return facade
          .getTypeInfo(initializer, context)
          .replaceType(components.dataFlowAnalyzer.checkStatementType(multiDeclaration, context));
    }
  }

  @Override
  public KotlinTypeInfo visitNamedFunction(
      @NotNull KtNamedFunction function, ExpressionTypingContext context) {
    return functions.visitNamedFunction(
        function, context, /* isDeclaration= */ function.getName() != null, scope);
  }
  ...
  public KotlinTypeInfo visitClass(@NotNull KtClass klass, ExpressionTypingContext context) {
    components.localClassifierAnalyzer.processClassOrObject(
        ...
        klass);
    return TypeInfoFactoryKt.createTypeInfo(
        components.dataFlowAnalyzer.checkStatementType(klass, context), context);
  }
  ...
  public KotlinTypeInfo visitDeclaration(
    ...
    return TypeInfoFactoryKt.createTypeInfo(
        components.dataFlowAnalyzer.checkStatementType(dcl, context), context);
  }

  @Override
  public KotlinTypeInfo visitBinaryExpression(
      @NotNull KtBinaryExpression expression, ExpressionTypingContext context) {
  ...
  @NotNull
  protected KotlinTypeInfo visitAssignmentOperation(
      KtBinaryExpression expression, ExpressionTypingContext contextWithExpectedType) {
    ...
    TemporaryTraceAndCache temporary =
    ...
    KotlinTypeInfo leftInfo =
        ExpressionTypingUtils.getTypeInfoOrNullType(leftOperand, context, facade);
    ...
    if (leftType == null) {
      ...
      // Cache the type info for the right hand side so that we don't evaluate it twice if there is
      // no valid plusAssign.
      // We skip over function literals and references, since ArgumentTypeResolver will only resolve
      // the shape of the
    ...
    ExpressionReceiver receiver =
        ExpressionReceiver.Companion.create(left, leftType, context.trace.getBindingContext());
    ...
    TemporaryTraceAndCache temporaryForAssignmentOperation =
        TemporaryTraceAndCache.create(
    ...
    OverloadResolutionResults<FunctionDescriptor> assignmentOperationDescriptors =
        components.callResolver.resolveBinaryCall(
            ...
            expression,
            name);
    KotlinType assignmentOperationType =
        OverloadResolutionResultsUtil.getResultingType(assignmentOperationDescriptors, context);

    OverloadResolutionResults<FunctionDescriptor> binaryOperationDescriptors;
    KotlinType binaryOperationType;
    TemporaryTraceAndCache temporaryForBinaryOperation =
        TemporaryTraceAndCache.create(
            context, "trace to check binary operation like '+' for", expression);
    TemporaryBindingTrace ignoreReportsTrace =
        TemporaryBindingTrace.create(context.trace, "Trace for checking assignability");
    ExpressionTypingContext contextForBinaryOperation = null;

    boolean lhsAssignable =
        basic.checkLValue(ignoreReportsTrace, context, left, right, expression, false);
    ...
    if (assignmentOperationType == null || lhsAssignable) {
      ...
      // Check for '+'
      // We should clear calls info for coroutine inference within right side as here we analyze it
      // a second time in another context
      if (context.inferenceSession instanceof BuilderInferenceSession) {
        ((BuilderInferenceSession) context.inferenceSession)
            .clearCallsInfoByContainingElement(right);
      }
      ...
      binaryOperationDescriptors =
          components.callResolver.resolveBinaryCall(
              contextForBinaryOperation, receiver, expression, counterpartName);

      binaryOperationType =
          OverloadResolutionResultsUtil.getResultingType(binaryOperationDescriptors, context);
    } else {
    ...
    KotlinType type =
        assignmentOperationType != null ? assignmentOperationType : binaryOperationType;
    KotlinTypeInfo rightInfo = leftInfo;

    boolean hasRemAssignOperation =
        atLeastOneOperation(
            assignmentOperationDescriptors.getResultingCalls(), OperatorNameConventions.REM_ASSIGN);
    boolean hasRemBinaryOperation =
        atLeastOneOperation(
            binaryOperationDescriptors.getResultingCalls(), OperatorNameConventions.REM);

    boolean oneTypeOfModRemOperations = hasRemAssignOperation == hasRemBinaryOperation;

    boolean maybeAmbiguity =
        assignmentOperationDescriptors.isSuccess()
            && binaryOperationDescriptors.isSuccess()
            && oneTypeOfModRemOperations;
    boolean isResolvedToPlusAssign =
        assignmentOperationType != null
            && (assignmentOperationDescriptors.isSuccess()
                || !binaryOperationDescriptors.isSuccess())
            && (!hasRemBinaryOperation || !binaryOperationDescriptors.isSuccess());

    KotlinTypeInfo rhsResolutionResult;
    // We complete resolution for 'plus' only if there may be ambiguity (in this case we can
    // disambiguate it),
    // or it definitely won't be resolved to plus assign (in this case we would analyse right side
    // twice)
    if (maybeAmbiguity || !isResolvedToPlusAssign) {
      rhsResolutionResult =
          completePlusResolution(
              contextForBinaryOperation, expression, binaryOperationType, left, leftInfo);
    } else {
    ...
    if (maybeAmbiguity && rhsResolutionResult != null) {
      ...
      if (!KotlinTypeChecker.DEFAULT.equalTypes(
          components.builtIns.getUnitType(), assignmentOperationType)) {
        context.trace.report(
          ...
          context
              ...
              .replaceBindingTrace(
                  TemporaryBindingTrace.create(
                      ...
                      expression));
      basic.resolveImplicitArrayAccessSetMethod(
          (KtArrayAccessExpression) leftDeparentized,
          rightOperand,
          contextForResolve,
          context.trace);
    }
    KotlinTypeInfo rightInfo =
        facade.getTypeInfo(rightOperand, context.replaceDataFlowInfo(leftInfo.getDataFlowInfo()));

    boolean refineJavaFieldInTypeProperly =
        components.languageVersionSettings.supportsFeature(
            LanguageFeature.RefineTypeCheckingOnAssignmentsToJavaFields);

    BindingContext bindingContext = context.trace.getBindingContext();
    KotlinType leftType = leftInfo.getType();

    KotlinType expectedType =
        refineJavaFieldInTypeProperly
  ...
  private static boolean atLeastOneOperation(
      Collection<? extends ResolvedCall<FunctionDescriptor>> calls, Name operationName) {
  ...
  private static KotlinType refineTypeFromPropertySetterIfPossible(
    ...
    VariableDescriptor descriptor =
        BindingContextUtils.extractVariableFromResolvedCall(bindingContext, leftOperand);
  ...
  @NotNull
  protected KotlinTypeInfo visitAssignment(
      KtBinaryExpression expression, ExpressionTypingContext contextWithExpectedType) {
    ExpressionTypingContext context =
        contextWithExpectedType
            .replaceExpectedType(NO_EXPECTED_TYPE)
    ...
    KotlinTypeInfo leftInfo =
        ExpressionTypingUtils.getTypeInfoOrNullType(
            left,
    ...
    List<AssignResolutionAltererExtension> assignAlterers =
        AssignResolutionAltererExtension.Companion.getInstances(expression.getProject());
    if (!assignAlterers.isEmpty()) {
      KotlinTypeInfo alteredTypeInfo =
          assignAlterers.stream()
    ...
    if (right != null) {
      ...
      if (left != null && expectedType != null && rightType != null) {
        DataFlowValue leftValue =
            components.dataFlowValueFactory.createDataFlowValue(left, expectedType, context);
        DataFlowValue rightValue =
            components.dataFlowValueFactory.createDataFlowValue(right, rightType, context);
        // We cannot say here anything new about rightValue except it has the same value as
        // leftValue
        resultInfo =
            resultInfo.replaceDataFlowInfo(
                dataFlowInfo.assign(leftValue, rightValue, components.languageVersionSettings));
        NewSchemeOfIntegerOperatorResolutionChecker.checkArgument(
            expectedType, right, context.trace, components.moduleDescriptor);
      }
    } else {
      resultInfo = leftInfo;
    }
    if (expectedType != null
        && leftOperand != null) { // if expectedType == null, some other error has been generated
  ...
  private void checkPropertyInTypeWithWarnings(
    ...
    KotlinType expectedTypeByInType =
        refineTypeByPropertyInType(context.trace.getBindingContext(), lhsOperand, lhsType);

    if (expectedTypeByInType != null
        && expectedType != expectedTypeByInType
      ...
      components.dataFlowAnalyzer.checkType(
          ...
          expression,
          context
              .replaceExpectedType(expectedTypeByInType)
  ...
  @Override
  public KotlinTypeInfo visitExpression(
      @NotNull KtExpression expression, ExpressionTypingContext context) {
    return facade.getTypeInfo(expression, context);
  }
  ...
  public KotlinTypeInfo visitIfExpression(
      @NotNull KtIfExpression expression, ExpressionTypingContext context) {
    return controlStructures.visitIfExpression(expression, context);
  }

  @Override
  public KotlinTypeInfo visitWhenExpression(
      @NotNull KtWhenExpression expression, ExpressionTypingContext context) {
