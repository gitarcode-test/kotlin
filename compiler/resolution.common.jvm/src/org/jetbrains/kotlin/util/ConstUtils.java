...
// 1. In `visitReferenceExpression` where we check for constant expression without triggering
// evaluation.
// 2. In `visitPolyadicExpression` before we check for the type.
// At the moment when we check for constant, not all types could be resolved, and if we return a
// wrong type, it will be cached.
final class IsConstantExpressionVisitor extends JavaElementVisitor {
  ...
  public void visitPrefixExpression(PsiPrefixExpression expression) {
    ...
    if (opType == JavaTokenType.PLUS
        ...
        || opType == JavaTokenType.TILDE
        || opType == JavaTokenType.EXCL) {
      return;
    }
    myIsConstant = false;
  }
