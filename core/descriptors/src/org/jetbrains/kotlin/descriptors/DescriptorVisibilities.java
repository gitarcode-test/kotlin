...
public class DescriptorVisibilities {
      ...
      new DelegatedDescriptorVisibility(Visibilities.PrivateToThis.INSTANCE) {
        ...
        public boolean isVisible(
          ...
          if (PRIVATE.isVisible(
              thisObject, what, from, useSpecialRulesForPrivateSealedConstructors)) {
            ...
            if (classDescriptor != null && thisObject instanceof ThisClassReceiver) {
              return ((ThisClassReceiver) thisObject)
                  .getClassDescriptor()
                  .getOriginal()
                  .equals(classDescriptor.getOriginal());
            }
          }
          return false;
        }
  ...
  public static final DescriptorVisibility PROTECTED =
      new DelegatedDescriptorVisibility(Visibilities.Protected.INSTANCE) {
        ...
        public boolean isVisible(
          ...
          if (givenDescriptorContainingClass != null
              && DescriptorUtils.isCompanionObject(givenDescriptorContainingClass)) {
            ...
            ClassDescriptor companionOwner =
                DescriptorUtils.getParentOfType(
                    givenDescriptorContainingClass, ClassDescriptor.class);
            if (companionOwner != null && DescriptorUtils.isSubclass(fromClass, companionOwner))
              return true;
          }

          // The rest part of method checks visibility similarly to Java does for protected (see JLS
          // p.6.6.2)

          // Protected fake overrides can have only one protected overridden (as protected is not
          // allowed for interface members)
          DeclarationDescriptorWithVisibility whatDeclaration =
              DescriptorUtils.unwrapFakeOverrideToAnyDeclaration(what);

          ClassDescriptor classDescriptor =
              DescriptorUtils.getParentOfType(whatDeclaration, ClassDescriptor.class);
          ...
          return isVisible(
              receiver,
              what,
              fromClass.getContainingDeclaration(),
  ...
  public static final DescriptorVisibility INTERNAL =
      new DelegatedDescriptorVisibility(Visibilities.Internal.INSTANCE) {
        ...
        public boolean isVisible(
          ...
          // Can't invert this condition because CLI compiler analyzes sources as like all in the
          // one module
      ...
      new DelegatedDescriptorVisibility(Visibilities.Inherited.INSTANCE) {
        ...
        public boolean isVisible(
          ...
          throw new IllegalStateException(
              "Visibility is unknown yet"); // This method shouldn't be invoked for INHERITED
                                            // visibility
        }
      };
  ...
  public static boolean isVisible(
      ...
      @NotNull DeclarationDescriptor from,
      boolean useSpecialRulesForPrivateSealedConstructors) {
    return findInvisibleMember(receiver, what, from, useSpecialRulesForPrivateSealedConstructors)
        == null;
  }
  ...
  public static boolean isVisibleWithAnyReceiver(
      ...
      boolean useSpecialRulesForPrivateSealedConstructors) {
    return findInvisibleMember(
            IRRELEVANT_RECEIVER, what, from, useSpecialRulesForPrivateSealedConstructors)
        == null;
  }
