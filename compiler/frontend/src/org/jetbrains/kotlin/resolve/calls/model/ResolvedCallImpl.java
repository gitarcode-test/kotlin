...
public class ResolvedCallImpl<D extends CallableDescriptor> implements MutableResolvedCall<D> {
  ...
  @NotNull private Map<TypeParameterDescriptor, KotlinType> typeArguments;
  @NotNull private final Map<ValueParameterDescriptor, ResolvedValueArgument> valueArguments;
  private final MutableDataFlowInfoForArguments dataFlowInfoForArguments;
  @NotNull private final Map<ValueArgument, ArgumentMatchImpl> argumentToParameterMap;
  ...
  @NotNull
  private static Map<ValueParameterDescriptor, ResolvedValueArgument> createValueArgumentsMap(
  ...
  @NotNull
  private static Map<ValueArgument, ArgumentMatchImpl> createArgumentsToParameterMap(
      CallableDescriptor descriptor) {
    return descriptor.getValueParameters().isEmpty() ? Collections.emptyMap() : new HashMap<>();
  }

  @NotNull
  private static Map<TypeParameterDescriptor, KotlinType> createTypeArgumentsMap(
      CallableDescriptor descriptor) {
  ...
  public void setResultingSubstitutor(@NotNull TypeSubstitutor substitutor) {
    D descriptorToSubstitute =
        resultingDescriptor != null
                && DescriptorUtilsKt.shouldBeSubstituteWithStubTypes(resultingDescriptor)
            ? resultingDescriptor
            : candidateDescriptor;
  ...
  public void setResolvedCallSubstitutor(@NotNull TypeSubstitutor substitutor) {
    for (TypeParameterDescriptor typeParameter : candidateDescriptor.getTypeParameters()) {
      TypeProjection typeArgumentProjection =
          substitutor.getSubstitution().get(typeParameter.getDefaultType());
    ...
    typeArguments =
        typeArguments.entrySet().stream()
            .collect(
                Collectors.toMap(
                    Map.Entry::getKey,
                    e -> substitutor.safeSubstitute(e.getValue(), e.getKey().getVariance())));

    if (dispatchReceiver instanceof ExpressionReceiver) {
      dispatchReceiver =
          dispatchReceiver.replaceType(
              substitutor.safeSubstitute(dispatchReceiver.getType(), Variance.IN_VARIANCE));
    }
    if (extensionReceiver instanceof ExtensionReceiver) {
      extensionReceiver =
          extensionReceiver.replaceType(
              substitutor.safeSubstitute(extensionReceiver.getType(), Variance.IN_VARIANCE));
    }
    ...
    Collection<Map.Entry<ValueParameterDescriptor, ResolvedValueArgument>>
        valueArgumentsBeforeSubstitution = new SmartList<>(valueArguments.entrySet());

    valueArguments.clear();

    for (Map.Entry<ValueParameterDescriptor, ResolvedValueArgument> entry :
        valueArgumentsBeforeSubstitution) {
      ValueParameterDescriptor substitutedVersion =
          substitutedParameters.get(entry.getKey().getIndex());
    ...
    for (Map.Entry<ValueArgument, ArgumentMatchImpl> entry : unsubstitutedArgumentMappings) {
      ...
      ValueParameterDescriptor substitutedVersion =
          substitutedParameters.get(valueParameterDescriptor.getIndex());
      assert substitutedVersion != null : valueParameterDescriptor;
      argumentToParameterMap.put(
          entry.getKey(), argumentMatch.replaceValueParameter(substitutedVersion));
    }
  }
  ...
  @Override
  public void recordValueArgument(
      @NotNull ValueParameterDescriptor valueParameter,
      @NotNull ResolvedValueArgument valueArgument) {
  ...
  public List<ResolvedValueArgument> getValueArgumentsByIndex() {
    List<ResolvedValueArgument> arguments =
        new ArrayList<>(candidateDescriptor.getValueParameters().size());
    for (int i = 0; i < candidateDescriptor.getValueParameters().size(); ++i) {
      arguments.add(null);
    }

    for (Map.Entry<ValueParameterDescriptor, ResolvedValueArgument> entry :
        valueArguments.entrySet()) {
  ...
  @Override
  public void recordArgumentMatchStatus(
      @NotNull ValueArgument valueArgument, @NotNull ArgumentMatchStatus matchStatus) {
  ...
  public ArgumentMapping getArgumentMapping(@NotNull ValueArgument valueArgument) {
    ...
    if (argumentMatch == null) {
      if (ArgumentMappingKt.isReallySuccess(this)) {
        LOG.error(
            "ArgumentUnmapped for "
                + valueArgument
                + " in successfully resolved call: "
                + call.getCallElement().getText());
      }
  ...
  public boolean hasInferredReturnType() {
    ...
    assert hasInferredReturnType != null
  ...
  @Override
  public void updateExtensionReceiverWithSmartCastIfNeeded(
      @NotNull KotlinType smartCastExtensionReceiverType) {
    if (extensionReceiver instanceof ImplicitClassReceiver) {
      extensionReceiver =
          new CastImplicitClassReceiver(
