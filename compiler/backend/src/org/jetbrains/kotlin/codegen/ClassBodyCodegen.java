...
public abstract class ClassBodyCodegen extends MemberCodegen<KtPureClassOrObject> {
  @NotNull public final KtPureClassOrObject myClass;

  @NotNull public final OwnerKind kind;

  @NotNull public final ClassDescriptor descriptor;
  ...
  protected ClassBodyCodegen(
    ...
    this.descriptor =
        SyntheticClassOrObjectDescriptorKt.findClassDescriptor(myClass, bindingContext);
  }
  ...
  protected void generateBody() {
    ...
    if (kind != OwnerKind.DEFAULT_IMPLS) {
      Collection<DeclarationDescriptor> classifiers =
          descriptor
  ...
  protected void generateDeclaration(KtDeclaration declaration) {
    if (declaration instanceof KtProperty
        ...
        || declaration instanceof KtTypeAlias) {
      if (shouldGenerateNonClassMembers()) {
        genSimpleMember(declaration);
      }
    } else if (declaration instanceof KtClassOrObject) {
      if (declaration instanceof KtEnumEntry
          && !enumEntryNeedSubclass(bindingContext, (KtEnumEntry) declaration)) {
        return;
      }
  ...
  private void generatePrimaryConstructorProperties() {
    ClassConstructorDescriptor constructor =
        CollectionsKt.firstOrNull(descriptor.getConstructors());
  ...
  protected void generateDelegatesToDefaultImpl() {
    ...
    JvmKotlinType receiverType =
        new JvmKotlinType(typeMapper.mapType(descriptor), descriptor.getDefaultType());

    for (Map.Entry<FunctionDescriptor, FunctionDescriptor> entry :
        CodegenUtil.getNonPrivateTraitMethods(descriptor).entrySet()) {
  ...
  public static void generateDelegationToDefaultImpl(
    ...
    CallableMemberDescriptor actualImplementation =
        interfaceFun.getKind().isReal()
            ? interfaceFun
            : findImplementationFromInterface(interfaceFun);
    ...
    if (CodegenUtilKt.isDefinitelyNotDefaultImplsMethod(actualImplementation)
        || JvmAnnotationUtilKt.isCallableMemberCompiledToJvmDefault(
            actualImplementation, state.getJvmDefaultMode())) {
      return;
    }
    ...
    functionCodegen.generateMethod(
        new JvmDeclarationOrigin(
            ...
            descriptorToDeclaration(interfaceFun),
            interfaceFun,
            null),
        inheritedFun,
        new FunctionGenerationStrategy.CodegenBased(state) {
          ...
          public void doGenerateBody(
            ...
            Method traitMethod =
                typeMapper.mapAsmMethod(originalInterfaceFun, OwnerKind.DEFAULT_IMPLS);

            putArgumentsOnStack(codegen, signature, traitMethod);
            InstructionAdapter iv = codegen.v;

            if (KotlinBuiltIns.isCloneable(containingTrait)
                && traitMethod.getName().equals("clone")) {
              // A special hack for Cloneable: there's no kotlin/Cloneable$DefaultImpls class at
              // runtime,
              ...
              iv.invokestatic(
                  traitImplType.getInternalName(),
                  traitMethod.getName(),
                  traitMethod.getDescriptor(),
                  false);
            }

            Type returnType = signature.getReturnType();
            StackValue.onStack(traitMethod.getReturnType(), originalInterfaceFun.getReturnType())
                .put(returnType, iv);
          ...
          private void putArgumentsOnStack(
            ...
            KotlinType interfaceKotlinType =
                ((ClassDescriptor) inheritedFun.getContainingDeclaration()).getDefaultType();
            ...
            assert myParameters.size() == toParameters.size()
                : "Inconsistent value parameters between delegating fun "
                    + inheritedFun
                    + "and interface fun "
                    + interfaceFun;
            ...
            for (; myArgI < myArgTypes.length; myArgI++, toArgI++) {
              ...
              KotlinType myArgKotlinType =
                  myParametersIterator.hasNext() ? myParametersIterator.next().getType() : null;
              KotlinType toArgKotlinType =
                  toParametersIterator.hasNext() ? toParametersIterator.next().getType() : null;
            ...
            assert toArgI == toArgTypes.length
                : "Invalid trait implementation signature: "
                    + signature
                    + " vs "
                    + defaultImplsMethod
                    + " for "
