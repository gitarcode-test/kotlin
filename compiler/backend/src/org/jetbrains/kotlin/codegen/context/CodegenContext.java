...
public abstract class CodegenContext<T extends DeclarationDescriptor> {
  ...
  private final Map<ClassDescriptor, AccessorForCompanionObjectInstanceFieldDescriptor>
      accessorsForCompanionObjects = new LinkedHashMap<>();
  ...
  private static class AccessorForPropertyDescriptorFactory {
    ...
    public PropertyDescriptor getOrCreateAccessorIfNeeded(
        boolean getterAccessorRequired, boolean setterAccessorRequired) {
      if (getterAccessorRequired && setterAccessorRequired) {
        ...
        if (withSyntheticGetter == null) {
          withSyntheticGetter =
              new AccessorForPropertyDescriptor(
                  property,
  ...
  public CodegenContext(
    ...
    this.outerExpression =
        LockBasedStorageManager.NO_LOCKS.createNullableLazyValue(this::computeOuterExpression);
  ...
  public CodegenContext<? extends ClassOrPackageFragmentDescriptor>
  ...
  /**
   * This method returns not null only if context descriptor corresponds to method or function which
   * has receiver
   */
  @Nullable
  ...
  private StackValue getOuterExpression(
      @Nullable StackValue prefix, boolean ignoreNoOuter, boolean captureThis) {
    if (outerExpression.invoke() == null) {
      if (!ignoreNoOuter) {
        throw new UnsupportedOperationException(
            "Don't know how to generate outer expression: " + this);
      }
      return null;
    }
    if (captureThis) {
  ...
  @NotNull
  public PackageContext intoPackagePart(
  ...
  public MultifileClassPartContext intoMultifileClassPart(
    ...
    return new MultifileClassPartContext(
        descriptor, this, multifileClassType, filePartType, sourceFile);
  }
  ...
  public ClassContext intoDefaultImplsClass(
      ClassDescriptor descriptor, ClassContext interfaceContext, GenerationState state) {
    return new DefaultImplsClassContext(
        state.getTypeMapper(), descriptor, OwnerKind.DEFAULT_IMPLS, this, null, interfaceContext);
  }

  @NotNull
  public ClassContext intoClass(
    ...
    if (shouldAddChild(descriptor)) {
      ...
      if (savedContext != null) {
        assert savedContext.getContextKind() == kind
            : "Kinds should be same, but: " + savedContext.getContextKind() + "!= " + kind;
        return (ClassContext) savedContext;
      }
    }

    ClassContext classContext =
        new ClassContext(state.getTypeMapper(), descriptor, kind, this, null);
    ...
    if (descriptor.getCompanionObjectDescriptor() != null) {
      ...
      // object
      classContext.intoClass(
          descriptor.getCompanionObjectDescriptor(), OwnerKind.IMPLEMENTATION, state);
    }
    return classContext;
  }
  ...
  public ClassContext intoAnonymousClass(
    ...
    return new AnonymousClassContext(
        codegen.getState().getTypeMapper(), descriptor, ownerKind, this, codegen);
  }

  @NotNull
  public MethodContext intoFunction(
      FunctionDescriptor descriptor, boolean isDefaultFunctionContext) {
  ...
  @NotNull
  public MethodContext intoInlinedLambda(
      FunctionDescriptor descriptor, boolean isCrossInline, boolean isPropertyReference) {
    return new InlineLambdaContext(
        descriptor, getContextKind(), this, null, isCrossInline, isPropertyReference);
  }

  @NotNull
  public ConstructorContext intoConstructor(
      @NotNull ConstructorDescriptor descriptor, @NotNull KotlinTypeMapper typeMapper) {
  ...
  public ClosureContext intoCoroutineClosure(
    ...
    return new ClosureContext(
        typeMapper, jvmViewOfSuspendLambda, this, localLookup, originalSuspendLambdaDescriptor);
  }

  public ClassContext intoWrapperForErasedInlineClass(
      ClassDescriptor descriptor, GenerationState state) {
    return new ClassContext(
        state.getTypeMapper(), descriptor, OwnerKind.ERASED_INLINE_CLASS, this, null);
  }
  ...
  private PropertyDescriptor getPropertyAccessor(
    ...
    return getAccessor(
        ...
        setterAccessorRequired);
  }

  @SuppressWarnings("unchecked")
  public <D extends CallableMemberDescriptor> D getAccessorForJvmDefaultCompatibility(
      @NotNull D descriptor) {
    if (descriptor instanceof PropertyAccessorDescriptor) {
      PropertyDescriptor propertyAccessor =
          getAccessor(
      ...
      return descriptor instanceof PropertyGetterDescriptor
          ? (D) propertyAccessor.getGetter()
          : (D) propertyAccessor.getSetter();
    }
  ...
  private <D extends CallableMemberDescriptor> D getAccessor(
      @NotNull D descriptor, @Nullable ClassDescriptor superCallTarget) {
  ...
  public <D extends CallableMemberDescriptor> D getAccessorForSuperCallIfNeeded(
      ...
      @Nullable ClassDescriptor superCallTarget,
      @NotNull GenerationState state) {
    if (superCallTarget != null
        && !isNonDefaultInterfaceMember(descriptor, state.getJvmDefaultMode())) {
  ...
  public <D extends CallableMemberDescriptor> D getAccessor(
      ...
      @Nullable ClassDescriptor superCallTarget) {
    // TODO this corresponds to default behavior for properties before fixing KT-9717. Is it Ok in
    // general case?
    // Does not matter for other descriptor kinds.
    return getAccessor(
        possiblySubstitutedDescriptor,
        accessorKind,
        delegateType,
        superCallTarget,
  ...
  private <D extends CallableMemberDescriptor> D getAccessor(
    ...
    // NB should check for property accessor factory first (or change property accessor tracking
    // under propertyAccessorFactory creation)
    AccessorForPropertyDescriptorFactory propertyAccessorFactory =
        propertyAccessorFactories.get(key);
    if (propertyAccessorFactory != null) {
      return (D)
          propertyAccessorFactory.getOrCreateAccessorIfNeeded(
    ...
    if (accessor != null) {
      assert accessorKind == AccessorKind.NORMAL
          ...
          : "There is already exists accessor with isForBackingField = false in this context";
      return (D) accessor;
    }

    String nameSuffix =
        SyntheticAccessorUtilKt.getAccessorNameSuffix(
            descriptor, key.superCallLabelTarget, accessorKind);
    if (descriptor instanceof SimpleFunctionDescriptor) {
      accessor =
          new AccessorForFunctionDescriptor(
      ...
      accessor =
          new AccessorForConstructorDescriptor(
      ...
      if (accessorKind == AccessorKind.NORMAL
          || accessorKind == AccessorKind.JVM_DEFAULT_COMPATIBILITY) {
        AccessorForPropertyDescriptorFactory factory =
            new AccessorForPropertyDescriptorFactory(
                propertyDescriptor, contextDescriptor, superCallTarget, nameSuffix, accessorKind);
        ...
        return (D)
            factory.getOrCreateAccessorIfNeeded(getterAccessorRequired, setterAccessorRequired);
      }

      accessor =
          new AccessorForPropertyBackingField(
              propertyDescriptor,
              contextDescriptor,
              delegateType,
              accessorKind == AccessorKind.IN_CLASS_COMPANION
                  ? null
                  : propertyDescriptor.getExtensionReceiverParameter(),
              accessorKind == AccessorKind.IN_CLASS_COMPANION
                  ? null
                  : propertyDescriptor.getDispatchReceiverParameter(),
              nameSuffix,
              accessorKind);
    } else {
      throw new UnsupportedOperationException(
  ...
  public StackValue lookupInContext(
    ...
    if (closure != null) {
      ...
      if (capturedVariable != null) {
        return StackValue.changeReceiverForFieldAndSharedVar(
            capturedVariable.getInnerValue(), result);
      }
    ...
    if (myOuter != null && getEnclosingClass() == d) {
      ...
      resultValue =
          enclosingClassContext != null
              ? enclosingClassContext.lookupInContext(d, result, state, ignoreNoOuter)
              : null;
    }
  ...
  public Collection<? extends AccessorForCallableDescriptor<?>> getAccessors() {
    return accessors == null
        ? Collections.<AccessorForCallableDescriptor<CallableMemberDescriptor>>emptySet()
        : accessors.values();
  }
  ...
  public <D extends CallableMemberDescriptor> D accessibleDescriptor(
    ...
    boolean sameJvmDefault =
        JvmAnnotationUtilKt.isCompiledToJvmDefault(descriptor, getState().getJvmDefaultMode())
                == isCallableMemberCompiledToJvmDefault(
                    properContext.contextDescriptor, getState().getJvmDefaultMode())
    ...
    return (D)
        properContext.accessibleDescriptorIfNeeded(descriptor, superCallTarget, isInliningContext);
  }
  ...
  private <D extends CallableMemberDescriptor> D accessibleDescriptorIfNeeded(
    ...
    if (descriptorContext == null && DescriptorUtils.isCompanionObject(enclosed)) {
      CodegenContext classContext =
          findParentContextWithDescriptor(enclosed.getContainingDeclaration());
    ...
    if (descriptorContext == null
      ...
      descriptorContext =
          ExpressionCodegen.getParentContextSubclassOf((ClassDescriptor) enclosed, this);
    ...
    if (descriptorContext == null && descriptor instanceof ClassConstructorDescriptor) {
      ClassDescriptor classDescriptor =
          ((ClassConstructorDescriptor) descriptor).getContainingDeclaration();
      if (DescriptorUtils.isSealedClass(classDescriptor)) {
        CodegenContext parentContextForClass =
            findParentContextWithDescriptor(classDescriptor.getContainingDeclaration());
    ...
    if (descriptor instanceof PropertyDescriptor) {
      ...
      if (setter != null
          && setter.getVisibility().normalize() != DescriptorVisibilities.INVISIBLE_FAKE) {
      ...
      return (D)
          descriptorContext.getPropertyAccessor(
  ...
  private static boolean isAccessorRequired(
    ...
    return isSuperCall && withinInline
        ...
        || ((accessFlag & ACC_PROTECTED) != 0
            && (withinInline
                || !isInSamePackage(
                    unwrappedDescriptor, descriptorContext.getContextDescriptor())));
  }
  ...
  private static boolean shouldAddChild(@NotNull DeclarationDescriptor childContextDescriptor) {
    return DescriptorUtils.isCompanionObject(childContextDescriptor)
        || DescriptorUtils.isSealedClass(childContextDescriptor);
  }
    ...
    return accessorsForCompanionObjects.computeIfAbsent(
        ...
        it ->
            new AccessorForCompanionObjectInstanceFieldDescriptor(
                it, Name.identifier(JvmCodegenUtil.getCompanionObjectAccessorName(it))));
  }

  @NotNull
  public Collection<AccessorForCompanionObjectInstanceFieldDescriptor>
      getRequiredAccessorsForCompanionObjects() {
