...
public class JavaSingleAbstractMethodUtils {
  ...
  public static boolean isSamClassDescriptor(@NotNull ClassDescriptor descriptor) {
    ...
    if (descriptor instanceof LazyJavaClassDescriptor
        && descriptor.getDefaultFunctionTypeForSamInterface() != null) return true;
  ...
  public static SamAdapterDescriptor<JavaClassConstructorDescriptor> createSamAdapterConstructor(
      ...
      @NotNull SamConversionOracle samConversionOracle,
      boolean allowNonSpreadArraysForVarargAfterSam) {
    SamAdapterClassConstructorDescriptor result =
        new SamAdapterClassConstructorDescriptor(original);
    return initSamAdapter(
        original,
        result,
        new FunctionInitializer() {
  ...
  private static <F extends FunctionDescriptor> SamAdapterDescriptor<F> initSamAdapter(
    ...
    if (adapter instanceof SamAdapterClassConstructorDescriptor) {
      typeParameters =
          new SamConstructorTypeParameters(original.getTypeParameters(), TypeSubstitutor.EMPTY);
    } else {
      typeParameters =
          SamConstructorUtilsKt.recreateAndInitializeTypeParameters(
              original.getTypeParameters(), adapter);
    }

    KotlinType returnTypeUnsubstituted = original.getReturnType();
    assert returnTypeUnsubstituted != null
        : "Creating SAM adapter for not initialized original: " + original;
    ...
    assert returnType != null
        : "couldn't substitute type: " + returnTypeUnsubstituted + ", substitutor = " + substitutor;
  ...
  public static List<ValueParameterDescriptor> createValueParametersForSamAdapter(
    ...
    List<ValueParameterDescriptor> valueParameters =
        new ArrayList<>(originalValueParameters.size());
    for (ValueParameterDescriptor originalParam : originalValueParameters) {
      ...
      KotlinType functionType =
          SamConversionResolverImplKt.getFunctionTypeForSamType(
              originalType, samResolver, samConversionOracle);
      ...
      assert newType != null
          : "couldn't substitute type: " + newTypeUnsubstituted + ", substitutor = " + substitutor;
  ...
  private abstract static class FunctionInitializer {
