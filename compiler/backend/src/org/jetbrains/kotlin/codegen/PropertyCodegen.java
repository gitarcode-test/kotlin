...
public class PropertyCodegen {
  ...
  public void gen(@NotNull KtProperty property) {
    ...
    if (!(variableDescriptor instanceof PropertyDescriptor)) {
      throw ExceptionLogger.logDescriptorNotFound(
          "Property "
              + property.getName()
              + " should have a property descriptor: "
              + variableDescriptor,
          property);
    }
  ...
  public void genDestructuringDeclaration(@NotNull KtDestructuringDeclarationEntry entry) {
    ...
    if (!(variableDescriptor instanceof PropertyDescriptor)) {
      throw ExceptionLogger.logDescriptorNotFound(
          "Destructuring declaration entry"
              + entry.getName()
              + " should have a property descriptor: "
              + variableDescriptor,
          entry);
  ...
  public void generateInPackageFacade(
      @NotNull DeserializedPropertyDescriptor deserializedProperty) {
    assert context instanceof MultifileClassFacadeContext
        : "should be called only for generating facade: " + context;
  ...
  private void genBackingFieldAndAnnotations(@NotNull PropertyDescriptor descriptor) {
    // Fields and '$annotations' methods for non-private const properties are generated in the
    // multi-file facade
    boolean isBackingFieldOwner =
        descriptor.isConst() && !DescriptorVisibilities.isPrivate(descriptor.getVisibility())
  ...
  public static boolean isReferenceablePropertyWithGetter(@NotNull PropertyDescriptor descriptor) {
    ...
    KtDeclaration ktDeclaration =
        psiElement instanceof KtDeclaration ? (KtDeclaration) psiElement : null;
    if (ktDeclaration instanceof KtProperty) {
      ...
      return isAccessorNeeded(
          descriptor, ktProperty.getGetter(), isDefaultGetterAndSetter, OwnerKind.IMPLEMENTATION);
  ...
  /**
   * Determines if it's necessary to generate an accessor to the property, i.e. if this property can
   * be referenced via getter/setter for any reason
   *
  ...
  private static boolean isAccessorNeeded(
    ...
    // Companion object properties should have accessors for non-private properties because these
    // properties can be referenced
    // via getter/setter. But these accessors getter/setter are not required for private properties
    // that have a default getter
    // and setter, in this case, the property can always be accessed through the accessor
    // 'access<property name>$cp' and avoid some
    ...
    if (accessor != null
        && accessor.isSetter()
        && DescriptorVisibilities.isPrivate(descriptor.getSetter().getVisibility())) {
      return !isDefaultAccessor;
    }

    // Non-public API (private and internal) primary vals of inline classes don't have getter
    if (InlineClassesUtilsKt.isUnderlyingPropertyOfInlineClass(descriptor)
        && !descriptor.getVisibility().isPublicAPI()) {
      return false;
    }
  ...
  private static boolean areAccessorsNeededForPrimaryConstructorProperty(
      @NotNull PropertyDescriptor descriptor, @NotNull OwnerKind kind) {
  ...
  public void generateConstructorPropertyAsMethodForAnnotationClass(
    ...
    AnnotationCodegen.forMethod(mv, memberCodegen, state)
        .genAnnotations(getter, asmMethod.getReturnType(), null);

    KtExpression defaultValue =
        loadAnnotationArgumentDefaultValue(parameter, descriptor, expectedAnnotationConstructor);
    if (defaultValue != null) {
      ...
      assert !state.getClassBuilderMode().generateBodies || constant != null
          : "Default value for annotation parameter should be compile time value: "
              + defaultValue.getText();
      if (constant != null) {
        AnnotationCodegen annotationCodegen =
            AnnotationCodegen.forAnnotationDefaultValue(mv, memberCodegen, state);
  ...
  private KtExpression loadAnnotationArgumentDefaultValue(
    ...
    if (expectedAnnotationConstructor != null) {
      ValueParameterDescriptor expectedParameter =
          CollectionsKt.single(
              expectedAnnotationConstructor.getValueParameters(),
              parameter -> parameter.getName().equals(descriptor.getName()));
      PsiElement element = DescriptorToSourceUtils.descriptorToDeclaration(expectedParameter);
      if (!(element instanceof KtParameter)) {
        state
            .getDiagnostics()
            .report(EXPECTED_FUNCTION_SOURCE_WITH_DEFAULT_ARGUMENTS_NOT_FOUND.on(ktParameter));
        return null;
      }
  ...
  private void generateBackingField(
      @NotNull PropertyDescriptor descriptor, boolean isBackingFieldOwner) {
    ...
    if (descriptor.isDelegated()) {
      defaultValue = null;
        ...
        bindingContext.get(BindingContext.BACKING_FIELD_REQUIRED, descriptor))) {
  ...
  // Annotations on properties are stored in bytecode on an empty synthetic method. This way they're
  // still
  // accessible via reflection, and 'deprecated' and 'synthetic' flags prevent this method from
  ...
  private void generateBackingField(
      ...
      boolean isBackingFieldOwner) {
    FieldDescriptor annotatedField =
        isDelegate ? propertyDescriptor.getDelegateField() : propertyDescriptor.getBackingField();
    ...
    KotlinType kotlinType =
        isDelegate
            ? getDelegateTypeForProperty(propertyDescriptor, bindingContext)
    ...
    if (DescriptorAsmUtil.isInstancePropertyWithStaticBackingField(propertyDescriptor)) {
      ...
      if (DescriptorsJvmAbiUtil.isPropertyWithBackingFieldInOuterClass(propertyDescriptor)) {
        ...
        if (DescriptorVisibilities.isPrivate(
            ((ClassDescriptor) propertyDescriptor.getContainingDeclaration()).getVisibility())) {
    ...
    v.getSerializationBindings()
        .put(FIELD_FOR_PROPERTY, propertyDescriptor, new Pair<>(type, name));

    if (isBackingFieldOwner) {
      String signature =
          isDelegate ? null : typeMapper.mapFieldSignature(kotlinType, propertyDescriptor);
      FieldVisitor fv =
          builder.newField(
              JvmDeclarationOriginKt.OtherOrigin(propertyDescriptor),
              modifiers,
              name,
              type.getDescriptor(),
              signature,
      ...
      if (annotatedField != null) {
        ...
        boolean skipNullabilityAnnotations =
            (modifiers & ACC_SYNTHETIC) != 0 || propertyDescriptor.isLateInit();
      ...
      if (propertyDescriptor.getContainingDeclaration() instanceof ClassDescriptor
          && JvmAnnotationUtilKt.isJvmRecord(
  ...
  public static KotlinType getDelegateTypeForProperty(
      @NotNull PropertyDescriptor propertyDescriptor, @NotNull BindingContext bindingContext) {
    ...
    KtProperty property =
        (KtProperty) DescriptorToSourceUtils.descriptorToDeclaration(propertyDescriptor);
    ...
    if (delegateType == null) {
      ...
      delegateType =
          ErrorUtils.createErrorType(
              ErrorTypeKind.TYPE_FOR_DELEGATION, delegateExpression.getText());
    }
    return delegateType;
  }
  ...
  private boolean shouldWriteFieldInitializer(@NotNull PropertyDescriptor descriptor) {
    if (!descriptor.isConst()
        && state
            ...
            .supportsFeature(LanguageFeature.NoConstantValueAttributeForNonConstVals)) {
      return false;
    }
  ...
  private void generateGetter(
      @NotNull PropertyDescriptor descriptor, @Nullable KtPropertyAccessor getter) {
    generateAccessor(
        getter,
  ...
  private void generateSetter(
      @NotNull PropertyDescriptor descriptor, @Nullable KtPropertyAccessor setter) {
  ...
  private void generateAccessor(
      @Nullable KtPropertyAccessor accessor, @NotNull PropertyAccessorDescriptor descriptor) {
    ...
    functionCodegen.generateMethod(
        JvmDeclarationOriginKt.OtherOrigin(descriptor), descriptor, strategy);
  }
  ...
  private static class DefaultPropertyAccessorStrategy
    ...
    public DefaultPropertyAccessorStrategy(
        @NotNull GenerationState state, @NotNull PropertyAccessorDescriptor descriptor) {
      super(state);
    ...
    public void doGenerateBody(
      ...
      StackValue property =
          codegen.intermediateValueForProperty(propertyDescriptor, true, null, StackValue.LOCAL_0);
      ...
      if (propertyAccessorDescriptor instanceof PropertyGetterDescriptor) {
        ...
        List<ValueParameterDescriptor> valueParameters =
            propertyAccessorDescriptor.getValueParameters();
        assert valueParameters.size() == 1
            : "Property setter should have only one value parameter but has "
                + propertyAccessorDescriptor;
        int parameterIndex = codegen.lookupLocalIndex(valueParameters.get(0));
        assert parameterIndex >= 0
            : "Local index for setter parameter should be positive or zero: "
                + propertyAccessorDescriptor;
  ...
  public static boolean isDelegatedPropertyWithOptimizedMetadata(
    ...
    return Boolean.TRUE
        == bindingContext.get(DELEGATED_PROPERTY_WITH_OPTIMIZED_METADATA, descriptor);
  }
  ...
  public static StackValue getDelegatedPropertyMetadata(
    ...
    List<VariableDescriptorWithAccessors> allDelegatedProperties =
        bindingContext.get(DELEGATED_PROPERTIES_WITH_METADATA, owner);
    ...
    StackValue.Field array =
        StackValue.field(
            Type.getType("[" + K_PROPERTY_TYPE),
            owner,
  ...
  private static class DelegatedPropertyAccessorStrategy
    ...
    public void doGenerateBody(
      ...
      ResolvedCall<FunctionDescriptor> resolvedCall =
          bindingContext.get(
              BindingContext.DELEGATED_PROPERTY_RESOLVED_CALL, propertyAccessorDescriptor);
      assert resolvedCall != null
          : "Resolve call should be recorded for delegate call " + signature.toString();

      PropertyDescriptor propertyDescriptor = propertyAccessorDescriptor.getCorrespondingProperty();
      StackValue.Property property =
          codegen.intermediateValueForProperty(propertyDescriptor, true, null, StackValue.LOCAL_0);
      ...
      StackValue lastValue =
          invokeDelegatedPropertyConventionMethod(
              codegen, resolvedCall, delegate, propertyDescriptor);
  ...
  public void genDelegate(
      @NotNull PropertyDescriptor delegate,
      @NotNull PropertyDescriptor delegateTo,
      @NotNull StackValue field) {
