...
public class FileCopier {
  ...
  private void syncDirsImpl(
    ...
    for (Path path : Files.walk(sourceBase).collect(Collectors.toList())) {
      ...
      // remove current files from previousSnapshots map so only removed files would remain in
      // previousSnapshots
  ...
  private void writeFileSnapshots(
      @NotNull Map<String, FileSnapshot> snapshots, @NotNull File outputFile) {
