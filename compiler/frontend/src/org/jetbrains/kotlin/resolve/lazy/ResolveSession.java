...
public class ResolveSession implements KotlinCodeAnalyzer, LazyClassContext {
  ...
  @Inject
  public void setDeclarationScopeProvider(
      @NotNull DeclarationScopeProviderImpl declarationScopeProvider) {
  ...
  @Inject
  public void setPlatformDiagnosticSuppressor(
      @NotNull PlatformDiagnosticSuppressor platformDiagnosticSuppressor) {
  ...
  @Inject
  public void setAdditionalClassPartsProvider(
      @NotNull AdditionalClassPartsProvider additionalClassPartsProvider) {
    this.additionalClassPartsProvider = additionalClassPartsProvider;
  }

  @Inject
  public void setSealedClassInheritorsProvider(
      @NotNull SealedClassInheritorsProvider sealedClassInheritorsProvider) {
  ...
  public ResolveSession(
    ...
    this.packageFragmentProvider =
        new PackageFragmentProviderOptimized() {
          @Override
    ...
    fileAnnotations =
        storageManager.createMemoizedFunction(
            file -> createAnnotations(file, file.getAnnotationEntries()));

    danglingAnnotations =
        storageManager.createMemoizedFunction(
            file -> createAnnotations(file, file.getDanglingAnnotations()));
  ...
  private LazyAnnotations createAnnotations(
      KtFile file, List<KtAnnotationEntry> annotationEntries) {
  ...
  public LazyPackageDescriptor getPackageFragment(@NotNull FqName fqName) {
    ...
    return packages.computeIfAbsent(
        fqName, () -> new LazyPackageDescriptor(module, fqName, this, provider));
  }

  @NotNull
  @Override
  public LazyPackageDescriptor getPackageFragmentOrDiagnoseFailure(
      @NotNull FqName fqName, @Nullable KtFile from) {
    LazyPackageDescriptor packageDescriptor = getPackageFragment(fqName);
    if (packageDescriptor == null) {
  ...
  public Collection<ClassifierDescriptor> getTopLevelClassifierDescriptors(
      @NotNull FqName fqName, @NotNull LookupLocation location) {
    if (fqName.isRoot()) return Collections.emptyList();

    PackageMemberDeclarationProvider provider =
        declarationProviderFactory.getPackageMemberDeclarationProvider(fqName.parent());
    ...
    result.addAll(
        ContainerUtil.mapNotNull(
            ...
            classOrObjectInfo ->
                getClassDescriptor(classOrObjectInfo.getCorrespondingClassOrObject(), location)));
  ...
  public ClassDescriptor getClassDescriptor(
      @NotNull KtClassOrObject classOrObject, @NotNull LookupLocation location) {
  ...
  public DeclarationDescriptor resolveToDescriptor(@NotNull KtDeclaration declaration) {
    ...
    if (!areDescriptorsCreatedForDeclaration(declaration)) {
      throw new IllegalStateException(
          "No descriptors are created for declarations of type "
  ...
  public static boolean areDescriptorsCreatedForDeclaration(@NotNull KtDeclaration declaration) {
    return !(declaration instanceof KtAnonymousInitializer
        || declaration instanceof KtDestructuringDeclaration);
  }
  ...
  private List<LazyPackageDescriptor> collectAllPackages(
      @NotNull List<LazyPackageDescriptor> result, @NotNull LazyPackageDescriptor current) {
    result.add(current);
    for (FqName subPackage :
        packageFragmentProvider.getSubPackagesOf(
            current.getFqName(), MemberScope.Companion.getALL_NAME_FILTER())) {
