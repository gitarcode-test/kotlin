...
public class JavaMethodImpl extends JavaMemberImpl<PsiMethod> implements JavaMethod {
  public JavaMethodImpl(@NotNull JavaElementPsiSource<PsiMethod> psiMethodSource) {
    ...
    assert !method.isConstructor()
        : "PsiMethod which is a constructor should be wrapped in JavaConstructorImpl: "
            + method.getName();
  }
  ...
  public JavaType getReturnType() {
    return JavaTypeImpl.create(
        Objects.requireNonNull(getPsi().getReturnType()),
        createMethodReturnTypeSource(psiElementSource));
  }
}
