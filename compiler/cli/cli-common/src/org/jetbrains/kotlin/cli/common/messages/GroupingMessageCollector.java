...
public class GroupingMessageCollector implements MessageCollector {
  ...
  private boolean hasExplicitErrors() {
    return groupedMessages.entries().stream()
        .anyMatch(entry -> entry.getValue().severity.isError());
  }

  private boolean hasWarnings() {
    return groupedMessages.entries().stream()
        .anyMatch(entry -> entry.getValue().severity.isWarning());
  }
  ...
  public void flush() {
    ...
    List<CompilerMessageSourceLocation> sortedKeys =
        CollectionsKt.sortedWith(
            groupedMessages.keySet(),
            Comparator.nullsFirst(CompilerMessageLocationComparator.INSTANCE));
  ...
  private static class CompilerMessageLocationComparator
    ...
    public static final CompilerMessageLocationComparator INSTANCE =
        new CompilerMessageLocationComparator();

    // First, output all messages without any location information. Then, only those with the file
    // path.
    // Next, all messages with the file path and the line number. Next, all messages with file path,
