...
public class TypeIntersector {
  ...
  public static KotlinType intersectTypes(@NotNull Collection<KotlinType> types) {
    assert !types.isEmpty()
        : "Attempting to intersect empty collection of types, this case should be dealt with on the"
              + " call site.";
    ...
    if (nullabilityStripped.isEmpty()) {
      ...
      return ErrorUtils.createErrorType(
          ErrorTypeKind.INTERSECTION_OF_ERROR_TYPES, types.toString());
    }
    ...
    for (KotlinType type : nullabilityStripped) {
      if (!TypeUtils.canHaveSubtypes(typeChecker, type)) {
        ...
        for (KotlinType other : nullabilityStripped) {
          ...
          boolean relative =
              typeChecker.isSubtypeOf(type, other) || typeChecker.isSubtypeOf(other, type);
    ...
    if (resultingTypes.isEmpty()) {
      // If we ended up here, it means that all types from `nullabilityStripped` were excluded by
      // the code above
      // most likely, this is because they are all semantically interchangeable (e.g. List<Foo>! and
      // List<Foo>),
      // in that case, we can safely select the best representative out of that set and return it
      // TODO: maybe return the most specific among the types that are subtypes to all others in the
      // `nullabilityStripped`?
  ...
  /**
   * Note: this method was used in overload and override bindings to approximate type parameters
   * with several bounds, but as it turned out at some point, that logic was inconsistent with Java
   * rules, so it was simplified. Most of the other usages of this method are left untouched but
   * probably should be investigated closely if they're still valid.
   */
  @NotNull
  ...
  public static KotlinType intersectUpperBounds(
      @NotNull TypeParameterDescriptor descriptor, @NotNull List<KotlinType> upperBounds) {
  ...
  private static class TypeUnifier {
    private static class TypeParameterUsage {
      ...
      public TypeParameterUsage(
          TypeParameterDescriptor typeParameterDescriptor, Variance howTheTypeParameterIsUsed) {
    ...
    private static void processAllTypeParameters(
      ...
      if (descriptor instanceof TypeParameterDescriptor) {
        ...
        result.invoke(
            new TypeParameterUsage((TypeParameterDescriptor) descriptor, howThisTypeIsUsed));
      ...
      for (TypeProjection projection : type.getArguments()) {
        ...
        processAllTypeParameters(
            projection.getType(), projection.getProjectionKind(), result, containsParameter);
      }
    }
  }
}
