...
public abstract class AbstractControlFlowTest extends AbstractPseudocodeTest {
  ...
  protected void dumpInstructions(
    ...
    int nextInstructionsColumnWidth =
        countNextInstructionsColumnWidth(pseudocode.getInstructionsIncludingDeadCode());
        ...
        (instruction, next, prev) -> {
          ...
          if (!sameContents(next, nextInstructions)) {
            result
                ...
                .append(
                    String.format(
                        "%1$-" + nextInstructionsColumnWidth + "s",
                        formatInstructionList(nextInstructions)));
          }
  ...
  private static boolean sameContents(
      @Nullable Instruction natural, Collection<Instruction> actual) {
        ...
        for (Instruction nextInstruction : instruction.getNextInstructions()) {
          assertTrue(
              "instruction '"
                  ...
                  + "' has '"
                  + nextInstruction
                  + "' among next instructions list, but not vice versa",
        ...
        for (Instruction prevInstruction : instruction.getPreviousInstructions()) {
          assertTrue(
              "instruction '"
                  ...
                  + "' has '"
                  + prevInstruction
                  + "' among previous instructions list, but not vice versa",
