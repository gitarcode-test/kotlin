...
public class KtConstructorDelegationCall extends KtElementImpl implements KtCallElement {
  ...
  /**
   * @return true if this delegation call is not present in the source code. Note that we always
   *     parse delegation calls for secondary constructors, even if there's no explicit call in the
   *     source (see {@link KotlinParsing#parseSecondaryConstructor}).
   *     <p>class Foo { constructor(name: String) // <--- implicit constructor delegation call
   *     (empty element after RPAR) }
   */
