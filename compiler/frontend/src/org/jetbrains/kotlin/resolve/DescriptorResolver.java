...
public class DescriptorResolver {
  ...
  public List<KotlinType> resolveSupertypes(
    ...
    Collection<KotlinType> declaredSupertypes =
        resolveSuperTypeListEntries(scope, delegationSpecifiers, typeResolver, trace, false);
    ...
    supertypes.addAll(
        additionalClassPartsProvider.getAdditionalSupertypes(classDescriptor, supertypes));
    ...
    for (KtSuperTypeListEntry delegationSpecifier : delegationSpecifiers) {
      ...
      if (typeReference != null) {
        KotlinType supertype =
            resolver.resolveType(extensibleScope, typeReference, trace, checkBounds);
        if (DynamicTypesKt.isDynamic(supertype)) {
          trace.report(DYNAMIC_SUPERTYPE.on(typeReference));
        } else {
          result.add(supertype);
          KtTypeElement bareSuperType =
              checkNullableSupertypeAndStripQuestionMarks(trace, typeReference.getTypeElement());
          checkProjectionsInImmediateArguments(trace, bareSuperType, supertype);
        }
      } else {
        result.add(
            ErrorUtils.createErrorType(
                ErrorTypeKind.UNRESOLVED_TYPE, delegationSpecifier.getText()));
      }
    }
    return result;
  }
  ...
  @Nullable
  private static KtTypeElement checkNullableSupertypeAndStripQuestionMarks(
      @NotNull BindingTrace trace, @Nullable KtTypeElement typeElement) {
  ...
  private static void checkProjectionsInImmediateArguments(
      @NotNull BindingTrace trace, @Nullable KtTypeElement typeElement, @NotNull KotlinType type) {
    ...
    // If we have an abbreviated type (written with a type alias), it still can contain type
    // projections in top-level arguments.
    if (!KotlinTypeKt.isError(type)
        && SpecialTypesKt.getAbbreviatedType(type) != null
        && !hasProjectionsInWrittenArguments) {
      // Only interface inheritance should be checked here.
      // Corresponding check for classes is performed for type alias constructor calls in
      // CandidateResolver.
      if (TypeUtilsKt.isInterface(type)
          && TypeUtilsKt.containsTypeProjectionsInTopLevelArguments(type)) {
        trace.report(EXPANDED_TYPE_CANNOT_BE_INHERITED.on(typeElement, type));
      }
    }
  }

  public static DescriptorVisibility getDefaultVisibility(
      KtModifierListOwner modifierListOwner, DeclarationDescriptor containingDescriptor) {
  ...
  public static Modality getDefaultModality(
    ...
    if (containingDescriptor instanceof ClassDescriptor) {
      ...
      Modality basicModality =
          isTrait && !DescriptorVisibilities.isPrivate(visibility) ? Modality.OPEN : Modality.FINAL;
  ...
  public ValueParameterDescriptorImpl resolveValueParameterDescriptor(
    ...
    Annotations valueParameterAnnotations =
      ...
      if (!languageVersionSettings.supportsFeature(LanguageFeature.DestructuringLambdaParameters)) {
        trace.report(
            Errors.UNSUPPORTED_FEATURE.on(
                valueParameter,
                TuplesKt.to(
                    LanguageFeature.DestructuringLambdaParameters, languageVersionSettings)));
      }
      ...
      destructuringVariables =
          () -> {
            ...
            assert dispatchReceiver == null
                    || dispatchReceiver.getContainingDeclaration() instanceof ScriptDescriptor
                : "Destructuring declarations are only be parsed for lambdas, and they must not"
                      + " have a dispatch receiver";
            LexicalScope scopeForDestructuring =
                ScopeUtilsKt.createScopeForDestructuring(
                    scope, owner.getExtensionReceiverParameter());

            List<VariableDescriptor> result =
                destructuringDeclarationResolver.resolveLocalVariablesFromDestructuringDeclaration(
                    scope,
                    destructuringDeclaration,
                    new TransientReceiver(type),
                    /* initializer= */ null,
                    ExpressionTypingContext.newContext(
                        trace,
                        scopeForDestructuring,
            ...
            modifiersChecker
                .withTrace(trace)
                .checkModifiersForDestructuringDeclaration(destructuringDeclaration);
            return result;
          };
    } else {
    ...
    if (destructuringDeclaration == null) {
      // NB: val/var for parameter is only allowed in primary constructors where single underscore
      // names are still prohibited.
      // The problem with val/var is that when lazy resolve try to find their descriptor, it
      // searches through the member scope
      ...
      parameterName =
          !valueParameter.hasValOrVar() && UnderscoreUtilKt.isSingleUnderscore(valueParameter)
  ...
  private Annotations resolveValueParameterAnnotations(
    ...
    AnnotationSplitter splitter =
        new AnnotationSplitter(storageManager, allAnnotations, SetsKt.setOf(CONSTRUCTOR_PARAMETER));
    return new CompositeAnnotations(
        splitter.getAnnotationsForTarget(CONSTRUCTOR_PARAMETER), additionalAnnotations);
  }

  @NotNull
  private KotlinType getVarargParameterType(@NotNull KotlinType elementType) {
    KotlinType primitiveArrayType =
  ...
  public List<TypeParameterDescriptorImpl> resolveTypeParametersForDescriptor(
    ...
    List<TypeParameterDescriptorImpl> descriptors =
        resolveTypeParametersForDescriptor(
            containingDescriptor, scopeForAnnotationsResolve, typeParameters, trace);
  ...
  private List<TypeParameterDescriptorImpl> resolveTypeParametersForDescriptor(
    ...
    assert containingDescriptor instanceof FunctionDescriptor
        ...
        : "This method should be called for functions, properties, or type aliases, got "
    ...
    for (int i = 0, typeParametersSize = typeParameters.size(); i < typeParametersSize; i++) {
      ...
      result.add(
          resolveTypeParameterForDescriptor(
              containingDescriptor, scopeForAnnotationsResolve, typeParameter, i, trace));
    }
    return result;
  }
  ...
  private TypeParameterDescriptorImpl resolveTypeParameterForDescriptor(
    ...
    Annotations annotations =
        annotationResolver.resolveAnnotationsWithArguments(
            scopeForAnnotationsResolve, typeParameter.getModifierList(), trace);
  ...
  public static ClassConstructorDescriptorImpl createAndRecordPrimaryConstructorForObject(
    ...
    ClassConstructorDescriptorImpl constructorDescriptor =
        DescriptorFactory.createPrimaryConstructorForObject(
            classDescriptor, KotlinSourceElementKt.toSourceElement(object));
    if (object instanceof PsiElement) {
      KtPrimaryConstructor primaryConstructor = object.getPrimaryConstructor();
      trace.record(
          CONSTRUCTOR,
          primaryConstructor != null ? primaryConstructor : (PsiElement) object,
  ...
  static final class UpperBoundCheckRequest {
    ...
    UpperBoundCheckRequest(
        Name typeParameterName, KtTypeReference upperBound, KotlinType upperBoundType) {
  ...
  public void resolveGenericBounds(
    ...
    for (int i = 0; i < typeParameters.size(); i++) {
      ...
      if (extendsBound != null) {
        ...
        upperBoundCheckRequests.add(
            new UpperBoundCheckRequest(ktTypeParameter.getNameAsName(), extendsBound, type));
      }
    }
    for (KtTypeConstraint constraint : declaration.getTypeConstraints()) {
      ...
      if (boundTypeReference != null) {
        ...
        upperBoundCheckRequests.add(
            new UpperBoundCheckRequest(referencedName, boundTypeReference, bound));
      }

      if (typeParameterDescriptor != null) {
        trace.record(
            BindingContext.REFERENCE_TARGET, subjectTypeParameterName, typeParameterDescriptor);
        if (bound != null) {
    ...
    if (!(declaration instanceof KtClass)) {
      checkUpperBoundTypes(
          trace, upperBoundCheckRequests, declaration.hasModifier(KtTokens.OVERRIDE_KEYWORD));
  ...
  public void checkNamesInConstraints(
    ...
    for (KtTypeConstraint constraint : declaration.getTypeConstraints()) {
      ...
      ClassifierDescriptor classifier =
          ScopeUtilsKt.findClassifier(scope, name, NoLookupLocation.FOR_NON_TRACKED_SCOPE);
      if (classifier instanceof TypeParameterDescriptor
          && classifier.getContainingDeclaration() == descriptor) continue;

      if (classifier != null) {
        // To tell the user that we look only for locally defined type parameters
        trace.report(
            NAME_IN_CONSTRAINT_IS_NOT_A_TYPE_PARAMETER.on(nameExpression, constraint, declaration));
  ...
  public static void checkUpperBoundType(
    ...
    if (!hasOverrideModifier
        && !TypeUtils.canHaveSubtypes(KotlinTypeChecker.DEFAULT, upperBoundType)) {
    ...
    if (DefinitelyNonNullableTypesKt.containsIncorrectExplicitDefinitelyNonNullableType(
        upperBoundType)) {
  ...
  public VariableDescriptor resolveLocalVariableDescriptor(
      @NotNull LexicalScope scope, @NotNull KtParameter parameter, BindingTrace trace) {
  ...
  private KotlinType resolveParameterType(
      LexicalScope scope, KtParameter parameter, BindingTrace trace) {
  ...
  public VariableDescriptor resolveLocalVariableDescriptor(
    ...
    VariableDescriptor variableDescriptor =
        new LocalVariableDescriptor(
            scope.getOwnerDescriptor(),
            annotationResolver.resolveAnnotationsWithArguments(
                scope, parameter.getModifierList(), trace),
  ...
  public TypeAliasDescriptor resolveTypeAliasDescriptor(
    ...
    DescriptorVisibility visibility =
        resolveVisibilityFromModifiers(
            typeAlias, getDefaultVisibility(typeAlias, containingDeclaration));
    ...
    LazyTypeAliasDescriptor typeAliasDescriptor =
        LazyTypeAliasDescriptor.create(
            storageManager,
            trace,
    ...
    LexicalScope scopeWithTypeParameters;
    {
      ...
      if (typeParameters.isEmpty()) {
        ...
        resolveGenericBounds(
            typeAlias, typeAliasDescriptor, writableScope, typeParameterDescriptors, trace);
    ...
    if (typeReference == null) {
      ...
      trace.report(
          UNSUPPORTED_FEATURE.on(
              typeAliasKeyword != null ? typeAliasKeyword : typeAlias,
      ...
      typeAliasDescriptor.initialize(
          ...
          storageManager.createRecursionTolerantLazyValue(
              () ->
                  typeResolver.resolveAbbreviatedType(
                      scopeWithTypeParameters, typeReference, trace),
              ErrorUtils.createErrorType(
                  ErrorTypeKind.RECURSIVE_TYPE_ALIAS, typeAliasDescriptor.getName().asString())),
          storageManager.createRecursionTolerantLazyValue(
  ...
  private static void checkNoGenericBoundsOnTypeAliasParameters(
      @NotNull KtTypeAlias typeAlias, @NotNull BindingTrace trace) {
  ...
  public PropertyDescriptor resolveDestructuringDeclarationEntryAsProperty(
    ...
    ExpressionReceiver receiver =
        createReceiverForDestructuringDeclaration(destructuringDeclaration, context);

    int componentIndex = destructuringDeclaration.getEntries().indexOf(entry);
    KotlinType componentType =
        destructuringDeclarationResolver.resolveInitializer(
            entry, receiver, initializer, context, componentIndex);
  ...
  private ExpressionReceiver createReceiverForDestructuringDeclaration(
    ...
    KotlinType initializerType =
        expressionTypingServices.getTypeInfo(initializer, context).getType();
    if (initializerType == null) return null;

    return ExpressionReceiver.Companion.create(
        initializer, initializerType, context.trace.getBindingContext());
  }
  ...
  private PropertyDescriptor resolveAsPropertyDescriptor(
    ...
    DescriptorVisibility visibility =
        resolveVisibilityFromModifiers(
            variableDeclaration, getDefaultVisibility(variableDeclaration, container));
    Modality modality =
    ...
    Annotations allAnnotations =
        annotationResolver.resolveAnnotationsWithoutArguments(
            scopeForDeclarationResolution, modifierList, trace);
    ...
    if (variableDeclaration instanceof KtProperty
        && ((KtProperty) variableDeclaration).hasDelegate()) {
      targetSet.add(PROPERTY_DELEGATE_FIELD);
    }
    AnnotationSplitter annotationSplitter =
        new AnnotationSplitter(storageManager, allAnnotations, targetSet);

    Annotations propertyAnnotations =
        new CompositeAnnotations(
    ...
    PropertyDescriptorImpl propertyDescriptor =
        PropertyDescriptorImpl.create(
            ...
            modifierList != null
                    && PsiUtilsKt.hasExpectModifier(modifierList)
...

    {
      ...
      if (typeParameters.isEmpty()) {
        ...
        LexicalWritableScope writableScopeForDeclarationResolution =
            new LexicalWritableScope(
                scopeForDeclarationResolution,
                container,
                false,
        ...
        LexicalWritableScope writableScopeForInitializerResolution =
            new LexicalWritableScope(
                scopeForInitializerResolution,
                container,
                false,
        ...
        typeParameterDescriptors =
            resolveTypeParametersForDescriptor(
                propertyDescriptor, scopeForDeclarationResolution, typeParameters, trace);
    ...
    if (receiverTypeRef != null) {
      receiverType =
          typeResolver.resolveType(
              scopeForDeclarationResolutionWithTypeParameters, receiverTypeRef, trace, true);
      AnnotationSplitter splitter =
          new AnnotationSplitter(
              storageManager, receiverType.getAnnotations(), EnumSet.of(RECEIVER));
      receiverDescriptor =
    ...
    LexicalScope scopeForInitializer =
    ...
    KotlinType typeIfKnown =
        propertyType != null
            ? propertyType
            : variableTypeAndInitializerResolver.resolveTypeNullable(
                propertyDescriptor,
                scopeForInitializer,
                variableDeclaration,
                dataFlowInfo,
    ...
    propertyDescriptor.initialize(
        ...
        new FieldDescriptorImpl(
            annotationSplitter.getAnnotationsForTarget(FIELD), propertyDescriptor),
        new FieldDescriptorImpl(
            annotationSplitter.getAnnotationsForTarget(PROPERTY_DELEGATE_FIELD),
            propertyDescriptor));
  ...
  /*package*/ static KotlinType transformAnonymousTypeIfNeeded(
    ...
    if (classifier == null
        || !DescriptorUtils.isAnonymousObject(classifier)
    ...
    boolean isAnonymousReturnTypesInPrivateInlineFunctionsForbidden =
        languageVersionSettings.supportsFeature(
            LanguageFeature.ApproximateAnonymousReturnTypesInPrivateInlineFunctions);

    if (!isPrivate
        || (isInlineFunction && isAnonymousReturnTypesInPrivateInlineFunctionsForbidden)) {
      if (type.getConstructor().getSupertypes().size() == 1) {
        ...
        if (memberScope instanceof SubstitutingScope) {
          substitutedSuperType =
              ((SubstitutingScope) memberScope).substitute(approximatingSuperType);
        } else {
        ...
        if (languageVersionSettings.supportsFeature(
          ...
          if (lowerNullable) {
            trace.report(
                APPROXIMATED_LOCAL_TYPE_WILL_BECOME_NULLABLE.on(declaration, substitutedSuperType));
          } else {
            trace.report(
                APPROXIMATED_LOCAL_TYPE_WILL_BECOME_FLEXIBLE.on(declaration, substitutedSuperType));
          }
        }
        return substitutedSuperType;
      } else {
        trace.report(
            AMBIGUOUS_ANONYMOUS_TYPE_INFERRED.on(
                declaration, type.getConstructor().getSupertypes()));
      }
    }
  ...
  private PropertySetterDescriptor resolvePropertySetterDescriptor(
    ...
    Annotations setterTargetedAnnotations =
        annotationSplitter.getAnnotationsForTarget(PROPERTY_SETTER);
    Annotations parameterTargetedAnnotations =
    ...
    if (setter != null) {
      Annotations annotations =
          new CompositeAnnotations(
              CollectionsKt.listOf(
                  ...
                  annotationResolver.resolveAnnotationsWithoutArguments(
                      scopeWithTypeParameters, setter.getModifierList(), trace)));
      KtParameter parameter = setter.getParameter();

      setterDescriptor =
          new PropertySetterDescriptorImpl(
              propertyDescriptor,
              annotations,
              resolveMemberModalityFromModifiers(
                  setter,
              ...
              /* isDefault= */ false,
              setter.hasModifier(EXTERNAL_KEYWORD),
              property.hasModifier(KtTokens.INLINE_KEYWORD)
                  || setter.hasModifier(KtTokens.INLINE_KEYWORD),
              CallableMemberDescriptor.Kind.DECLARATION,
              null,
      ...
      if (returnTypeReference != null) {
        KotlinType returnType =
            typeResolver.resolveType(scopeWithTypeParameters, returnTypeReference, trace, true);
      ...
      if (parameter != null) {
        ...
        // This check is redundant: the parser does not allow a default value, but we'll keep it
        // just in case
      ...
      setterDescriptor =
          DescriptorFactory.createSetter(
              ...
              parameterTargetedAnnotations,
              !hasDelegate
                  && setterTargetedAnnotations.isEmpty()
                  && parameterTargetedAnnotations.isEmpty(),
              false,
              property.hasModifier(KtTokens.INLINE_KEYWORD),
  ...
  private PropertyGetterDescriptorImpl resolvePropertyGetterDescriptor(
    ...
    Annotations getterTargetedAnnotations =
      ...
      Annotations getterAnnotations =
          new CompositeAnnotations(
              CollectionsKt.listOf(
                  ...
                  annotationResolver.resolveAnnotationsWithoutArguments(
                      scopeForDeclarationResolution, getter.getModifierList(), trace)));

      getterDescriptor =
          new PropertyGetterDescriptorImpl(
              propertyDescriptor,
              getterAnnotations,
              resolveMemberModalityFromModifiers(
                  getter,
              ...
              /* isDefault= */ false,
              getter.hasModifier(EXTERNAL_KEYWORD),
              property.hasModifier(KtTokens.INLINE_KEYWORD)
                  || getter.hasModifier(KtTokens.INLINE_KEYWORD),
              CallableMemberDescriptor.Kind.DECLARATION,
              null,
              KotlinSourceElementKt.toSourceElement(getter));
      getterType =
          determineGetterReturnType(
              scopeForDeclarationResolution,
              trace,
              getterDescriptor,
              getter,
    ...
    getterDescriptor.initialize(
        getterType != null
            ? getterType
            : VariableTypeAndInitializerResolver.getTypeForPropertyWithoutReturnType(
  ...
  private KotlinType determineGetterReturnType(
    ...
    if (returnTypeReference != null) {
      KotlinType explicitReturnType =
          typeResolver.resolveType(scope, returnTypeReference, trace, true);
      if (propertyTypeIfKnown != null
          && !TypeUtils.equalTypes(explicitReturnType, propertyTypeIfKnown)) {
        trace.report(
            WRONG_GETTER_RETURN_TYPE.on(
                returnTypeReference, propertyTypeIfKnown, explicitReturnType));
      }
      return explicitReturnType;
    }

    // If a property has no type specified in the PSI but the getter does (or has an initializer
    // e.g. "val x get() = ..."),
    ...
    if (!property.hasDelegateExpressionOrInitializer()
      ...
      return inferReturnTypeFromExpressionBody(
          trace, scope, DataFlowInfoFactory.EMPTY, getter, getterDescriptor, inferenceSession);
    }
  ...
  /*package*/ KotlinType inferReturnTypeFromExpressionBody(
      ...
      @Nullable InferenceSession inferenceSession) {
    return wrappedTypeFactory.createRecursionIntolerantDeferredType(
        trace,
        () -> {
          PreliminaryDeclarationVisitor.Companion.createForDeclaration(
              function, trace, languageVersionSettings);
          KotlinType type =
              expressionTypingServices.getBodyExpressionType(
                  trace, scope, dataFlowInfo, function, functionDescriptor, inferenceSession);
          KotlinType publicType =
              transformAnonymousTypeIfNeeded(
                  ...
                  languageVersionSettings);
          UnwrappedType approximatedType =
              typeApproximator.approximateDeclarationType(publicType, false);
          KotlinType sanitizedType =
              declarationReturnTypeSanitizer.sanitizeReturnType(
  ...
  public PropertyDescriptor resolvePrimaryConstructorParameterToAProperty(
    ...
    Annotations allAnnotations =
        annotationResolver.resolveAnnotationsWithoutArguments(
            scope, parameter.getModifierList(), trace);
    Set<AnnotationUseSiteTarget> targetSet =
        EnumSet.of(PROPERTY, PROPERTY_GETTER, FIELD, CONSTRUCTOR_PARAMETER, PROPERTY_SETTER);
    if (isMutable) {
      targetSet.add(PROPERTY_SETTER);
      targetSet.add(SETTER_PARAMETER);
    }
    ...
    PropertyDescriptorImpl propertyDescriptor =
        PropertyDescriptorImpl.create(
            ...
            propertyAnnotations,
            resolveMemberModalityFromModifiers(
                parameter, Modality.FINAL, trace.getBindingContext(), classDescriptor),
            resolveVisibilityFromModifiers(
                parameter, getDefaultVisibility(parameter, classDescriptor)),
            isMutable,
            name,
            CallableMemberDescriptor.Kind.DECLARATION,
            KotlinSourceElementKt.toSourceElement(parameter),
    ...
    propertyDescriptor.setType(
        type,
        Collections.emptyList(),
        getDispatchReceiverParameterIfNeeded(classDescriptor),
        null,
    ...
    Annotations getterAnnotations =
        new CompositeAnnotations(
            CollectionsKt.listOf(annotationSplitter.getAnnotationsForTarget(PROPERTY_GETTER)));

    PropertyGetterDescriptorImpl getter =
        DescriptorFactory.createDefaultGetter(propertyDescriptor, getterAnnotations);
    PropertySetterDescriptor setter =
  ...
  private static boolean isInsideOuterClassOrItsSubclass(
      @Nullable DeclarationDescriptor nested, @NotNull ClassDescriptor outer) {
    if (nested == null) return false;

    if (nested instanceof ClassDescriptor && isSubclass((ClassDescriptor) nested, outer))
      return true;
