...
public class InternalFinallyBlockInliner extends CoveringTryCatchNodeProcessor {
  ...
  private static class FinallyBlockInfo {
    ...
    private FinallyBlockInfo(
        @NotNull AbstractInsnNode inclusiveStart, @NotNull AbstractInsnNode exclusiveEnd) {
  ...
  public static void processInlineFunFinallyBlocks(
    ...
    for (TryCatchBlockNode block : inlineFun.tryCatchBlocks) {
      inlineFunTryBlockInfo.add(
          new TryCatchBlockNodeInfo(block, index++ < lambdaTryCatchBlockNodes));
    }
    ...
    if (hasFinallyBlocks(inlineFunTryBlockInfo)) {
      new InternalFinallyBlockInliner(
              inlineFun, inlineFunTryBlockInfo, localVars, finallyParamOffset, properFinallySplit)
          .processInlineFunFinallyBlocks();
    }
  }

  @NotNull private final MethodNode inlineFun;
  private final boolean properFinallySplit;

  // lambdaTryCatchBlockNodes is number of TryCatchBlockNodes that was inlined with lambdas into
  // function
  ...
  private int initAndGetVarIndexForNonLocalReturnValue() {
    MaxLocalsCalculator tempCalcNode =
        new MaxLocalsCalculator(Opcodes.API_VERSION, inlineFun.access, inlineFun.desc, null);
  ...
  private void processInlineFunFinallyBlocks() {
    ...
    while (curIns != null) {
      ...
      // At this point only global return is possible, local one already substituted with: goto
      // endLabel
      if (!isReturnOpcode(curIns.getOpcode()) || !isMarkedReturn(curIns)) {
      ...
      if (currentCoveringNodesFromInnermost.isEmpty()
          || currentCoveringNodesFromInnermost
              .get(currentCoveringNodesFromInnermost.size() - 1)
      ...
      assert markedReturn.getNext() instanceof LabelNode
          : "Label should be occurred after non-local return";
      LabelNode newFinallyEnd = (LabelNode) markedReturn.getNext();
      Type nonLocalReturnType = getReturnType(markedReturn.getOpcode());

      // Generally there could be several tryCatch blocks (group) on one code interval (same start
      // and end labels, but maybe different handlers) -
      // all of them refer to one try/*catches*/finally or try/catches.
      // Each group that corresponds to try/*catches*/finally contains tryCatch block with default
      // handler.
      ...
      List<TryBlockCluster<TryCatchBlockNodeInfo>> clustersFromInnermost =
          TryBlockClusteringKt.doClustering(currentCoveringNodesFromInnermost);
      Iterator<TryBlockCluster<TryCatchBlockNodeInfo>> tryCatchBlockIterator =
          clustersFromInnermost.iterator();
      ...
      while (tryCatchBlockIterator.hasNext()) {
        ...
        TryCatchBlockNodeInfo nodeWithDefaultHandlerIfExists =
            clusterBlocks.get(clusterBlocks.size() - 1);

        FinallyBlockInfo finallyInfo =
            findFinallyBlockBody(
                nodeWithDefaultHandlerIfExists, getTryBlocksMetaInfo().getAllIntervals());
        if (finallyInfo == null) {
        ...
        List<TryCatchBlockNodePosition> tryCatchBlockInlinedInFinally =
            findTryCatchBlocksInlinedInFinally(finallyInfo);
        ...
        boolean generateAloadAstore =
            nonLocalReturnType != Type.VOID_TYPE && !finallyInfo.isEmpty();
        if (generateAloadAstore) {
          finallyBlockCopy.visitVarInsn(
              nonLocalReturnType.getOpcode(Opcodes.ISTORE), nextTempNonLocalVarIndex);
        }
        finallyBlockCopy.visitLabel(newFinallyStart);

        // Keep some information about label nodes, we need it to understand whether it's jump
        // inside finally block or outside
        // in first case we do call VISIT on instruction otherwise recreating jump instruction (see
        // below)
        ...
        while (currentIns != finallyInfo.endInsExclusive) {
          ...
          copyInstruction(
              finallyBlockCopy, currentIns, isInsOrJumpInsideFinally, originalDepthIndex);
        ...
        if (generateAloadAstore) {
          finallyBlockCopy.visitVarInsn(
              nonLocalReturnType.getOpcode(Opcodes.ILOAD), nextTempNonLocalVarIndex);
        ...
        updateExceptionTable(
            properFinallySplit ? nestedUnsplitBlocksWithoutFinally : clusterBlocks,
            newFinallyStart,
            newFinallyEnd,
            tryCatchBlockInlinedInFinally,
            labelsInsideFinally,
            (LabelNode) insertedBlockEnd.info);
  ...
  private static void copyInstruction(
    ...
    if (isInsOrJumpInsideFinally) {
      ...
      finallyBlockCopy.instructions.add(
          new JumpInsnNode(currentIns.getOpcode(), ((JumpInsnNode) currentIns).label));
    }
  }

  private static void checkCoveringBlocksInvariant(
      @ReadOnly @NotNull List<TryCatchBlockNodeInfo> currentCoveringNodesFromOuterMost) {
    ...
    for (TryCatchBlockNodeInfo info : currentCoveringNodesFromOuterMost) {
      assert !isWasOnlyLocal || info.getOnlyCopyNotProcess()
          : "There are some problems with try-catch structure";
  ...
  private void updateExceptionTable(
    ...
    List<TryBlockCluster<TryCatchBlockNodePosition>> clusters =
        TryBlockClusteringKt.doClustering(tryCatchBlockPresentInFinally);
    ...
    for (TryBlockCluster<TryCatchBlockNodePosition> cluster : clusters) {
      ...
      if (clusterPosition == TryCatchPosition.INNER) {
        for (TryCatchBlockNodePosition position : clusterBlocks) {
          ...
          assert inlineFun.instructions.indexOf(tryCatchBlockNode.start)
              <= inlineFun.instructions.indexOf(tryCatchBlockNode.end);

          TryCatchBlockNode additionalTryCatchBlock =
              new TryCatchBlockNode(
                  (LabelNode) tryCatchBlockNode.start.getLabel().info,
          ...
          tryBlocksMetaInfo.addNewInterval(
              new TryCatchBlockNodeInfo(additionalTryCatchBlock, true));
        }
        ...
        TryCatchBlockNodePosition defaultHandler = cluster.getDefaultHandler();
        assert defaultHandler != null : "Default handler should be present";
        TryBlockCluster<TryCatchBlockNodePosition> endCluster =
            handler2Cluster.remove(defaultHandler.getHandler());
        assert endCluster != null : "Could find start cluster for  " + clusterPosition;

        // at this point only external finallies could occurs
        // they don't collision with updatingClusterBlocks, but may with external ones on next
        // updateExceptionTable invocation
        ...
        for (TryCatchBlockNodePosition endBlockPosition : endCluster.getBlocks()) {
          ...
          assert Objects.equal(startNode.getType(), endNode.getType())
              : "Different handler types : " + startNode.getType() + " " + endNode.getType();

          getTryBlocksMetaInfo()
              .split(
                  endNode,
                  new SimpleInterval(
                      (LabelNode) endNode.getNode().end.getLabel().info,
                      (LabelNode) startNode.getStartLabel().getLabel().info),
                  false);
        }
      }
    }
    ...
    if (handler2Cluster.size() == 1) {
      TryBlockCluster<TryCatchBlockNodePosition> singleCluster =
          handler2Cluster.values().iterator().next();
      if (singleCluster.getBlocks().get(0).getPosition() == TryCatchPosition.END) {
        // Pair that starts on default handler don't added to tryCatchBlockPresentInFinally cause
        // it's out of finally block
        ...
        for (TryCatchBlockNodePosition endBlockPosition : singleCluster.getBlocks()) {
          ...
          getTryBlocksMetaInfo()
              .split(
                  ...
                  new SimpleInterval(
                      (LabelNode) endNode.getNode().end.getLabel().info,
                      (LabelNode) insertedBlockEnd.getLabel().info),
                  false);
        }
    ...
    for (TryCatchBlockNodeInfo block : updatingClusterBlocks) {
      ...
      SplitPair<TryCatchBlockNodeInfo> split =
          tryBlocksMetaInfo.splitAndRemoveIntervalFromCurrents(block, splitBy, false);
  ...
  private static LabelNode getNewOrOldLabel(
      LabelNode oldHandler, @NotNull Set<LabelNode> labelsInsideFinally) {
  ...
  // As described above all tryCatch group that have finally block also should contains
  // tryCatchBlockNode with default handler.
  // So we assume that instructions between end of tryCatchBlock and start of next tryCatchBlock
  // with same default handler is required finally body.
  // There is at least two tryCatchBlockNodes in list cause there is always tryCatchBlockNode on
  // first instruction of default handler:
  ...
  private FinallyBlockInfo findFinallyBlockBody(
    ...
    for (TryCatchBlockNodeInfo block : tryCatchBlocks) {
      ...
      if (afterStartBlock) {
        if (block.getNode().type == null
            && (firstLabelInChain(tryCatchBlock.getNode().start)
                        == firstLabelInChain(block.getNode().start)
                    && firstLabelInChain(tryCatchBlock.getNode().end)
                        == firstLabelInChain(block.getNode().end)
    ...
    FinallyBlockInfo finallyInfo =
        new FinallyBlockInfo(startFinallyChain.getNext(), endFinallyChainExclusive);
  ...
  private void checkFinally(AbstractInsnNode startIns, AbstractInsnNode endInsExclusive) {
    if (inlineFun.instructions.indexOf(startIns)
        >= inlineFun.instructions.indexOf(endInsExclusive)) {
      throw new AssertionError(
          "Inconsistent finally: block end occurs before start "
              + traceInterval(endInsExclusive, startIns));
    }
  }

  @NotNull
  private List<TryCatchBlockNodePosition> findTryCatchBlocksInlinedInFinally(
    ...
    for (AbstractInsnNode curInstr = finallyInfo.startIns;
        curInstr != finallyInfo.endInsExclusive;
      ...
      for (TryCatchBlockNodeInfo block : startedTryBlocks) {
        assert !processedBlocks.containsKey(block)
            : "Try catch block already processed before start label!!! " + block;
        TryCatchBlockNodePosition info =
            new TryCatchBlockNodePosition(block, TryCatchPosition.START);
  ...
  private void flushCurrentState(@NotNull AbstractInsnNode curNonLocal) {
    substituteTryBlockNodes(inlineFun);
    System.out.println(
        "Will process instruction at : "
            + inlineFun.instructions.indexOf(curNonLocal)
            + " "
            + curNonLocal.toString());
