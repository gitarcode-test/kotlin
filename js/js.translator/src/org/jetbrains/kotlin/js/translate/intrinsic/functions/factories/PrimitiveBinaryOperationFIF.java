...
public enum PrimitiveBinaryOperationFIF implements FunctionIntrinsicFactory {
  ...
  private abstract static class BinaryOperationIntrinsicBase
      extends FunctionIntrinsicWithReceiverComputed {
    @NotNull
    public abstract JsExpression doApply(
  ...
  private static final BinaryOperationIntrinsicBase INT_MULTIPLICATION_INTRINSIC =
      new BinaryOperationIntrinsicBase() {
        // IEEE 754 mantissa is 52 bits long. Assuming one argument may be up to 2^31, the second
        // argument should be
        // not greater than 2^21 in order their product don't exceed 2^52. We preserve two extra
        // bits to be more safe.
  ...
  private static final NamePredicate BINARY_OPERATIONS =
      new NamePredicate(OperatorNameConventions.BINARY_OPERATION_NAMES);
  ...
  private static final DescriptorPredicate SIMPLE_INT_MULTIPLICATION =
      pattern("Byte|Short.times(Byte|Short)");
  private static final DescriptorPredicate INT_DIVISION =
      pattern("Byte|Short|Int.div(Byte|Short|Int)");
  ...
  private static final DescriptorPredicate
      PRIMITIVE_INTEGRAL_NUMBERS_COMPARE_TO_INTEGRAL_OPERATIONS =
  ...
  private static final Predicate<FunctionDescriptor> INT_WITH_BIT_OPERATIONS =
      pattern("Int.or|and|xor|shl|shr|ushr").or(pattern("Short|Byte.or|and|xor"));
  ...
  private static final DescriptorPredicate NUMBER_REM =
      pattern("Byte|Short|Int.rem(Byte|Short|Int)");
  ...
  private static final DescriptorPredicate NUMBER_RANGE_TO =
      pattern("Byte|Short|Int.rangeTo(Byte|Short|Int)");
  private static final DescriptorPredicate NUMBER_RANGE_UNTIL =
      pattern("Byte|Short|Int|Long.rangeUntil(Byte|Short|Int|Long)");

  private static final ImmutableMap<String, JsBinaryOperator> BINARY_BITWISE_OPERATIONS =
      ImmutableMap.<String, JsBinaryOperator>builder()
  ...
  private static final Predicate<FunctionDescriptor> PREDICATE =
      PRIMITIVE_NUMBERS_BINARY_OPERATIONS
          ...
          .or(STRING_PLUS)
          .or(INT_WITH_BIT_OPERATIONS)
          .or(PRIMITIVE_NUMBERS_COMPARE_TO_OPERATIONS);

  @Nullable
  @Override
  public FunctionIntrinsic getIntrinsic(
    ...
    if (KotlinBuiltIns.isBuiltIn(descriptor)
        && descriptor.getName().equals(OperatorNameConventions.COMPARE_TO)) {
      return BUILTINS_COMPARE_TO_INTRINSIC;
    }
  ...
  private static JsBinaryOperator getOperator(@NotNull FunctionDescriptor descriptor) {
    ...
    Name descriptorName =
        descriptor.getName().equals(OperatorNameConventions.MOD)
  ...
  private static class PrimitiveBinaryOperationFunctionIntrinsic
    ...
    @NotNull private final JsBinaryOperator operator;
  ...
  private static class CharAndIntBinaryOperationFunctionIntrinsic
    ...
    @NotNull private final BinaryOperationIntrinsicBase functionIntrinsic;

    private CharAndIntBinaryOperationFunctionIntrinsic(
        @NotNull BinaryOperationIntrinsicBase functionIntrinsic) {
  ...
  private static class CharAndCharBinaryOperationFunctionIntrinsic
    ...
    private CharAndCharBinaryOperationFunctionIntrinsic(
        @NotNull BinaryOperationIntrinsicBase functionIntrinsic) {
      this.functionIntrinsic = functionIntrinsic;
    }
  ...
  private static class StringAndCharBinaryOperationFunctionIntrinsic
    ...
    private StringAndCharBinaryOperationFunctionIntrinsic(
        @NotNull BinaryOperationIntrinsicBase functionIntrinsic) {
      this.functionIntrinsic = functionIntrinsic;
    }
    ...
    public JsExpression doApply(
        ...
        @NotNull JsExpression right,
        @NotNull TranslationContext context) {
      return functionIntrinsic.doApply(
          left, TopLevelFIF.TO_STRING.apply(right, Collections.emptyList(), context), context);
    }
  }
}
