 ...
 *
 * <p>It is based on the C source files jsparse.c and jsparse.h in the jsref package.
 *
 * @see TokenStream
 */
public class Parser {
      ...
      case TokenStream.SWITCH:
        {
          ...
          while ((tt = ts.getToken()) != TokenStream.RC && tt != TokenStream.EOF) {
            ...
            while ((tt = ts.peekToken()) != TokenStream.RC
                && tt != TokenStream.CASE
                && tt != TokenStream.DEFAULT
                && tt != TokenStream.EOF) {
          ...
          if (peek == TokenStream.CATCH) {
            while (ts.matchToken(TokenStream.CATCH)) {
              ...
              catchblocks.addChildToBack(
                  nf.createCatch(varName, catchCond, statements(ts), catchPosition));
  ...
  private Node variables(TokenStream ts, boolean inForInit, CodePosition position)
      throws IOException, JavaScriptException {
      ...
      case TokenStream.DELPROP:
        {
          ...
          if (!isValidDeleteArgument(argument)) {
            Context.reportError(
                "msg.wrong.delete argument", argument.getPosition(), ts.lastPosition);
          }
  ...
  private Node memberExprTail(TokenStream ts, boolean allowCallSyntax, Node pn)
      throws IOException, JavaScriptException {
    ...
    while ((tt = ts.getToken()) > TokenStream.EOF) {
      ...
      if (tt == TokenStream.DOT) {
        ...
        pn =
            nf.createBinary(
                TokenStream.DOT, pn, nf.createName(ts.getString(), ts.tokenPosition), position);
        /*
          ...
          if (!matched) {
            do {
              ...
              if (tt == TokenStream.COMMA) {
                pn.addChildToBack(
                    nf.createLeaf(TokenStream.PRIMARY, TokenStream.UNDEFINED, position));
              } else {
