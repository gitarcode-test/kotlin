...
public abstract class CodegenTestCase extends KotlinBaseTest<KotlinBaseTest.TestFile> {
  ...
  private static final String DEFAULT_JVM_TARGET =
      System.getProperty("kotlin.test.default.jvm.target");
  public static final String BOX_IN_SEPARATE_PROCESS_PORT =
      System.getProperty("kotlin.test.box.in.separate.process.port");
  ...
  protected final void createEnvironmentWithMockJdkAndIdeaAnnotations(
      @NotNull ConfigurationKind configurationKind, @NotNull File... javaSourceRoots) {
    createEnvironmentWithMockJdkAndIdeaAnnotations(
        configurationKind, Collections.emptyList(), TestJdkKind.MOCK_JDK, javaSourceRoots);
  }
  ...
  @NotNull
  public static CodegenTestFiles loadMultiFiles(
      @NotNull List<TestFile> files, @NotNull Project project) {
    Collections.sort(files);

    List<KtFile> ktFiles = new ArrayList<>(files.size());
    for (TestFile file : files) {
      if (file.name.endsWith(".kt") || file.name.endsWith(".kts")) {
        // `rangesToDiagnosticNames` parameter is not-null only for diagnostic tests, it's using for
        // lazy diagnostics
        String content =
            CheckerTestUtil.INSTANCE.parseDiagnosedRanges(file.content, new ArrayList<>(0), null);
  ...
  protected String relativePath(@NotNull File file) {
    return FilesKt.toRelativeString(
        file.getAbsoluteFile(), new File(codegenTestBasePath()).getAbsoluteFile());
  }
  ...
  protected GeneratedClassLoader generateAndCreateClassLoader(boolean reportProblems) {
    ...
    if (!CodegenTestUtil.verifyAllFilesWithAsm(
        generateClassesInFile(reportProblems), reportProblems)) {
  ...
  protected GeneratedClassLoader createClassLoader() {
    ...
    return new GeneratedClassLoader(generateClassesInFile(), classLoader, getClassPathURLs());
  }
  ...
  protected URL[] getClassPathURLs() {
    ...
    if (externalImportsProvider != null) {
      myEnvironment
          ...
          .forEach(
              file -> {
                ScriptCompilationConfigurationWrapper refinedConfiguration =
                    externalImportsProvider.getScriptConfiguration(file);
  ...
  protected Class<?> generateFacadeClass() {
    FqName facadeClassFqName =
        JvmFileClassUtil.getFileClassInfoNoResolve(myFiles.getPsiFile()).getFacadeClassFqName();
  ...
  private ClassFileFactory generateClassesInFile(boolean reportProblems) {
    ...
    try {
      ...
      boolean ignoreDexing =
          myFiles.getPsiFiles().stream()
              .anyMatch(
  ...
  private static void setCustomDefaultJvmTarget(CompilerConfiguration configuration) {
    if (DEFAULT_JVM_TARGET != null) {
      ...
      if (originalTarget == null
          || customDefaultTarget.getMajorVersion() > originalTarget.getMajorVersion()) {
  ...
  protected void compile(
      @NotNull List<TestFile> files, boolean reportProblems, boolean dumpKotlinFiles) {
    ...
    // If there are Java files, they should be compiled against the class files produced by Kotlin,
    // so we dump them to the disk
    ...
    if (compileJavaFiles) {
      ...
      List<String> finalJavacOptions =
          prepareJavacOptions(
              javaClasspath, javacOptions, javaClassesOutputDirectory, isJava9Module);

      try {
        runJavacTask(
            findJavaSourcesInDirectory(javaSourceDir).stream()
                .map(File::new)
  ...
  protected void doTestWithTransformer(
      @NotNull String filePath, @NotNull Function<String, String> sourceTransformer) {
  ...
  @Override
  @NotNull
  protected List<TestFile> createTestFilesFromFile(
      @NotNull File file, @NotNull String expectedText) {
  ...
  public static List<TestFile> createTestFilesFromFile(
    ...
    List<TestFile> testFiles =
        TestFiles.createTestFiles(
            file.getName(),
            expectedText,
            new TestFiles.TestFileFactoryNoModules<TestFile>() {
              @NotNull
              @Override
    ...
    if (InTextDirectivesUtils.isDirectiveDefined(expectedText, "WITH_HELPERS")) {
      testFiles.add(
          new TestFile(
              ...
              TestHelperGeneratorKt.createTextForCodegenTestHelpers(backend)));
    }
    return testFiles;
  }
  ...
  private void callBoxMethodAndCheckResult(
      URLClassLoader classLoader, Class<?> aClass, Method method)
      throws IOException, IllegalAccessException, InvocationTargetException {
    callBoxMethodAndCheckResult(classLoader, aClass, method, false);
  }

  protected void callBoxMethodAndCheckResult(
      URLClassLoader classLoader, Class<?> aClass, Method method, boolean unexpectedBehaviour)
      throws IOException, IllegalAccessException, InvocationTargetException {
    String result;
    if (BOX_IN_SEPARATE_PROCESS_PORT != null) {
      ...
      if (savedClassLoader != classLoader) {
        // otherwise the test infrastructure used in the test may conflict with the one from the
        // context classloader
  ...
  @NotNull
  private String invokeBoxInSeparateProcess(URLClassLoader classLoader, Class<?> aClass)
      throws IOException {
    ...
    if (classLoader instanceof GeneratedClassLoader) {
      ...
      SimpleOutputFileCollection currentOutput =
          new SimpleOutputFileCollection(
              ((GeneratedClassLoader) classLoader).getAllGeneratedFiles());
      writeAllTo(currentOutput, outDir);
      classPath.add(0, outDir.toURI().toURL());
    }

    return new TestProxy(
            Integer.valueOf(BOX_IN_SEPARATE_PROCESS_PORT), aClass.getCanonicalName(), classPath)
        .runTest();
  }
  ...
  protected void printReport(File wholeFile) {
    boolean isIgnored =
        InTextDirectivesUtils.isIgnoredTarget(
            getBackend(), wholeFile, getIgnoreBackendDirectivePrefix());
    if (!isIgnored) {
      System.out.println(generateToText());
    }
  }
