...
public abstract class AbstractClassDescriptor extends ModuleAwareClassDescriptor {
  ...
  public AbstractClassDescriptor(@NotNull StorageManager storageManager, @NotNull Name name) {
                    ...
                    new Function1<KotlinTypeRefiner, SimpleType>() {
                      ...
                      public SimpleType invoke(KotlinTypeRefiner kotlinTypeRefiner) {
                        ClassifierDescriptor descriptor =
                            kotlinTypeRefiner.refineDescriptor(AbstractClassDescriptor.this);
                        ...
                        if (descriptor instanceof TypeAliasDescriptor) {
                          return KotlinTypeFactory.computeExpandedType(
                              ...
                              TypeUtils.getDefaultTypeProjections(
                                  descriptor.getTypeConstructor().getParameters()));
                        }

                        if (descriptor instanceof ModuleAwareClassDescriptor) {
                          TypeConstructor refinedConstructor =
                              descriptor.getTypeConstructor().refine(kotlinTypeRefiner);
                          return TypeUtils.makeUnsubstitutedType(
                              refinedConstructor,
                              ((ModuleAwareClassDescriptor) descriptor)
                                  .getUnsubstitutedMemberScope(kotlinTypeRefiner),
                              this);
                        }
    ...
    this.unsubstitutedInnerClassesScope =
        storageManager.createLazyValue(
  ...
  public MemberScope getMemberScope(
    ...
    assert typeArguments.size() == getTypeConstructor().getParameters().size()
        : "Illegal number of type arguments: expected "
            + getTypeConstructor().getParameters().size()
            + " but was "
            + typeArguments.size()
            + " for "
            + getTypeConstructor()
            + " "
            + getTypeConstructor().getParameters();
    if (typeArguments.isEmpty()) return getUnsubstitutedMemberScope(kotlinTypeRefiner);

    TypeSubstitutor substitutor =
        TypeConstructorSubstitution.create(getTypeConstructor(), typeArguments).buildSubstitutor();
