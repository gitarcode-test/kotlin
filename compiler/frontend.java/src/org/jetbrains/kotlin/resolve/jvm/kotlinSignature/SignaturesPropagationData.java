...
public class SignaturesPropagationData {
  ...
  private static final KotlinToJvmSignatureMapper SIGNATURE_MAPPER =
      ServiceLoader.load(
              KotlinToJvmSignatureMapper.class, KotlinToJvmSignatureMapper.class.getClassLoader())
          .iterator()
          .next();
  ...
  public SignaturesPropagationData(
    ...
    assert receiverType == null
        : "Parameters before propagation have receiver type,"
    ...
    JavaMethodDescriptor autoMethodDescriptor =
        createAutoMethodDescriptor(
            containingClass, method, autoReturnType, autoValueParameters, autoTypeParameters);

    superFunctions = getSuperFunctionsForMethod(method, autoMethodDescriptor, containingClass);
    modifiedValueParameters =
  ...
  private ValueParameters modifyValueParametersAccordingToSuperMethods(
    ...
    for (ValueParameterDescriptor originalParam : parameters) {
      ...
      List<TypeAndName> typesFromSuperMethods =
          CollectionsKt.map(
              superFunctions,
              superFunction -> {
                ReceiverParameterDescriptor receiver =
                    superFunction.getExtensionReceiverParameter();
                int index = receiver != null ? originalIndex - 1 : originalIndex;
                if (index == -1) {
                  assert receiver != null
                      : "can't happen: index is -1, while function is not extension";
      ...
      if (shouldBeExtension && originalIndex == 0) {
        ...
        for (int i = 0; i < superFunctions.size(); i++) {
          if (superFunctions.get(i).hasStableParameterNames()) {
            // When there's more than one stable name in super functions, we pick the first one.
            // This behaviour is similar to
            // the compiler front-end, except that it reports a warning in such cases
            // TODO: report a warning somewhere if there's more than one stable name in super
            // functions
        ...
        resultParameters.add(
            new ValueParameterDescriptorImpl(
                ...
                varargCheckResult.isVararg
                    ? DescriptorUtilsKt.getBuiltIns(originalParam).getArrayElementType(altType)
                    : null,
                SourceElement.NO_SOURCE));
      }
    }
  ...
  private static List<FunctionDescriptor> getSuperFunctionsForMethod(
    ...
    boolean autoMethodContainsVararg =
        SignaturePropagationUtilKt.containsVarargs(autoMethodDescriptor);
    for (KotlinType supertype : containingClass.getTypeConstructor().getSupertypes()) {
      Collection<? extends SimpleFunctionDescriptor> superFunctionCandidates =
          supertype
              .getMemberScope()
              .getContributedFunctions(name, NoLookupLocation.WHEN_GET_SUPER_MEMBERS);

      if (!autoMethodContainsVararg
          && !SignaturePropagationUtilKt.containsAnyNotTrivialSignature(superFunctionCandidates))
        continue;
      ...
      for (FunctionDescriptor candidate : superFunctionCandidates) {
        ...
        if (KotlinToJvmSignatureMapperKt.erasedSignaturesEqualIgnoringReturnTypes(
            autoSignature, candidateSignature)) {
    ...
    superFunctions.sort(
        Comparator.comparing(fun -> getFqName(fun.getContainingDeclaration()).asString()));
  ...
  private VarargCheckResult checkVarargInSuperFunctions(
    ...
    for (FunctionDescriptor superFunction : superFunctions) {
      int originalIndex = originalParam.getIndex();
      int index =
          superFunction.getExtensionReceiverParameter() != null ? originalIndex - 1 : originalIndex;
      if (index != -1
          && superFunction.getValueParameters().get(index).getVarargElementType() != null) {
