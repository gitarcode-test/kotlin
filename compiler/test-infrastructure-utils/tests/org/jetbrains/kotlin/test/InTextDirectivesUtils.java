...
public final class InTextDirectivesUtils {
  ...
  public static final String[] IGNORE_BACKEND_DIRECTIVE_PREFIXES = {
    IGNORE_BACKEND_DIRECTIVE_PREFIX, IGNORE_BACKEND_K1_DIRECTIVE_PREFIX
  };

  public static final String TARGET_BACKEND_DIRECTIVE_PREFIX = "// TARGET_BACKEND: ";
  public static final String DORT_TARGET_EXACT_BACKEND_DIRECTIVE_PREFIX =
  ...
  @NotNull
  public static String[] findArrayWithPrefixes(
      @NotNull String fileText, @NotNull String... prefixes) {
    return ArrayUtil.toStringArray(findListWithPrefixes(fileText, prefixes));
  }

  @NotNull
  public static List<String> findListWithPrefixes(
      @NotNull String fileText, @NotNull String... prefixes) {
  ...
  @Nullable
  public static String findStringWithPrefixes(
      @NotNull String fileText, @NotNull String... prefixes) {
    ...
    if (strings.size() != 1) {
      throw new IllegalStateException(
          "There is more than one string with given prefixes "
  ...
  @NotNull
  public static List<String> findLinesWithPrefixesRemoved(
      @NotNull String fileText, @NotNull String... prefixes) {
    return findLinesByPrefixRemoved(fileText, true, true, prefixes).values().stream()
        .flatMap(Collection::stream)
        .collect(Collectors.toList());
  }

  @NotNull
  public static Map<String, List<String>> findLinesByPrefixRemoved(
      @NotNull String fileText, @NotNull String... prefixes) {
  ...
  @NotNull
  public static Map<String, List<String>> findLinesByPrefixRemoved(
      @NotNull String fileText, boolean trim, boolean strict, @NotNull String... prefixes) {
      ...
      for (String prefix : cleanedPrefixes) {
        if (line.startsWith(prefix)) {
          ...
          if (noPrefixLine.isEmpty()
            ...
            result
                .computeIfAbsent(prefix, s -> new ArrayList<>())
                .add(
                    trim
                        ? noPrefixLine.trim()
                        : StringUtil.trimTrailing(StringsKt.drop(noPrefixLine, 1)));
            break;
          } else if (strict) {
            throw new AssertionError(
                "Line starts with prefix \""
                    + prefix
                    + "\", but doesn't have space symbol after it: "
                    + line);
          }
        }
      }
  ...
  public static void assertHasUnknownPrefixes(
      @NotNull String fileText, @NotNull Collection<String> knownPrefixes) {
  ...
  public static boolean isCompatibleTarget(
      @NotNull TargetBackend targetBackend, @NotNull File file) {
  ...
  public static boolean isCompatibleTarget(
      @NotNull TargetBackend targetBackend, Map<String, List<String>> directives) {
    if (targetBackend == TargetBackend.ANY) return true;

    List<String> doNotTarget =
        directives.getOrDefault("DONT_TARGET_EXACT_BACKEND: ", Collections.emptyList());
    doNotTarget =
        doNotTarget.stream()
            .flatMap((s) -> Arrays.stream(s.split(" ")))
            .collect(Collectors.toList());
    if (doNotTarget.contains(targetBackend.name())) return false;
  ...
  private static boolean isCompatibleTargetExceptAny(
      @NotNull TargetBackend targetBackend, @NotNull List<String> backends) {
    if (targetBackend == TargetBackend.ANY) return false;
    return backends.isEmpty()
        || backends.contains(targetBackend.name())
        || isCompatibleTargetExceptAny(targetBackend.getCompatibleWith(), backends);
  }
  ...
  private static boolean isIgnoredTarget(
      @NotNull TargetBackend targetBackend, boolean includeAny, List<String> ignoredBackends) {
  ...
  public static boolean dontRunGeneratedCode(
      @NotNull TargetBackend targetBackend, @NotNull File file) {
    List<String> backends =
        findListWithPrefixes(textWithDirectives(file), "// DONT_RUN_GENERATED_CODE: ");
