...
public abstract class AssignmentTranslator extends AbstractTranslator {
  ...
  public static JsExpression translate(
    ...
    if (hasCorrespondingFunctionIntrinsic(context, expression)) {
      ...
      // The right implementation is: get assignment via OverloadedAssignmentTranslator and see at
      // the result.
      // If it's an expression like `a = a + b` then represent it as `a += b`
      // Another right implementation is: enumerate explicitly which descriptors are OK to
      // intrinsify like this.
      CallableDescriptor operationDescriptor =
          getCallableDescriptorForOperationExpression(context.bindingContext(), expression);
      if (operationDescriptor == null
          || operationDescriptor.getExtensionReceiverParameter() == null) {
  ...
  @NotNull protected final KtBinaryExpression expression;
  protected final boolean isVariableReassignment;

  protected AssignmentTranslator(
      @NotNull KtBinaryExpression expression, @NotNull TranslationContext context) {
    super(context);
  ...
  protected final AccessTranslator createAccessTranslator(
      @NotNull KtExpression left, boolean forceOrderOfEvaluation) {
