...
final class ObjectNode {
  ...
  private ObjectNode(@NotNull Disposable object, boolean parentIsRoot) {
    myObject = object;
    myTrace =
        parentIsRoot && Disposer.isDebugMode() ? ThrowableInterner.intern(new Throwable()) : null;
  }
    ...
    for (ObjectNode childNode : myChildren.getAllNodes()) {
      ...
      String message =
          "Memory leak detected: '"
              ...
              + "' ("
              + object.getClass()
              + ") was registered in Disposer"
              + " as a child of '"
              + getObject()
              + "' ("
              + getObject().getClass()
              + ") but wasn't disposed.\n"
              + "Register it with a proper parentDisposable or ensure that it's always disposed by"
              + " direct Disposer.dispose call.\n"
              + "See https://jetbrains.org/intellij/sdk/docs/basics/disposers.html for more"
              + " details.\n"
  ...
  @NotNull
  ObjectNode moveChildNodeToOtherParent(
      @NotNull Disposable child, @NotNull ObjectNode otherParentNode) {
  ...
  private static final class MapNodeChildren implements NodeChildren {
    ...
    MapNodeChildren(@NotNull List<ObjectNode> children) {
      Reference2ObjectLinkedOpenHashMap<Disposable, ObjectNode> map =
          new Reference2ObjectLinkedOpenHashMap<>(children.size());
    ...
    public void removeChildren(
        @Nullable Predicate<? super Disposable> condition,
        @NotNull Consumer<? super ObjectNode> deletedNodeConsumer) {
  ...
  /**
   * A collection of child ObjectNodes. Backed either by an {@code ArrayList<ObjectNode>} (when the
   * number of children is small) or {@code Map<Disposable, ObjectNode>} otherwise, to speed up the
   * lookups.
   */
  private interface NodeChildren {
    @Nullable
    ObjectNode removeChildNode(@NotNull Disposable object);

    @Nullable
    ObjectNode findChildNode(@NotNull Disposable object);

    @NotNull // return a new instance of NodeChildren when the underlying data-structure changed,
             // e.g., list->map
