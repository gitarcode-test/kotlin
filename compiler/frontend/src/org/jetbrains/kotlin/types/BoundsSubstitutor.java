...
public class BoundsSubstitutor {
  ...
  public static <D extends CallableDescriptor> D substituteBounds(@NotNull D functionDescriptor) {
    ...
    D substitutedFunction =
        (D) functionDescriptor.substitute(createUpperBoundsSubstitutor(typeParameters));
  ...
  @NotNull
  public static <D extends CallableDescriptor> TypeSubstitutor createUpperBoundsSubstitutor(
      @NotNull D callableDescriptor) {
    return createUpperBoundsSubstitutor(callableDescriptor.getTypeParameters());
  }

  @NotNull
  private static TypeSubstitutor createUpperBoundsSubstitutor(
    ...
    for (TypeParameterDescriptor descriptor : topologicallySortTypeParameters(typeParameters)) {
      ...
      KotlinType substitutedUpperBoundsAsType =
          substitutor.substitute(upperBoundsAsType, Variance.INVARIANT);
      mutableSubstitution.put(
          descriptor.getTypeConstructor(), new TypeProjectionImpl(substitutedUpperBoundsAsType));
    }
  ...
  @NotNull
  private static List<TypeParameterDescriptor> topologicallySortTypeParameters(
      @NotNull List<TypeParameterDescriptor> typeParameters) {
    ...
    List<TypeParameterDescriptor> topOrder =
        DFS.topologicalOrder(
            typeParameters, current -> getTypeParametersFromUpperBounds(current, typeParameters));

    assert topOrder.size() == typeParameters.size()
        : "All type parameters must be visited, but only " + topOrder + " were";
