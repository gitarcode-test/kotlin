...
public class GeneralCommandLine implements UserDataHolder {
   ...
   *
   * <p>{@code NONE} means a child process will receive an empty environment. <br>
   * {@code SYSTEM} will provide it with the same environment as an IDE. <br>
   * {@code CONSOLE} provides the child with a similar environment as if it was launched from, well,
   * a console. On OS X, a console environment is simulated (see {@link
   * EnvironmentUtil#getEnvironmentMap()} for reasons it's needed and details on how it works). On
   * Windows and Unix hosts, this option is no different from {@code SYSTEM} since there is no
   * drastic distinction in environment between GUI and console apps.
   */
  ...
  @NotNull
  public GeneralCommandLine withEnvironment(
      @NonNls @NotNull String key, @NonNls @NotNull String value) {
  ...
  public void setPassParentEnvironment(boolean passParentEnvironment) {
    withParentEnvironmentType(
        passParentEnvironment ? ParentEnvironmentType.CONSOLE : ParentEnvironmentType.NONE);
  }
  ...
  /**
   * Returns an environment as seen by a child process, that is the {@link #getEnvironment()
   * environment} merged with the {@link #getParentEnvironment() parent} one.
   */
  @NotNull
  ...
  private List<String> validateAndPrepareCommandLine() throws ExecutionException {
    ...
    for (Map.Entry<String, String> entry : myEnvParams.entrySet()) {
      ...
      if (!EnvironmentUtil.isValidName(name))
        throw new IllegalEnvVarException("run.configuration.invalid.env.name: " + name);
      if (!EnvironmentUtil.isValidValue(value))
        throw new IllegalEnvVarException(
            "run.configuration.invalid.env.value: " + name + ", " + value);
    }

    String exePath = myExePath;
    if (SystemInfoRt.isMac
        && myParentEnvironmentType == ParentEnvironmentType.CONSOLE
        && exePath.indexOf(File.separatorChar) == -1) {
   ...
   *     Windows command line handling is extremely fragile and vague, and the exact escaping of a
   *     particular argument may vary depending on values of the preceding arguments.
   *     <pre>
   *   [foo] [^] -> [foo] [^^]
   * </pre>
   *     but:
   ...
   *     <p>If you need to alter the command line passed in, override the {@link
   *     #prepareCommandLine(String, List, Platform)} method instead.
   */
  @NotNull
  ...
  // Ideally, it should have been startProcess(ProcessBuilder), and the design would be more
  // straightforward.
  @NotNull
  ...
  /**
   * Executed with pre-filled ProcessBuilder as the param and gives the last chance to configure
   * starting process parameters before a process is started
   *
   ...
   * But some commands (e.g. {@code 'cmd /c start "title" ...'}) should get their quotes
   * non-escaped. Wrapping a parameter by this method (instead of using quotes) will do exactly
   * this.
   *
