...
public abstract class IncrementTranslator extends AbstractTranslator {
  ...
  public static JsExpression translate(
      @NotNull KtUnaryExpression expression, @NotNull TranslationContext context) {
  ...
  @NotNull protected final KtUnaryExpression expression;
  @NotNull protected final AccessTranslator accessTranslator;

  @NotNull private final JsBlock accessBlock = new JsBlock();

  protected IncrementTranslator(
      @NotNull KtUnaryExpression expression, @NotNull TranslationContext context) {
    super(context);
    this.expression = expression;
    KtExpression baseExpression = getBaseExpression(expression);
    this.accessTranslator =
        getAccessTranslator(baseExpression, context().innerBlock(accessBlock)).getCached();
  }
  ...
  private JsExpression asPrefix() {
    ...
    JsExpression reassignment =
        variableReassignment(context().innerBlock(accessBlock), getExpression).source(expression);
  ...
  private JsExpression asPostfix() {
    ...
    TemporaryVariable t1 =
        context()
            .declareTemporary(accessTranslator.translateAsGet().source(expression), expression);
    accessBlock.getStatements().add(t1.assignmentStatement());
    JsExpression variableReassignment =
        variableReassignment(context().innerBlock(accessBlock), t1.reference()).source(expression);
    accessBlock
        .getStatements()
        .add(JsAstUtils.asSyntheticStatement(variableReassignment.source(expression)));

    JsExpression result;
    if (accessBlock.getStatements().size() == 2) {
      result =
          JsAstUtils.newSequence(
              Arrays.asList(t1.assignmentExpression(), variableReassignment, t1.reference()));
    } else {
      context().getCurrentBlock().getStatements().addAll(accessBlock.getStatements());
      result = t1.reference();
    }
  ...
  private JsExpression variableReassignment(
      @NotNull TranslationContext context, @NotNull JsExpression toCallMethodUpon) {
    JsExpression overloadedMethodCallOnPropertyGetter =
        operationExpression(context, toCallMethodUpon);
    return accessTranslator.translateAsSet(overloadedMethodCallOnPropertyGetter);
  }

  @NotNull
  abstract JsExpression operationExpression(
      @NotNull TranslationContext context, @NotNull JsExpression receiver);
