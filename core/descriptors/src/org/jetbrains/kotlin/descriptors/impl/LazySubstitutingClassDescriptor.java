...
public class LazySubstitutingClassDescriptor extends ModuleAwareClassDescriptor {
  ...
  public LazySubstitutingClassDescriptor(
      ModuleAwareClassDescriptor descriptor, TypeSubstitutor substitutor) {
    ...
    if (newSubstitutor == null) {
      if (originalSubstitutor.isEmpty()) {
        ...
        List<TypeParameterDescriptor> originalTypeParameters =
            original.getTypeConstructor().getParameters();
        typeConstructorParameters =
            new ArrayList<TypeParameterDescriptor>(originalTypeParameters.size());
        newSubstitutor =
        ...
        declaredTypeParameters =
            CollectionsKt.filter(
                typeConstructorParameters,
                new Function1<TypeParameterDescriptor, Boolean>() {
                  @Override
  ...
  public TypeConstructor getTypeConstructor() {
    ...
    if (typeConstructor == null) {
      ...
      typeConstructor =
          new ClassTypeConstructorImpl(
              this, typeConstructorParameters, supertypes, LockBasedStorageManager.NO_LOCKS);
    }
  ...
  public MemberScope getMemberScope(
      @NotNull List<? extends TypeProjection> typeArguments,
  ...
  @Override
  public MemberScope getMemberScope(
      @NotNull TypeSubstitution typeSubstitution, @NotNull KotlinTypeRefiner kotlinTypeRefiner) {
  ...
  public MemberScope getUnsubstitutedMemberScope() {
    return getUnsubstitutedMemberScope(
        DescriptorUtilsKt.getKotlinTypeRefiner(DescriptorUtils.getContainingModule(original)));
  }
  ...
  public SimpleType getDefaultType() {
    List<TypeProjection> typeProjections =
        TypeUtils.getDefaultTypeProjections(getTypeConstructor().getParameters());
  ...
  public Collection<ClassConstructorDescriptor> getConstructors() {
    ...
    Collection<ClassConstructorDescriptor> result =
        new ArrayList<ClassConstructorDescriptor>(originalConstructors.size());
  ...
  public ClassDescriptor substitute(@NotNull TypeSubstitutor substitutor) {
    ...
    return new LazySubstitutingClassDescriptor(
        ...
        TypeSubstitutor.createChainedSubstitutor(
            substitutor.getSubstitution(), getSubstitutor().getSubstitution()));
  }
  ...
  public ValueClassRepresentation<SimpleType> getValueClassRepresentation() {
    ...
    return representation.mapUnderlyingType(
        new Function1<SimpleType, SimpleType>() {
          @Override
  ...
  private SimpleType substituteSimpleType(@Nullable SimpleType type) {
    ...
    assert substitutedType instanceof SimpleType
        : "Substitution for SimpleType should also be a SimpleType, but it is "
            + substitutedType
            + "\n"
            + "Unsubstituted: "
            + type;
