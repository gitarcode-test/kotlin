...
public class DataFlowAnalyzer {
  ...
  @Nullable
  private FunctionDescriptor getOverriddenDescriptorFromClass(
    ...
    for (FunctionDescriptor overridden : overriddenDescriptors) {
      ...
      if (DescriptorUtils.isClass(containingDeclaration)
  ...
  private boolean typeHasOverriddenEquals(
      @NotNull KotlinType type, @NotNull KtElement lookupElement) {
    // `equals` from `String` is not fake override because it is marked as
    // `IntrinsicConstEvaluation`
    if (KotlinBuiltIns.isString(type)) return false;

    Collection<? extends SimpleFunctionDescriptor> members =
        type.getMemberScope()
    ...
    for (FunctionDescriptor member : members) {
      ...
      ClassifierDescriptor superClassDescriptor =
          (ClassifierDescriptor) fromSuperClass.getContainingDeclaration();
  ...
  public DataFlowInfo extractDataFlowInfoFromCondition(
      @Nullable KtExpression condition, boolean conditionValue, ExpressionTypingContext context) {
    ...
    condition.accept(
        new KtVisitorVoid() {
          ...
          public void visitIsExpression(@NotNull KtIsExpression expression) {
            if (conditionValue && !expression.isNegated()
                || !conditionValue && expression.isNegated()) {
              result.set(
                  context.trace.get(BindingContext.DATAFLOW_INFO_AFTER_CONDITION, expression));
            }
          }
          ...
          public void visitBinaryExpression(@NotNull KtBinaryExpression expression) {
            ...
            if (OperatorConventions.BOOLEAN_OPERATIONS.containsKey(operationToken)) {
              DataFlowInfo dataFlowInfo =
                  extractDataFlowInfoFromCondition(expression.getLeft(), conditionValue, context);
                ...
                DataFlowInfo rightInfo =
                    extractDataFlowInfoFromCondition(
                        ...
                        and == conditionValue
                            ...
                            : context);
                if (and
                    == conditionValue) { // this means: and && conditionValue || !and &&
                                         // !conditionValue
              ...
              DataFlowInfo expressionFlowInfo =
                  facade.getTypeInfo(expression, context).getDataFlowInfo();
              ...
              DataFlowValue leftValue =
                  dataFlowValueFactory.createDataFlowValue(left, lhsType, context);
              DataFlowValue rightValue =
                  dataFlowValueFactory.createDataFlowValue(right, rhsType, context);

              Boolean equals = null;
              if (operationToken == KtTokens.EQEQ || operationToken == KtTokens.EQEQEQ) {
                equals = true;
              } else if (operationToken == KtTokens.EXCLEQ
                  || operationToken == KtTokens.EXCLEQEQEQ) {
                equals = false;
              }
              if (equals != null) {
                if (equals
                    == conditionValue) { // this means: equals && conditionValue || !equals &&
                                         // !conditionValue
                  boolean identityEquals =
                      operationToken == KtTokens.EQEQEQ
                          || operationToken == KtTokens.EXCLEQEQEQ
          ...
          public void visitUnaryExpression(@NotNull KtUnaryExpression expression) {
            IElementType operationTokenType =
                expression.getOperationReference().getReferencedNameElementType();
            if (operationTokenType == KtTokens.EXCL) {
              KtExpression baseExpression = expression.getBaseExpression();
              if (baseExpression != null) {
                result.set(
                    extractDataFlowInfoFromCondition(baseExpression, !conditionValue, context));
              }
            } else {
    ...
    DataFlowInfo infoFromEffectSystem =
        effectSystem.extractDataFlowInfoFromCondition(
            condition,
            conditionValue,
            context.trace,
  ...
  @NotNull
  public KotlinTypeInfo checkType(
  ...
  private KotlinType checkTypeInternal(
    ...
    if (!noExpectedType(c.expectedType)
        && TypeUtilsKt.contains(
            expressionType, (type) -> type instanceof StubTypeForBuilderInference)) {
      if (c.inferenceSession instanceof BuilderInferenceSession) {
        ((BuilderInferenceSession) c.inferenceSession)
            .addExpectedTypeConstraint(expression, expressionType, c.expectedType);
      }
    }
    if (noExpectedType(c.expectedType)
        || !c.expectedType.getConstructor().isDenotable()
    ...
    if (expression instanceof KtConstantExpression && reportErrorForTypeMismatch) {
      ConstantValue<?> constantValue =
          constantExpressionEvaluator.evaluateToConstantValue(expression, c.trace, c.expectedType);
      boolean error =
          new CompileTimeConstantChecker(c, module, true)
              .checkConstantExpressionType(
                  constantValue, (KtConstantExpression) expression, c.expectedType);
      hasError.set(error);
      return expressionType;
    }
    ...
    if (reportErrorForTypeMismatch
        && !DiagnosticUtilsKt.reportTypeMismatchDueToTypeProjection(
            c, expression, c.expectedType, expressionType)
        && !DiagnosticUtilsKt.reportTypeMismatchDueToScalaLikeNamedFunctionSyntax(
            c, expression, c.expectedType, expressionType)) {
  ...
  public KotlinType checkType(
    ...
    KotlinType result =
        checkTypeInternal(expressionType, expression, c, hasError, reportErrorForTypeMismatch);
  ...
  public SmartCastResult checkPossibleCast(
    ...
    return smartCastManager.checkAndRecordPossibleCast(
        dataFlowValue, c.expectedType, expression, c, null, false, null);
  }

  public void recordExpectedType(
      @NotNull BindingTrace trace,
      @NotNull KtExpression expression,
      @NotNull KotlinType expectedType) {
    if (expectedType != NO_EXPECTED_TYPE) {
      KotlinType normalizeExpectedType =
          expectedType == UNIT_EXPECTED_TYPE ? builtIns.getUnitType() : expectedType;
      trace.record(BindingContext.EXPECTED_EXPRESSION_TYPE, expression, normalizeExpectedType);
    }
  }

  @Nullable
  public KotlinType checkStatementType(
  ...
  @NotNull
  public KotlinTypeInfo illegalStatementType(
      @NotNull KtExpression expression,
  ...
  public static Collection<KotlinType> getAllPossibleTypes(
      @NotNull KtExpression expression, @NotNull KotlinType type, @NotNull ResolutionContext c) {
  ...
  public KotlinTypeInfo createCompileTimeConstantTypeInfo(
    ...
    if (value instanceof IntegerValueTypeConstant) {
      ...
      if (context.contextDependency == INDEPENDENT) {
        ...
        constantExpressionEvaluator.updateNumberType(
            expressionType, expression, context.statementFilter, context.trace);
      } else {
    ...
    NewSchemeOfIntegerOperatorResolutionChecker.checkArgument(
        context.expectedType, expression, context.trace, module);
