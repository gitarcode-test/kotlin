...
class LambdaExpressionElementType extends IErrorCounterReparseableElementType {
  ...
  public ASTNode parseContents(ASTNode chameleon) {
    ...
    PsiBuilder builder =
        PsiBuilderFactory.getInstance()
            .createBuilder(project, chameleon, null, KotlinLanguage.INSTANCE, chameleon.getChars());
  ...
  public boolean isParsable(
      @Nullable ASTNode parent, CharSequence buffer, Language fileLanguage, Project project) {
  ...
  private static boolean wasParameterCommaMovedOrDeleted(
  ...
  private static KtLambdaExpression findLambdaExpression(@Nullable ASTNode parent) {
    ...
    KtLambdaExpression[] lambdaExpressions =
        PsiTreeUtil.getChildrenOfType(parentPsi, KtLambdaExpression.class);
  ...
  private static boolean hasTokenMoved(
      String oldText, CharSequence buffer, int oldOffset, IElementType tokenType) {
    ...
    while (true) {
      ...
      if (oldType == null)
        break; // Didn't find an expected token. Consider it as no token was present.
