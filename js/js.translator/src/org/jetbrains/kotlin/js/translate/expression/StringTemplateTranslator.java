...
public final class StringTemplateTranslator extends AbstractTranslator {
  ...
  private StringTemplateTranslator(
      @NotNull KtStringTemplateExpression expression, @NotNull TranslationContext context) {
    super(context);

    expressionEntries = expression.getEntries();
    assert expressionEntries.length != 0
        : message(expression, "String template must have one or more entries.");
  }
  ...
  private final class EntryVisitor extends KtVisitorVoid {
    ...
    @Nullable private JsExpression resultingExpression = null;
    ...
    public void visitStringTemplateEntryWithExpression(
      ...
      JsExpression translatedExpression =
          Translation.translateAsExpression(entryExpression, context());
      ...
      if (type != null && KotlinBuiltIns.isCharOrNullableChar(type)) {
        if (type.isMarkedNullable()) {
          ...
          append(
              new JsConditional(
                  JsAstUtils.equality(tmp.assignmentExpression(), new JsNullLiteral()),
                  new JsNullLiteral(),
        ...
        append(
            TopLevelFIF.TO_STRING.apply(
                (JsExpression) null, new SmartList<>(translatedExpression), context()));
