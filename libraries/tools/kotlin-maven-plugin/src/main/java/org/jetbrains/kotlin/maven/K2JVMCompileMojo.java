...
public class K2JVMCompileMojo extends KotlinCompileMojoBase<K2JVMCompilerArguments> {
  ...
  @Parameter(
      property = "kotlin.compiler.incremental.cache.root",
      defaultValue = "${project.build.directory}/kotlin-ic")
  ...
  protected List<String> getSourceFilePaths() {
    ...
    File sourcesDir =
        AnnotationProcessingManager.getGeneratedSourcesDirectory(project, getSourceSetName());
    if (sourcesDir.isDirectory()) {
      paths.add(sourcesDir.getAbsolutePath());
    }

    File kotlinSourcesDir =
        AnnotationProcessingManager.getGeneratedKotlinSourcesDirectory(project, getSourceSetName());
  ...
  protected void configureSpecificCompilerArguments(
    ...
    if (module != null || testModule != null) {
      getLog()
          .warn(
              "Parameters module and testModule are deprecated and ignored, they will be removed in"
                  + " further release.");
    }
                ...
                file.getName().equals(PsiJavaModule.MODULE_INFO_FILE)
                    || file.isDirectory()
                        && Arrays.stream(file.listFiles())
                            .anyMatch(
                                child -> child.getName().equals(PsiJavaModule.MODULE_INFO_FILE)));
  }
  ...
  private ExitCode runIncrementalCompiler(
      MessageCollector messageCollector, K2JVMCompilerArguments arguments, List<File> sourceRoots)
      throws MojoExecutionException {
    ...
    try {
      ...
      CompilerRunnerUtils.makeJvmIncrementally(
          cachesDir, sourceRoots, arguments, messageCollector, icReporter);

      int compiledKtFilesCount = icReporter.getCompiledKotlinFiles().size();
      getLog()
          .info(
              "Compiled "
                  + icReporter.getCompiledKotlinFiles().size()
                  + " Kotlin files using incremental compiler");
