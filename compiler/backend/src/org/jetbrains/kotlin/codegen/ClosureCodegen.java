...
public class ClosureCodegen extends MemberCodegen<KtElement> {
  ...
  public ClosureCodegen(
    ...
    this.functionReferenceTarget =
        functionReferenceCall != null ? functionReferenceCall.getResultingDescriptor() : null;
    ...
    this.shouldHaveBoundReferenceReceiver =
        CallableReferenceUtilKt.isForBoundCallableReference(closure);

    ClassifierDescriptor superClassDescriptor =
        superClassType.getConstructor().getDeclarationDescriptor();
  ...
  protected void generateDeclaration() {
    ...
    for (int i = 0; i < superInterfaceTypes.size(); i++) {
      ...
      if (samType != null
          && superInterfaceType.getConstructor() == samType.getType().getConstructor()) {
    ...
    v.defineClass(
        ...
        ACC_FINAL
            | ACC_SUPER
            | visibilityFlag
            | getSyntheticAccessFlagForLambdaClass(classDescriptor),
  ...
  protected void generateClosureBody() {
    functionCodegen.generateMethod(
        JvmDeclarationOriginKt.OtherOrigin(element, funDescriptor), funDescriptor, strategy);
  ...
  protected void generateBridges() {
    ...
    List<KotlinType> bridgeParameterKotlinTypes =
        CollectionsKt.map(erasedInterfaceFunction.getValueParameters(), ValueDescriptor::getType);
  ...
  static void generateBridgesForSAM(
    ...
    descriptorForBridges.initialize(
        erasedInterfaceFunction.getExtensionReceiverParameter(),
  ...
  protected void generateKotlinMetadataAnnotation() {
    ...
    FunctionDescriptor freeLambdaDescriptor =
        FakeDescriptorsForReferencesKt.createFreeFakeLambdaDescriptor(
  ...
  public StackValue putInstanceOnStack(
      @NotNull ExpressionCodegen codegen, @Nullable StackValue functionReferenceReceiver) {
    return StackValue.operation(
        ...
        v -> {
          if (isConst(closure)) {
            ...
            codegen.pushClosureOnStack(
                classDescriptor, true, codegen.defaultCallGenerator, functionReferenceReceiver);
            v.invokespecial(
  ...
  private void generateBridge(
    ...
    MemberCodegen.markLineNumberForDescriptor(
        DescriptorUtils.getParentOfType(funDescriptor, ClassDescriptor.class), iv);

    Type[] bridgeParameterTypes = bridge.getArgumentTypes();
    if (isVarargInvoke) {
      assert bridgeParameterTypes.length == 1
              && bridgeParameterTypes[0].equals(AsmUtil.getArrayType(OBJECT_TYPE))
    ...
    List<ParameterDescriptor> calleeParameters =
        CollectionsKt.plus(
    ...
    StackValue.onStack(delegate.getReturnType(), delegateReturnType)
  ...
  private void generateFunctionReferenceMethods(@NotNull FunctionDescriptor descriptor) {
...

    {
      MethodVisitor mv =
          v.newMethod(
              ...
              flags,
              "getOwner",
              Type.getMethodDescriptor(K_DECLARATION_CONTAINER_TYPE),
              null,
              null);
      if (generateBody) {
        mv.visitCode();
...

    {
      MethodVisitor mv =
          v.newMethod(
              NO_ORIGIN, flags, "getName", Type.getMethodDescriptor(JAVA_STRING_TYPE), null, null);
      if (generateBody) {
        mv.visitCode();
        InstructionAdapter iv = new InstructionAdapter(mv);
...

    {
      MethodVisitor mv =
          v.newMethod(
              ...
              flags,
              "getSignature",
              Type.getMethodDescriptor(JAVA_STRING_TYPE),
              null,
              null);
      if (generateBody) {
        mv.visitCode();
  ...
  protected Method generateConstructor() {
    List<FieldInfo> args =
        calculateConstructorParameters(
            typeMapper, state.getLanguageVersionSettings(), closure, asmType);
    ...
    if (state.getClassBuilderMode().generateBodies) {
      ...
      Pair<Integer, FieldInfo> receiverIndexAndFieldInfo =
          CallableReferenceUtilKt.generateClosureFieldsInitializationFromParameters(
              iv, closure, args);
      ...
      if (superClassAsmType.equals(LAMBDA)
        ...
        if (isOptimizedFunctionReference || isAdaptedFunctionReference) {
          ...
          CallableReferenceUtilKt.generateFunctionReferenceSignature(
              iv, functionReferenceTarget, state);
          int flags =
        ...
        assert !shouldHaveBoundReferenceReceiver
            : "Unexpected bound reference with supertype " + superClassAsmType;
      }
      iv.invokespecial(
          superClassAsmType.getInternalName(),
          "<init>",
  ...
  private static int calculateFunctionReferenceFlags(
      @NotNull ResolvedCall<?> call, @NotNull FunctionDescriptor anonymousAdapterFunction) {
    ...
    for (int i = shift;
        i < anonymousAdapterFunction.getValueParameters().size()
            && i - shift < target.getValueParameters().size();
        i++) {
      ValueParameterDescriptor targetParameter = target.getValueParameters().get(i - shift);
      ValueParameterDescriptor adaptedParameter =
          anonymousAdapterFunction.getValueParameters().get(i);

      // Vararg to element conversion is happening if the target parameter is vararg (e.g. `vararg
      // xs: Int`),
      // but the adapted parameter's type is not equal to the target parameter's type (which is
      // `IntArray`).
    ...
    boolean hasCoercionToUnit =
        KotlinBuiltIns.isUnit(anonymousAdapterFunction.getReturnType())
    ...
    if (captureReceiverType != null) {
      ...
      args.add(
          FieldInfo.createForHiddenField(
              ownerType, typeMapper.mapType(captureReceiverType), captureReceiverType, fieldName));
    }
  ...
  public static FunctionDescriptor getErasedInvokeFunction(@NotNull FunctionDescriptor function) {
    ClassDescriptor functionClass =
        DescriptorUtilsKt.getBuiltIns(function)
            .getFunction(
                function.getValueParameters().size()
                    + (function.getExtensionReceiverParameter() != null ? 1 : 0));
