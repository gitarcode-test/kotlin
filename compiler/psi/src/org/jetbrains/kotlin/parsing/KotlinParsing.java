...
public class KotlinParsing extends AbstractKotlinParsing {
  ...
  private static final TokenSet TYPE_PARAMETER_GT_RECOVERY_SET =
      TokenSet.create(WHERE_KEYWORD, LPAR, COLON, LBRACE, GT);
  public static final TokenSet PARAMETER_NAME_RECOVERY_SET =
  ...
  private static final TokenSet IMPORT_RECOVERY_SET =
      TokenSet.create(AS_KEYWORD, DOT, EOL_OR_SEMICOLON);
  private static final TokenSet TYPE_REF_FIRST =
      TokenSet.create(LBRACKET, IDENTIFIER, LPAR, HASH, DYNAMIC_KEYWORD);
  ...
  private static final TokenSet SOFT_KEYWORDS_AT_MEMBER_START =
      TokenSet.create(CONSTRUCTOR_KEYWORD, INIT_KEYWORD);
  ...
  private static final TokenSet IDENTIFIER_RBRACKET_LBRACKET_SET =
      TokenSet.create(IDENTIFIER, RBRACKET, LBRACKET);
  private static final TokenSet LBRACE_RBRACE_SET = TokenSet.create(LBRACE, RBRACE);
  private static final TokenSet COMMA_SEMICOLON_RBRACE_SET =
      TokenSet.create(COMMA, SEMICOLON, RBRACE);
  private static final TokenSet VALUE_ARGS_RECOVERY_SET =
      TokenSet.create(LBRACE, SEMICOLON, RPAR, EOL_OR_SEMICOLON, RBRACE);
  private static final TokenSet PROPERTY_NAME_FOLLOW_SET =
      TokenSet.create(
          ...
          FUN_KEYWORD,
          CLASS_KEYWORD);
  private static final TokenSet PROPERTY_NAME_FOLLOW_MULTI_DECLARATION_RECOVERY_SET =
      TokenSet.orSet(PROPERTY_NAME_FOLLOW_SET, PARAMETER_NAME_RECOVERY_SET);
  private static final TokenSet PROPERTY_NAME_FOLLOW_FUNCTION_OR_PROPERTY_RECOVERY_SET =
      TokenSet.orSet(PROPERTY_NAME_FOLLOW_SET, LBRACE_RBRACE_SET, TOP_LEVEL_DECLARATION_FIRST);
  private static final TokenSet IDENTIFIER_EQ_COLON_SEMICOLON_SET =
      TokenSet.create(IDENTIFIER, EQ, COLON, SEMICOLON);
  ...
  private static final TokenSet RPAR_IDENTIFIER_COLON_LBRACE_EQ_SET =
      TokenSet.create(RPAR, IDENTIFIER, COLON, LBRACE, EQ);
  ...
  private static final TokenSet LBRACKET_LBRACE_RBRACE_LPAR_SET =
      TokenSet.create(LBRACKET, LBRACE, RBRACE, LPAR);
  private static final TokenSet FUNCTION_NAME_FOLLOW_SET =
      TokenSet.create(LT, LPAR, RPAR, COLON, EQ);
  private static final TokenSet FUNCTION_NAME_RECOVERY_SET =
      TokenSet.orSet(
          TokenSet.create(LT, LPAR, RPAR, COLON, EQ),
          LBRACE_RBRACE_SET,
          TOP_LEVEL_DECLARATION_FIRST);
  private static final TokenSet VALUE_PARAMETERS_FOLLOW_SET =
      TokenSet.create(EQ, LBRACE, RBRACE, SEMICOLON, RPAR);
  private static final TokenSet LPAR_VALUE_PARAMETERS_FOLLOW_SET =
      TokenSet.orSet(TokenSet.create(LPAR), VALUE_PARAMETERS_FOLLOW_SET);
  private static final TokenSet LPAR_LBRACE_COLON_CONSTRUCTOR_KEYWORD_SET =
      TokenSet.create(LPAR, LBRACE, COLON, CONSTRUCTOR_KEYWORD);
  private static final TokenSet definitelyOutOfReceiverSet =
      TokenSet.orSet(
          TokenSet.create(EQ, COLON, LBRACE, RBRACE, BY_KEYWORD), TOP_LEVEL_DECLARATION_FIRST);
  private static final TokenSet EOL_OR_SEMICOLON_RBRACE_SET =
      TokenSet.create(EOL_OR_SEMICOLON, RBRACE);
  private static final TokenSet CLASS_INTERFACE_SET =
      TokenSet.create(CLASS_KEYWORD, INTERFACE_KEYWORD);
  ...
  private static KotlinParsing createForByClause(
      SemanticWhitespaceAwarePsiBuilder builder, boolean isLazy) {
    return new KotlinParsing(
        new SemanticWhitespaceAwarePsiBuilderForByClause(builder), false, isLazy);
  }
  ...
  private KotlinParsing(
      SemanticWhitespaceAwarePsiBuilder builder, boolean isTopLevel, boolean isLazy) {
  ...
  private void checkUnclosedBlockComment() {
    if (BLOCK_DOC_COMMENT_SET.contains(myBuilder.rawLookup(-1))) {
      ...
      if (!(tokenChars.length() > 2
          && tokenChars
              .subSequence(tokenChars.length() - 2, tokenChars.length())
              .toString()
              .equals("*/"))) {
  ...
  private void parsePreamble() {
    ...
    if (at(PACKAGE_KEYWORD)) {
      ...
      // When package directive is omitted we should not report error on non-file annotations at the
      // beginning of the file.
      // So, we rollback the parsing position and reparse file annotation list without report error
      // on non-file annotations.
      firstEntry.rollbackTo();

      parseFileAnnotationList(FILE_ANNOTATIONS_WHEN_PACKAGE_OMITTED);
      packageDirective = mark();
      packageDirective.done(PACKAGE_DIRECTIVE);
      // Need to skip everything but shebang comment to allow comments at the start of the file to
      // be bound to the first declaration.
  ...
  private void parseImportDirective() {
    ...
    while (at(DOT) && lookahead(1) != MUL) {
      ...
      if (closeImportWithErrorIfNewline(
          importDirective, null, "Import must be placed on a single line")) {
        qualifiedName.drop();
        return;
      }
  ...
  private void parseImportDirectives() {
    ...
    if (!at(IMPORT_KEYWORD)) {
      // this is necessary to allow comments at the start of the file to be bound to the first
      // declaration
  ...
  private void parseTopLevelDeclaration() {
    ...
    IElementType declType =
        parseCommonDeclaration(
            detector, NameParsingMode.REQUIRED, DeclarationParsingMode.MEMBER_OR_TOPLEVEL);
   ...
   * (modifier | annotation)*
   *
   * <p>Feeds modifiers (not annotations) into the passed consumer, if it is not null
   *
   * @param noModifiersBefore is a token set with elements indicating when met them that previous
   *     token must be parsed as an identifier rather than modifier
   */
  boolean parseModifierList(
    ...
    return doParseModifierList(
        tokenConsumer, MODIFIER_KEYWORDS, AnnotationParsingMode.DEFAULT, noModifiersBefore);
  }
  ...
  private boolean doParseModifierList(
    ...
    boolean empty =
        doParseModifierListBody(
            tokenConsumer, modifierKeywords, annotationParsingMode, noModifiersBefore);
  ...
  private boolean parseAnnotationOrList(AnnotationParsingMode mode) {
    if (at(AT)) {
      ...
      if ((nextRawToken == IDENTIFIER || ANNOTATION_TARGETS.contains(nextRawToken))
          && lookahead(2) == COLON) {
  ...
  private boolean parseAnnotationTargetIfNeeded(AnnotationParsingMode mode) {
    ...
    if (mode == FILE_ANNOTATIONS_WHEN_PACKAGE_OMITTED
        && !(targetKeyword == FILE_KEYWORD && lookahead(1) == COLON)) {
      return false;
    }
  ...
  private boolean parseAnnotation(AnnotationParsingMode mode) {
    ...
    boolean whitespaceAfterAnnotation =
        WHITE_SPACE_OR_COMMENT_BIT_SET.contains(myBuilder.rawLookup(-1));
    boolean shouldBeParsedNextAsFunctionalType =
        at(LPAR) && whitespaceAfterAnnotation && mode.withSignificantWhitespaceBeforeArguments;
  ...
  private IElementType parseClassOrObject(
    ...
    if (expectKindKeyword) {
      ...
      assert enumClass
          : "Currently classifiers without class/interface/object are only allowed for enums";
  ...
  private IElementType parseMemberDeclarationRest(@NotNull ModifierDetector modifierDetector) {
    IElementType declType =
        parseCommonDeclaration(
            modifierDetector,
            modifierDetector.isCompanionDetected()
                ? NameParsingMode.ALLOWED
  ...
  private void parseSecondaryConstructor() {
    ...
    if (at(LPAR)) {
      ...
      errorWithRecovery(
          "Expecting '('", TokenSet.orSet(VALUE_ARGS_RECOVERY_SET, TokenSet.create(COLON)));
    }
  ...
  enum DeclarationParsingMode {
    ...
    DeclarationParsingMode(
        ...
        boolean canBeEnumUsedAsSoftKeyword) {
  ...
  public IElementType parseProperty(DeclarationParsingMode mode) {
    ...
    boolean typeParametersDeclared =
        at(LT) && parseTypeParameterList(IDENTIFIER_EQ_COLON_SEMICOLON_SET);
    ...
    if (at(COLON)) {
      ...
      parseTypeRef();
      errorIf(
          type, multiDeclaration, "Type annotations are not allowed on destructuring declarations");
    }

    parseTypeConstraintsGuarded(typeParametersDeclared);

    if (!parsePropertyDelegateOrAssignment()
        && isNameOnTheNextLine
        && noTypeReference
        && !receiverTypeDeclared) {
    ...
    if (mode.accessorsAllowed) {
      ...
      boolean hasNewLineWithSemicolon =
          consumeIf(SEMICOLON) && myBuilder.newlineBeforeCurrentToken();
      myBuilder.restoreNewlinesState();
        ...
        if (!atSet(EOL_OR_SEMICOLON_RBRACE_SET)) {
          if (getLastToken() != SEMICOLON) {
            errorUntil(
                ...
                TokenSet.orSet(
                    DECLARATION_FIRST, TokenSet.create(EOL_OR_SEMICOLON, LBRACE, RBRACE)));
          }
        } else {
          consumeIf(SEMICOLON);
        }
   ...
   */
  @Nullable
  private PropertyComponentKind parsePropertyComponent(
      PropertyComponentKind.Collector notAllowedKind) {
    ...
    if (propertyComponentKind != PropertyComponentKind.FIELD) {
      ...
      if (!at(RPAR)) {
        errorUntil(
            "Expecting ')'", TokenSet.create(RPAR, COLON, LBRACE, RBRACE, EQ, EOL_OR_SEMICOLON));
      }
      if (at(RPAR)) {
        advance();
      }
  ...
  private int lastDotAfterReceiver() {
    AbstractTokenStreamPattern pattern =
        at(LPAR) ? lastDotAfterReceiverLParPattern : lastDotAfterReceiverNotLParPattern;
    pattern.reset();
   ...
   * IDENTIFIER
   */
  private void parseFunctionOrPropertyName(
      boolean receiverFound,
      String title,
      TokenSet nameFollow,
      TokenSet recoverySet,
    ...
    errorWithRecovery(
        "Expecting " + title + " name" + (!receiverFound ? " or receiver type" : ""), recoverySet);
  }
  ...
  private void parseTypeConstraint() {
    ...
    if (expect(
        ...
        COLON_COMMA_LBRACE_RBRACE_TYPE_REF_FIRST_SET)) {
      reference.done(REFERENCE_EXPRESSION);
    } else {
      reference.drop();
    }
  ...
  private void parseTypeRef(TokenSet extraRecoverySet, boolean allowSimpleIntersectionTypes) {
    PsiBuilder.Marker typeRefMarker =
        parseTypeRefContents(extraRecoverySet, allowSimpleIntersectionTypes);
  ...
  private PsiBuilder.Marker parseTypeRefContents(
      TokenSet extraRecoverySet, boolean allowSimpleIntersectionTypes) {
    ...
    if (at(IDENTIFIER)
        && !(lookahead == DOT && lookahead2 == IDENTIFIER)
        && lookahead != LT
        && at(DYNAMIC_KEYWORD)) {
      ...
      advance(); // LPAR
      parseTypeRefContents(TokenSet.EMPTY, /* allowSimpleIntersectionTypes */ true)
  ...
  private boolean recoverOnParenthesizedWordForPlatformTypes(
      int offset, String word, boolean consume) {
  ...
  boolean tryParseTypeArgumentList(TokenSet extraRecoverySet) {
    ...
    while (true) {
      ...
      // Annotations on other kinds of type arguments should be parsed as common type annotations
      // (within parseTypeRef call)
  ...
  private static final TokenSet NO_MODIFIER_BEFORE_FOR_VALUE_PARAMETER =
      TokenSet.create(COMMA, COLON, EQ, RPAR);
  ...
  private void parseValueParameterList(
      boolean isFunctionTypeContents, boolean typeRequired, TokenSet recoverySet) {
    assert _at(LPAR);
    ...
    if (!at(RPAR) && !atSet(recoverySet)) {
      while (true) {
        ...
        if (at(COMMA)) {
          ...
          // recovery for the case "fun bar(x: Array<Int> : Int)" when we've just parsed "x:
          // Array<Int>"
          ...
          if (!atSet(isFunctionTypeContents ? LAMBDA_VALUE_PARAMETER_FIRST : VALUE_PARAMETER_FIRST))
            break;
        }
      }
    }
  ...
  private boolean parseFunctionParameterRest(boolean typeRequired) {
    ...
    if ((at(IDENTIFIER) && lookahead(1) == LT) || at(COLON)) {
      ...
      if (at(COLON)) {
        // We keep noErrors == true so that unnamed parameters starting with ":" are not rolled back
        // during parsing of functional types
        advance(); // COLON
      } else {
        noErrors = false;
      }
      ...
      if (at(COLON)) {
        ...
        if (at(IDENTIFIER) && lookahead(1) == COLON) {
          // recovery for the case "fun foo(x: y: Int)" when we're at "y: " it's likely that this is
          // a name of the next parameter,
