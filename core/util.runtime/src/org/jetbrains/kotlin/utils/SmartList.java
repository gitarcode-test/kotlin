...
/**
 * A List which is optimised for the sizes of 0 and 1, in which cases it would not allocate array at
 * all.
 *
 * <p>This class was copied from com.intellij.util.SmartList.
 */
...
public class SmartList<E> extends AbstractList<E> implements RandomAccess {
  ...
  public SmartList(@NotNull Collection<? extends E> elements) {
    ...
    if (size == 1) {
      E element =
          elements instanceof List ? (E) ((List) elements).get(0) : elements.iterator().next();
      add(element);
    } else if (size > 0) {
      mySize = size;
    ...
    protected void checkCoModification() {
      if (modCount != myInitialModCount) {
        throw new ConcurrentModificationException(
            "ModCount: " + modCount + "; expected: " + myInitialModCount);
      }
    }
  ...
  /**
   * Trims the capacity of this list to be the list's current size. An application can use this
   * operation to minimize the storage of a list instance.
   */
