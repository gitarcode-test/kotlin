...
public class LazyClassDescriptor extends ClassDescriptorBase
  ...
  private static final Function1<KotlinType, Boolean> VALID_SUPERTYPE =
      type -> {
        assert !KotlinTypeKt.isError(type)
  ...
  private final MemoizedFunctionToNotNull<KtObjectDeclaration, ClassDescriptor>
  ...
  public LazyClassDescriptor(
    ...
    this.staticScope =
        kind == ClassKind.ENUM_CLASS
            ? new StaticScopeForKotlinEnum(storageManager, this, /* enumEntriesCanBeUsed= */ true)
    ...
    this.visibility =
        isLocal
            ...
            : resolveVisibilityFromModifiers(
                modifierList, DescriptorVisibilities.DEFAULT_VISIBILITY);

    this.isInner =
        modifierList != null && modifierList.hasModifier(INNER_KEYWORD) && !isIllegalInner(this);
    ...
    this.isExpect =
        modifierList != null && PsiUtilsKt.hasExpectModifier(modifierList)
            || containingDeclaration instanceof ClassDescriptor
                && ((ClassDescriptor) containingDeclaration).isExpect();
    ...
    this.extraCompanionObjectDescriptors =
        storageManager.createMemoizedFunction(this::computeCompanionObjectDescriptor);
    ...
    this.resolutionScopesSupport =
        new ClassResolutionScopesSupport(
            this, storageManager, c.getLanguageVersionSettings(), this::getOuterScope);
            ...
            .createLazyValue(
                () -> {
                  ...
                  boolean isAnonymousObject =
                      (classInfo.getClassKind() == ClassKind.CLASS)
                          && (classInfo.getCorrespondingClassOrObject()
                              instanceof KtObjectDeclaration);
                  ...
                  if (isAnonymousObject) {
                    ...
                    if (c.getLanguageVersionSettings()
                        .supportsFeature(
                            LanguageFeature.ProhibitTypeParametersInAnonymousObjects)) {
                  ...
                  for (int i = 0; i < typeParameters.size(); i++) {
                      ...
                      lazyAnnotations =
                          new LazyAnnotations(
                              new LazyAnnotationsContext(
                                  c.getAnnotationResolver(), storageManager, c.getTrace()) {
                                @NotNull
                                @Override
                                public LexicalScope getScope() {
                                  return getOuterScope();
                    ...
                    parameters.add(
                        new LazyTypeParameterDescriptor(c, this, parameter, lazyAnnotations, i));
                  }
        ...
        storageManager.createLazyValue(
            () -> {
              if (getModality() == Modality.SEALED) {
                return c.getSealedClassInheritorsProvider()
                    .computeSealedSubclasses(this, freedomForSealedInterfacesSupported);
              } else {
    ...
    this.contextReceivers =
        storageManager.createLazyValue(
            () -> {
                            ...
                            KotlinType kotlinType =
                                c.getTypeResolver()
                                    .resolveType(
                                        ...
                                        true);
                            Name label =
                                contextReceiver.labelNameAsName() != null
              ...
              if (c.getLanguageVersionSettings()
                  .supportsFeature(LanguageFeature.ContextReceivers)) {
                HashMultimap<String, ReceiverParameterDescriptor> labelNameToReceiverMap =
                    HashMultimap.create();

                for (int i = 0; i < contextReceivers.size(); i++) {
                  labelNameToReceiverMap.put(
                      contextReceivers.get(i).name(), contextReceiverDescriptors.get(i));
                }
  ...
  protected ScopesHolderForClass<LazyClassMemberScope> createScopesHolderForClass(
      @NotNull LazyClassContext c, @NotNull ClassMemberDeclarationProvider declarationProvider) {
    return ScopesHolderForClass.Companion.create(
        ...
        kotlinTypeRefinerForDependentModule -> {
          LazyClassMemberScope scopeForDeclaredMembers =
              !kotlinTypeRefinerForDependentModule.isRefinementNeededForModule(
                      c.getModuleDescriptor())
                  ? null
                  : scopesHolderForClass.getScope(
                      c.getKotlinTypeCheckerOfOwnerModule()
                          .getKotlinTypeRefiner()); // essentially, a scope for owner-module
  ...
  @Nullable
  private ClassDescriptorWithResolutionScopes computeCompanionObjectDescriptor(
  ...
  @Nullable
  private static KtClassLikeInfo getCompanionObjectInfo(
      @Nullable KtObjectDeclaration companionObject) {
  ...
  private KtObjectDeclaration getCompanionObjectIfAllowed() {
    KtObjectDeclaration companionObject =
        firstOrNull(declarationProvider.getOwnerInfo().getCompanionObjects());
  ...
  public ValueClassRepresentation<SimpleType> getValueClassRepresentation() {
    ...
    InlineClassRepresentation<SimpleType> invalidValueClassRepresentation =
        new InlineClassRepresentation<>(
            SpecialNames.SAFE_IDENTIFIER_FOR_NO_NAME,
            c.getModuleDescriptor().getBuiltIns().getAnyType());
    if (constructor == null) {
    ...
    if (isRecursiveInlineClass(constructor, new HashSet<>())) {
      return new InlineClassRepresentation<>(
          parameters.get(0).getName(), (SimpleType) parameters.get(0).getType());
    }
    if (parameters.size() == 0) {
      return invalidValueClassRepresentation;
    }
    List<Pair<Name, SimpleType>> fields =
        parameters.stream()
  ...
  private static boolean isRecursiveInlineClass(
      @Nullable ClassConstructorDescriptor constructor, @NotNull Set<ClassDescriptor> visited) {
    if (constructor == null
        || constructor.getValueParameters().size() != 1
        || !(constructor.getConstructedClass().isValue()
            || constructor.getConstructedClass().isInline())) {
      return false;
    }
    ...
    if (descriptor instanceof ClassDescriptor) {
      ClassConstructorDescriptor newConstructor =
          ((ClassDescriptor) descriptor).getUnsubstitutedPrimaryConstructor();
  ...
  private class LazyClassTypeConstructor extends AbstractClassTypeConstructor {
    ...
    protected boolean getShouldReportCyclicScopeWithCompanionWarning() {
      return !c.getLanguageVersionSettings()
          .supportsFeature(
              LanguageFeature.ProhibitVisibilityOfNestedClassifiersFromSupertypesOfCompanion);
    }
