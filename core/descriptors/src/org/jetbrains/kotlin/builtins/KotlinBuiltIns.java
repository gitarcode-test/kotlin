...
public abstract class KotlinBuiltIns {
  ...
  private final NotNullLazyValue<Collection<PackageViewDescriptor>>
  ...
  protected KotlinBuiltIns(@NotNull StorageManager storageManager) {
    ...
    this.builtInPackagesImportedByDefault =
        storageManager.createLazyValue(
            new Function0<Collection<PackageViewDescriptor>>() {
              @Override
    ...
    this.primitives =
        storageManager.createLazyValue(
            new Function0<Primitives>() {
              @Override
              public Primitives invoke() {
                Map<PrimitiveType, SimpleType> primitiveTypeToArrayKotlinType =
                    new EnumMap<PrimitiveType, SimpleType>(PrimitiveType.class);
                Map<KotlinType, SimpleType> primitiveKotlinTypeToKotlinArrayType =
                    new HashMap<KotlinType, SimpleType>();
                Map<SimpleType, SimpleType> kotlinArrayTypeToPrimitiveKotlinType =
                ...
                for (PrimitiveType primitive : PrimitiveType.values()) {
                  ...
                  SimpleType arrayType =
                      getBuiltInTypeByClassName(primitive.getArrayTypeName().asString());
    ...
    this.builtInClassesByName =
        storageManager.createMemoizedFunction(
            new Function1<Name, ClassDescriptor>() {
              @Override
              public ClassDescriptor invoke(Name name) {
                ClassifierDescriptor classifier =
                    getBuiltInsPackageScope()
                        .getContributedClassifier(name, NoLookupLocation.FROM_BUILTINS);
                if (classifier == null) {
                  throw new AssertionError(
                      "Built-in class " + BUILT_INS_PACKAGE_FQ_NAME.child(name) + " is not found");
                }
                if (!(classifier instanceof ClassDescriptor)) {
                  throw new AssertionError(
                      "Must be a class descriptor " + name + ", but was " + classifier);
                }
  ...
  protected void createBuiltInsModule(boolean isFallback) {
    ...
    builtInsModule.initialize(
        BuiltInsLoader.Companion.getInstance()
  ...
  public void setPostponedBuiltinsModuleComputation(
      @NotNull Function0<ModuleDescriptorImpl> computation) {
  ...
  public ModuleDescriptorImpl getBuiltInsModule() {
    assert builtInsModule != null || postponedBuiltInsModule != null
        : "Uninitialized built-ins module";
    if (builtInsModule == null) {
  ...
  /**
   * Checks if the given descriptor is declared in the deserialized built-in package fragment, i.e.
   * if it was loaded as a part of loading .kotlin_builtins definition files.
   *
   * <p>NOTE: this method returns false for descriptors loaded from .class files or other binaries,
   * even if they are "built-in" in some other sense! For example, it returns false for the class
   * descriptor of `kotlin.IntRange` loaded from `kotlin/IntRange.class`. In case you need to check
   * if the class is "built-in" in another sense, you should probably do it by inspecting its FQ
   * name, or the FQ name of its containing package.
   */
  public static boolean isBuiltIn(@NotNull DeclarationDescriptor descriptor) {
    return DescriptorUtils.getParentOfType(descriptor, BuiltInsPackageFragment.class, false)
        != null;
  }

  /**
   * @return true if the containing package of the descriptor is "kotlin" or any subpackage of
   *     "kotlin"
   */
  ...
  public ClassDescriptor getBuiltInClassByFqName(@NotNull FqName fqName) {
    ClassDescriptor descriptor =
        DescriptorUtilKt.resolveClassByFqName(
            getBuiltInsModule(), fqName, NoLookupLocation.FROM_BUILTINS);
  ...
  public ClassDescriptor getSuspendFunction(int parameterCount) {
    return getBuiltInClassByFqName(
        COROUTINES_PACKAGE_FQ_NAME.child(Name.identifier(getSuspendFunctionName(parameterCount))));
  }
  ...
  public ClassDescriptor getKSuspendFunction(int parameterCount) {
    Name name =
        Name.identifier(
            FunctionTypeKind.KSuspendFunction.INSTANCE.getClassNamePrefix() + parameterCount);
  ...
  public KotlinType getArrayElementType(@NotNull KotlinType arrayType) {
    ...
    KotlinType primitiveType =
        primitives.invoke().kotlinArrayTypeToPrimitiveKotlinType.get(notNullArrayType);
  ...
  @Nullable
  private static KotlinType getElementTypeForUnsignedArray(
      @NotNull KotlinType notNullArrayType, @NotNull ModuleDescriptor module) {
    ...
    ClassDescriptor elementClassDescriptor =
        FindClassInModuleKt.findClassAcrossModuleDependencies(module, elementClassId);
   ...
   */
  @Nullable
  public SimpleType getPrimitiveArrayKotlinTypeByPrimitiveKotlinType(
      @NotNull KotlinType kotlinType) {
    SimpleType primitiveArray =
        primitives.invoke().primitiveKotlinTypeToKotlinArrayType.get(kotlinType);
    ...
    if (UnsignedTypes.INSTANCE.isUnsignedType(kotlinType)) {
      ...
      ClassId unsignedClassId =
          DescriptorUtilsKt.getClassId(kotlinType.getConstructor().getDeclarationDescriptor());
      assert unsignedClassId != null
          : "unsignedClassId should not be null for unsigned type " + kotlinType;

      ClassId arrayClassId =
          UnsignedTypes.INSTANCE.getUnsignedArrayClassIdByUnsignedClassId(unsignedClassId);
      assert arrayClassId != null
          : "arrayClassId should not be null for unsigned type " + unsignedClassId;

      ClassDescriptor arrayClassDescriptor =
          FindClassInModuleKt.findClassAcrossModuleDependencies(module, arrayClassId);
  ...
  @NotNull
  public SimpleType getArrayType(
      @NotNull Variance projectionType,
      @NotNull KotlinType argument,
    ...
    return KotlinTypeFactory.simpleNotNullType(
        TypeAttributesKt.toDefaultAttributes(annotations), getArray(), types);
  }
  ...
  public SimpleType getEnumType(@NotNull SimpleType argument) {
    ...
    List<TypeProjectionImpl> types =
        Collections.singletonList(new TypeProjectionImpl(projectionType, argument));
    return KotlinTypeFactory.simpleNotNullType(
        TypeAttributes.Companion.getEmpty(), getEnum(), types);
  }
  ...
  public static boolean isConstructedFromGivenClass(
      @NotNull KotlinType type, @NotNull FqName fqName) {
    return isConstructedFromGivenClass(type, fqName.toUnsafe());
  }

  public static boolean isTypeConstructorForGivenClass(
      @NotNull TypeConstructor typeConstructor, @NotNull FqNameUnsafe fqName) {
    ClassifierDescriptor descriptor = typeConstructor.getDeclarationDescriptor();
    return descriptor instanceof ClassDescriptor && classFqNameEquals(descriptor, fqName);
  }

  private static boolean classFqNameEquals(
      @NotNull ClassifierDescriptor descriptor, @NotNull FqNameUnsafe fqName) {
    // Quick check to avoid creation of full FqName instance
    return descriptor.getName().equals(fqName.shortName()) && fqName.equals(getFqName(descriptor));
  }
  ...
  public static boolean isSpecialClassWithNoSupertypes(@NotNull ClassDescriptor descriptor) {
    return classFqNameEquals(descriptor, FqNames.any)
        || classFqNameEquals(descriptor, FqNames.nothing);
  }
  ...
  public static boolean mayReturnNonUnitValue(@NotNull FunctionDescriptor descriptor) {
    ...
    for (FunctionDescriptor overriddenDescriptor :
        descriptor.getOriginal().getOverriddenDescriptors()) {
  ...
  // This function only checks presence of Deprecated annotation at declaration-site, it doesn't
  // take into account @DeprecatedSinceKotlin
  // To check that a referenced descriptor is actually deprecated at call-site, use
  // DeprecationResolver
