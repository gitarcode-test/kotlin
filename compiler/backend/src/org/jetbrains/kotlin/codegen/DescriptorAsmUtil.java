...
public class DescriptorAsmUtil {
  ...
  private static String getLabeledThisNameForReceiver(
    ...
    if (descriptor instanceof FunctionDescriptor) {
      String labelName =
          bindingContext.get(
              CodegenBinding.CALL_LABEL_FOR_LAMBDA_ARGUMENT, (FunctionDescriptor) descriptor);
      if (labelName != null) {
  ...
  @NotNull
  public static Type boxType(
      @NotNull Type type, @NotNull KotlinType kotlinType, @NotNull KotlinTypeMapper typeMapper) {
  ...
  public static boolean isAbstractMethod(
      FunctionDescriptor functionDescriptor, OwnerKind kind, JvmDefaultMode jvmDefaultMode) {
  ...
  public static boolean isStaticMethod(
      OwnerKind kind, CallableMemberDescriptor functionDescriptor) {
  ...
  public static boolean isStaticKind(OwnerKind kind) {
    return kind == OwnerKind.PACKAGE
        || kind == OwnerKind.DEFAULT_IMPLS
        || kind == OwnerKind.ERASED_INLINE_CLASS;
  }

  public static int getMethodAsmFlags(
      FunctionDescriptor functionDescriptor, OwnerKind kind, GenerationState state) {
    return getMethodAsmFlags(
        functionDescriptor, kind, state.getDeprecationProvider(), state.getJvmDefaultMode());
  }
  ...
  public static int getMethodAsmFlags(
    ...
    if (CodegenUtilKt.isJvmStaticInCompanionObject(functionDescriptor)) {
      // Native method will be a member of the class, the companion object method will be delegated
      // to it
      flags &= ~Opcodes.ACC_NATIVE;
    }
  ...
  private static boolean isInlineClassWrapperConstructor(
      @NotNull FunctionDescriptor functionDescriptor, @Nullable OwnerKind kind) {
    if (!(functionDescriptor instanceof ConstructorDescriptor)) return false;
    ClassDescriptor classDescriptor =
        ((ConstructorDescriptor) functionDescriptor).getConstructedClass();
    return InlineClassesUtilsKt.isInlineClass(classDescriptor) && kind == OwnerKind.IMPLEMENTATION;
  }

  public static int getCommonCallableFlags(
      FunctionDescriptor functionDescriptor, @NotNull GenerationState state) {
  ...
  private static int getVisibilityAccessFlag(
      @NotNull MemberDescriptor descriptor, @Nullable OwnerKind kind) {
    ...
    if (defaultMapping == null) {
      throw new IllegalStateException(
          visibility
              + " is not a valid visibility in backend for "
  ...
  */
  public static int getVisibilityAccessFlagForClass(@NotNull ClassDescriptor descriptor) {
    ...
    if (descriptor.getVisibility() == DescriptorVisibilities.PUBLIC
        ...
        ||
        // TODO: should be package private, but for now Kotlin's reflection can't access members of
        // such classes
  ...
  public static int getSyntheticAccessFlagForLambdaClass(@NotNull ClassDescriptor descriptor) {
    return descriptor instanceof SyntheticClassDescriptorForLambda
            && ((SyntheticClassDescriptorForLambda) descriptor).isCallableReference()
        ? ACC_SYNTHETIC
        : 0;
  }
  ...
  @Nullable
  private static Integer specialCaseVisibility(
      @NotNull MemberDescriptor memberDescriptor, @Nullable OwnerKind kind) {
    ...
    if (kind != OwnerKind.ERASED_INLINE_CLASS
        ...
        && InlineClassManglingRulesKt.shouldHideConstructorDueToValueClassTypeValueParameters(
            (ConstructorDescriptor) memberDescriptor)) {
      return ACC_PRIVATE;
    }

    // Sealed class constructors should be ACC_PRIVATE.
    // In 1.4 and before, sealed class constructors had PRIVATE visibility, and were represented as
    // private methods in bytecode.
    // In 1.5 (+AllowSealedInheritorsInDifferentFilesOfSamePackage), sealed class constructors
    // became INTERNAL,
    // but still should be represented as private methods in bytecode in order to prevent inheriting
    // from sealed classes on JVM.
    ...
    if (memberVisibility == DescriptorVisibilities.LOCAL
    ...
    if (memberDescriptor instanceof ConstructorDescriptor
        && isAnonymousObject(memberDescriptor.getContainingDeclaration())) {
      return getVisibilityAccessFlagForAnonymous(
          (ClassDescriptor) memberDescriptor.getContainingDeclaration());
    }

    if (memberDescriptor instanceof SyntheticJavaPropertyDescriptor) {
      return getVisibilityAccessFlag(
          ((SyntheticJavaPropertyDescriptor) memberDescriptor).getGetMethod());
    }
    if (memberDescriptor instanceof PropertyAccessorDescriptor) {
      PropertyDescriptor property =
          ((PropertyAccessorDescriptor) memberDescriptor).getCorrespondingProperty();
      if (property instanceof SyntheticJavaPropertyDescriptor) {
        FunctionDescriptor method =
            memberDescriptor == property.getGetter()
    ...
    if (memberDescriptor instanceof CallableDescriptor
        && memberVisibility == DescriptorVisibilities.PROTECTED) {
      for (CallableDescriptor overridden :
          DescriptorUtils.getAllOverriddenDescriptors((CallableDescriptor) memberDescriptor)) {
    ...
    if (captureReceiverType != null && !CallableReferenceUtilKt.isForCallableReference(closure)) {
      String fieldName =
          closure.getCapturedReceiverFieldName(
              typeMapper.getBindingContext(), languageVersionSettings);
  ...
  public static int genAssignInstanceFieldFromParam(
      FieldInfo info, int index, InstructionAdapter iv, int ownerIndex, boolean cast) {
  ...
  public static void genInvokeAppendMethod(
    ...
    CallableMethod specializedToString =
        getSpecializedToStringCallableMethodOrNull(kotlinType, typeMapper);
  ...
  public static StackValue genToString(
    ...
    return StackValue.operation(
        ...
        v -> {
          CallableMethod specializedToString =
              getSpecializedToStringCallableMethodOrNull(receiverKotlinType, typeMapper);
          if (specializedToString != null) {
          ...
          if (receiverKotlinType != null
              && InlineClassesUtilsKt.isInlineClassType(receiverKotlinType)) {
            type = OBJECT_TYPE;
          ...
          v.invokestatic(
              "java/lang/String",
              "valueOf",
              "(" + type.getDescriptor() + ")Ljava/lang/String;",
              false);
          return null;
        });
  }
  ...
  private static CallableMethod getSpecializedToStringCallableMethodOrNull(
      @Nullable KotlinType receiverKotlinType, @Nullable KotlinTypeMapper typeMapper) {
    ...
    DeclarationDescriptor receiverTypeDescriptor =
        receiverKotlinType.getConstructor().getDeclarationDescriptor();
    assert receiverTypeDescriptor != null
            && InlineClassesUtilsKt.isInlineClass(receiverTypeDescriptor)
    ...
    FunctionDescriptor toStringDescriptor =
        receiverClassDescriptor
            ...
            .orElseThrow(
                () ->
                    new AssertionError(
                        "'toString' not found in member scope of " + receiverClassDescriptor));
  ...
  public static void genHashCode(
      MethodVisitor mv, InstructionAdapter iv, Type type, JvmTarget jvmTarget) {
  ...
  public static StackValue genEqualsForExpressionsOnStack(
      @NotNull IElementType opToken, @NotNull StackValue left, @NotNull StackValue right) {
  ...
  public static BranchedValue genTotalOrderEqualsForExpressionOnStack(
      @NotNull StackValue left, @NotNull StackValue right, @NotNull Type asmType) {
    return new BranchedValue(left, right, asmType, Opcodes.IFEQ) {
      @Override
      public void condJump(
          @NotNull Label jumpLabel, @NotNull InstructionAdapter iv, boolean jumpIfFalse) {
  ...
  public static void genIEEE754EqualForNullableTypesCall(
      InstructionAdapter v, Type left, Type right) {
    v.invokestatic(
        IntrinsicMethods.INTRINSICS_CLASS_NAME,
        "areEqual",
        "(" + left.getDescriptor() + right.getDescriptor() + ")Z",
        false);
  }
  ...
  static void genNotNullAssertionsForParameters(
    ...
    // currently when resuming a suspend function we pass default values instead of real arguments
    // (i.e. nulls for references)
    ...
    if (getVisibilityAccessFlag(descriptor) == ACC_PRIVATE) {
      ...
      // unless we have a private operator function, in which we should generate a parameter
      // assertion for an extension receiver.
        ...
        if (receiverParameter != null) {
          String name =
              getNameForReceiverParameter(
                  descriptor, state.getBindingContext(), state.getLanguageVersionSettings());
  ...
  private static void genParamAssertion(
    ...
    if (asmType.getSort() == Type.OBJECT || asmType.getSort() == Type.ARRAY) {
      ...
      if (JvmCodegenUtil.isDeclarationOfBigArityFunctionInvoke(containingDeclaration)
        ...
        value =
            StackValue.arrayElement(
                OBJECT_TYPE,
                null,
                StackValue.local(1, getArrayType(OBJECT_TYPE)),
                StackValue.constant(index));
      } else {
      ...
      v.visitLdcInsn(name);
      String methodName =
          state.getConfig().getUnifiedNullChecks()
  ...
  private static int getIndexOfParameterInVarargInvokeArray(
      @NotNull ParameterDescriptor parameter) {
    ...
    assert parameter instanceof ValueParameterDescriptor
        : "Non-extension-receiver parameter must be a value parameter: " + parameter;
    int extensionShift =
        ((CallableDescriptor) container).getExtensionReceiverParameter() == null ? 0 : 1;

    return extensionShift + ((ValueParameterDescriptor) parameter).getIndex();
  }

  // At the beginning of the vararg invoke of a function with big arity N, generates an assert that
  // the vararg parameter has N elements
  ...
  public static boolean isInstancePropertyWithStaticBackingField(
  ...
  public static int getVisibilityForBackingField(
      @NotNull PropertyDescriptor propertyDescriptor, boolean isDelegate) {
  ...
  private static MemberDescriptor descriptorForVisibility(
    ...
    if (!propertyDescriptor.isVar()) {
      ...
      return propertyDescriptor.getSetter() != null
  ...
  public static boolean isPropertyWithBackingFieldCopyInOuterClass(
    ...
    return propertyDescriptor.isConst()
        && isCompanionObject(propertyContainer)
        && isJvmInterface(propertyContainer.getContainingDeclaration())
  ...
  public static void writeAnnotationData(
      @NotNull AnnotationVisitor av,
      @NotNull MessageLite message,
      @NotNull JvmStringTable stringTable) {
    AsmUtil.writeAnnotationData(
        av,
        JvmProtoBufUtil.writeData(message, stringTable),
        ArrayUtil.toStringArray(stringTable.getStrings()));
  }
  ...
  public static int getReceiverIndex(
      @NotNull CodegenContext context, @NotNull CallableMemberDescriptor descriptor) {
