...
public abstract class KotlinCompileMojoBase<A extends CommonCompilerArguments>
  ...
  @Component protected MojoExecution mojoExecution;
  ...
  @Parameter private List<String> sourceDirs;

  /** A list of kotlin compiler plugins to be applied. */
  @Parameter private List<String> compilerPlugins;

  /** A list of plugin options in format (pluginId):(parameter)=(value) */
  @Parameter private List<String> pluginOptions;

  @Parameter private boolean multiPlatform = false;
  ...
  protected List<String> getSourceFilePaths() {
    ...
    return sourceFilePaths.stream()
        .map(path -> new File(path).toPath().normalize().toString())
        .distinct()
          ...
          if (plugin != null) {
            for (PluginExecution pluginExecution : plugin.getExecutions()) {
              if (pluginExecution.getGoals() != null
                  && pluginExecution.getGoals().contains("metadata")) {
  ...
  private void printCompilerArgumentsIfDebugEnabled(
      @NotNull A arguments, @NotNull CLICompiler<A> compiler) {
  ...
  protected abstract void configureSpecificCompilerArguments(
      @NotNull A arguments, @NotNull List<File> sourceRoots) throws MojoExecutionException;
  ...
  private List<String> getCompilerPluginClassPaths() {
    ...
    List<ArtifactRepository> remoteRepos =
        session == null ? null : session.getCurrentProject().getPluginArtifactRepositories();
  ...
  @NotNull
  private Map<String, KotlinMavenPluginExtension> loadCompilerPlugins()
    ...
    for (String pluginName : compilerPlugins) {
      ...
      try {
        KotlinMavenPluginExtension extension =
            container.lookup(KotlinMavenPluginExtension.class, pluginName);
        loadedPlugins.put(pluginName, extension);
        getLog()
            .debug(
                "Got plugin instance " + pluginName + " of type " + extension.getClass().getName());
  ...
  @NotNull
  private List<PluginOption> getCompilerPluginOptions()
      throws PluginNotFoundException, PluginOptionIllegalFormatException {
  ...
  private static List<PluginOption> parseUserProvidedPluginOptions(
      @NotNull List<String> rawOptions, @NotNull Map<String, KotlinMavenPluginExtension> plugins)
  ...
  private List<File> getSourceRoots() throws MojoExecutionException {
    ...
    for (File sourceDir : getSourceDirs()) {
      ...
      // unfortunately there is no good way to detect generated sources directory so we simply keep
      // hardcoded value
  ...
  private void configureCompilerArguments(
      @NotNull A arguments, @NotNull CLICompiler<A> compiler, @NotNull List<File> sourceRoots)
      throws MojoExecutionException {
    if (getLog().isDebugEnabled()) {
    ...
    if (args != null && args.contains(null)) {
      throw new MojoExecutionException(
          "Empty compiler argument passed in the <configuration> section");
    }
    ...
    if (!pluginClassPaths.isEmpty()) {
      if (arguments.getPluginClasspaths() == null || arguments.getPluginClasspaths().length == 0) {
        arguments.setPluginClasspaths(
            pluginClassPaths.toArray(new String[pluginClassPaths.size()]));
      } else {
      ...
      if (getLog().isDebugEnabled()) {
        getLog()
            .debug(
                "Plugin classpaths are: " + Joiner.on(", ").join(arguments.getPluginClasspaths()));
      }
    }
