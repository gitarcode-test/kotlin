...
/**
 * A scope is a factory for creating and allocating {@link JsName}s. A JavaScript AST is built in
 * terms of abstract name objects without worrying about obfuscation, keyword/identifier
 ...
 * <p>
 *
 * <p>Scopes are associated with {@link JsFunction}s, but the two are not equivalent. Functions
 * <i>have</i> scopes, but a scope does not necessarily have an associated Function. Examples of
 * this include the {@link JsRootScope} and synthetic scopes that might be created by a client.
 *
 * <p>
 *
 * <p>
 *
 * <p>Scopes can have parents to provide constraints when allocating actual identifiers for names.
 * Specifically, names in child scopes are chosen such that they do not conflict with names in their
 * parent scopes. The ultimate parent is usually the global scope (see {@link
 * JsProgram#getRootScope()}), but parentless scopes are useful for managing names that are always
 * accessed with a qualifier and could therefore never be confused with the global scope hierarchy.
 */
public abstract class JsScope {
  ...
  /**
   * Gets a name object associated with the specified identifier in this scope, creating it if
   * necessary.<br>
   * If the JsName does not exist yet, a new JsName is created. The identifier, short name, and
   * original name of the newly created JsName are equal to the given identifier.
   *
  ...
  /**
   * Creates a new variable with an unique ident in this scope. The generated JsName is guaranteed
   * to have an identifier that does not clash with any existing variables in the scope. Future
   * declarations of variables might however clash with the temporary (unless they use this
   * function).
   */
  @NotNull
  ...
  /**
   * Creates a temporary variable with an unique name in this scope. The generated temporary is
   * guaranteed to have an identifier (but not short name) that does not clash with any existing
   * variables in the scope. Future declarations of variables might however clash with the
   * temporary.
   */
  @NotNull
  ...
  /**
   * During inlining names can be refreshed multiple times, so "a" becomes "a_0", then becomes
   * "a_0_0" in case a_0 has been declared in calling scope.
   *
   * <p>That's ugly. To resolve it, we rename clashing names with "[_$]\\d+" suffix, incrementing
   * last number.
   *
   * <p>Fresh name for "a0" should still be "a0_0".
   */
  @NotNull
