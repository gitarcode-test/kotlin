...
public final class StaticContext {
  ...
  @NotNull private final JsProgramFragment fragment;

  @NotNull private final BindingTrace bindingTrace;
  @NotNull private final Namer namer;

  @NotNull private final Intrinsics intrinsics;

  @NotNull private final JsScope rootScope;

  @NotNull private final Generator<JsName> innerNames = new InnerNameGenerator();
  @NotNull private final Generator<JsScope> scopes = new ScopeGenerator();
  @NotNull private final Generator<JsName> objectInstanceNames = new ObjectInstanceNameGenerator();

  @NotNull private final Map<JsScope, JsFunction> scopeToFunction = new HashMap<>();
  ...
  @NotNull private final JsConfig config;

  @NotNull private final ModuleDescriptor currentModule;

  @NotNull private final JsImportedModule currentModuleAsImported;

  @NotNull private final NameSuggestion nameSuggestion;

  @NotNull private final Map<DeclarationDescriptor, JsName> nameCache = new HashMap<>();

  @NotNull
  private final Map<VariableDescriptorWithAccessors, JsName> backingFieldNameCache =
      new HashMap<>();

  @NotNull private final Map<DeclarationDescriptor, JsExpression> fqnCache = new HashMap<>();
  ...
  @NotNull private final DeclarationExporter exporter = new DeclarationExporter(this);

  @NotNull private final Map<FqName, JsScope> packageScopes = new HashMap<>();

  @NotNull private final ClassModelGenerator classModelGenerator;

  @Nullable private JsName nameForImportsForInline;
  ...
  private final Map<SpecialFunction, JsName> specialFunctions =
      new EnumMap<>(SpecialFunction.class);

  private final Map<String, JsName> intrinsicNames = new HashMap<>();

  @NotNull private final SourceFilePathResolver sourceFilePathResolver;
  ...
  public StaticContext(
    ...
    this.currentModuleAsImported =
        new JsImportedModule(
            Namer.getRootPackageName(), rootScope.declareName(Namer.getRootPackageName()), null);

    JsName kotlinName = rootScope.declareName(Namer.KOTLIN_NAME);
    createImportedModule(
        new JsImportedModuleKey(Namer.KOTLIN_LOWER_NAME, null),
        Namer.KOTLIN_LOWER_NAME,
        kotlinName,
        null);
    ...
    ClassDescriptor exceptionClass =
        findClassAcrossModuleDependencies(
            moduleDescriptor, ClassId.topLevel(new FqName("kotlin.Exception")));
    isStdlib =
        exceptionClass != null
            && DescriptorUtils.getContainingModule(exceptionClass) == moduleDescriptor;
  }
  ...
  private JsExpression buildQualifiedExpression(@NotNull DeclarationDescriptor descriptor) {
    if (descriptor instanceof ClassDescriptor) {
      ...
      } else if (KotlinBuiltIns.isInt(type)
    ...
    if (config.getModuleKind() != ModuleKind.PLAIN) {
      ...
      if (moduleName != null) {
        return JsAstUtils.pureFqn(
            getImportedModule(moduleName, suggested.getDescriptor()).getInternalName(), null);
      }
    }
    ...
    // Don't generate qualifier for local declarations
        ...
        || suggested.getDescriptor() instanceof CallableDescriptor
            && suggested.getScope() instanceof FunctionDescriptor) {
      expression = null;
    } else {
    ...
    if (isNativeObject(suggested.getDescriptor())
      ...
      String fileModuleName =
          AnnotationsUtils.getFileModuleName(getBindingContext(), suggested.getDescriptor());
      ...
      String qualifier =
          AnnotationsUtils.getFileQualifier(getBindingContext(), suggested.getDescriptor());
      if (qualifier != null) {
  ...
  public JsName getNameForDescriptor(@NotNull DeclarationDescriptor descriptor) {
    if (descriptor instanceof ClassDescriptor
        && KotlinBuiltIns.isAny((ClassDescriptor) descriptor)) {
    ...
    if (suggested == null) {
      throw new IllegalArgumentException(
          "Can't generate name for root declarations: " + descriptor);
    }
  ...
  public JsName getNameForBackingField(@NotNull VariableDescriptorWithAccessors property) {
    ...
    if (name == null) {
      ...
      String baseName =
          NameSuggestion.getPrivateMangledName(fqn.getNames().get(0), property) + "_0";
  ...
  private List<JsName> getActualNameFromSuggested(@NotNull SuggestedName suggested) {
    ...
    if (suggested.getStable()) {
      ...
      for (String namePart : suggested.getNames()) {
        ...
        if (tag != null
            && !AnnotationsUtils.isNativeObject(suggested.getDescriptor())
      ...
      assert suggested.getNames().size() == 1
          : "Private names must always consist of exactly one name";
  ...
  JsName importDeclaration(
      @NotNull String suggestedName, @NotNull String tag, @NotNull JsExpression declaration) {
  ...
  @NotNull
  private JsName importDeclarationImpl(
      @NotNull String suggestedName, @NotNull String tag, @NotNull JsExpression declaration) {
  ...
  @NotNull
  private JsName localOrImportedName(
      @NotNull DeclarationDescriptor descriptor, @NotNull String suggestedName) {
  ...
  // When compiling stdlib, we may have sources for built-in declaration. In this case we have two
  // distinct descriptors
  // for declaration with one signature. One descriptor is from current module, another descriptor
  // is from built-in module.
  // Different declarations refer different descriptors. This may cause single name to be both
  // imported and declared locally,
  // which in turn causes runtime error. We avoid this by detecting this case and turning off
  // import.
      ...
      addRule(
          descriptor -> {
            if (descriptor instanceof PackageFragmentDescriptor
                && DescriptorUtils.getContainingModule(descriptor) == currentModule) {
              return exporter.getLocalPackageName(
                  ((PackageFragmentDescriptor) descriptor).getFqName());
            }
            if (descriptor instanceof FunctionDescriptor) {
              FunctionDescriptor initialDescriptor =
                  ((FunctionDescriptor) descriptor).getInitialSignatureDescriptor();
            ...
            if (descriptor instanceof LocalVariableDescriptor
                || descriptor instanceof ParameterDescriptor) {
              return getNameForDescriptor(descriptor);
            }
            if (descriptor instanceof ConstructorDescriptor) {
              if (((ConstructorDescriptor) descriptor).isPrimary()) {
                return getInnerNameForDescriptor(
                    ((ConstructorDescriptor) descriptor).getConstructedClass());
              }
            }
  ...
  public static String getSuggestedName(@NotNull DeclarationDescriptor descriptor) {
    ...
    if (descriptor instanceof PropertyGetterDescriptor) {
      ...
      assert descriptor != null
          : "ConstructorDescriptor should have containing declaration: " + constructor;
    } else {
    ...
    if (!(descriptor instanceof PackageFragmentDescriptor)
      ...
      assert container != null
          : "We just figured out that descriptor is not for a top-level declaration: " + descriptor;
      suggestedName =
  ...
  private final class ScopeGenerator extends Generator<JsScope> {
    public ScopeGenerator() {
      Rule<JsScope> generateNewScopesForClassesWithNoAncestors =
          descriptor -> {
            if (!(descriptor instanceof ClassDescriptor)) {
              return null;
            }
            if (getSuperclass((ClassDescriptor) descriptor) == null) {
              JsFunction function =
                  new JsFunction(new JsRootScope(program), new JsBlock(), descriptor.toString());
      ...
      Rule<JsScope> generateInnerScopesForDerivedClasses =
          descriptor -> {
            ...
            return getScopeForDescriptor(superclass)
                .innerObjectScope("Scope for class " + descriptor.getName());
          };
      ...
      Rule<JsScope> generateInnerScopesForMembers =
          descriptor ->
              fragment.getScope().innerObjectScope("Scope for member " + descriptor.getName());
      Rule<JsScope> createFunctionObjectsForCallableDescriptors =
          descriptor -> {
            if (!(descriptor instanceof CallableDescriptor)) {
              return null;
            }

            JsFunction correspondingFunction =
                JsAstUtils.createFunctionWithEmptyBody(fragment.getScope());
            assert (!scopeToFunction.containsKey(correspondingFunction.getScope()))
                : "Scope to function value overridden for " + descriptor;
            scopeToFunction.put(correspondingFunction.getScope(), correspondingFunction);
            correspondingFunction.setSource(
                PsiSourceElementKt.getPsi(((CallableDescriptor) descriptor).getSource()));
            return correspondingFunction.getScope();
          };
      Rule<JsScope> scopeForPackage =
          descriptor -> {
  ...
  @NotNull
  public JsImportedModule getImportedModule(
      @NotNull String baseName, @Nullable DeclarationDescriptor descriptor) {
    String plainName =
        descriptor != null && config.getModuleKind() == ModuleKind.UMD
            ? getPlainId(descriptor)
            : null;
    ...
    if (module == null) {
      JsName internalName =
          JsScope.declareTemporaryName(
              Namer.LOCAL_MODULE_PREFIX + Namer.suggestedModuleName(baseName));
      module =
          createImportedModule(
              key, baseName, internalName, plainName != null ? pureFqn(plainName, null) : null);
    }
    return module;
  }

  private JsImportedModule createImportedModule(
      JsImportedModuleKey key, String baseName, JsName internalName, JsExpression plainName) {
  ...
  private String getPlainId(@NotNull DeclarationDescriptor declaration) {
    ...
    assert suggestedName != null
        : "Declaration should not be ModuleDescriptor, therefore suggestedName should be non-null";
  ...
  public void putClassOrConstructorClosure(
      @NotNull MemberDescriptor localClass, @NotNull List<DeclarationDescriptor> closure) {
    classOrConstructorClosure.put(localClass, Lists.newArrayList(closure));
  }

  @Nullable
  public List<DeclarationDescriptor> getClassOrConstructorClosure(
  ...
  public void addClass(@NotNull ClassDescriptor classDescriptor) {
    if (!AnnotationsUtils.isNativeObject(classDescriptor)
        && !AnnotationsUtils.isLibraryObject(classDescriptor)) {
      fragment
          .getClasses()
          .put(
              getInnerNameForDescriptor(classDescriptor),
              classModelGenerator.generateClassModel(classDescriptor));
    }
  }
  ...
  public void addInlineCall(@NotNull CallableDescriptor descriptor) {
    ...
    JsExpression moduleExpression =
        exportModuleForInline(DescriptorUtils.getContainingModule(descriptor));
  ...
  public JsExpression exportModuleForInline(
      @NotNull String moduleId, @NotNull JsImportedModule moduleName) {
    ...
    if (moduleRef == null) {
      ...
      JsExpressionStatement importStmt =
          new JsExpressionStatement(
              JsAstUtils.assignment(lhsModuleRef, moduleName.getInternalName().makeRef()));
  ...
  public JsName getNameForSpecialFunction(@NotNull SpecialFunction specialFunction) {
    return specialFunctions.computeIfAbsent(
        specialFunction,
        f -> {
          JsExpression expression = Namer.createSpecialFunction(specialFunction);
          JsName name =
              importDeclaration(
                  f.getSuggestedName(), TranslationUtils.getTagForSpecialFunction(f), expression);
  ...
  public JsName getNameForIntrinsic(@NotNull String name) {
    JsName resultName =
        intrinsicNames.computeIfAbsent(
            name,
            k -> {
              if (isStdlib) {
              ...
              return importDeclaration(
                  NameSuggestion.sanitizeName(name),
                  "intrinsic:" + name,
                  TranslationUtils.getIntrinsicFqn(name));
            });
  ...
  public JsName getVariableForPropertyMetadata(@NotNull VariableDescriptorWithAccessors property) {
    return propertyMetadataVariables.computeIfAbsent(
        property,
        p -> {
          String id = getSuggestedName(property) + "_metadata";
          JsName name = JsScope.declareTemporaryName(NameSuggestion.sanitizeName(id));

          // Unexpectedly! However, the only thing, for which 'imported' property is relevant, is a
          // import clener.
