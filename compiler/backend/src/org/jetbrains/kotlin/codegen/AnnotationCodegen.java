...
public abstract class AnnotationCodegen {
  ...
  public static final List<JvmFlagAnnotation> METHOD_FLAGS =
      Arrays.asList(
          ...
          new JvmFlagAnnotation(
              SYNCHRONIZED_ANNOTATION_FQ_NAME.asString(), Opcodes.ACC_SYNCHRONIZED));

  private static final AnnotationVisitor NO_ANNOTATION_VISITOR =
      new AnnotationVisitor(Opcodes.API_VERSION) {
        @Override
  ...
  private AnnotationCodegen(
      @NotNull InnerClassConsumer innerClassConsumer, @NotNull GenerationState state) {
  ...
  public void genAnnotations(
    ...
    for (AnnotationDescriptor annotation : annotations) {
      ...
      if (annotated instanceof ClassDescriptor
        ...
        if (classDescriptor.getVisibility() == DescriptorVisibilities.LOCAL) {
          assert applicableTargets.contains(KotlinTarget.EXPRESSION)
              : "Inconsistent target list for object literal annotation: "
                  + applicableTargets
                  + " on "
                  + annotated;
          continue;
    ...
    generateAdditionalAnnotations(
        annotated, returnType, annotationDescriptorsAlreadyPresent, parameterContainer);
  }
  ...
  private void generateAdditionalAnnotations(
    ...
    if (annotated instanceof CallableDescriptor) {
      generateAdditionalCallableAnnotations(
          (CallableDescriptor) annotated,
          returnType,
          annotationDescriptorsAlreadyPresent,
          parameterContainer);
    } else if (annotated instanceof FieldDescriptor) {
      generateAdditionalCallableAnnotations(
          ((FieldDescriptor) annotated).getCorrespondingProperty(),
          returnType,
          annotationDescriptorsAlreadyPresent,
          parameterContainer);
    } else if (annotated instanceof ClassDescriptor) {
      generateAdditionalClassAnnotations(
          annotationDescriptorsAlreadyPresent, (ClassDescriptor) annotated);
    }
  }
  ...
  private void generateAdditionalCallableAnnotations(
    ...
    if (returnType != null && !AsmUtil.isPrimitive(returnType) && !skipNullabilityAnnotations) {
      generateNullabilityAnnotation(
          descriptor.getReturnType(), annotationDescriptorsAlreadyPresent);
    }
  }
  ...
  private static boolean isInvisibleFromTheOutside(@Nullable DeclarationDescriptor descriptor) {
    ...
    if (descriptor instanceof MemberDescriptor) {
      return DescriptorAsmUtil.getVisibilityAccessFlag((MemberDescriptor) descriptor)
          == Opcodes.ACC_PRIVATE;
    }
    return false;
  }
  ...
  private void generateNullabilityAnnotation(
      @Nullable KotlinType type, @NotNull Set<String> annotationDescriptorsAlreadyPresent) {
    if (type == null) return;

    if (isBareTypeParameterWithNullableUpperBound(type)) {
      // This is to account for the case of, say
      //   class Function<R> { fun invoke(): R }
      // it would be a shame to put @Nullable on the return type of the function, and force all
      // callers to check for null,
      // so we put no annotations
      return;
    }

    if (FlexibleTypesKt.isFlexible(type)) {
      // A flexible type whose lower bound in not-null and upper bound is nullable, should not be
      // annotated
      FlexibleType flexibleType = FlexibleTypesKt.asFlexibleType(type);

      if (!TypeUtils.isNullableType(flexibleType.getLowerBound())
          && TypeUtils.isNullableType(flexibleType.getUpperBound())) {
        AnnotationDescriptor notNull =
            type.getAnnotations().findAnnotation(JvmAnnotationNames.JETBRAINS_NOT_NULL_ANNOTATION);
        if (notNull != null) {
          generateAnnotationIfNotPresent(
              annotationDescriptorsAlreadyPresent,
              Type.getType(NotNull.class).getDescriptor(),
              false);
        }
        return;
      }
    ...
    generateAnnotationIfNotPresent(
        ...
        TypeUtils.isNullableType(type)
            ? Type.getType(Nullable.class).getDescriptor()
            : Type.getType(NotNull.class).getDescriptor(),
        false);
  }

  private static final Map<KotlinTarget, ElementType> annotationTargetMap =
      new EnumMap<>(KotlinTarget.class);
  ...
  private void generateTargetAnnotation(
    ...
    Set<KotlinTarget> targets =
        AnnotationChecker.Companion.applicableTargetSetFromTargetAnnotationOrNull(classDescriptor);
    ...
    for (ElementType javaTarget : javaTargets) {
      arrayVisitor.visitEnum(
          null, Type.getType(ElementType.class).getDescriptor(), javaTarget.name());
    }
    arrayVisitor.visitEnd();
    visitor.visitEnd();
  }

  private void generateRetentionAnnotation(
      @NotNull ClassDescriptor classDescriptor,
      @NotNull Set<String> annotationDescriptorsAlreadyPresent) {
  ...
  private void generateDocumentedAnnotation(
      @NotNull ClassDescriptor classDescriptor,
      @NotNull Set<String> annotationDescriptorsAlreadyPresent) {
  ...
  private static boolean isBareTypeParameterWithNullableUpperBound(@NotNull KotlinType type) {
    ...
    return !type.isMarkedNullable()
        ...
        && TypeUtils.hasNullableSuperType(type);
  }

  public void generateAnnotationDefaultValue(
      @NotNull ConstantValue<?> value, @NotNull KotlinType expectedType) {
  ...
  private String genAnnotation(
    ...
    if (rp == RetentionPolicy.SOURCE
        && !typeMapper.getClassBuilderMode().generateSourceRetentionAnnotations) {
      return null;
    }

    // We do not generate annotations whose classes are optional (annotated with
    // `@OptionalExpectation`) because if an annotation entry
    // is resolved to the expected declaration, this means that annotation has no actual class, and
    // thus should not be generated.
  ...
  private void genAnnotationArguments(
      AnnotationDescriptor annotationDescriptor, AnnotationVisitor annotationVisitor) {
    ClassDescriptor annotationClass = DescriptorUtilsKt.getAnnotationClass(annotationDescriptor);
    for (Map.Entry<Name, ConstantValue<?>> entry :
        annotationDescriptor.getAllValueArguments().entrySet()) {
      genCompileTimeValue(
          getAnnotationArgumentJvmName(annotationClass, entry.getKey()),
          entry.getValue(),
          annotationVisitor);
    }
  }

  private String getAnnotationArgumentJvmName(
      @Nullable ClassDescriptor annotationClass, @NotNull Name parameterName) {
    if (annotationClass == null) return parameterName.asString();

    Collection<? extends PropertyDescriptor> variables =
        annotationClass
            .getUnsubstitutedMemberScope()
            .getContributedVariables(parameterName, NoLookupLocation.FROM_BACKEND);
  ...
  private void genCompileTimeValue(
    ...
    AnnotationArgumentVisitor<Void, Void> argumentVisitor =
        new AnnotationArgumentVisitor<Void, Void>() {
          @Override
          ...
          public Void visitAnnotationValue(AnnotationValue value, Void data) {
            ...
            innerClassConsumer.addInnerClassInfoFromAnnotation(
                DescriptorUtils.getClassDescriptorForType(classType));
            AnnotationVisitor visitor =
                annotationVisitor.visitAnnotation(
                    name, typeMapper.mapType(classType).getDescriptor());
          ...
          private Void visitUnsupportedValue(ConstantValue<?> value) {
            ...
            if (mode.generateBodies) {
              throw new IllegalStateException(
                  "Don't know how to compile annotation value " + value);
            } else {
              return null;
            }
          }
  ...
  private static final Map<KotlinRetention, RetentionPolicy> annotationRetentionMap =
      new EnumMap<>(KotlinRetention.class);
  ...
  private static Set<ElementType> getJavaTargetList(ClassDescriptor descriptor) {
    AnnotationDescriptor targetAnnotation =
        descriptor.getAnnotations().findAnnotation(new FqName(Target.class.getName()));
    if (targetAnnotation != null) {
      Collection<ConstantValue<?>> valueArguments =
          targetAnnotation.getAllValueArguments().values();
  ...
  private static RetentionPolicy getRetentionPolicy(@NotNull Annotated descriptor) {
    ...
    AnnotationDescriptor retentionAnnotation =
        descriptor.getAnnotations().findAnnotation(new FqName(Retention.class.getName()));
    if (retentionAnnotation != null) {
      ConstantValue<?> value =
          CollectionsKt.firstOrNull(retentionAnnotation.getAllValueArguments().values());
      if (value instanceof EnumValue) {
        FqName enumClassFqName = ((EnumValue) value).getEnumClassId().asSingleFqName();
  ...
  public static AnnotationCodegen forParameter(
    ...
    return new AnnotationCodegen(innerClassConsumer, state, skipNullabilityAnnotations) {
      ...
      AnnotationVisitor visitTypeAnnotation(String descr, TypePath path, boolean visible) {
        return safe(
            mv.visitTypeAnnotation(
                TypeReference.newFormalParameterReference(parameter).getValue(),
                path,
                descr,
                visible));
      }
