...
public abstract class MemberCodegen<T extends KtPureElement /* TODO: & KtDeclarationContainer*/>
  ...
  private final ReifiedTypeParametersUsages reifiedTypeParametersUsages =
      new ReifiedTypeParametersUsages();
  ...
  protected MemberCodegen(
      @NotNull MemberCodegen<T> wrapped, T declaration, FieldOwnerContext codegenContext) {
    ...
    boolean shouldGenerateSyntheticParts =
        !(element instanceof KtClassOrObject)
            || state
                ...
                .shouldGenerateClassMembers((KtClassOrObject) element);
  ...
  public static void markLineNumberForDescriptor(
      @Nullable ClassDescriptor declarationDescriptor, @NotNull InstructionAdapter v) {
    if (declarationDescriptor == null) {
      return;
    }

    PsiElement classElement =
        DescriptorToSourceUtils.getSourceFromDescriptor(declarationDescriptor);
    if (classElement != null) {
      markLineNumberForElement(classElement, v);
    }
  }

  public static void markLineNumberForElement(
      @NotNull PsiElement element, @NotNull InstructionAdapter v) {
  ...
  protected void done() {
    ...
    v.visitSMAP(
        ...
        !state
            ...
            .supportsFeature(LanguageFeature.CorrectSourceMappingSyntax));
    ...
    if (declaration instanceof KtNamedFunction) {
      try {
        ...
        throw new CompilationException(
            "Failed to generate function " + declaration.getName(), e, declaration);
      }
      ...
      try {
        ...
        PlatformExceptionUtilsKt.rethrowIntellijPlatformExceptionIfNeeded(e);
        throw new CompilationException(
            "Failed to generate property " + declaration.getName(), e, declaration);
      }
      ...
      try {
        ...
        PlatformExceptionUtilsKt.rethrowIntellijPlatformExceptionIfNeeded(e);
        throw new CompilationException(
            "Failed to generate destructuring declaration entry " + declaration.getName(),
            e,
            declaration);
      }
    } else {
  ...
  private void genTypeAlias(@NotNull KtTypeAlias typeAlias) {
    ...
    if (typeAliasDescriptor == null) {
      throw ExceptionLogger.logDescriptorNotFound(
          "Type alias " + typeAlias.getName() + " should have a descriptor", typeAlias);
    }
  ...
  private void genTypeAliasAnnotationsMethodIfRequired(TypeAliasDescriptor typeAliasDescriptor) {
    boolean isAnnotationsMethodOwner =
        CodegenContextUtil.isImplementationOwner(context, typeAliasDescriptor);
  ...
  protected void generateSyntheticAnnotationsMethod(
    ...
    int flags =
        ACC_DEPRECATED
            | ACC_STATIC
            | ACC_SYNTHETIC
            | DescriptorAsmUtil.getVisibilityAccessFlag(descriptor);
    MethodVisitor mv =
        v.newMethod(
    ...
    AnnotationCodegen.forMethod(mv, this, state)
        .genAnnotations(new AnnotatedImpl(annotations), Type.VOID_TYPE, null);
    mv.visitCode();
    ...
    ClassBuilder classBuilder =
        state
            ...
            .newVisitor(
                ...
                classType,
                aClass.getContainingKtFile());
    ClassContext classContext =
        parentContext.intoClass(descriptor, OwnerKind.IMPLEMENTATION, state);
    new ImplementationBodyCodegen(aClass, classContext, classBuilder, state, parentCodegen, false)
        .generate();
  }
  ...
  private void writeInnerClasses() {
    ...
    for (SyntheticInnerClassInfo syntheticInnerClass : syntheticInnerClasses) {
      v.visitInnerClass(
          syntheticInnerClass.getInternalName(), null, null, syntheticInnerClass.getFlags());
    }
  }

  protected void addParentsToInnerClassesIfNeeded(
      @NotNull Collection<ClassDescriptor> innerClasses) {
  ...
  // It's necessary for proper recovering of classId by plain string JVM descriptor when loading
  // annotations
  ...
  public static void writeInnerClass(
      @NotNull ClassDescriptor innerClass,
    ...
    v.visitInnerClass(
        ...
        innerName,
        calculateInnerClassAccessFlags(innerClass));
  }
  ...
  protected void writeOuterClassAndEnclosingMethod() {
    ...
    Type enclosingAsmType =
        computeOuterClass(typeMapper, state.getJvmDefaultMode(), element, context);
  ...
  public static CodegenContext<?> getNonInlineOuterContext(CodegenContext<?> parentContext) {
    ...
    while (context instanceof InlineLambdaContext) {
      // If this is a lambda which will be inlined, skip its MethodContext and enclosing
      // ClosureContext
  ...
  public static Type computeOuterClass(
    ...
    CodegenContext<? extends ClassOrPackageFragmentDescriptor> outermost =
        context.getClassOrPackageParentContext();
    if (outermost instanceof ClassContext) {
    ...
    } else if (outermost instanceof MultifileClassFacadeContext
        || outermost instanceof DelegatingToPartContext) {
      ...
      if (implementationOwnerType != null) {
        ...
        return Type.getObjectType(
            JvmFileClassUtil.getFileClassInternalName(element.getContainingKtFile()));
      }
    }
  ...
  @Nullable
  public static Method computeEnclosingMethod(
      @NotNull KotlinTypeMapper typeMapper, @NotNull CodegenContext context) {
    if (context instanceof MethodContext) {
      FunctionDescriptor functionDescriptor = ((MethodContext) context).getFunctionDescriptor();
  ...
  public final ExpressionCodegen createOrGetClInitCodegen() {
    if (clInit == null) {
      ...
      SimpleFunctionDescriptorImpl clInitDescriptor =
          createClInitFunctionDescriptor(contextDescriptor);
      MethodVisitor mv =
          v.newMethod(
              JvmDeclarationOriginKt.OtherOrigin(contextDescriptor),
              ACC_STATIC,
              "<clinit>",
              "()V",
              null,
      ...
      clInit =
          new ExpressionCodegen(
              ...
              new FrameMap(),
              Type.VOID_TYPE,
              context.intoFunction(clInitDescriptor),
              state,
              this);
    }
    return clInit;
  ...
  @NotNull
  private SimpleFunctionDescriptorImpl createClInitFunctionDescriptor(
  ...
  protected void generateInitializers(@NotNull Function0<ExpressionCodegen> createCodegen) {
    ...
    for (int i = 0; i < declarations.size(); i++) {
      ...
      if (declaration instanceof KtProperty) {
        ...
        codegen
            .invoke()
            .initializeDestructuringDeclaration((KtDestructuringDeclaration) declaration, true);
  ...
  // Requires public access, because it is used by serialization plugin to generate initializer in
  ...
  public void initializeProperty(@NotNull ExpressionCodegen codegen, @NotNull KtProperty property) {
    ...
    assert initializer != null
        : "shouldInitializeProperty must return false if initializer is null";

    StackValue.Property propValue =
        codegen.intermediateValueForProperty(
  ...
  // Public accessible for serialization plugin to check whether call to initializeProperty(..) is
  // legal.
  public boolean shouldInitializeProperty(@NotNull KtProperty property) {
    ...
    ConstantValue<?> initializerValue =
        initializer != null
            ? ExpressionCodegen.getCompileTimeConstant(
                initializer, bindingContext, state.getConfig().getShouldInlineConstVals())
            : null;
  ...
  @NotNull
  private KotlinType getPropertyOrDelegateType(
      @NotNull KtProperty property, @NotNull PropertyDescriptor descriptor) {
  ...
  protected void generatePropertyMetadataArrayFieldIfNeeded(@NotNull Type thisAsmType) {
    List<VariableDescriptorWithAccessors> delegatedProperties =
        bindingContext.get(CodegenBinding.DELEGATED_PROPERTIES_WITH_METADATA, thisAsmType);
    if (delegatedProperties == null || delegatedProperties.isEmpty()) return;

    int additionalFlags =
        context.getContextKind() != OwnerKind.DEFAULT_IMPLS
                && isInterface(context.getContextDescriptor())
            ? ACC_PUBLIC
            : 0;
    v.newField(
        NO_ORIGIN,
        ACC_STATIC | ACC_FINAL | ACC_SYNTHETIC | additionalFlags,
        JvmAbi.DELEGATED_PROPERTIES_ARRAY_NAME,
        "[" + K_PROPERTY_TYPE,
        null,
        null);
  ...
  public static void generatePropertyReference(
      ...
      @NotNull GenerationState state) {
    int receiverCount =
        (property.getDispatchReceiverParameter() != null ? 1 : 0)
            + (property.getExtensionReceiverParameter() != null ? 1 : 0);
    Type implType =
        property.isVar()
            ? MUTABLE_PROPERTY_REFERENCE_IMPL[receiverCount]
            : PROPERTY_REFERENCE_IMPL[receiverCount];
    iv.anew(implType);
    iv.dup();
    ...
    iv.invokespecial(
        ...
        "<init>",
        Type.getMethodDescriptor(Type.VOID_TYPE, superCtorArgTypes.toArray(new Type[0])),
        false);
    Method wrapper =
        PropertyReferenceCodegen.getWrapperMethodForPropertyReference(property, receiverCount);
  ...
  public SourceMapper getOrCreateSourceMapper() {
    if (sourceMapper == null) {
      sourceMapper =
          new SourceMapper(
              element instanceof KtElement
                  ? SourceInfo.Companion.createFromPsi((KtElement) element, getClassName())
                  : null);
    }
    return sourceMapper;
  }
  ...
  protected void generateConstInstance(@NotNull Type thisAsmType, @NotNull Type fieldAsmType) {
    ...
    if (state.getClassBuilderMode().generateBodies) {
      ...
      iv.putstatic(
          thisAsmType.getInternalName(), JvmAbi.INSTANCE_FIELD, fieldAsmType.getDescriptor());
    }
  }
  ...
  protected final void generateSyntheticAccessors() {
    for (AccessorForCallableDescriptor<?> accessor : ((CodegenContext<?>) context).getAccessors()) {
      boolean compiledToJvmDefault =
          JvmAnnotationUtilKt.isCompiledToJvmDefault(
              accessor.getCalleeDescriptor(), state.getJvmDefaultMode());
    ...
    if (!accessorsForCompanionObjects.isEmpty()) {
      ...
      for (AccessorForCompanionObjectInstanceFieldDescriptor accessor :
          sortedAccessorsForCompanionObjects) {
  ...
  private final Comparator<AccessorForCompanionObjectInstanceFieldDescriptor> BY_COMPANION_FQN =
      (o1, o2) -> {
        String companionFQN1 =
            DescriptorUtils.getFqName(o1.getCompanionObjectDescriptor()).asString();
        String companionFQN2 =
            DescriptorUtils.getFqName(o2.getCompanionObjectDescriptor()).asString();
        return companionFQN1.compareTo(companionFQN2);
      };

  private void generateSyntheticAccessorForCompanionObject(
      @NotNull AccessorForCompanionObjectInstanceFieldDescriptor accessor) {
    ClassDescriptor companionObjectDescriptor = accessor.getCompanionObjectDescriptor();
    DeclarationDescriptor hostClassDescriptor =
        companionObjectDescriptor.getContainingDeclaration();
    assert hostClassDescriptor instanceof ClassDescriptor
        : "Class descriptor expected: " + hostClassDescriptor;
    functionCodegen.generateMethod(
        ...
        new FunctionGenerationStrategy.CodegenBased(state) {
          ...
          public void doGenerateBody(
            ...
            StackValue.singleton(companionObjectDescriptor, typeMapper)
                .put(companionObjectType, codegen.v);
  ...
  private void generateSyntheticAccessor(
      @NotNull AccessorForCallableDescriptor<?> accessorForCallableDescriptor) {
    if (accessorForCallableDescriptor instanceof FunctionDescriptor) {
      ...
      FunctionDescriptor original =
          (FunctionDescriptor) accessorForCallableDescriptor.getCalleeDescriptor();
      functionCodegen.generateMethod(
          Synthetic(null, original),
          accessor,
          new FunctionGenerationStrategy.CodegenBased(state) {
            @Override
            ...
            public void doGenerateBody(
              ...
              if (accessorForCallableDescriptor.getAccessorKind()
                  == AccessorKind.JVM_DEFAULT_COMPATIBILITY) {
                FunctionDescriptor descriptor =
                    unwrapFakeOverrideToAnyDeclaration(original).getOriginal();
                ...
                generateMethodCallTo(descriptor, accessor, codegen.v)
      ...
      AccessorForPropertyDescriptor accessor =
          (AccessorForPropertyDescriptor) accessorForCallableDescriptor;
      ...
      class PropertyAccessorStrategy extends FunctionGenerationStrategy.CodegenBased {
        ...
        public void doGenerateBody(
          ...
          if (accessorForCallableDescriptor.getAccessorKind()
            ...
            PropertyAccessorDescriptor originalAccessor =
                isGetter ? descriptor.getGetter() : descriptor.getSetter();
            PropertyAccessorDescriptor accessorDescriptor =
                isGetter ? accessor.getGetter() : accessor.getSetter();
          ...
          boolean forceFieldForCompanionProperty =
              DescriptorsJvmAbiUtil.isPropertyWithBackingFieldInOuterClass(original)
                  && !isCompanionObject(accessor.getContainingDeclaration());
          boolean forceField =
              forceFieldForCompanionProperty
      ...
      if (accessor.isWithSyntheticGetterAccessor()) {
        ...
        assert getter != null;
        functionCodegen.generateMethod(
            Synthetic(null, original.getGetter() != null ? original.getGetter() : original),
            getter,
      ...
      if (accessor.isVar() && accessor.isWithSyntheticSetterAccessor()) {
        ...
        functionCodegen.generateMethod(
            Synthetic(null, original.getSetter() != null ? original.getSetter() : original),
            setter,
  ...
  private boolean isProhibitedAccessorToPrivatePropertySetter(PropertyDescriptor original) {
    ...
    if (overriddenSetter == null
        || !DescriptorVisibilities.isPrivate(overriddenSetter.getVisibility())) return false;
    DeclarationDescriptor contextDescriptor = context.getContextDescriptor();
  ...
  protected StackValue generateMethodCallTo(
    ...
    CallableMethod callableMethod =
        typeMapper.mapToCallableMethod(
            ...
            accessorDescriptor instanceof AccessorForCallableDescriptor
                && (((AccessorForCallableDescriptor) accessorDescriptor).getSuperCallTarget()
                        != null
                    || ((AccessorForCallableDescriptor) accessorDescriptor).getAccessorKind()
                        == AccessorKind.JVM_DEFAULT_COMPATIBILITY),
            (accessorDescriptor != null
                    && TypeMapperUtilsKt.isInlineClassConstructorAccessor(accessorDescriptor)
                ? OwnerKind.ERASED_INLINE_CLASS
                : null));

    boolean isJvmStaticInObjectOrClass =
        CodegenUtilKt.isJvmStaticInObjectOrClassOrInterface(functionDescriptor);
    boolean hasDispatchReceiver =
        !isStaticDeclaration(functionDescriptor)
    ...
    Type dispatchReceiverType =
        dispatchReceiver != null && !accessorIsConstructor
    ...
    if (!accessorIsConstructor && functionDescriptor instanceof ConstructorDescriptor) {
      ...
      reg = 0;
      accessorParam = 0;
    } else if (KotlinTypeMapper.isAccessor(accessorDescriptor)
        && (hasDispatchReceiver || accessorIsConstructor)) {
  ...
  public void generateAssertField() {
    ...
    AssertCodegenUtilKt.generateAssertionsDisabledFieldInitialization(
        v, createOrGetClInitCodegen().v, v.getThisName());
