...
public class JvmCodegenUtil {
  ...
  private static boolean isCallInsideSameClassAsFieldRepresentingProperty(
    ...
    DeclarationDescriptor containingDeclaration =
        descriptor.getContainingDeclaration().getOriginal();
    if (DescriptorsJvmAbiUtil.isPropertyWithBackingFieldInOuterClass(descriptor)) {
      // For property with backed field, check if the access is done in the same class containing
      // the backed field and
    ...
    return !isFakeOverride
        ...
        && (((context.hasThisDescriptor() && containingDeclaration == context.getThisDescriptor())
                || ((context.getParentContext() instanceof FacadePartWithSourceFile)
                    && isWithinSameFile(
                        ((FacadePartWithSourceFile) context.getParentContext()).getSourceFile(),
                        descriptor)))
  ...
  private static boolean isWithinSameFile(
    ...
    if (containingDeclaration instanceof PackageFragmentDescriptor) {
      ...
      return callerFile != null
          && callerFile != SourceFile.NO_SOURCE_FILE
  ...
  public static boolean isCallInsideSameModuleAsDeclared(
    ...
    if (directMember instanceof DeserializedCallableMemberDescriptor) {
      return ModuleVisibilityUtilsKt.isContainedByCompiledPartOfOurModule(
          directMember, outDirectory);
    } else {
  ...
  public static boolean couldUseDirectAccessToProperty(
    ...
    if (!isCallInsideSameClassAsFieldRepresentingProperty(property, context)) {
      ...
      if (isAnnotationValue || !isDebuggerContext(context)) {
        // Unless we are evaluating expression in debugger context, only properties of the same
        // class can be directly accessed
        return false;
      } else {
    ...
    // If the accessor is non-default (i.e. it has some code) we should call that accessor and not
    // use direct access
    if (DescriptorPsiUtilsKt.hasBody(accessor)) return false;

    // If the accessor is private or final, it can't be overridden in the subclass and thus we can
    // use direct access
    return DescriptorVisibilities.isPrivate(accessor.getVisibility())
        || accessor.getModality() == FINAL;
  }
  ...
  public static boolean isDebuggerContext(@NotNull CodegenContext context) {
    ...
    if (contextDescriptor instanceof DeclarationDescriptorWithSource) {
      SourceElement sourceElement =
          ((DeclarationDescriptorWithSource) contextDescriptor).getSource();
  ...
  public static boolean isCompanionObjectInInterfaceNotIntrinsic(
    ...
    return isCompanionObject(companionObject)
        ...
        && !DescriptorsJvmAbiUtil.isMappedIntrinsicCompanionObject(
  ...
  public static boolean isDeclarationOfBigArityFunctionInvoke(
    ...
    return descriptor instanceof FunctionInvokeDescriptor
        && ((FunctionInvokeDescriptor) descriptor).hasBigArity();
  }

  public static boolean isDeclarationOfBigArityCreateCoroutineMethod(
      @Nullable DeclarationDescriptor descriptor) {
    return descriptor instanceof SimpleFunctionDescriptor
        && descriptor.getName().asString().equals(SUSPEND_FUNCTION_CREATE_METHOD_NAME)
        && ((SimpleFunctionDescriptor) descriptor).getValueParameters().size()
            >= BuiltInFunctionArity.BIG_ARITY - 1
        && descriptor.getContainingDeclaration() instanceof AnonymousFunctionDescriptor
        && ((AnonymousFunctionDescriptor) descriptor.getContainingDeclaration()).isSuspend();
  }
  ...
  public static boolean isPolymorphicSignature(@NotNull FunctionDescriptor descriptor) {
    return descriptor
        .getAnnotations()
        .hasAnnotation(PolymorphicSignatureCallChecker.polymorphicSignatureFqName);
  }
  ...
  public static boolean typeHasSpecializedInlineClassEquality(
    ...
    if (!(descriptor instanceof DeserializedClassDescriptor)) return true;

    DeserializedClassDescriptor classDescriptor = (DeserializedClassDescriptor) descriptor;

    // The Result class is the only inline class in the standard library without special rules for
    // equality.
    // We only call Result.equals-impl0 if we are compiling for Kotlin 1.4 or later. Otherwise, the
    // code
    ...
    if (DescriptorUtils.getFqNameSafe(classDescriptor).equals(StandardNames.RESULT_FQ_NAME)) {
      return state.getLanguageVersionSettings().getApiVersion().compareTo(ApiVersion.KOTLIN_1_4)
          >= 0;
    } else {
