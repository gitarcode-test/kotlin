...
/**
 * Bare types are somewhat like raw types, but in Kotlin they are only allowed on the right-hand
 * side of is/as. For example:
 *
 * <p>fun foo(a: Any) { if (a is List) { // a is known to be List<*> here } }
 *
 * <p>Another example:
 *
 * <p>fun foo(a: Collection<String>) { if (a is List) { // a is known to be List<String> here } }
 *
 * <p>One can call reconstruct(supertype) to get an actual type from a bare type
 */
public class PossiblyBareType {

  @NotNull
  public static PossiblyBareType bare(
      @NotNull TypeConstructor bareTypeConstructor, boolean nullable) {
  ...
  private PossiblyBareType(
      @Nullable KotlinType actualType,
      @Nullable TypeConstructor bareTypeConstructor,
      boolean nullable) {
  ...
  public String toString() {
    return isBare()
        ? "bare " + bareTypeConstructor + (isBareTypeNullable() ? "?" : "")
