...
public class ExpressionCodegen extends KtVisitor<StackValue, StackValue>
    implements LocalLookup, BaseExpressionCodegen {
  ...
  public ExpressionCodegen(
    ...
    boolean inlineScopesEnabled =
        state.getConfiguration().getBoolean(JVMConfigurationKeys.USE_INLINE_SCOPES_NUMBERS);
    this.inlineScopesGenerator = inlineScopesEnabled ? new InlineScopesGenerator() : null;
  }

  @Nullable
  private static FunctionDescriptor getOriginalSuspendLambdaDescriptorFromContext(
  ...
  static class LoopBlockStackElement extends BlockStackElement {
    ...
    LoopBlockStackElement(
        Label breakLabel, Label continueLabel, KtSimpleNameExpression targetLabel) {
  ...
  public ObjectLiteralResult generateObjectLiteral(@NotNull KtObjectLiteralExpression literal) {
    ...
    ClassContext objectContext =
        context.intoAnonymousClass(classDescriptor, this, OwnerKind.IMPLEMENTATION);

    MemberCodegen literalCodegen =
  ...
  private static void addReifiedParametersFromSignature(
      @NotNull MemberCodegen<?> member, @NotNull ClassDescriptor descriptor) {
  ...
  private static void processTypeArguments(@NotNull MemberCodegen<?> member, KotlinType type) {
    for (TypeProjection supertypeArgument : type.getArguments()) {
      ...
      TypeParameterDescriptor parameterDescriptor =
          TypeUtils.getTypeParameterDescriptorOrNull(supertypeArgument.getType());
      if (parameterDescriptor != null) {
        if (parameterDescriptor.isReified()) {
          member
              .getReifiedTypeParametersUsages()
              .addUsedReifiedParameter(parameterDescriptor.getName().asString());
        }
      } else {
  ...
  private static class ObjectLiteralResult {
    ...
    public ObjectLiteralResult(
        boolean wereReifiedMarkers, @NotNull ClassDescriptor classDescriptor) {
  ...
  private StackValue castToRequiredTypeOfInterfaceIfNeeded(
      StackValue inner, @NotNull ClassDescriptor provided, @NotNull ClassDescriptor required) {
  ...
  private StackValue genQualified(
      StackValue receiver, KtElement selector, KtVisitor<StackValue, StackValue> visitor) {
    if (tempVariables.containsKey(selector)) {
      throw new IllegalStateException(
          "Inconsistent state: expression saved to a temporary variable is a selector");
    }
    ...
    try {
      ...
      if (selector instanceof KtExpression) {
        ...
        runtimeAssertionInfo =
            bindingContext.get(JvmBindingContextSlices.RUNTIME_ASSERTION_INFO, expression);
        if (runtimeAssertionInfo == null
            && state
                .getLanguageVersionSettings()
                .supportsFeature(LanguageFeature.StrictJavaNullabilityAssertions)) {
          runtimeAssertionInfo =
              bindingContext.get(JvmBindingContextSlices.BODY_RUNTIME_ASSERTION_INFO, expression);
        }
      }

      if (BuiltinSpecialBridgesKt.isValueArgumentForCallToMethodWithTypeCheckBarrier(
          selector, bindingContext)) return stackValue;
      ...
      throw new CompilationException(
          "Failed to generate expression: " + selector.getClass().getSimpleName(), e, selector);
    }
  }

  private StackValue coerceAndBoxInlineClassIfNeeded(KtElement selector, StackValue stackValue) {
    ResolvedCall<? extends CallableDescriptor> resolvedCall =
        CallUtilKt.getResolvedCall(selector, bindingContext);
    ...
    // When we call suspend operator fun invoke using parens, we cannot box receiver as return type
    // inline class
    if (resolvedCall instanceof VariableAsFunctionResolvedCall
        && functionDescriptor.isOperator()
        && functionDescriptor.getName().getIdentifier().equals("invoke")) return stackValue;
    ...
    KotlinType originalKotlinType =
        unboxedInlineClass != null ? unboxedInlineClass : stackValueToWrap.kotlinType;
    Type originalType =
        unboxedInlineClass != null ? typeMapper.mapType(unboxedInlineClass) : stackValueToWrap.type;

    stackValue =
        new StackValue(originalType, originalKotlinType) {
          @Override
          public void putSelector(
              @NotNull Type type, @Nullable KotlinType kotlinType, @NotNull InstructionAdapter v) {
            if (((originalKotlinType != null
                    && InlineClassesUtilsKt.isInlineClassType(originalKotlinType))
                || (kotlinType != null && InlineClassesUtilsKt.isInlineClassType(kotlinType)))) {
              stackValueToWrap.put(v);
              // Suspend functions always return Ljava/lang/Object;, so, before inline-class
              // boxing/unboxing we need to generate CHECKCAST
  ...
  private StackValue visitClassOrObject(KtClassOrObject declaration) {
    ...
    ClassContext objectContext =
        context.intoAnonymousClass(descriptor, this, OwnerKind.IMPLEMENTATION);
    new ImplementationBodyCodegen(
            declaration,
            objectContext,
            classBuilder,
  ...
  @Override
  public StackValue visitObjectDeclaration(
      @NotNull KtObjectDeclaration declaration, StackValue data) {
  ...
  public StackValue visitExpression(@NotNull KtExpression expression, StackValue receiver) {
    throw new UnsupportedOperationException(
        "Codegen for " + expression + " is not yet implemented");
  }
  ...
  public static ClassDescriptor getSuperCallLabelTarget(
      @NotNull CodegenContext<?> context, @NotNull KtSuperExpression expression) {
    KotlinType thisTypeForSuperCall =
        context
            .getState()
            .getBindingContext()
            .get(BindingContext.THIS_TYPE_FOR_SUPER_EXPRESSION, expression);
    assert thisTypeForSuperCall != null
        : "This type for superCall ''" + expression.getText() + "'' should be not null!";
    ClassifierDescriptor descriptor =
        thisTypeForSuperCall.getConstructor().getDeclarationDescriptor();
    assert descriptor instanceof ClassDescriptor
        : "'This' reference target for ''"
            + expression.getText()
            + "''should be class descriptor, but was "
            + descriptor;
  ...
  @Override
  public StackValue visitParenthesizedExpression(
      @NotNull KtParenthesizedExpression expression, StackValue receiver) {
    return genQualified(receiver, expression.getExpression());
  }

  @Override
  public StackValue visitAnnotatedExpression(
      @NotNull KtAnnotatedExpression expression, StackValue receiver) {
  ...
  private static boolean isEmptyExpression(@Nullable KtElement expr) {
    ...
    if (expr instanceof KtBlockExpression) {
      ...
      if (statements.size() == 0
          || statements.size() == 1 && isEmptyExpression(statements.get(0))) {
        return true;
      }
    }
    return false;
  ...
  /* package */ StackValue generateIfExpression(
      @NotNull KtIfExpression expression, boolean isStatement) {
  ...
  public StackValue visitWhileExpression(
      @NotNull KtWhileExpression expression, StackValue receiver) {
    return StackValue.operation(
        Type.VOID_TYPE,
        adapter -> {
  ...
  @Override
  public StackValue visitDoWhileExpression(
      @NotNull KtDoWhileExpression expression, StackValue receiver) {
    return StackValue.operation(
        Type.VOID_TYPE,
        adapter -> {
  ...
  private void generateDoWhile(@NotNull KtDoWhileExpression expression) {
    ...
    blockStackElements.push(
        new LoopBlockStackElement(breakLabel, continueLabel, targetLabel(expression)));
    ...
    if (body instanceof KtBlockExpression) {
      // If body's a block, it can contain variable declarations which may be used in the condition
      // of a do-while loop.
      // We handle this case separately because otherwise such variable will be out of the frame map
      ...
      // Need to split leave task and condition cause otherwise BranchedValue optimizations wouldn't
      // work
  ...
  @Override
  public StackValue visitForExpression(
      @NotNull KtForExpression forExpression, StackValue receiver) {
    return StackValue.operation(
        Type.VOID_TYPE,
        adapter -> {
  ...
  @NotNull
  public static KotlinType getExpectedReceiverType(
      @NotNull ResolvedCall<? extends CallableDescriptor> resolvedCall) {
    ReceiverParameterDescriptor extensionReceiver =
        resolvedCall.getResultingDescriptor().getExtensionReceiverParameter();
    assert extensionReceiver != null : "Extension receiver should be non-null";
    return extensionReceiver.getType();
  }

  @Nullable
  public static KtExpression getSingleArgumentExpression(
      @NotNull ResolvedCall<? extends CallableDescriptor> resolvedCall) {
  ...
  private void generateForLoop(ForLoopGenerator generator) {
    ...
    // Renew line number cause it could be reset by inline (resetLastLineNumber) in
    // generator.checkPreCondition(loopExit).
    markStartLineNumber(generator.getForExpression());
    v.nop();
    ...
    blockStackElements.push(
        new LoopBlockStackElement(
            loopExit, continueLabel, targetLabel(generator.getForExpression())));
    generator.body();
    blockStackElements.pop();
    v.mark(continueLabel);
  ...
  public StackValue visitBreakExpression(
      @NotNull KtBreakExpression expression, StackValue receiver) {
    return generateBreakOrContinueExpression(expression, true, new Label(), new ArrayList<>());
  }

  @Override
  public StackValue visitContinueExpression(
      @NotNull KtContinueExpression expression, StackValue receiver) {
  ...
  private StackValue generateBreakOrContinueExpression(
    ...
    if (stackElement instanceof TryWithFinallyBlockStackElement) {
      TryWithFinallyBlockStackElement tryWithFinallyBlockStackElement =
          (TryWithFinallyBlockStackElement) stackElement;
      genFinallyBlockOrGoto(
          tryWithFinallyBlockStackElement,
          null,
          afterBreakContinueLabel,
          nestedTryBlocksWithoutFinally);
      ...
      if (labelElement == null
          || loopBlockStackElement.targetLabel != null
              && labelElement
                  .getReferencedName()
                  .equals(loopBlockStackElement.targetLabel.getReferencedName())) {
        Label label =
            isBreak ? loopBlockStackElement.breakLabel : loopBlockStackElement.continueLabel;
        return StackValue.operation(
            Type.VOID_TYPE,
            getNothingType(),
            adapter -> {
    ...
    StackValue result =
        generateBreakOrContinueExpression(
            expression, isBreak, afterBreakContinueLabel, nestedTryBlocksWithoutFinally);
  ...
  @Override
  public StackValue visitConstantExpression(
      @NotNull KtConstantExpression expression, StackValue receiver) {
    ConstantValue<?> compileTimeValue = getPrimitiveOrStringCompileTimeConstant(expression);
    assert compileTimeValue != null;
    return StackValue.constant(
        compileTimeValue.getValue(), expressionType(expression), kotlinType(expression));
  }

  @Nullable
  public ConstantValue<?> getCompileTimeConstant(@NotNull KtExpression expression) {
    return getCompileTimeConstant(
        expression, bindingContext, state.getConfig().getShouldInlineConstVals());
  }

  @Nullable
  public ConstantValue<?> getPrimitiveOrStringCompileTimeConstant(
      @NotNull KtExpression expression) {
    return getPrimitiveOrStringCompileTimeConstant(
        expression, bindingContext, state.getConfig().getShouldInlineConstVals());
  }
  ...
  public static ConstantValue<?> getCompileTimeConstant(
      ...
      boolean shouldInlineConstVals) {
    return JvmConstantsKt.getCompileTimeConstant(
        expression, bindingContext, takeUpConstValsAsConst, shouldInlineConstVals);
  }

  @Override
  public StackValue visitStringTemplateExpression(
      @NotNull KtStringTemplateExpression expression, StackValue receiver) {
  ...
  private void invokeAppendForEntries(
      StringConcatGenerator generator, List<StringTemplateEntry> entries) {
    for (StringTemplateEntry entry : entries) {
      if (entry instanceof StringTemplateEntry.Expression) {
        ...
        if (value.length() == 1) {
          generator.putValueOrProcessConstant(
              StackValue.constant(value.charAt(0), Type.CHAR_TYPE, null));
        } else {
  ...
  private abstract static class StringTemplateEntry {
  ...
  private @NotNull List<StringTemplateEntry> preprocessStringTemplate(
    ...
    for (KtStringTemplateEntry entry : entries) {
      if (entry instanceof KtLiteralStringTemplateEntry) {
        ...
        ConstantValue<?> compileTimeConstant =
            getPrimitiveOrStringCompileTimeConstant(entryExpression);

        if (compileTimeConstant != null
            && isConstantValueInlinableInStringTemplate(compileTimeConstant)) {
  ...
  private static boolean isConstantValueInlinableInStringTemplate(
  ...
  @Override
  public StackValue visitBlockExpression(
      @NotNull KtBlockExpression expression, StackValue receiver) {
  ...
  public StackValue visitNamedFunction(
      @NotNull KtNamedFunction function, StackValue data, boolean isStatement) {
  ...
  @Override
  public StackValue visitLambdaExpression(
      @NotNull KtLambdaExpression expression, StackValue receiver) {
  ...
  private StackValue genClosure(KtDeclarationWithBody declaration, @Nullable SamType samType) {
    ...
    return genClosure(
        declaration,
        descriptor,
        new ClosureGenerationStrategy(state, declaration),
        samType,
        null,
        null);
  }
  ...
  private StackValue genClosure(
    ...
    ClosureCodegen coroutineCodegen =
        CoroutineCodegenForLambda.create(this, descriptor, declaration, cv);
  ...
  private StackValue putClosureInstanceOnStack(
      @NotNull ClosureCodegen closureCodegen, @Nullable StackValue functionReferenceReceiver) {
  ...
  @Override
  public StackValue visitObjectLiteralExpression(
      @NotNull KtObjectLiteralExpression expression, StackValue receiver) {
    ...
    return StackValue.operation(
        ...
        v -> {
          ...
          pushClosureOnStack(
              classDescriptor, true, defaultCallGenerator, /* functionReferenceReceiver= */ null);

          ConstructorDescriptor primaryConstructor =
              classDescriptor.getUnsubstitutedPrimaryConstructor();
          assert primaryConstructor != null
              : "There should be primary constructor for object literal";
          ResolvedCall<ConstructorDescriptor> superCall =
              getDelegationConstructorCall(bindingContext, primaryConstructor);
          if (superCall != null) {
            // For an anonymous object, we should also generate all non-default arguments that it
            // captures for its super call
            ConstructorDescriptor superConstructor = superCall.getResultingDescriptor();
            ConstructorDescriptor constructorToCall =
                SamCodegenUtil.resolveSamAdapter(superConstructor);
            List<ValueParameterDescriptor> superValueParameters =
                superConstructor.getValueParameters();
            int params = superValueParameters.size();
            List<Type> superMappedTypes =
                typeMapper.mapToCallableMethod(constructorToCall, false).getValueParameterTypes();
            ...
            ArgumentGenerator argumentGenerator =
                new CallBasedArgumentGenerator(
                    this, defaultCallGenerator, valueParameters, mappedTypes);
          ...
          assert constructors.size() == 1
              : "Unexpected number of constructors for class: "
                  + classDescriptor
                  + " "
                  + constructors;
          ...
          Method constructor =
              typeMapper.mapAsmMethod(SamCodegenUtil.resolveSamAdapter(constructorDescriptor));
  ...
  public void pushClosureOnStack(
    ...
    for (Map.Entry<DeclarationDescriptor, EnclosedValueDescriptor> entry :
        closure.getCaptureVariables().entrySet()) {
    ...
    if (superClass != null) {
      pushClosureOnStack(
          superClass,
          putThis && closure.getCapturedOuterClassDescriptor() == null,
          callGenerator,
          /* functionReferenceReceiver= */ null);
    }
    ...
    if (closure.isSuspend()) {
      ...
      if (closure.isSuspendLambda()) {
        ...
        boolean isCrossinlineLambda =
            (callGenerator instanceof PsiInlineCodegen)
                && Objects.requireNonNull(
                        ((PsiInlineCodegen) callGenerator).getActiveLambda(),
                        "no active lambda found")
                    .isCrossInline();
        ...
        assert context.getFunctionDescriptor().isSuspend()
            : "Coroutines closure must be created only inside suspend functions";
        ValueParameterDescriptor continuationParameter =
            CollectionsKt.last(context.getFunctionDescriptor().getValueParameters());
        StackValue continuationValue = findLocalOrCapturedValue(continuationParameter);

        assert continuationValue != null
            : "Couldn't find a value for continuation parameter of "
                + context.getFunctionDescriptor();

        callGenerator.putCapturedValueOnStack(
            continuationValue, continuationValue.type, paramIndex);
      }
    }
  }

  @NotNull
  private static CallableDescriptor unwrapOriginalReceiverOwnerForSuspendLambda(
      @NotNull MethodContext context) {
    FunctionDescriptor originalForInvokeSuspend =
        context
            .getFunctionDescriptor()
            .getUserData(CoroutineCodegenUtilKt.INITIAL_SUSPEND_DESCRIPTOR_FOR_INVOKE_SUSPEND);
    ...
    if (context.getFunctionDescriptor().isSuspend()) {
      return CoroutineCodegenUtilKt.unwrapInitialDescriptorForSuspendFunction(
          context.getFunctionDescriptor());
    }
  ...
  /* package */ StackValue generateBlock(
      @NotNull KtBlockExpression expression, boolean isStatement) {
    if (expression.getParent() instanceof KtNamedFunction) {
      // For functions end of block should be end of function label
      return generateBlock(
          expression.getStatements(), isStatement, null, context.getMethodEndLabel());
    }
    ...
    for (LocalLookup.LocalLookupCase aCase : LocalLookup.LocalLookupCase.values()) {
      if (aCase.isCase(descriptor)) {
        ...
        if (asSharedVar && outerValue instanceof StackValue.FieldForSharedVar) {
          StackValue.FieldForSharedVar fieldForSharedVar =
              (StackValue.FieldForSharedVar) outerValue;
  ...
  private StackValueWithLeaveTask generateBlock(
    ...
    for (Iterator<KtExpression> iterator = statements.iterator(); iterator.hasNext(); ) {
      ...
      // Note that this statementResult value is potentially unused (in case of handleResult
      // coroutine call)
      // It's supposed here that no bytecode is emitted until 'put' call on relevant StackValue
      // object
      StackValue statementResult =
          isExpression ? gen(possiblyLabeledStatement) : genStatement(possiblyLabeledStatement);
    ...
    assert blockResult != null
        : "Block result should be initialized in the loop or the condition above";

    return new StackValueWithLeaveTask(
        blockResult,
        value -> {
  ...
  @Nullable
  private StackValue getCoroutineInstanceValueForSuspensionPoint(
      @NotNull ResolvedCall<?> resolvedCall) {
    FunctionDescriptor enclosingSuspendLambdaForSuspensionPoint =
        bindingContext.get(
            ENCLOSING_SUSPEND_FUNCTION_FOR_SUSPEND_FUNCTION_CALL, resolvedCall.getCall());
  ...
  @Nullable
  public StackValue genCoroutineInstanceForSuspendLambda(
      @NotNull FunctionDescriptor suspendFunction) {
    ...
    ClassDescriptor suspendLambdaClassDescriptor =
        bindingContext.get(CodegenBinding.CLASS_FOR_CALLABLE, suspendFunction);
  ...
  private Type getVariableTypeNoSharing(@NotNull VariableDescriptor variableDescriptor) {
    KotlinType varType =
        isDelegatedLocalVariable(variableDescriptor)
            ? JvmCodegenUtil.getPropertyDelegateType(
                (VariableDescriptorWithAccessors) variableDescriptor, bindingContext)
  ...
  private void putDescriptorIntoFrameMap(@NotNull KtElement statement) {
    ...
    if (statement instanceof KtNamedFunction) {
      ...
      assert descriptor instanceof FunctionDescriptor
          : "Couldn't find function declaration in binding context " + statement.getText();
  ...
  private void putLocalVariableIntoFrameMap(@NotNull KtVariableDeclaration statement) {
    ...
    if (isDelegatedLocalVariable(variableDescriptor)) {
      myFrameMap.enter(
          getDelegatedLocalVariableMetadata(variableDescriptor, bindingContext),
          AsmTypes.K_PROPERTY0_TYPE);
    }
  ...
  private void addLeaveTaskToRemoveDescriptorFromFrameMap(
    ...
    if (statement instanceof KtVariableDeclaration) {
      addLeaveTaskToRemoveLocalVariableFromFrameMap(
          (KtVariableDeclaration) statement, blockEnd, leaveTasks);
    }

    if (statement instanceof KtNamedFunction) {
      addLeaveTaskToRemoveNamedFunctionFromFrameMap(
          (KtNamedFunction) statement, blockEnd, leaveTasks);
    }
  }
    ...
    leaveTasks.add(
        answer -> {
          ...
          v.visitLocalVariable(
              variableDescriptor.getName().asString(),
              type.getDescriptor(),
              null,
              scopeStart,
              blockEnd,
  ...
  private void addLeaveTaskToRemoveNamedFunctionFromFrameMap(
    ...
    FunctionDescriptor functionDescriptor =
        (FunctionDescriptor) bindingContext.get(DECLARATION_TO_DESCRIPTOR, localFunction);
    ...
    leaveTasks.add(
        answer -> {
          ...
          String localVariableName =
              AsmUtil.LOCAL_FUNCTION_VARIABLE_PREFIX
                  + CommonVariableAsmNameManglingUtils.mangleNameIfNeeded(
                      functionDescriptor.getName().asString())
                  + "$"
                  + functionIndex;

          v.visitLocalVariable(
              localVariableName, type.getDescriptor(), null, scopeStart, blockEnd, index);
          return null;
        });
  }
  ...
  private boolean doFinallyOnReturn(
    ...
    if (!blockStackElements.isEmpty()) {
      ...
      if (stackElement instanceof TryWithFinallyBlockStackElement) {
        TryWithFinallyBlockStackElement tryWithFinallyBlockStackElement =
            (TryWithFinallyBlockStackElement) stackElement;
        genFinallyBlockOrGoto(
            tryWithFinallyBlockStackElement, null, afterReturnLabel, nestedTryBlocksWithoutFinally);
        nestedTryBlocksWithoutFinally.clear();
      } else if (stackElement instanceof TryBlockStackElement) {
        nestedTryBlocksWithoutFinally.add((TryBlockStackElement) stackElement);
      } else if (stackElement instanceof LoopBlockStackElement) {
  ...
  private void genFinallyBlockOrGoto(
    ...
    if (tryWithFinallyBlockStackElement != null) {
      finallyDepth++;
      assert tryWithFinallyBlockStackElement.gaps.size() % 2 == 0
          : "Finally block gaps are inconsistent";

      BlockStackElement topOfStack = blockStackElements.pop();
      assert topOfStack == tryWithFinallyBlockStackElement
          : "Top element of stack doesn't equals processing finally block";
      ...
      addGapLabelsForNestedTryCatchWithoutFinally(
          state, nestedTryBlocksWithoutFinally, finallyStart);
  ...
  @Override
  public StackValue visitReturnExpression(
      @NotNull KtReturnExpression expression, StackValue receiver) {
    return StackValue.operation(
        Type.VOID_TYPE,
        getNothingType(),
        adapter -> {
          ...
          if (isNonLocalReturn && state.getConfig().isInlineDisabled()) {
            state
                .getDiagnostics()
                .report(Errors.NON_LOCAL_RETURN_IN_DISABLED_INLINE.on(expression));
            genThrow(
                v,
          ...
          if (isNonLocalReturn) {
            ...
            if (returnTarget == null || !returnTarget.isSuspend()) {
              ...
              // Find inline-site and check, whether it is suspend functions returning unboxed
              // inline class
              CodegenContext<?> inlineSiteContext =
                  this.context.getFirstCrossInlineOrNonInlineContext();
              KotlinType originalInlineClass = null;
              if (inlineSiteContext instanceof MethodContext) {
                FunctionDescriptor view =
                    CoroutineCodegenUtilKt.getOrCreateJvmSuspendFunctionView(returnTarget, state);
                originalInlineClass =
              ...
              if (originalInlineClass != null) {
                // As an optimization, suspend functions, returning inline classes with reference
                // underlying
            ...
            KotlinType originalInlineClass =
                CoroutineCodegenUtilKt
                    .originalReturnTypeOfSuspendFunctionReturningUnboxedInlineClass(
          ...
          StackValue valueToReturn =
              returnedExpression != null ? gen(returnedExpression) : StackValue.none();
  ...
  public void generateFinallyBlocksIfNeeded(
    ...
    if (hasFinallyBlocks()) {
      if (!Type.VOID_TYPE.equals(returnType)) {
        ...
        StackValue.Local localForReturnValue =
            StackValue.local(returnValIndex, returnType, returnKotlinType);
  ...
  @Nullable
  private NonLocalReturnInfo getNonLocalReturnInfo(
      @NotNull CallableMemberDescriptor descriptor, @NotNull KtReturnExpression expression) {
    // call inside lambda
    if (isFunctionLiteral(descriptor) || isFunctionExpression(descriptor)) {
      if (expression.getLabelName() == null) {
        if (isFunctionLiteral(descriptor)) {
          ...
          FunctionDescriptor containingFunction =
              BindingContextUtils.getContainingFunctionSkipFunctionLiterals(descriptor, true)
                  .getFirst();
      ...
      if (element
          != DescriptorToSourceUtils.getSourceFromDescriptor(context.getContextDescriptor())) {
        DeclarationDescriptor elementDescriptor =
            typeMapper.getBindingContext().get(DECLARATION_TO_DESCRIPTOR, element);
  ...
  public void returnExpression(@NotNull KtExpression expr) {
    boolean isBlockedNamedFunction =
        expr instanceof KtBlockExpression && expr.getParent() instanceof KtNamedFunction;

    FunctionDescriptor originalSuspendLambdaDescriptor =
        getOriginalSuspendLambdaDescriptorFromContext(context);
    boolean isVoidCoroutineLambda =
        originalSuspendLambdaDescriptor != null
            && DescriptorBasedTypeSignatureMappingKt.hasVoidReturnType(
    ...
    // If generating body for named block-bodied function or Unit-typed coroutine lambda, generate
    // it as sequence of statements
  ...
  private static boolean endsWithReturn(@NotNull KtElement bodyExpression) {
    if (bodyExpression instanceof KtBlockExpression) {
      ...
      return statements.size() > 0
          && statements.get(statements.size() - 1) instanceof KtReturnExpression;
    }

    return bodyExpression instanceof KtReturnExpression;
  }

  private static boolean isLambdaVoidBody(
      @NotNull KtElement bodyExpression, @NotNull Type returnType) {
  ...
  @Override
  public StackValue visitSimpleNameExpression(
      @NotNull KtSimpleNameExpression expression, @NotNull StackValue receiver) {
    ...
    if (resolvedCall == null) {
      ...
      StackValue intrinsicResult =
          applyIntrinsic(descriptor, KCallableNameProperty.class, resolvedCall, receiver);
    ...
    StackValue intrinsicResult =
        applyIntrinsic(descriptor, IntrinsicPropertyGetter.class, resolvedCall, receiver);
    if (intrinsicResult != null) return intrinsicResult;

    return generateNonIntrinsicSimpleNameExpression(
        expression, receiver, descriptor, resolvedCall, isSyntheticField);
  }
  ...
  public StackValue generateNonIntrinsicSimpleNameExpression(
    ...
    if (descriptor instanceof PropertyDescriptor) {
      ...
      if (InlineClassesUtilsKt.isUnderlyingPropertyOfInlineClass(propertyDescriptor)) {
        ...
        return StackValue.underlyingValueOfInlineClass(
            typeMapper.mapType(propertyType), propertyType, receiver);
      }
      ...
      if (!codegenExtensions.isEmpty() && resolvedCall != null) {
        ExpressionCodegenExtension.Context context =
            new ExpressionCodegenExtension.Context(this, typeMapper, v);
      ...
      ClassDescriptor superCallTarget =
          resolvedCall == null ? null : getSuperCallTarget(resolvedCall.getCall());
    ...
    if (descriptor instanceof TypeAliasDescriptor) {
      ...
      assert classDescriptor != null
          : "Type alias "
              + descriptor
              + " static member reference should be rejected by type checker, "
    ...
    if (localOrCaptured != null) {
      if (descriptor instanceof ValueParameterDescriptor) {
        ...
        if (InlineClassesCodegenUtilKt.isInlineClassWithUnderlyingTypeAnyOrAnyN(inlineClassType)
          ...
          ClassDescriptor inlineClass =
              (ClassDescriptor) inlineClassType.getConstructor().getDeclarationDescriptor();
          InlineClassRepresentation<SimpleType> representation =
              getInlineClassRepresentation(inlineClass);
  ...
  private StackValue generateCompanionObjectInstance(ClassDescriptor companionObjectDescriptor) {
    ...
    CodegenContext accessorContext =
        getContextForCompanionObjectAccessorIfRequiredOrNull(companionObjectDescriptor, context);
    ...
    assert accessorContextDescriptor instanceof ClassDescriptor
        : "Expected to put accessor for companion object "
            + companionObjectDescriptor
            + " into a class, but got "
            + accessorContextDescriptor;
    ...
    // TODO we actually have corresponding accessor descriptor, it might be a better idea to use
    // general method call generation.
    return StackValue.operation(
        companionObjectType,
    ...
    while (context != null) {
      if (context instanceof ConstructorContext) {
        ...
        ClassDescriptor constructedClass =
            constructorContext.getConstructorDescriptor().getConstructedClass();
  ...
  private static boolean isInInnerClassesChainFor(
      ClassDescriptor innerClass, ClassDescriptor outerClass) {
  ...
  private StackValue applyIntrinsic(
    ...
    if (descriptor instanceof CallableMemberDescriptor) {
      CallableMemberDescriptor memberDescriptor =
          DescriptorUtils.unwrapFakeOverride((CallableMemberDescriptor) descriptor);
      ...
      if (intrinsicType.isInstance(intrinsic)) {
        ...
        return ((IntrinsicPropertyGetter) intrinsic)
            .generate(resolvedCall, this, returnType, receiver);
      }
    }
    return null;
  }
  ...
  public StackValue findCapturedValue(@NotNull DeclarationDescriptor descriptor) {
    ...
    if (isDelegatedLocalVariable(descriptor) && value != null) {
      VariableDescriptor metadata =
          getDelegatedLocalVariableMetadata((VariableDescriptor) descriptor, bindingContext);
      StackValue metadataValue =
          context.lookupInContext(metadata, StackValue.LOCAL_0, state, false);
      assert metadataValue != null
          : "Metadata stack value should be non-null for local delegated property: " + descriptor;
      return delegatedVariableValue(
          value, metadataValue, (VariableDescriptorWithAccessors) descriptor, typeMapper);
    }
  ...
  @Nullable
  private StackValue lookupCapturedValueInConstructorParameters(
      @NotNull DeclarationDescriptor descriptor) {
    ...
    int parameterOffsetInConstructor =
        context.closure.getCapturedParameterOffsetInConstructor(descriptor);
    // when captured parameter is singleton
    // see compiler/testData/codegen/box/objects/objectInLocalAnonymousObject.kt (fun local()
    // captured in A)
    if (parameterOffsetInConstructor == -1) return adjustVariableValue(parentResult, descriptor);

    assert parentResult instanceof StackValue.Field
            || parentResult instanceof StackValue.FieldForSharedVar
    ...
    return adjustVariableValue(
        StackValue.local(parameterOffsetInConstructor, parentResult.type, parentResult.kotlinType),
        descriptor);
  }
    ...
    if (descriptor instanceof VariableDescriptor) {
      ...
      KotlinType delegateKotlinType =
          isDelegatedLocalVariable(descriptor)
              ? JvmCodegenUtil.getPropertyDelegateType(
                  (VariableDescriptorWithAccessors) descriptor, bindingContext)
              : null;
      if (sharedVarType != null) {
  ...
  public int lookupLocalIndex(DeclarationDescriptor descriptor) {
    ...
    DeclarationDescriptor synonym =
        bindingContext.get(CodegenBinding.PARAMETER_SYNONYM, (ValueParameterDescriptor) descriptor);
  ...
  public StackValue.Property intermediateValueForProperty(
      ...
      @NotNull StackValue receiver) {
    return intermediateValueForProperty(
        propertyDescriptor, forceField, false, superCallTarget, false, receiver, null, false);
  }

  private CodegenContext getBackingFieldContext(
      @NotNull AccessorKind accessorKind, @NotNull DeclarationDescriptor containingDeclaration) {
    switch (accessorKind) {
      case NORMAL:
        if (containingDeclaration instanceof ClassDescriptor) {
          CodegenContext parentWithDescriptor =
              context.findParentContextWithDescriptor(containingDeclaration);
      ...
      case IN_CLASS_COMPANION:
        return context.findParentContextWithDescriptor(
            containingDeclaration.getContainingDeclaration());
  ...
  public StackValue.Property intermediateValueForProperty(
    ...
    if (propertyDescriptor instanceof SyntheticJavaPropertyDescriptor) {
      return intermediateValueForSyntheticExtensionProperty(
          (SyntheticJavaPropertyDescriptor) propertyDescriptor, receiver);
    }

    if (propertyDescriptor instanceof PropertyImportedFromObject) {
      propertyDescriptor =
          ((PropertyImportedFromObject) propertyDescriptor).getCallableFromObject();
    }

    DeclarationDescriptor containingDeclaration = propertyDescriptor.getContainingDeclaration();

    boolean isBackingFieldMovedFromCompanion =
        DescriptorsJvmAbiUtil.isPropertyWithBackingFieldInOuterClass(propertyDescriptor);
    ...
    if (skipLateinitAssertion) {
      ...
      fieldAccessorKind =
          JvmCodegenUtil.isDebuggerContext(context)
              ? AccessorKind.NORMAL
              : AccessorKind.IN_CLASS_COMPANION;
    } else //noinspection ConstantConditions
    if ((syntheticBackingField
        && context.getFirstCrossInlineOrNonInlineContext().getParentContext().getContextDescriptor()
    ...
    boolean isStaticBackingField =
        DescriptorUtils.isStaticDeclaration(propertyDescriptor)
    ...
    KotlinType delegateType =
        JvmCodegenUtil.getPropertyDelegateType(propertyDescriptor, bindingContext);
    ...
    CodegenContext<?> backingFieldContext =
        getBackingFieldContext(fieldAccessorKind, containingDeclaration);
    boolean isPrivateProperty =
        fieldAccessorKind != AccessorKind.NORMAL
            && (DescriptorAsmUtil.getVisibilityForBackingField(
                        propertyDescriptor, isDelegatedProperty)
                    & ACC_PRIVATE)
                != 0;
    ...
    PropertyDescriptor originalPropertyDescriptor =
        DescriptorUtils.unwrapFakeOverride(propertyDescriptor);
    boolean directAccessToGetter =
        couldUseDirectAccessToProperty(
            ...
            isDelegatedProperty,
            context,
            state.getConfig().getShouldInlineConstVals());
    ...
    if (fieldAccessorKind == AccessorKind.LATEINIT_INTRINSIC) {
      ...
      if (!skipPropertyAccessors) {
        if (isBackingFieldMovedFromCompanion
            && context.getContextDescriptor() instanceof AccessorForPropertyBackingField) {
          CodegenContext<?> parentContext = backingFieldContext.getParentContext();
          propertyDescriptor =
              parentContext.getAccessor(
                  propertyDescriptor,
                  AccessorKind.IN_CLASS_COMPANION,
                  delegateType,
                  superCallTarget);
    ...
    } else if (fieldAccessorKind == AccessorKind.IN_CLASS_COMPANION
        || fieldAccessorKind == AccessorKind.FIELD_FROM_LOCAL) {
      // Do not use accessor 'access<property name>$cp' when the property can be accessed directly
      // by its backing field.
      skipPropertyAccessors =
          skipAccessorsForPrivateFieldInOuterClass
      ...
      ownerDescriptor =
          isBackingFieldMovedFromCompanion ? containingDeclaration : propertyDescriptor;
    }

    if (!skipPropertyAccessors) {
      if (!directAccessToGetter || !directAccessToSetter) {
        propertyDescriptor =
            context.getAccessorForSuperCallIfNeeded(propertyDescriptor, superCallTarget, state);
    ...
    if (isExtensionProperty && !isDelegatedProperty) {
      fieldName = null;
    } else if (originalPropertyDescriptor.getContainingDeclaration()
        == backingFieldContext.getContextDescriptor()) {
      assert backingFieldContext instanceof FieldOwnerContext
          : "Actual context is "
              + backingFieldContext
              + " but should be instance of FieldOwnerContext";
      fieldName =
          ((FieldOwnerContext) backingFieldContext)
              .getFieldName(propertyDescriptor, isDelegatedProperty);
    } else {
    ...
    if (propertyDescriptor instanceof JavaPropertyDescriptor
        && InlineClassesUtilsKt.isInlineClassType(propertyType)) {
  ...
  private StackValue.Property intermediateValueForSyntheticExtensionProperty(
      @NotNull SyntheticJavaPropertyDescriptor propertyDescriptor, @NotNull StackValue receiver) {
    ...
    CallableMethod callableGetter =
        typeMapper.mapToCallableMethod(
            context.accessibleDescriptor(propertyDescriptor.getGetMethod(), null), false);
    FunctionDescriptor setMethod = propertyDescriptor.getSetMethod();
    CallableMethod callableSetter =
        setMethod != null
            ? typeMapper.mapToCallableMethod(context.accessibleDescriptor(setMethod, null), false)
            : null;
    return StackValue.property(
        propertyDescriptor,
        null,
  ...
  public StackValue visitCallExpression(@NotNull KtCallExpression expression, StackValue receiver) {
    ...
    if (descriptor.getOriginal() instanceof SamConstructorDescriptor) {
      ...
      assert argumentExpression != null
          : "Argument expression is not saved for a SAM constructor: " + descriptor;
  ...
  public StackValue visitCollectionLiteralExpression(
    ...
    ResolvedCall<FunctionDescriptor> resolvedCall =
        bindingContext.get(COLLECTION_LITERAL_CALL, expression);
    assert resolvedCall != null
        : "No resolved call for " + PsiUtilsKt.getTextWithLocation(expression);
  ...
  private StackValue genSamInterfaceValue(
    ...
    Type asmType =
        state
            ...
            .getSamWrapperClass(
                samType, expression.getContainingKtFile(), this, context.getContextDescriptor());
    ...
    return StackValue.operation(
        ...
        v -> {
          ...
          v.invokespecial(
              ...
              "<init>",
              Type.getMethodDescriptor(Type.VOID_TYPE, functionType),
              false);
  ...
  private boolean shouldForceAccessorForConstructor(FunctionDescriptor descriptor) {
    ...
    if (descriptor.getContainingDeclaration()
  ...
  private AccessorForConstructorDescriptor createAccessorForHiddenConstructor(
      @NotNull ResolvedCall<?> resolvedCall, FunctionDescriptor descriptor) {
  ...
  public StackValue invokeFunction(
      @NotNull ResolvedCall<?> resolvedCall, @NotNull StackValue receiver) {
    return invokeFunction(resolvedCall.getCall(), resolvedCall, receiver);
  }

  @NotNull
  public StackValue invokeFunction(
      @NotNull Call call, @NotNull ResolvedCall<?> resolvedCall, @NotNull StackValue receiver) {
    ...
    if (callWithRealDescriptor != null) {
      prepareCoroutineArgumentForSuspendCall(
          resolvedCall, callWithRealDescriptor.getFakeContinuationExpression());
  ...
  private void prepareCoroutineArgumentForSuspendCall(
      @NotNull ResolvedCall<?> resolvedCall, @NotNull KtExpression continuationExpression) {
    StackValue coroutineInstanceValueForSuspensionPoint =
        getCoroutineInstanceValueForSuspensionPoint(resolvedCall);
  ...
  private StackValue getContinuationParameterFromEnclosingSuspendFunction(
    ...
    assert enclosingSuspendFunction != null
        : "Suspend functions may be called either as suspension points or from another suspend"
              + " function";
    ...
    assert enclosingSuspendFunctionJvmView != null
        : "No JVM view function found for " + enclosingSuspendFunction;

    return getContinuationParameterFromEnclosingSuspendFunctionDescriptor(
        enclosingSuspendFunctionJvmView);
  }

  @Nullable
  public StackValue getContinuationParameterFromEnclosingSuspendFunctionDescriptor(
      @NotNull FunctionDescriptor enclosingSuspendFunctionJvmView) {
    ValueParameterDescriptor continuationParameter =
        enclosingSuspendFunctionJvmView
            .getValueParameters()
  ...
  public static CodegenContext getParentContextSubclassOf(
      ClassDescriptor descriptor, CodegenContext context) {
  ...
  @Nullable
  public static CodegenContext getCompanionObjectContextSubclassOf(
      ClassDescriptor descriptor, CodegenContext context) {
    for (CodegenContext current = context; current != null; current = current.getParentContext()) {
      ...
      ClassDescriptor companionObject =
          classContext.getContextDescriptor().getCompanionObjectDescriptor();
  ...
  Callable resolveToCallable(
      @NotNull FunctionDescriptor fd, boolean superCall, @NotNull ResolvedCall<?> resolvedCall) {
    ...
    if (valueArguments != null
        && valueArguments.stream().anyMatch(it -> it instanceof DefaultValueArgument)) {
      // When we invoke a function with some arguments mapped as defaults,
      // we later reroute this call to an overridden function in a base class that processes the
      // default arguments.
      // If the base class is generic, this overridden function can have a different Kotlin
      // signature
  ...
  public void invokeMethodWithArguments(
    ...
    assert callGenerator == defaultCallGenerator
            || !tailRecursionCodegen.isTailRecursion(resolvedCall)
    ...
    invokeMethodWithArguments(
        callableMethod, resolvedCall, receiver, callGenerator, argumentGenerator);
  }
  ...
  public void invokeMethodWithArguments(
    ...
    if (AssertCodegenUtilKt.isAssertCall(resolvedCall)
        && !state.getConfig().getAssertionsMode().equals(JVMAssertionsMode.LEGACY)) {
      AssertCodegenUtilKt.generateAssert(
          state.getConfig().getAssertionsMode(), resolvedCall, this, parentCodegen);
      return;
    }

    SuspensionPointKind suspensionPointKind =
        CoroutineCodegenUtilKt.isSuspensionPoint(resolvedCall, this);
    boolean maybeSuspensionPoint =
        suspensionPointKind != SuspensionPointKind.NEVER && !insideCallableReference();
    boolean isConstructor = resolvedCall.getResultingDescriptor() instanceof ConstructorDescriptor;
    if (!(callableMethod instanceof IntrinsicWithSpecialReceiver)) {
      putReceiverAndInlineMarkerIfNeeded(
          callableMethod, resolvedCall, receiver, maybeSuspensionPoint, isConstructor);
    }
    ...
    if (callableMethod instanceof CallableMethod) {
      List<JvmMethodParameterSignature> callableParameters =
          ((CallableMethod) callableMethod).getValueParameters();
      for (JvmMethodParameterSignature parameter : callableParameters) {
        if (parameter.getKind() == JvmMethodParameterKind.CONSTRUCTOR_MARKER) {
          callGenerator.putValueIfNeeded(
              new JvmKotlinType(parameter.getAsmType(), null),
              StackValue.constant(null, parameter.getAsmType()));
        }
      }
    }
    ...
    // If a suspend function returns unboxed inline class, we should check for COROUTINE_SUSPENDED
    // and only then box the inline class.
    if (insideCallableReference()
        && resolvedCall.getResultingDescriptor() instanceof FunctionDescriptor) {
      KotlinType unboxedInlineClass =
          CoroutineCodegenUtilKt.originalReturnTypeOfSuspendFunctionReturningUnboxedInlineClass(
  ...
  private boolean insideCallableReference() {
    return (parentCodegen instanceof ClosureCodegen)
        && ((ClosureCodegen) parentCodegen).isCallableReference();
  }
  ...
  private void putReceiverAndInlineMarkerIfNeeded(
    ...
    boolean isSafeCallOrOnStack =
        receiver instanceof StackValue.SafeCall || receiver instanceof StackValue.OnStack;
    ...
    if (!isConstructor) { // otherwise already
      ...
      // In regular cases we add an inline marker just before receiver is loaded (to spill the stack
      // before a suspension)
      ...
      // The solution is to spill stack just after receiver is loaded (after IFNULL) in case of safe
      // call.
      // But the problem is that we should leave the receiver itself on the stack, so we store it in
      ...
      if (isSuspendCall && isSafeCallOrOnStack) {
        ...
        Type firstReceiverType =
            bothReceivers ? ((CallReceiver) receiver).getDispatchReceiver().type : receiver.type;
  ...
  private CallGenerator getOrCreateCallGenerator(
    ...
    boolean isIntrinsic =
        descriptor instanceof CallableMemberDescriptor
            && state.getIntrinsics().getIntrinsic((CallableMemberDescriptor) descriptor) != null;

    boolean isInline =
        (InlineUtil.isInline(descriptor) && !isIntrinsic)
            || InlineUtil.isArrayConstructorWithLambda(descriptor);
    ...
    boolean shouldInline =
        isInline
            && (!state.getConfig().isInlineDisabled()
    ...
    FunctionDescriptor original =
        CoroutineCodegenUtilKt.getOriginalSuspendFunctionView(
            unwrapInitialSignatureDescriptor(
                DescriptorUtils.unwrapFakeOverride((FunctionDescriptor) descriptor.getOriginal())),
            bindingContext);

    FunctionDescriptor functionDescriptor =
        InlineUtil.isArrayConstructorWithLambda(original)
            ? FictitiousArrayConstructor.create((ConstructorDescriptor) original)
            : original.getOriginal();
    PsiSourceCompilerForInline sourceCompiler =
        new PsiSourceCompilerForInline(this, callElement, functionDescriptor);

    JvmMethodSignature signature =
        typeMapper.mapSignatureWithGeneric(
            functionDescriptor, sourceCompiler.getContext().getContextKind());
    if (signature.getAsmMethod().getName().contains("-")
        && !state.getConfig().getUseOldManglingSchemeForFunctionsWithInlineClassesInSignatures()) {
      Boolean classFileContainsMethod =
          InlineClassesCodegenUtilKt.classFileContainsMethod(
              functionDescriptor, state, signature.getAsmMethod());
      if (classFileContainsMethod != null && !classFileContainsMethod) {
        typeMapper.setUseOldManglingRulesForFunctionAcceptingInlineClass(true);
        signature =
            typeMapper.mapSignatureWithGeneric(
    ...
    if (isDefaultCompilation) {
      return new InlineCodegenForDefaultBody(
          functionDescriptor, this, state, signature, sourceCompiler);
    } else {
  ...
  protected CallGenerator getOrCreateCallGeneratorForDefaultImplBody(
      @NotNull FunctionDescriptor descriptor, @Nullable KtNamedFunction function) {
  ...
  @NotNull
  CallGenerator getOrCreateCallGenerator(
      @NotNull ResolvedCall<?> resolvedCall, @NotNull CallableDescriptor descriptor) {
    TypeParameterMappings<KotlinType> mappings =
        new TypeParameterMappings<>(
            typeSystem,
            ...
            (KotlinType type, BothSignatureWriter sw) ->
                typeMapper.mapTypeArgument(approximateCapturedType(type), sw));
  ...
  private KotlinType approximateCapturedType(@NotNull KotlinType type) {
    if (state.getLanguageVersionSettings().supportsFeature(LanguageFeature.NewInference)) {
      TypeApproximator approximator =
          new TypeApproximator(state.getModule().getBuiltIns(), state.getLanguageVersionSettings());

      KotlinType approximatedType =
          TypeUtils.contains(
                  type, (containedType) -> CapturedTypeConstructorKt.isCaptured(containedType))
              ? (KotlinType)
  ...
  private static Map<TypeParameterDescriptor, KotlinType> getTypeArgumentsForResolvedCall(
    ...
    TypeAliasConstructorDescriptor typeAliasConstructorDescriptor =
        (TypeAliasConstructorDescriptor) descriptor;
    ClassConstructorDescriptor underlyingConstructorDescriptor =
        typeAliasConstructorDescriptor.getUnderlyingConstructorDescriptor();
    ...
    List<TypeParameterDescriptor> typeParameters =
        underlyingConstructorDescriptor.getTypeParameters();

    assert typeParameters.size() == typeArgumentsForReturnType.size()
        : "Type parameters of the underlying constructor "
            + underlyingConstructorDescriptor
            + "should correspond to type arguments for the resulting type "
            + resultingType;

    Map<TypeParameterDescriptor, KotlinType> typeArgumentsMap =
        Maps.newHashMapWithExpectedSize(typeParameters.size());
  ...
  public StackValue generateReceiverValue(@Nullable ReceiverValue receiverValue, boolean isSuper) {
    if (receiverValue instanceof ImplicitClassReceiver) {
      ClassDescriptor receiverDescriptor =
          ((ImplicitClassReceiver) receiverValue).getDeclarationDescriptor();
      ...
      return generateExtensionReceiver(
          ((ExtensionReceiver) receiverValue).getDeclarationDescriptor());
      ...
      if (!state.getConfig().isReceiverAssertionsDisabled()) {
        RuntimeAssertionInfo runtimeAssertionInfo =
            bindingContext.get(
                JvmBindingContextSlices.RECEIVER_RUNTIME_ASSERTION_INFO, expressionReceiver);
  ...
  private StackValue generateInstanceReceiver(
      @NotNull ClassDescriptor receiverDescriptor, boolean isSuper, boolean castReceiver) {
    ...
    if (DescriptorUtils.isCompanionObject(receiverDescriptor)) {
      ...
      if (contextDescriptor instanceof FunctionDescriptor
          && receiverDescriptor == contextDescriptor.getContainingDeclaration()) {
        return StackValue.LOCAL_0;
      } else if (isPossiblyUninitializedSingleton(receiverDescriptor)
          && isInsideSingleton(receiverDescriptor)) {
      ...
      StackValue contextReceiverValueOrNull =
          generateScriptReceiver((ScriptDescriptor) receiverDescriptor);

      return contextReceiverValueOrNull != null
          ? contextReceiverValueOrNull
          : StackValue.thisOrOuter(this, receiverDescriptor, isSuper, castReceiver);
    } else {
  ...
  private CodegenContext getContextForCompanionObjectAccessorIfRequiredOrNull(
    ...
    if (!state
        .getLanguageVersionSettings()
        .supportsFeature(LanguageFeature.ProperVisibilityForCompanionObjectInstanceField)) {
      return null;
    }
    ...
    ClassDescriptor containingClassOfCompanionObject =
        DescriptorUtils.getContainingClass(companionObjectDescriptor);

    if ((accessFlag & ACC_PRIVATE) != 0) {
      // Private companion object
      if (!isInlineMethodContext
          && contextDescriptor.getContainingDeclaration() == containingClassOfCompanionObject) {
        return null;
      }
      ...
      if (!isInlineMethodContext
          && canAccessProtectedMembers(contextDescriptor, containingClassOfCompanionObject)) {
        return null;
      }
      CodegenContext accessorContext =
          getParentContextSubclassOf(containingClassOfCompanionObject, context);
      if (accessorContext == null) {
        accessorContext =
            getCompanionObjectContextSubclassOf(containingClassOfCompanionObject, context);
      }
      assert accessorContext != null
          : "Class context for accessor to a protected companion object "
              + companionObjectDescriptor
              + " not found:\n"
  ...
  private static boolean canAccessProtectedMembers(
      DeclarationDescriptor contextDescriptor, ClassDescriptor classDescriptor) {
  ...
  public StackValue generateExtensionReceiver(@NotNull CallableDescriptor descriptor) {
    ...
    if (myFrameMap.getIndex(parameter) != -1) {
      ...
      return StackValue.local(myFrameMap.getIndex(parameter), typeMapper.mapType(type), type);
    }
    ...
    while (cur != null) {
      ...
      if (cur instanceof ScriptContext) {
        ...
        return StackValue.field(
            classType, receiverKotlinType, currentScriptType, fieldName, false, result, receiver);
      }
  ...
  @NotNull
  public StackValue generateThisOrOuter(
      @NotNull ClassDescriptor calleeContainingClass, boolean isSuper) {
  ...
  @NotNull
  public StackValue generateThisOrOuter(
      @NotNull ClassDescriptor thisOrOuterClass, boolean isSuper, boolean forceOuter) {
  ...
  private StackValue generateThisOrOuterFromContext(
      @NotNull ClassDescriptor thisOrOuterClass, boolean isSuper, boolean forceOuter) {
    CodegenContext cur = context;
    ...
    while (cur != null) {
      ...
      // We use equals on type constructors (instead of reference equality on classes) to support
      // the case where default parameter
      // values of actual functions loaded from the expected function refer to the expected class.
      if (!isSuper
          && thisDescriptor.getTypeConstructor().equals(thisOrOuterClass.getTypeConstructor())) {
        return result;
      }
      ...
      // for constructor super call we should access to outer instance through parameter in locals,
      // in other cases through field for captured outer
      if (inStartConstructorContext) {
        ...
        if (!(thisOrOuterClass instanceof ScriptDescriptor)) {
          if (cur instanceof ScriptLikeContext) {
            ...
            if (closure != null) {
              StackValue captured =
                  ((ScriptLikeContext) enclosingContext).captureVariable(closure, thisOrOuterClass);
              if (captured != null) {
                return captured;
              }
            }
  ...
  private boolean canSkipArrayCopyForSpreadArgument(KtExpression spreadArgument) {
    ResolvedCall<? extends CallableDescriptor> resolvedCall =
        CallUtilKt.getResolvedCall(spreadArgument, bindingContext);
  ...
  @NotNull
  public StackValue genVarargs(
      @NotNull VarargValueArgument valueArgument, @NotNull KotlinType outType) {
    ...
    // There can be only single such argument for a given vararg parameter, but here it doesn't
    // really matter:
    // we'll just build a copy of array as if an array argument was passed as positional with an
    ...
    boolean hasSpreadOperator =
        state
            ...
            ? arguments.stream()
                .anyMatch(argument -> argument.getSpreadElement() != null || argument.isNamed())
    ...
    if (hasSpreadOperator) {
      ...
      if (size == 1) {
            ...
            adapter -> {
              ...
              if (!canSkipArrayCopyForSpreadArgument(spreadArgument)) {
                ...
                v.invokestatic(
                    "java/util/Arrays",
                    "copyOf",
                    Type.getMethodDescriptor(arrayType, arrayType, Type.INT_TYPE),
                    false);
              }
        ...
        if (arrayOfReferences) {
          ...
          String spreadBuilderClassName =
              AsmUtil.asmPrimitiveTypeToLangPrimitiveType(elementType).getTypeName().getIdentifier()
                  + "SpreadBuilder";
            ...
            adapter -> {
              ...
              for (int i = 0; i != size; ++i) {
                ...
                if (argument.getSpreadElement() != null) {
                  ...
                  if (argumentKotlinType != null
                      && InlineClassesUtilsKt.isInlineClassType(argumentKotlinType)) {
                    // we're going to pass value of inline class type to j/l/Object, which would
                    // result in boxing and then
                    // will cause check cast error on toArray() call. To mitigate this problem, we
                    // unbox this value and pass
                    // primitive array to the method. Note that bytecode optimisations will remove
                    // box/unbox calls.
                    StackValue.coerce(
                        OBJECT_TYPE,
                        argumentKotlinType,
                        asmType(argumentKotlinType),
                        argumentKotlinType,
                        v);
                  }
      ...
      return StackValue.operation(
          ...
          adapter -> {
            ...
            newArrayInstruction(outType);
            KotlinType elementKotlinType =
                outType.getConstructor().getBuiltIns().getArrayElementType(outType);
            for (int i = 0; i != size; ++i) {
              v.dup();
              ...
              StackValue.arrayElement(
                      elementType,
                      elementKotlinType,
                      StackValue.onStack(type, outType),
  ...
  public int indexOfLocalNotDelegated(KtReferenceExpression lhs) {
    ...
    if (declarationDescriptor instanceof LocalVariableDescriptor
        && ((LocalVariableDescriptor) declarationDescriptor).isDelegated()) {
      return -1;
    }
  ...
  @Override
  public StackValue visitClassLiteralExpression(
      @NotNull KtClassLiteralExpression expression, StackValue data) {
  ...
  public StackValue visitCallableReferenceExpression(
      @NotNull KtCallableReferenceExpression expression, StackValue data) {
    ResolvedCall<?> resolvedCall =
        CallUtilKt.getResolvedCallWithAssert(expression.getCallableReference(), bindingContext);
    ...
    return generatePropertyReference(
        expression,
        getVariableDescriptorNotNull(expression),
  ...
  public StackValue generateCallableReferenceReceiver(@NotNull ResolvedCall<?> resolvedCall) {
    ...
    return StackValue.coercion(
        generateReceiverValue(receiver, false), asmType(receiverType), receiverType);
  }
  ...
  private StackValue generatePropertyReference(
    ...
    if (classDescriptor == null) {
      throw new IllegalStateException(
          "Property reference class was not found: "
              + variableDescriptor
              + "\nTried to generate: "
              + element.getText());
    }
    ...
    PropertyReferenceCodegen codegen =
        new PropertyReferenceCodegen(
            state,
            parentCodegen,
            context.intoAnonymousClass(classDescriptor, this, OwnerKind.IMPLEMENTATION),
            element,
            ...
            target,
            receiverValue != null
                ? new JvmKotlinType(receiverValue.type, receiverValue.kotlinType)
                : null);
    codegen.generate();
  ...
  @Override
  public StackValue visitDotQualifiedExpression(
      @NotNull KtDotQualifiedExpression expression, StackValue receiver) {
  ...
  private StackValue generateExpressionWithNullFallback(
      @NotNull KtExpression expression, @NotNull Label ifnull) {
    ...
    if (expression instanceof KtSafeQualifiedExpression && !isPrimitive(type)) {
      return StackValue.coercion(
          generateSafeQualifiedExpression((KtSafeQualifiedExpression) expression, ifnull),
          type,
          kotlinType);
    } else {
  ...
  private StackValue generateSafeQualifiedExpression(
      @NotNull KtSafeQualifiedExpression expression, @NotNull Label ifNull) {
    ...
    // Do not optimize for primitives cause in case of safe call extension receiver should be
    // generated before dispatch one
    StackValue newReceiver =
        new StackValue.SafeCall(
            receiverType,
            receiverKotlinType,
            receiverValue,
            isPrimitive(receiverType) ? null : ifNull);
    return genQualified(newReceiver, selector);
  }

  @Override
  public StackValue visitSafeQualifiedExpression(
      @NotNull KtSafeQualifiedExpression expression, StackValue unused) {
  ...
  @Override
  public StackValue visitBinaryExpression(
      @NotNull KtBinaryExpression expression, @NotNull StackValue receiver) {
    ...
    if (opToken == KtTokens.EQ) {
    ...
    } else if (opToken == KtTokens.EQEQ
        || opToken == KtTokens.EXCLEQ
        || opToken == KtTokens.EQEQEQ
        || opToken == KtTokens.EXCLEQEQEQ) {
    ...
    } else if (opToken == KtTokens.LT
        || opToken == KtTokens.LTEQ
        || opToken == KtTokens.GT
        || opToken == KtTokens.GTEQ) {
      ...
      return generateIn(
          StackValue.expression(expressionType(expression.getLeft()), expression.getLeft(), this),
          expression.getRight(),
          reference);
    } else {
      ...
      ResolvedCall<?> resolvedCall =
          CallUtilKt.getResolvedCallWithAssert(expression, bindingContext);
      FunctionDescriptor descriptor = (FunctionDescriptor) resolvedCall.getResultingDescriptor();
  ...
  private StackValue genLazyUnlessProvided(
      @Nullable StackValue pregenerated, @NotNull KtExpression expr, @NotNull Type type) {
  ...
  private StackValue genLazyUnlessProvided(
      ...
      @NotNull Type type,
      @Nullable KotlinType kotlinType) {
    return pregenerated != null
        ? StackValue.coercion(pregenerated, type, kotlinType)
        : genLazy(expr, type, kotlinType);
  }

  private StackValue genUnlessProvided(
      @Nullable StackValue pregenerated, @NotNull KtExpression expr, @NotNull Type type) {
  ...
  private StackValue generateEquals(
    ...
    boolean leftIsInlineClass =
        leftKotlinType != null && InlineClassesUtilsKt.isInlineClassType(leftKotlinType);
    boolean rightIsInlineClass =
        rightKotlinType != null && InlineClassesUtilsKt.isInlineClassType(rightKotlinType);
    ...
    boolean leftHasPrimitiveEquality =
        isPrimitive(leftType)
            && (!leftIsInlineClass || isInlineClassTypeWithPrimitiveEquality(leftKotlinType));
    boolean rightHasPrimitiveEquality =
        isPrimitive(rightType)
            && (!rightIsInlineClass || isInlineClassTypeWithPrimitiveEquality(rightKotlinType));
    ...
    if (leftIsInlineClass || rightIsInlineClass) {
      boolean leftIsUnboxed =
          leftIsInlineClass && StackValue.isUnboxedInlineClass(leftKotlinType, leftType);
      boolean rightIsUnboxed =
          rightIsInlineClass && StackValue.isUnboxedInlineClass(rightKotlinType, rightType);
      if (!leftIsUnboxed
          || !rightIsUnboxed
          || !leftHasPrimitiveEquality
          || !rightHasPrimitiveEquality) {
    ...
    if (pregeneratedSubject == null
        && left instanceof KtSafeQualifiedExpression
    ...
    if (isPrimitive(leftType)
      ...
      return genCmpPrimitiveToSafeCall(
          left, leftType, (KtSafeQualifiedExpression) right, opToken, pregeneratedSubject);
    }
  ...
  private StackValue genEqualsForInlineClasses(
      ...
      boolean rightIsUnboxed) {
    StackValue leftValue =
        genLazyUnlessProvided(pregeneratedSubject, left, leftType, leftKotlinType);
          ...
          if (useUnboxedEquals) {
            ...
            if (rightIsUnboxed) {
              ...
              if (!leftIsUnboxed) {
                ...
                StackValue.coerce(
                    leftType, leftKotlinType, nonNullableLeftType, nonNullableLeftKotlinType, v);
                afterTopType = nonNullableLeftType;
                AsmUtil.swap(v, afterTopType, topType);
              }
              String descriptor =
                  Type.getMethodDescriptor(Type.BOOLEAN_TYPE, afterTopType, afterTopType);
              v.invokestatic(
                  className,
                  InlineClassDescriptorResolver.SPECIALIZED_EQUALS_NAME.asString(),
                  descriptor,
                  false);
            } else {
              String descriptor =
                  Type.getMethodDescriptor(Type.BOOLEAN_TYPE, afterTopType, OBJECT_TYPE);
  ...
  @Nullable
  private static KotlinType getLeftOperandType(
      @Nullable PrimitiveNumericComparisonInfo numericComparisonInfo) {
  ...
  private StackValue genEqualsForExpressionsPreferIeee754Arithmetic(
      ...
      @Nullable StackValue pregeneratedLeft,
      @Nullable PrimitiveNumericComparisonInfo primitiveNumericComparisonInfo) {
    assert (opToken == KtTokens.EQEQ || opToken == KtTokens.EXCLEQ)
        : "Optoken should be '==' or '!=', but: " + opToken;

    TypeAndNullability left754Type =
        calcTypeForIeee754ArithmeticIfNeeded(
            left, getLeftOperandType(primitiveNumericComparisonInfo));
    TypeAndNullability right754Type =
        calcTypeForIeee754ArithmeticIfNeeded(
            right, getRightOperandType(primitiveNumericComparisonInfo));
      ...
      if (comparisonType == Type.FLOAT_TYPE || comparisonType == Type.DOUBLE_TYPE) {
        if (left754Type.type.equals(right754Type.type)) {
          ...
          if (left754Type.isNullable || right754Type.isNullable) {
            if (state.getLanguageVersionSettings().getApiVersion().compareTo(ApiVersion.KOTLIN_1_1)
                >= 0) {
              return StackValue.operation(
                  Type.BOOLEAN_TYPE,
                  v -> {
                    generate754EqualsForNullableTypesViaIntrinsic(
                        v, opToken, pregeneratedLeft, left, left754Type, right, right754Type);
                    return Unit.INSTANCE;
                  });
            } else {
              return StackValue.operation(
  ...
  private void generate754EqualsForNullableTypesViaIntrinsic(
    ...
    Type rightType =
        right754Type.isNullable ? AsmUtil.boxType(right754Type.type) : right754Type.type;
    gen(right, rightType);
  ...
  private void generate754EqualsForNullableTypes(
    ...
    if (left754Type.isNullable) {
      ...
      StackValue.cmp(opToken, leftType, nonNullLeftValue, nonNullRightValue)
          .put(Type.BOOLEAN_TYPE, null, v);
      v.goTo(end);
  ...
  private boolean isIntZero(KtExpression expr, Type exprType) {
    ...
    return isIntPrimitive(exprType)
        && exprValue != null
        && Integer.valueOf(0).equals(exprValue.getValue());
  }

  private StackValue genCmpWithZero(
      KtExpression exp, IElementType opToken, @Nullable StackValue pregeneratedExpr) {
    StackValue argument;
    ...
    return StackValue.compareIntWithZero(
        argument, (KtTokens.EQEQ == opToken || KtTokens.EQEQEQ == opToken) ? IFNE : IFEQ);
  }

  private StackValue genCmpWithNull(
      KtExpression exp, IElementType opToken, @Nullable StackValue pregeneratedExpr) {
    KotlinType kotlinType = kotlinType(exp);
    ...
    if (kotlinType == null
        ...
        || !InlineClassesUtilsKt.isInlineClassType(kotlinType)) {
      return StackValue.compareWithNull(
          argument, (KtTokens.EQEQ == opToken || KtTokens.EQEQEQ == opToken) ? IFNONNULL : IFNULL);
    } else {
    ...
    return StackValue.operation(
        ...
        v -> {
          ...
          Integer rightLineNumber =
              CodegenUtil.getLineNumberForElement(expression.getRight(), false);
  ...
  private StackValue generateComparison(KtBinaryExpression expression, StackValue receiver) {
    ...
    TypeAndNullability right754Type =
        calcTypeForIeee754ArithmeticIfNeeded(
            right, getRightOperandType(primitiveNumericComparisonInfo));
    boolean isSame754ArithmeticTypes =
        left754Type != null && right754Type != null && left754Type.type.equals(right754Type.type);
    ...
    if (properIeee754Comparisons
      ...
      type =
          isSame754ArithmeticTypes ? left754Type.type : comparisonOperandType(leftType, rightType);
      leftValue = gen(left);
      rightValue = gen(right);
    } else {
      type = Type.INT_TYPE;
  ...
  private TypeAndNullability calcTypeForIeee754ArithmeticIfNeeded(
      @Nullable KtExpression expression, @Nullable KotlinType inferredPrimitiveType) {
    if (expression == null) {
      return null;
    } else if (shouldUseProperIeee754Comparisons()) {
      return Ieee754Kt.calcProperTypeForIeee754ArithmeticIfNeeded(
          expression, bindingContext, inferredPrimitiveType, typeMapper);
    } else {
    ...
    return StackValue.operation(
        ...
        adapter -> {
          ...
          boolean keepReturnValue =
              Boolean.TRUE.equals(bindingContext.get(VARIABLE_REASSIGNMENT, expression))
                  || !KotlinBuiltIns.isUnit(descriptor.getReturnType());

          putCallAugAssignMethod(
              expression,
              resolvedCall,
              callable,
  ...
  private void putCallAugAssignMethod(
    ...
    StackValue receiver =
        StackValue.onStack(jvmKotlinType.getType(), jvmKotlinType.getKotlinType());

    callable
        .invokeMethodWithArguments(resolvedCall, receiver, this)
        .put(callable.getReturnType(), null, v);
  ...
  public void invokeAppend(StringConcatGenerator generator, KtExpression expr) {
    ...
    if (compileTimeConstant == null) {
      if (expr instanceof KtBinaryExpression) {
        ...
        List<StringTemplateEntry> entries =
            preprocessStringTemplate((KtStringTemplateExpression) expr);
        invokeAppendForEntries(generator, entries);
        return;
      }
    }
    ...
    if (exprKotlinType != null
        && InlineClassesUtilsKt.isInlineClassType(exprKotlinType)
  ...
  @Override
  public StackValue visitPrefixExpression(
      @NotNull KtPrefixExpression expression, @NotNull StackValue receiver) {
    ConstantValue<?> compileTimeConstant = getPrimitiveOrStringCompileTimeConstant(expression);
    if (compileTimeConstant != null) {
      return StackValue.constant(compileTimeConstant.getValue(), expressionType(expression));
    }

    DeclarationDescriptor originalOperation =
        bindingContext.get(REFERENCE_TARGET, expression.getOperationReference());
    ResolvedCall<?> resolvedCall = CallUtilKt.getResolvedCallWithAssert(expression, bindingContext);
  ...
  @Override
  public StackValue visitPostfixExpression(
      @NotNull KtPostfixExpression expression, StackValue receiver) {
    ...
    if (originalOperationName.equals("inc")) {
      ...
      throw new UnsupportedOperationException(
          "Unsupported postfix operation: " + originalOperationName + " " + op);
    }
    ...
    if (isPrimitiveNumberClassDescriptor && AsmUtil.isPrimitive(asmBaseType)) {
      ...
      // Optimization for j = i++, when j and i are Int without any smart cast: we just work with
      // primitive int
      if (operand instanceof KtReferenceExpression
          && asmResultType == Type.INT_TYPE
    ...
    return StackValue.operation(
        ...
        v -> {
          StackValue value =
              StackValue.complexWriteReadReceiver(gen(expression.getBaseExpression()));

          value.put(asmBaseType, kotlinBaseType, v);
          AsmUtil.dup(v, asmBaseType);

          StackValue previousValue =
              StackValue.local(myFrameMap.enterTemp(asmBaseType), asmBaseType, kotlinBaseType);
          ...
          if (isPrimitiveNumberClassDescriptor && AsmUtil.isPrimitive(asmBaseType)) {
            ...
            StackValue result =
                invokeFunction(resolvedCall, StackValue.onStack(asmBaseType, kotlinBaseType));
  ...
  @Override
  public StackValue visitDestructuringDeclaration(
      @NotNull KtDestructuringDeclaration multiDeclaration, StackValue receiver) {
    return initializeDestructuringDeclaration(multiDeclaration, false);
  }

  public StackValue initializeDestructuringDeclaration(
      @NotNull KtDestructuringDeclaration multiDeclaration, boolean asProperty) {
    ...
    initializeDestructuringDeclarationVariables(
        multiDeclaration, initializerAsReceiver, local, asProperty);
  ...
  public void initializeDestructuringDeclarationVariables(
    ...
    initializeDestructuringDeclarationVariables(
        destructuringDeclaration, receiver, receiverStackValue, false);
  }
  ...
  private void initializeDestructuringDeclarationVariables(
      ...
      @NotNull StackValue receiverStackValue,
      boolean asProperty) {
    for (KtDestructuringDeclarationEntry variableDeclaration :
        destructuringDeclaration.getEntries()) {
      ResolvedCall<FunctionDescriptor> resolvedCall =
          bindingContext.get(COMPONENT_RESOLVED_CALL, variableDeclaration);
      ...
      if (asProperty && variableDescriptor instanceof PropertyDescriptor) {
        StackValue.Property propertyValue =
        ...
        initializeLocalVariable(
            variableDeclaration, invokeFunction(call, resolvedCall, receiverStackValue));
      }
    }
  }

  @NotNull
  private StackValue getVariableMetadataValue(
      @NotNull VariableDescriptorWithAccessors variableDescriptor) {
    StackValue value =
        findLocalOrCapturedValue(
            getDelegatedLocalVariableMetadata(variableDescriptor, bindingContext));
    assert value != null
        : "Can't find stack value for local delegated variable metadata: " + variableDescriptor;
    return value;
  }

  @NotNull
  private StackValue adjustVariableValue(
      @NotNull StackValue varValue, DeclarationDescriptor descriptor) {
  ...
  private void initializeLocalVariable(
      @NotNull KtVariableDeclaration variableDeclaration, @Nullable StackValue initializer) {
    LocalVariableDescriptor variableDescriptor =
        (LocalVariableDescriptor) getVariableDescriptorNotNull(variableDeclaration);
    ...
    KotlinType delegateKotlinType =
        JvmCodegenUtil.getPropertyDelegateType(variableDescriptor, bindingContext);
    StackValue storeTo =
        sharedVarType == null
            ? StackValue.local(index, varType, variableDescriptor, delegateKotlinType)
    ...
    if (variableDescriptor.isLateInit()) {
      assert initializer == null
          : "Initializer should be null for lateinit var "
              + variableDescriptor
              + ": "
              + initializer;
      v.aconst(null);
    ...
    if (isDelegatedLocalVariable(variableDescriptor)) {
      ...
      initializePropertyMetadata(
          (KtProperty) variableDeclaration, variableDescriptor, metadataValue);

      ResolvedCall<FunctionDescriptor> provideDelegateCall =
          bindingContext.get(PROVIDE_DELEGATE_RESOLVED_CALL, variableDescriptor);
      if (provideDelegateCall != null) {
        StackValue provideDelegateValue =
            generateProvideDelegateCallForLocalVariable(
                initializer, metadataValue, provideDelegateCall);
  ...
  private void initializeLocalVariableWithFakeDefaultValue(
      @NotNull KtProperty variableDeclaration) {
    LocalVariableDescriptor variableDescriptor =
        (LocalVariableDescriptor) getVariableDescriptorNotNull(variableDeclaration);
    ...
    assert !variableDeclaration.hasDelegateExpressionOrInitializer()
        ...
        : variableDeclaration.getText() + " in not variable declaration without initializer";
  ...
  private StackValue generateProvideDelegateCallForLocalVariable(
    ...
    StackValue provideDelegateReceiver =
        StackValue.onStack(initializer.type, initializer.kotlinType);

    List<? extends ValueArgument> arguments =
        provideDelegateResolvedCall.getCall().getValueArguments();
    ...
    tempVariables.put(
        arguments.get(0).asElement(), StackValue.constant(null, AsmTypes.OBJECT_TYPE));
    tempVariables.put(
  ...
  public VariableDescriptor getVariableDescriptorNotNull(@NotNull KtElement declaration) {
    ...
    assert descriptor != null
        : "Couldn't find variable declaration in binding context " + declaration.getText();
    return descriptor;
  }
  ...
  private void initializePropertyMetadata(
    ...
    // We can use the $$delegatedProperties array as in non-inline functions and upon inlining,
    // detect elements at what indices
    // of that array are used in the inline function body, load the corresponding initializing
    // bytecode from <clinit> of the
    // container class (where the PropertyReferenceNImpl instance is created), copy and adapt it at
    // the call site
    StackValue value;
    if (PropertyCodegen.isDelegatedPropertyWithOptimizedMetadata(
        variableDescriptor, bindingContext)) {
      ...
      value =
          generatePropertyReference(
              variable.getDelegate(), variableDescriptor, variableDescriptor, null);
    } else {
  ...
  private StackValue generateNewCall(
      @NotNull KtCallExpression expression, @NotNull ResolvedCall<?> resolvedCall) {
    Type type = expressionType(expression);
    if (type.getSort() == Type.ARRAY) {
      KotlinType kotlinType = kotlinType(expression);
      assert kotlinType != null
          : "No kotlinType for expression of type " + type + ": " + expression.getText();
  ...
  @NotNull
  public ClassConstructorDescriptor getConstructorDescriptor(
      @NotNull ResolvedCall<?> resolvedCall) {
  ...
  @NotNull
  public StackValue generateConstructorCall(
      @NotNull ResolvedCall<?> resolvedCall, @NotNull Type objectType) {
    return StackValue.functionCall(
        ...
        v -> {
          ...
          if (dispatchReceiver != null) {
            ...
            boolean callSuper =
                containingDeclaration.isInner() && receiver instanceof ImplicitClassReceiver;
            generateReceiverValue(receiver, callSuper).put(receiverType, kotlinType, v);
          }

          // Resolved call to local class constructor doesn't have dispatchReceiver, so we need to
          // generate closure on stack
          ...
          OwnerKind inlineClassKindOrNull =
              InlineClassesUtilsKt.isInlineClass(constructor.getContainingDeclaration())
                  ? OwnerKind.ERASED_INLINE_CLASS
                  : null;
          CallableMethod method =
              typeMapper.mapToCallableMethod(constructor, false, inlineClassKindOrNull);
  ...
  public void newArrayInstruction(@NotNull KotlinType arrayType) {
    if (KotlinBuiltIns.isArray(arrayType)) {
      ...
      putReifiedOperationMarkerIfTypeIsReifiedParameter(
          this, elementKotlinType, ReifiedTypeInliner.OperationKind.NEW_ARRAY);
  ...
  @Override
  public StackValue visitArrayAccessExpression(
      @NotNull KtArrayAccessExpression expression, StackValue receiver) {
    ...
    FunctionDescriptor operationDescriptor =
        (FunctionDescriptor) bindingContext.get(REFERENCE_TARGET, expression);
    assert operationDescriptor != null;
    boolean isInlineClassType =
        arrayKotlinType != null && InlineClassesUtilsKt.isInlineClassType(arrayKotlinType);
    if (arrayType.getSort() == Type.ARRAY
      ...
      KotlinType elementKotlinType =
          state.getModule().getBuiltIns().getArrayElementType(arrayKotlinType);
      Type elementType;
      ...
      ResolvedCall<FunctionDescriptor> resolvedSetCall =
          bindingContext.get(INDEXED_LVALUE_SET, expression);
      ResolvedCall<FunctionDescriptor> resolvedGetCall =
          bindingContext.get(INDEXED_LVALUE_GET, expression);
      ...
      Callable callable =
          resolveToCallable(accessibleFunctionDescriptor(resolvedCall), false, resolvedCall);
      Callable callableMethod =
          typeMapper.mapToCallableMethod(
              SamCodegenUtil.resolveSamAdapter(operationDescriptor), false);
      Type[] argumentTypes = callableMethod.getParameterTypes();

      StackValue.CollectionElementReceiver collectionElementReceiver =
      ...
      Type elementType =
          isGetter ? callableMethod.getReturnType() : ArrayUtil.getLastElement(argumentTypes);
      KotlinType elementKotlinType =
          isGetter
              ? operationDescriptor.getOriginal().getReturnType()
              : CollectionsKt.last(operationDescriptor.getOriginal().getValueParameters())
                  .getType();
  ...
  private StackValue.CollectionElementReceiver createCollectionElementReceiver(
    ...
    List<ResolvedValueArgument> valueArguments = resolvedCall.getValueArgumentsByIndex();
    assert valueArguments != null
        : "Failed to arrange value arguments by index: " + operationDescriptor;

    if (!isGetter) {
      assert valueArguments.size() >= 2
          : "Setter call should have at least 2 arguments: " + operationDescriptor;
      // Skip generation of the right hand side of an indexed assignment, which is the last value
      // argument
  ...
  @Override
  public StackValue visitThrowExpression(
      @NotNull KtThrowExpression expression, StackValue receiver) {
    return StackValue.operation(
        Type.VOID_TYPE,
        getNothingType(),
        adapter -> {
  ...
  public StackValue visitThisExpression(@NotNull KtThisExpression expression, StackValue receiver) {
    DeclarationDescriptor descriptor =
        bindingContext.get(REFERENCE_TARGET, expression.getInstanceReference());
    if (descriptor instanceof ClassDescriptor) {
    ...
    throw new UnsupportedOperationException(
        "Neither this nor receiver: "
            + descriptor
            + expression.getParent().getContainingFile().getText());
  }
        ...
        v -> {
          ...
          for (int i = 0, size = clauses.size(); i < size; i++) {
            ...
            VariableDescriptor descriptor =
                bindingContext.get(VALUE_PARAMETER, clause.getCatchParameter());
            assert descriptor != null;
            ...
            genFinallyBlockOrGoto(
                tryWithFinallyBlockStackElement,
                i != size - 1 || finallyBlock != null ? end : null,
                null,
                new ArrayList<>());
          ...
          if (finallyBlock != null) {
            ...
            // do it before finally block generation
            // javac also generates entry in exception table for default catch clause too!!!! so
            // defaultCatchEnd as end parameter
            List<Label> defaultCatchRegions =
                getCurrentCatchIntervals(
                    tryWithFinallyBlockStackElement, tryStart, defaultCatchEnd);
  ...
  private void generateExceptionTable(
      @NotNull Label catchStart, @NotNull List<Label> catchedRegions, @Nullable String exception) {
  ...
  private static List<Label> getCurrentCatchIntervals(
    ...
    List<Label> gapsInBlock =
        finallyBlockStackElement != null
            ? new ArrayList<>(finallyBlockStackElement.gaps)
  ...
  public StackValue visitBinaryWithTypeRHSExpression(
      @NotNull KtBinaryExpressionWithTypeRHS expression, StackValue receiver) {
    KtExpression left = expression.getLeft();
  ...
  private StackValue generateIsCheck(
      StackValue expressionToMatch, KtTypeReference typeReference, boolean negated) {
  ...
  private StackValue generateIsCheck(
      StackValue expressionToGen, KotlinType rhsKotlinType, boolean leaveExpressionOnStack) {
    ...
    return StackValue.operation(
        ...
        v -> {
          ...
          if (TypeUtils.isReifiedTypeParameter(rhsKotlinType)) {
            putReifiedOperationMarkerIfTypeIsReifiedParameter(
                this, rhsKotlinType, ReifiedTypeInliner.OperationKind.IS);
            v.instanceOf(type);
            return null;
          }
  ...
  @Override
  public void propagateChildReifiedTypeParametersUsages(
      @NotNull ReifiedTypeParametersUsages usages) {
    parentCodegen
        .getReifiedTypeParametersUsages()
        .propagateChildUsagesWithinContext(
            usages,
            () ->
                context.getContextDescriptor().getTypeParameters().stream()
                    .filter(TypeParameterDescriptor::isReified)
    ...
    return StackValue.operation(
        ...
        v -> {
          ...
          SwitchCodegen switchCodegen =
              switchCodegenProvider.buildAppropriateSwitchCodegenIfPossible(
                  expression,
                  isStatement,
                  CodegenUtil.isExhaustive(bindingContext, expression, isStatement));
          ...
          if (subjectVariable != null) {
            subjectVariableDescriptor =
                bindingContext.get(BindingContext.VARIABLE, subjectVariable);
            assert subjectVariableDescriptor != null
                : "Unresolved subject variable: " + subjectVariable.getName();
            ...
            tempVariables.put(
                subjectExpression, StackValue.local(subjectLocal, subjectType, subjectKotlinType));
          ...
          if (subjectVariableDescriptor != null) {
            ...
            v.visitLocalVariable(
                subjectVariableDescriptor.getName().asString(),
                subjectType.getDescriptor(),
                null,
                begin,
                end,
                subjectLocal);
  ...
  public void putUnitInstanceOntoStackForNonExhaustiveWhen(
      @NotNull KtWhenExpression whenExpression, boolean isStatement) {
  ...
  private StackValue generateWhenCondition(
    ...
    if (condition instanceof KtWhenConditionInRange) {
      ...
      return generateIn(
          StackValue.local(subjectLocal, subjectType, subjectKotlinType),
          conditionInRange.getRangeExpression(),
          conditionInRange.getOperationReference());
    }
    StackValue.Local match =
        subjectLocal == -1 ? null : StackValue.local(subjectLocal, subjectType, subjectKotlinType);
    if (condition instanceof KtWhenConditionIsPattern) {
      ...
      return generateIsCheck(
          match, patternCondition.getTypeReference(), patternCondition.isNegated());
      ...
      return generateExpressionMatch(
          match, subjectExpression, subjectKotlinType, patternExpression);
    } else {
  ...
  public Call makeFakeCall(ReceiverValue initializerAsReceiver) {
    KtSimpleNameExpression fake =
        new KtPsiFactory(state.getProject(), false).createSimpleName("fake");
  ...
  public void addBlockStackElementsForNonLocalReturns(
      @NotNull Stack<BlockStackElement> elements, int finallyDepth) {
  ...
  private StackValue.Delegate delegatedVariableValue(
    ...
    return StackValue.localDelegate(
        typeMapper.mapType(variableDescriptor.getType()),
        delegateValue,
        metadataValue,
        variableDescriptor,
        this);
  ...
  public void consumeReifiedOperationMarker(@NotNull TypeParameterMarker typeParameter) {
    assert typeParameter instanceof TypeParameterDescriptor
        : "Type parameter should be a descriptor: " + typeParameter;
