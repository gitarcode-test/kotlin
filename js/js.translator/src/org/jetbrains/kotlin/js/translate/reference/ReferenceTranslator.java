...
public final class ReferenceTranslator {
  private static final Set<FqNameUnsafe> DECLARATIONS_WITHOUT_SIDE_EFFECTS =
      new HashSet<>(
          Arrays.asList(
  ...
  @NotNull
  public static JsExpression translateSimpleName(
  ...
  @NotNull
  public static JsExpression translateAsValueReference(
      ...
      if (descriptor instanceof ValueParameterDescriptor) {
        ...
        if (parameter.getContainingDeclaration() instanceof PropertySetterDescriptor) {
          PropertySetterDescriptor setter =
              (PropertySetterDescriptor) parameter.getContainingDeclaration();
          return TranslationUtils.getReturnTypeForCoercion(
              setter.getCorrespondingProperty(), false);
        }
      }
    ...
    if (DescriptorUtils.isObject(descriptor) || DescriptorUtils.isEnumEntry(descriptor)) {
      ...
      if (!isLocallyAvailableDeclaration(context, descriptor)) {
        ...
        JsExpression functionRef =
            JsAstUtils.pureFqn(context.getNameForObjectInstance(classDescriptor), null);
  ...
  @NotNull
  public static JsExpression translateAsTypeReference(
      @NotNull ClassDescriptor descriptor, @NotNull TranslationContext context) {
    if (AnnotationsUtils.isNativeObject(descriptor)
        || AnnotationsUtils.isLibraryObject(descriptor)) {
      return context.getInnerReference(descriptor);
    }
  ...
  @NotNull
  private static JsExpression getPrototypeIfNecessary(
      @NotNull ClassDescriptor descriptor, @NotNull JsExpression reference) {
    if (DescriptorUtils.isObject(descriptor) || DescriptorUtils.isEnumEntry(descriptor)) {
      JsNameRef getPrototypeRef =
          JsAstUtils.pureFqn("getPrototypeOf", JsAstUtils.pureFqn("Object", null));
  ...
  private static boolean isLocallyAvailableDeclaration(
      @NotNull TranslationContext context, @NotNull DeclarationDescriptor descriptor) {
    return context.isFromCurrentModule(descriptor)
        && !(context.isPublicInlineFunction()
            && DescriptorUtilsKt.shouldBeExported(descriptor, context.getConfig()));
  }

  @NotNull
  private static JsExpression getLazyReferenceToObject(
      @NotNull ClassDescriptor descriptor, @NotNull TranslationContext context) {
  ...
  private static boolean isLocalVarOrFunction(DeclarationDescriptor descriptor) {
    return descriptor.getContainingDeclaration() instanceof FunctionDescriptor
        && !(descriptor instanceof ClassDescriptor);
  }

  @NotNull
  public static AccessTranslator getAccessTranslator(
      @NotNull KtSimpleNameExpression referenceExpression, @NotNull TranslationContext context) {
    if (isBackingFieldReference(
        getDescriptorForReferenceExpression(context.bindingContext(), referenceExpression))) {
    ...
    if (CompanionObjectIntrinsicAccessTranslator.isCompanionObjectReference(
  ...
  public static boolean canBePropertyAccess(
    ...
    DeclarationDescriptor descriptor =
        getDescriptorForReferenceExpression(context.bindingContext(), simpleNameExpression);

    // Skip ValueParameterDescriptor because sometime we can miss resolved call for it, e.g. when
    // set something to delegated property.
