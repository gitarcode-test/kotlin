...
public class JsToStringGenerationVisitor extends JsVisitor {
  ...
  @NotNull private final SourceLocationConsumer sourceLocationConsumer;

  @NotNull private final List<Object> sourceInfoStack = new ArrayList<>();
  ...
  /**
   * Generate JavaScript code that evaluates to the supplied string. Adapted from {@link
   * org.mozilla.javascript.ScriptRuntime#escapeString(String)} . The difference is that we quote
   * with either &quot; or &apos; depending on which one is used less inside the string.
   */
  ...
  /**
   * Escapes any closing XML tags embedded in <code>str</code>, which could potentially cause a
   * parse failure in a browser, for example, embedding a closing <code>&lt;script&gt;</code> tag.
   *
  ...
  /**
   * "Global" blocks are either the global block of a fragment, or a block nested directly within
   * some other global block. This definition matters because the statements designated by
   * statementEnds and statementStarts are those that appear directly within these global blocks.
   */
  private Set<JsBlock> globalBlocks = new ObjectOpenHashSet<>();

  @NotNull protected final TextOutput p;

  public JsToStringGenerationVisitor(
      @NotNull TextOutput out, @NotNull SourceLocationConsumer sourceLocationConsumer) {
    p = out;
  ...
  public void visitBinaryExpression(@NotNull JsBinaryOperation binaryOperation) {
    ...
    if (operator == JsBinaryOperator.COMMA) {
      isParenOpened = false;
      space();
    } else if (arg2 instanceof JsBinaryOperation
    ...
    for (JsPropertyInitializer item : objectLiteral.getPropertyInitializers()) {
      ...
      if (labelExpr instanceof JsStringLiteral) {
        ...
        if (IdentifierPolicyKt.isValidES5Identifier(value)) {
          String escaped =
              IdentifierPolicyKt.getRESERVED_KEYWORDS().contains(value) ? "'" + value + "'" : value;
  ...
  private void printJsBlock(
      JsBlock x, boolean finalNewline, @Nullable Object defaultClosingBraceLocation) {
    ...
    while (iterator.hasNext()) {
      ...
      if (needSemi) {
        ...
        boolean lastStatement =
            !iterator.hasNext() && needBraces && !JsRequiresSemiVisitor.exec(statement);
        if (functionStmt) {
          if (lastStatement) {
            newline();
          } else {
  ...
  /**
   * Decide whether, if <code>op</code> is printed followed by <code>arg</code>, there needs to be a
   * space between the operator and expression.
   *
  ...
  private static boolean spaceCalc(JsOperator op, JsExpression arg) {
    ...
    if (arg instanceof JsBinaryOperation) {
       ...
       */
      return binary.getOperator().getPrecedence() > op.getPrecedence()
          && spaceCalc(op, binary.getArg1());
    }
