...
public final class FunctionBodyTranslator extends AbstractTranslator {
  ...
  public static JsBlock translateFunctionBody(
    ...
    LocalFunctionCollector functionCollector =
        new LocalFunctionCollector(functionBodyContext.bindingContext());
    ...
    for (FunctionDescriptor localFunction : functionCollector.getFunctions()) {
      String localIdent =
          localFunction.getName().isSpecial() ? "lambda" : localFunction.getName().asString();
    ...
    return (new FunctionBodyTranslator(descriptor, declarationWithBody, functionBodyContext))
        .translate();
  }
  ...
  public static List<JsStatement> setDefaultValueForArguments(
    ...
    List<ValueParameterDescriptor> valueParametersForDefaultValue =
        CodegenUtil.getFunctionParametersForDefaultValueGeneration(
            descriptor, context.bindingTrace());
    ...
    for (int i = 0; i < valueParameters.size(); i++) {
      ...
      ValueParameterDescriptor valueParameterForDefaultValue =
          valueParametersForDefaultValue.get(i);

      if (!valueParameterForDefaultValue.declaresDefaultValue()) continue;

      JsExpression jsNameRef =
          ReferenceTranslator.translateAsValueReference(valueParameter, context);
      ...
      JsExpression defaultValue =
          Translation.translateAsExpression(defaultArgument, context, defaultArgBlock);
      ...
      JsStatement thenStatement =
          JsAstUtils.mergeStatementInBlockIfNeeded(assignStatement, defaultArgBlock);
  ...
  @NotNull private final KtDeclarationWithBody declaration;

  private FunctionBodyTranslator(
      @NotNull FunctionDescriptor descriptor,
  ...
  private JsBlock translate() {
    ...
    if (ktExpression instanceof KtBlockExpression
      ...
      JsReturn jsReturn =
          new JsReturn(ReferenceTranslator.translateAsValueReference(unit, context()));
