...
public class FileBasedDeclarationProviderFactory extends AbstractDeclarationProviderFactory {
  ...
  public FileBasedDeclarationProviderFactory(
      @NotNull StorageManager storageManager, @NotNull Collection<KtFile> files) {
  ...
  /*package*/ Collection<FqName> getAllDeclaredSubPackagesOf(@NotNull FqName parent) {
    return CollectionsKt.filter(
        index.invoke().declaredPackages,
        fqName -> !fqName.isRoot() && fqName.parent().equals(parent));
  }

  @Nullable
  @Override
  protected PackageMemberDeclarationProvider createPackageMemberDeclarationProvider(
