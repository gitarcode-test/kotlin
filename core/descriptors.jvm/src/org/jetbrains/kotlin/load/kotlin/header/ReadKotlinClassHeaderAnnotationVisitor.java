...
public class ReadKotlinClassHeaderAnnotationVisitor implements AnnotationVisitor {
  ...
  private static final Map<ClassId, KotlinClassHeader.Kind> HEADER_KINDS =
      new HashMap<ClassId, KotlinClassHeader.Kind>();

  static {
    // TODO: delete this at some point
    HEADER_KINDS.put(ClassId.topLevel(new FqName("kotlin.jvm.internal.KotlinClass")), CLASS);
    HEADER_KINDS.put(
        ClassId.topLevel(new FqName("kotlin.jvm.internal.KotlinFileFacade")), FILE_FACADE);
    HEADER_KINDS.put(
        ClassId.topLevel(new FqName("kotlin.jvm.internal.KotlinMultifileClass")), MULTIFILE_CLASS);
    HEADER_KINDS.put(
        ClassId.topLevel(new FqName("kotlin.jvm.internal.KotlinMultifileClassPart")),
        MULTIFILE_CLASS_PART);
    HEADER_KINDS.put(
        ClassId.topLevel(new FqName("kotlin.jvm.internal.KotlinSyntheticClass")), SYNTHETIC_CLASS);
  }
  ...
  public KotlinClassHeader createHeader(JvmMetadataVersion metadataVersionFromLanguageVersion) {
    ...
    JvmMetadataVersion metadataVersion =
        new JvmMetadataVersion(
            metadataVersionArray,
            (extraInt & JvmAnnotationNames.METADATA_STRICT_VERSION_SEMANTICS_FLAG) != 0);

    if (!metadataVersion.isCompatible(metadataVersionFromLanguageVersion)) {
      incompatibleData = data;
      data = null;
    } else if (shouldHaveData() && data == null) {
      // This means that the annotation is found and its ABI version is compatible, but there's no
      // "data" string array in it.
  ...
  @Nullable
  @Override
  public AnnotationArgumentVisitor visitAnnotation(
      @NotNull ClassId classId, @NotNull SourceElement source) {
  ...
  /**
   * JVM bytecode format doesn't allow {@code null} as the annotation argument value, so we use the
   * default value of the argument's type to signify that the argument is absent. So if we read an
   * empty string, for example, we should interpret it as if there was no value at all.
   *
   * <p>If we didn't, it could lead to problems for arguments like {@link
   * ReadKotlinClassHeaderAnnotationVisitor#packageName} (corresponding to {@link
   * kotlin.Metadata#pn}): the class could be loaded as if it's JVM package name is the default
   * package, which makes no sense and leads to problems in IDE like KT-39492, KTIJ-18094.
   */
  private class KotlinMetadataArgumentVisitor implements AnnotationArgumentVisitor {
    ...
    public void visitClassLiteral(
        @Nullable Name name, @NotNull ClassLiteralValue classLiteralValue) {}
    ...
    public void visitEnum(
        @Nullable Name name, @NotNull ClassId enumClassId, @NotNull Name enumEntryName) {}

    @Nullable
    @Override
    public AnnotationArgumentVisitor visitAnnotation(
        @Nullable Name name, @NotNull ClassId classId) {
      return null;
    }
  ...
  private abstract static class CollectStringArrayAnnotationVisitor
      implements AnnotationArrayArgumentVisitor {
