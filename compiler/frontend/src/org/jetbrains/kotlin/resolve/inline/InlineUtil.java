...
public class InlineUtil {
  ...
  public static boolean isInlineParameterExceptNullability(
      @NotNull ParameterDescriptor valueParameterOrReceiver) {
    return !(valueParameterOrReceiver instanceof ValueParameterDescriptor
        ...
        && FunctionTypesKt.isBuiltinFunctionalType(
            valueParameterOrReceiver.getOriginal().getType());
  }
  ...
  public static boolean isPropertyWithAllAccessorsAreInline(
  ...
  public static boolean isInPublicInlineScope(@Nullable DeclarationDescriptor descriptor) {
    ...
    if (isInline(descriptor) && descriptor instanceof DeclarationDescriptorWithVisibility) {
      DescriptorVisibility visibility =
          ((DeclarationDescriptorWithVisibility) descriptor).getVisibility();
      if (!DescriptorVisibilities.isPrivate(visibility)) {
        ...
        if (containingClass == null
            || !DescriptorVisibilities.isPrivate(containingClass.getVisibility())) return true;
      }
    }
  ...
  public static boolean checkNonLocalReturnUsage(
    ...
    PsiElement containingFunction =
        context.getContextParentOfType(
            startExpression, KtClassOrObject.class, KtDeclarationWithBody.class);
    ...
    return checkNonLocalReturnUsage(
        fromFunction,
        context.trace.get(BindingContext.DECLARATION_TO_DESCRIPTOR, containingFunction),
        containingFunction,
  ...
  public static boolean checkNonLocalReturnUsage(
    ...
    while (canBeInlineArgument(containingFunction)
        && fromFunction != containingFunctionDescriptor) {
      Boolean isLambdaDefinitelyInline =
          bindingContext.get(
      ...
      containingFunctionDescriptor =
          getContainingClassOrFunctionDescriptor(containingFunctionDescriptor, true);
  ...
  public static ValueParameterDescriptor getInlineArgumentDescriptor(
      @NotNull KtFunction argument, @NotNull BindingContext bindingContext) {
    ...
    ValueArgument valueArgument =
        CallUtilKt.getValueArgumentForExpression(resolvedCall.getCall(), argument);
  ...
  public static boolean canBeInlineArgument(@Nullable PsiElement functionalExpression) {
    return functionalExpression instanceof KtFunctionLiteral
        || functionalExpression instanceof KtNamedFunction;
  }

  /**
   * @return true if the descriptor is the constructor of one of 9 array classes (Array&lt;T&gt;,
   *     IntArray, FloatArray, ...) which takes the size and an initializer lambda as parameters.
   *     Such constructors are marked as 'inline' but they are not loaded as such because the
   *     'inline' flag is not stored for constructors in the binary metadata. Therefore we pretend
  ...
  public static boolean isArrayConstructorWithLambda(@NotNull CallableDescriptor descriptor) {
    return descriptor.getValueParameters().size() == 2
        ...
        && KotlinBuiltIns.isArrayOrPrimitiveArray(
            ((ConstructorDescriptor) descriptor).getConstructedClass());
  }

  @Nullable
  public static DeclarationDescriptor getContainingClassOrFunctionDescriptor(
      @NotNull DeclarationDescriptor descriptor, boolean strict) {
