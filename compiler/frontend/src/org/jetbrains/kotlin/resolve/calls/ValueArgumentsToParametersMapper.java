...
public class ValueArgumentsToParametersMapper {
  ...
  public static <D extends CallableDescriptor> Status mapValueArgumentsToParameters(
    ...
    // return new ValueArgumentsToParametersMapper().process(call, tracing, candidateCall,
    // unmappedArguments);
  ...
  private static class Processor<D extends CallableDescriptor> {
      ...
      if (parameterByNameInOverriddenMethods == null) {
        ...
        for (ValueParameterDescriptor valueParameter : parameters) {
          for (ValueParameterDescriptor parameterDescriptor :
              valueParameter.getOverriddenDescriptors()) {
    ...
    private final ProcessorState positionedOnly =
        new ProcessorState() {
          ...
          private int numberOfParametersForPositionedArguments() {
            return call.getCallType() == Call.CallType.ARRAY_SET_METHOD
                ? parameters.size() - 1
                : parameters.size();
          }
          ...
          private void processArgument(
              @NotNull ValueArgument argument, @Nullable ValueParameterDescriptor parameter) {
            if (parameter != null) {
              ...
              report(
                  TOO_MANY_ARGUMENTS.on(
                      argument.asElement(), candidateCall.getCandidateDescriptor()));
        ...
        new ProcessorState() {
          ...
          public ProcessorState processNamedArgument(@NotNull ValueArgument argument) {
            ...
            ValueParameterDescriptor valueParameterDescriptor =
                parameterByName.get(argumentName.getAsName());
            ...
            if (nameReference != null) {
              if (candidate instanceof MemberDescriptor
                  && ((MemberDescriptor) candidate).isExpect()
                  && candidate.getContainingDeclaration() instanceof ClassDescriptor) {
                // We do not allow named arguments for members of expected classes until we're able
                // to use both
            ...
            if (candidate.hasStableParameterNames()
                && nameReference != null
                && candidate instanceof CallableMemberDescriptor
                && ((CallableMemberDescriptor) candidate).getKind()
                    == CallableMemberDescriptor.Kind.FAKE_OVERRIDE) {
              if (valueParameterDescriptor == null) {
                valueParameterDescriptor =
                    getParameterByNameInOverriddenMethods(argumentName.getAsName());
              }

              if (valueParameterDescriptor != null) {
                for (ValueParameterDescriptor parameterFromSuperclass :
                    valueParameterDescriptor.getOverriddenDescriptors()) {
                  if (OverrideResolver.Companion.shouldReportParameterNameOverrideWarning(
            ...
            if (valueParameterDescriptor == null) {
              ...
              if (nameReference != null) {
                candidateCall
                    .getTrace()
                    .record(REFERENCE_TARGET, nameReference, valueParameterDescriptor);
              }
          ...
          public ProcessorState processArraySetRHS(@NotNull ValueArgument argument) {
            throw new IllegalStateException(
                "Array set RHS cannot appear after a named argument syntactically: " + argument);
          }
        };
    ...
    public void process() {
      ...
      List<? extends ValueArgument> argumentsInParentheses =
          CallUtilKt.getValueArgumentsInParentheses(call);
      for (Iterator<? extends ValueArgument> iterator = argumentsInParentheses.iterator();
          iterator.hasNext(); ) {
    ...
    private void processFunctionLiteralArguments() {
      ...
      if (parameters.isEmpty()) {
        CallUtilKt.reportTrailingLambdaErrorOr(
            candidateCall.getTrace(),
            possiblyLabeledFunctionLiteral,
            expression ->
                TOO_MANY_ARGUMENTS.on(expression, candidateCall.getCandidateDescriptor()));
        setStatus(ERROR);
      } else {
    ...
    private void putVararg(
        ValueParameterDescriptor valueParameterDescriptor, ValueArgument valueArgument) {
      if (valueParameterDescriptor.getVarargElementType() != null) {
        VarargValueArgument vararg =
            varargs.computeIfAbsent(valueParameterDescriptor, k -> new VarargValueArgument());
