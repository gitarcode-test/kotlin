...
public abstract class KotlinLightClassStructureTest extends KotlinAsJavaTestBase {
  ...
  public static class DeclaredWithGenerics extends KotlinLightClassStructureTest {
    ...
    protected List<File> getKotlinSourceRoots() {
      return Collections.singletonList(
          new File(
              "compiler/testData/asJava/lightClasses/lightClassStructure/DeclaredWithGenerics.kt"));
    }
  ...
  public static class PlatformStaticMethodsWithGenerics extends KotlinLightClassStructureTest {
    ...
    protected List<File> getKotlinSourceRoots() {
      return Collections.singletonList(
          new File(
              "compiler/testData/asJava/lightClasses/lightClassStructure/PlatformStaticMethodsGenerics.kt"));
    }
    ...
    public void testInClassObjectActual() throws Exception {
      checkMethodGenericParameter(
          "test.PlatformStaticClass." + SpecialNames.DEFAULT_NAME_FOR_COMPANION_OBJECT.asString(),
          "inClassObject",
          0,
          "T");
    }
  ...
  public static class CodeWithErrors extends KotlinLightClassStructureTest {
    ...
    protected List<File> getKotlinSourceRoots() {
      return Collections.singletonList(
          new File("compiler/testData/asJava/lightClasses/lightClassStructure/CodeWithErrors.kt"));
    }
  ...
  protected void checkClassGenericParameter(
      String classFqName, int index, String name, String... bounds) {
    checkGenericParameter(findClass(classFqName).getTypeParameters()[index], index, name, bounds);
  }

  protected void checkMethodGenericParameter(
      String classFqName, String methodName, int index, String name, String... bounds) {
    ...
    for (PsiMethod psiMethod : aClass.getMethods()) {
      if (methodName.equals(psiMethod.getName())) {
        assertNull(
            String.format(
                "Several methods with name '%s' found in class '%s'", methodName, classFqName),
            method);
        method = psiMethod;
      }
    }
    ...
    assertNotNull(
        String.format("Methods name '%s' wasn't found in class '%s'", methodName, classFqName),
        method);

    checkGenericParameter(method.getTypeParameters()[index], index, name, bounds);
  }

  protected static void checkGenericParameter(
      PsiTypeParameter typeParameter, int index, String name, String[] bounds) {
    ...
    Set<String> actualBounds =
        Sets.newHashSet(
            ArraysKt.map(typeParameter.getExtendsListTypes(), PsiType::getCanonicalText));
