...
public class ConstructorCodegen {
  ...
  public void generatePrimaryConstructor(
      DelegationFieldsInfo delegationFieldsInfo, Type superClassAsmType) {
    ...
    ConstructorContext constructorContext =
        context.intoConstructor(constructorDescriptor, typeMapper);
    ...
    JvmDeclarationOrigin origin =
        JvmDeclarationOriginKt.OtherOrigin(
            primaryConstructor != null ? primaryConstructor : myClass.getPsiOrParent(),
    ...
    OwnerKind ownerKindForDefault =
        context.getContextKind() == OwnerKind.ERASED_INLINE_CLASS
    ...
    new DefaultParameterValueSubstitutor(state)
        .generatePrimaryConstructorOverloadsIfNeeded(
            constructorDescriptor, v, memberCodegen, kind, myClass);
  }
  ...
  public void generateSecondaryConstructor(
      @NotNull ClassConstructorDescriptor constructorDescriptor, @NotNull Type superClassAsmType) {
  ...
  private void generateDelegatorToConstructorCall(
    ...
    assert !((ConstructorContext) codegenContext).isThisInitialized()
        : "Delegating constructor call is already generated for "
            + ((ConstructorContext) codegenContext).getConstructorDescriptor();

    if (delegationConstructorCall == null) {
      genSimpleSuperCall(iv, superClassAsmType);
    } else {
      generateDelegationConstructorCall(
          iv, codegen, constructorDescriptor, delegationConstructorCall);
    }
    ...
    for (KtSuperTypeListEntry specifier : myClass.getSuperTypeListEntries()) {
      if (specifier instanceof KtDelegatedSuperTypeEntry) {
        genCallToDelegatorByExpressionSpecifier(
            iv, codegen, (KtDelegatedSuperTypeEntry) specifier, fieldsInfo);
      }
    }
    ...
    for (KtParameter parameter : classBodyCodegen.getPrimaryConstructorParameters()) {
      if (parameter.hasValOrVar()) {
        ...
        assert propertyDescriptor != null
            : "Property descriptor is not found for primary constructor parameter: " + parameter;
        iv.putfield(
            classAsmType.getInternalName(),
            context.getFieldName(propertyDescriptor, false),
            type.getDescriptor());
      }
      curParam++;
    }
  ...
  private void generateSecondaryConstructorImpl(
    ...
    KtSecondaryConstructor constructor =
        (KtSecondaryConstructor)
            DescriptorToSourceUtils.descriptorToDeclaration(constructorDescriptor);
    ...
    ResolvedCall<ConstructorDescriptor> constructorDelegationCall =
        getDelegationConstructorCall(bindingContext, constructorDescriptor);
    ConstructorDescriptor delegateConstructor =
        constructorDelegationCall == null
            ? null
            : constructorDelegationCall.getResultingDescriptor();

    generateDelegatorToConstructorCall(
        iv, codegen, constructorDescriptor, constructorDelegationCall, superClassAsmType);
  ...
  private void generateClosureInitialization(@NotNull InstructionAdapter iv) {
    ...
    if (closure != null) {
      List<FieldInfo> argsFromClosure =
          ClosureCodegen.calculateConstructorParameters(
  ...
  private void generateDelegationConstructorCall(
    ...
    ConstructorDescriptor delegateConstructor =
        SamCodegenUtil.resolveSamAdapter(
            codegen.getConstructorDescriptor(delegationConstructorCall));

    KotlinTypeMapper typeMapper = state.getTypeMapper();

    CallableMethod delegateConstructorCallable =
        typeMapper.mapToCallableMethod(delegateConstructor, false, kind);
    CallableMethod callable = typeMapper.mapToCallableMethod(constructorDescriptor, false, kind);

    List<JvmMethodParameterSignature> delegatingParameters =
        delegateConstructorCallable.getValueParameters();
    ...
    if (isSameClassConstructor(delegateConstructor)) {
      ...
      argumentGenerator =
          generateThisCallImplicitArguments(
              iv,
              codegen,
              delegateConstructor,
  ...
  private void genSimpleSuperCall(InstructionAdapter iv, Type superClassAsmType) {
    ...
    if (descriptor.getKind() == ClassKind.ENUM_CLASS
        || descriptor.getKind() == ClassKind.ENUM_ENTRY) {
      iv.load(1, JAVA_STRING_TYPE);
      iv.load(2, Type.INT_TYPE);
      iv.invokespecial(
          superClassAsmType.getInternalName(), "<init>", "(Ljava/lang/String;I)V", false);
    } else {
  ...
  private static ArgumentGenerator generateThisCallImplicitArguments(
    ...
    for (; index < delegatingParameters.size(); index++) {
      ...
      if (delegatingKind == JvmMethodParameterKind.VALUE) {
        assert index == parameters.size()
                || parameters.get(index).getKind() == JvmMethodParameterKind.VALUE
    ...
    return new CallBasedArgumentGenerator(
        codegen,
        codegen.defaultCallGenerator,
        delegatingConstructor.getValueParameters(),
  ...
  private ArgumentGenerator generateSuperCallImplicitArguments(
    ...
    // Here we match all the super constructor parameters except those with kind VALUE to the
    // derived constructor parameters, push
    // them all onto the stack and update "offset" variable so that in the end it points to the slot
    // of the first VALUE argument
    for (JvmMethodParameterSignature parameter : parameters) {
      ...
      if (superKind == JvmMethodParameterKind.VALUE
          && kind == JvmMethodParameterKind.SUPER_CALL_PARAM) {
        // Stop when we reach the actual value parameters present in the code; they will be
        // generated via ResolvedCall below
        break;
      }

      if (superKind == JvmMethodParameterKind.OUTER) {
        assert kind == JvmMethodParameterKind.OUTER
                || kind == JvmMethodParameterKind.SUPER_CALL_PARAM
            : String.format(
                "Non-outer parameter incorrectly mapped to outer for %s: %s vs %s",
                constructorDescriptor, parameters, superParameters);
        // Super constructor requires OUTER parameter, but our OUTER instance may be different from
        // what is expected by the super
        // constructor. We need to traverse our outer classes from the bottom up, to find the needed
        // class. See innerExtendsOuter.kt
        ClassDescriptor outerForSuper =
            (ClassDescriptor)
                superConstructor.getContainingDeclaration().getContainingDeclaration();
        codegen.generateThisOrOuter(outerForSuper, true, true).put(codegen.v);
        superIndex++;
      } else if (kind == JvmMethodParameterKind.SUPER_CALL_PARAM
    ...
    if (isAnonymousObject(descriptor)) {
      List<JvmMethodParameterSignature> superValues =
          superParameters.subList(superIndex, superParameters.size());
  ...
  private static void markLineNumberForConstructor(
    ...
    if (constructor == null) {
      markLineNumberForDescriptor(descriptor.getContainingDeclaration(), codegen.v);
        ...
        && !(constructor instanceof KtSecondaryConstructor
            && !((KtSecondaryConstructor) constructor).hasImplicitDelegationCall())) {
      KtBlockExpression bodyExpression = constructor.getBodyExpression();
      List<KtExpression> statements =
          bodyExpression != null ? bodyExpression.getStatements() : Collections.emptyList();
