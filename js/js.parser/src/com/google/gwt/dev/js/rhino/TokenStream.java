...
public class TokenStream {
  ...
  static final int TSF_NEWLINES = 1 << 0, // tokenize newlines
  ...
  public static final int
      ...
      /**
       * Added by Mike - these are JSOPs in the jsref, but I don't have them yet in the java
       * implementation... so they go here. Also whatever I needed.
       *
       * <p>Most of these go in the 'op' field when returning more general token types, eg. 'DIV' as
       * the op of 'ASSIGN'.
       */
      NOP = 128, // NOP
      NOT = 129, // etc.
      ...
      /** For JSOPs associated with keywords... eg. op = THIS; token = PRIMARY */
      VOID = 132,
  ...
  public void ungetToken(int tt) {
    if (this.pushbackToken != EOF && tt != ERROR) {
      String message =
          Context.getMessage2(
              "msg.token.replaces.pushback", tokenToString(tt), tokenToString(this.pushbackToken));
  ...
  public int peekTokenSameLine() throws IOException {
    ...
    if (this.pushbackToken == EOL) this.pushbackToken = EOF;
    return result;
  }
  ...
  /**
   * This method is called to match the fully-qualified type name that should appear after the '@'
   * in a JSNI reference.
   *
   * @param sepChar the character that will separate the Java idents (either a '.' or '/')
  ...
  /**
   * Positions hold offset of an corresponding token's end. So lastPosition holds an offset of char
   * that is next to last token.
   *
   * <p>Use secondToLastPosition for error reporting outside of TokenStream, because usually we want
   * to report beginning of erroneous token, which is end of second to last read token.
   */
  ...
  /**
   * Token errors are reported before tokes is read, so use lastPosition for reporting.
   *
