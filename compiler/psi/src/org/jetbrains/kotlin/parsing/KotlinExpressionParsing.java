...
public class KotlinExpressionParsing extends AbstractKotlinParsing {
  ...
  private static final TokenSet TOKEN_SET_TO_FOLLOW_AFTER_DESTRUCTURING_DECLARATION_IN_LAMBDA =
      TokenSet.create(ARROW, COMMA, COLON);
  ...
  private static final TokenSet IN_KEYWORD_R_PAR_COLON_SET =
      TokenSet.create(IN_KEYWORD, RPAR, COLON);
  ...
  /*package*/ static final TokenSet EXPRESSION_FOLLOW =
      TokenSet.create(EOL_OR_SEMICOLON, ARROW, COMMA, RBRACE, RPAR, RBRACKET);
  ...
  public enum Precedence {
    ...
    AS(AS_KEYWORD, AS_SAFE) {
      @Override
      public IElementType parseRightHandSide(
          IElementType operation, KotlinExpressionParsing parser) {
  ...
  public KotlinExpressionParsing(
      SemanticWhitespaceAwarePsiBuilder builder, KotlinParsing kotlinParsing) {
    this(builder, kotlinParsing, true);
  }

  public KotlinExpressionParsing(
      SemanticWhitespaceAwarePsiBuilder builder, KotlinParsing kotlinParsing, boolean isLazy) {
  ...
  private boolean parseDoubleColonSuffix(@NotNull PsiBuilder.Marker expression) {
    ...
    if (at(LPAR) && !myBuilder.newlineBeforeCurrentToken()) {
      ...
      parseCallSuffix();
      lpar.error(
          "This syntax is reserved for future use; to call a reference, enclose it in parentheses:"
              + " (foo::bar)(args)");
    }
  ...
  private void parsePostfixExpression() {
    ...
    boolean firstExpressionParsed =
        at(COLONCOLON) ? parseDoubleColonSuffix(mark()) : parseAtomicExpression();
  ...
  private static void doneOrDrop(
      @NotNull PsiBuilder.Marker marker, @NotNull IElementType type, boolean condition) {
    if (condition) {
      marker.done(type);
    } else {
      marker.drop();
  ...
  private boolean parseAtomicExpression() {
    ...
    if (!ok) {
      ...
      errorWithRecovery(
          ...
          TokenSet.orSet(EXPRESSION_FOLLOW, TokenSet.create(LONG_TEMPLATE_ENTRY_END)));
    }
    ...
    if (at(LPAR)) {
      ...
      if (at(VAL_KEYWORD) || at(VAR_KEYWORD)) {
        IElementType declType =
            myKotlinParsing.parseProperty(KotlinParsing.DeclarationParsingMode.LOCAL);

        atWhenStart.done(declType);
        atWhenStart.setCustomEdgeTokenBinders(
            PrecedingDocCommentsBinder.INSTANCE, TrailingCommentsBinder.INSTANCE);
      } else {
        atWhenStart.drop();
        parseExpression();
      }
  ...
  private void parseAsCollectionLiteralExpression(
      IElementType nodeType, boolean canBeEmpty, String missingElementErrorMessage) {
    assert _at(LBRACKET);
  ...
  private boolean parseLocalDeclaration(
      boolean rollbackIfDefinitelyNotExpression, boolean isScriptTopLevel) {
    ...
    IElementType declType =
        parseLocalDeclarationRest(detector, rollbackIfDefinitelyNotExpression, isScriptTopLevel);
   ...
   *
   * <p>Please update {@link org.jetbrains.kotlin.BlockExpressionElementType#isParsable(ASTNode,
   * CharSequence, Language, Project)} if any changes occurs!
   */
  ...
  private void parseStatement(boolean isScriptTopLevel) {
    if (!parseLocalDeclaration(
        /* rollbackIfDefinitelyNotExpression= */ false, /* isScriptTopLevel= */ isScriptTopLevel)) {
      if (!atSet(EXPRESSION_FIRST)) {
  ...
  private IElementType parseLocalDeclarationRest(
    ...
    return myKotlinParsing.parseCommonDeclaration(
        ...
        isScriptTopLevel
            ? KotlinParsing.DeclarationParsingMode.SCRIPT_TOPLEVEL
            : KotlinParsing.DeclarationParsingMode.LOCAL);
  }
    ...
    if (expect(LPAR, "Expecting '(' to open a loop range", EXPRESSION_FIRST)) {
      ...
      if (!at(RPAR)) {
        ...
        if (at(LPAR)) {
          PsiBuilder.Marker destructuringDeclaration = mark();
          myKotlinParsing.parseMultiDeclarationName(
              IN_KEYWORD_L_BRACE_SET, IN_KEYWORD_L_BRACE_RECOVERY_SET);
          destructuringDeclaration.done(DESTRUCTURING_DECLARATION);
        } else {
  ...
  void parseLabelDefinition() {
    assert isAtLabelDefinitionOrMissingIdentifier()
