...
public class LockBasedStorageManager implements StorageManager {
  private static final String PACKAGE_NAME =
      StringsKt.substringBeforeLast(LockBasedStorageManager.class.getCanonicalName(), ".", "");
  ...
  public static final StorageManager NO_LOCKS =
      new LockBasedStorageManager(
          "NO_LOCKS", ExceptionHandlingStrategy.THROW, EmptySimpleLock.INSTANCE) {
        @NotNull
        @Override
        protected <K, V> RecursionDetectedResult<V> recursionDetectedDefault(
            @NotNull String source, K input) {
  ...
  public static LockBasedStorageManager createWithExceptionHandling(
    ...
    return new LockBasedStorageManager(
        debugText,
  ...
  public <K, V> MemoizedFunctionToNotNull<K, V> createMemoizedFunction(
    ...
    return createMemoizedFunction(
        compute, onRecursiveCall, LockBasedStorageManager.<K>createConcurrentHashMap());
  }
  ...
  public <K, V> MemoizedFunctionToNullable<K, V> createMemoizedFunctionWithNullableValues(
    ...
    return createMemoizedFunctionWithNullableValues(
        compute, LockBasedStorageManager.<K>createConcurrentHashMap());
  }
  ...
  @NotNull
  @Override
  public <T> NullableLazyValue<T> createNullableLazyValue(
      @NotNull Function0<? extends T> computable) {
  ...
  public <T> NullableLazyValue<T> createRecursionTolerantNullableLazyValue(
      @NotNull Function0<? extends T> computable, final T onRecursiveCall) {
    return new LockBasedLazyValue<T>(this, computable) {
      @NotNull
      @Override
      protected RecursionDetectedResult<T> recursionDetected(boolean firstTime) {
  ...
  private static class LockBasedLazyValue<T> implements NullableLazyValue<T> {
    ...
    @Nullable private volatile Object value = NotValue.NOT_COMPUTED;

    public LockBasedLazyValue(
    ...
    public T invoke() {
      ...
      try {
        ...
        try {
          ...
          // Don't publish computed value till post compute is finished as it may cause a race
          // condition
  ...
  /**
   * Computed value has an early publication and accessible from the same thread while executing a
   * post-compute lambda. For other threads value will be accessible only after post-compute lambda
   * is finished (when a real lock is used).
   */
  private abstract static class LockBasedLazyValueWithPostCompute<T> extends LockBasedLazyValue<T> {
    @Nullable private volatile SingleThreadValue<T> valuePostCompute = null;
  ...
  private abstract static class LockBasedNotNullLazyValueWithPostCompute<T>
      extends LockBasedLazyValueWithPostCompute<T> implements NotNullLazyValue<T> {
  ...
  private static class LockBasedNotNullLazyValue<T> extends LockBasedLazyValue<T>
  ...
  private static class MapBasedMemoizedFunction<K, V> implements MemoizedFunctionToNullable<K, V> {
      ...
      return sanitizeStackTrace(
          new AssertionError(
              "Inconsistent key detected. "
                  ...
                  + " is expected, was: "
                  + oldValue
                  + ", most probably race condition detected on input "
                  + input
                  + " under "
                  + storageManager));
    }
    ...
    private AssertionError unableToRemoveKey(K input, Throwable throwable) {
      return sanitizeStackTrace(
          new AssertionError("Unable to remove " + input + " under " + storageManager, throwable));
    }
  ...
  private static class MapBasedMemoizedFunctionToNotNull<K, V>
      extends MapBasedMemoizedFunction<K, V> implements MemoizedFunctionToNotNull<K, V> {
  ...
  private static <T extends Throwable> T sanitizeStackTrace(@NotNull T throwable) {
    ...
    for (int i = 0; i < size; i++) {
      // Skip everything (memoized functions and lazy values) from package
    ...
    assert firstNonStorage >= 0
        : "This method should only be called on exceptions created in LockBasedStorageManager";
  ...
  private static class CacheWithNullableValuesBasedOnMemoizedFunction<K, V>
    ...
    private CacheWithNullableValuesBasedOnMemoizedFunction(
      ...
      super(
          storageManager,
          map,
          new Function1<KeyWithComputation<K, V>, V>() {
            @Override
  ...
  public <K, V> CacheWithNotNullValues<K, V> createCacheWithNotNullValues() {
    return new CacheWithNotNullValuesBasedOnMemoizedFunction<K, V>(
        this, LockBasedStorageManager.<KeyWithComputation<K, V>>createConcurrentHashMap());
  }
