...
public abstract class AbstractLoadJavaTest extends TestCaseWithTmpdir {
  // There are two modules in each test case (sources and dependencies), so we should render
  // declarations from both of them
  public static final Configuration COMPARATOR_CONFIGURATION =
      DONT_INCLUDE_METHODS_OF_OBJECT.renderDeclarationsFromOtherModules(true);
  ...
  protected void doTestCompiledJavaAndKotlin(@NotNull String expectedFileName) {
    ...
    Pair<PackageViewDescriptor, BindingContext> binaryPackageAndContext =
        compileJavaAndLoadTestPackageAndBindingContextFromBinary(
            javaSources, tmpdir, ConfigurationKind.JDK_ONLY, null);

    checkJavaPackage(
        expectedFile,
        binaryPackageAndContext.first,
        binaryPackageAndContext.second,
  ...
  private void doTestCompiledKotlinImpl(
    ...
    CompilerConfiguration configuration =
        newConfiguration(configurationKind, getJdkKind(), getClasspath(), Collections.emptyList());
    ...
    ModuleDescriptor module =
        compileKotlinToDirAndGetModule(Collections.singletonList(ktFile), tmpdir, environment);
    ...
    PackageViewDescriptor packageFromBinary =
        LoadDescriptorUtil.loadTestPackageAndBindingContextFromJavaRoot(
                tmpdir,
                getTestRootDisposable(),
                getJdkKind(),
                configurationKind,
    ...
    for (DeclarationDescriptor descriptor :
        DescriptorUtils.getAllDescriptors(packageFromBinary.getMemberScope())) {
      if (descriptor instanceof ClassDescriptor) {
        assert descriptor instanceof DeserializedClassDescriptor
            : DescriptorUtils.getFqName(descriptor) + " is loaded as " + descriptor.getClass();
      }
    }
    ...
    if (InTextDirectivesUtils.isDirectiveDefined(fileContent, "NO_CHECK_SOURCE_VS_BINARY")) {
      // If NO_CHECK_SOURCE_VS_BINARY is enabled, only check that binary descriptors correspond to
      // the .txt file content
  ...
  public static void updateConfigurationWithDirectives(
      String content, CompilerConfiguration configuration) {
    ...
    if (languageVersionSettings == null) {
      languageVersionSettings =
          CompilerTestLanguageVersionSettingsKt.defaultLanguageVersionSettings();
    }
    ...
    if (InTextDirectivesUtils.isDirectiveDefined(content, "JVM_ANNOTATIONS")) {
      JvmContentRootsKt.addJvmClasspathRoot(
          configuration, ForTestCompileRuntime.jvmAnnotationsForTests());
    }
  }
  ...
  private void doTestJavaAgainstKotlinImpl(String expectedFileName) throws Exception {
    ...
    FileUtil.copyDir(
        sourcesDir, new File(tmpdir, "test"), pathname -> pathname.getName().endsWith(".java"));

    CompilerConfiguration configuration =
        KotlinTestUtils.newConfiguration(ConfigurationKind.JDK_ONLY, getJdkKind());
    updateConfiguration(configuration);
    ContentRootsKt.addKotlinSourceRoot(configuration, sourcesDir.getAbsolutePath());
    JvmContentRootsKt.addJavaSourceRoot(
        configuration, new File("compiler/testData/loadJava/include"));
    ...
    AnalysisResult result =
        TopDownAnalyzerFacadeForJVM.analyzeFilesWithJavaIntegration(
            ...
            new NoScopeRecordCliBindingTrace(environment.getProject()),
            configuration,
            environment::createPackagePartProvider);

    PackageViewDescriptor packageView =
        result.getModuleDescriptor().getPackage(TEST_PACKAGE_FQNAME);
    checkJavaPackage(
        expectedFile, packageView, result.getBindingContext(), COMPARATOR_CONFIGURATION);
  }
  ...
  private void doTestKotlinAgainstCompiledJavaWithKotlinImpl(@NotNull String expectedFileName)
      throws Exception {
    ...
    KtFile ktFile =
        KtTestUtil.createFile(
            kotlinSrc.getPath(), FileUtil.loadFile(kotlinSrc, true), environment.getProject());

    ModuleDescriptor module =
        GenerationUtils.compileFiles(Collections.singletonList(ktFile), environment).getModule();
    PackageViewDescriptor packageView = module.getPackage(TEST_PACKAGE_FQNAME);
    assertFalse(packageView.isEmpty());

    validateAndCompareDescriptorWithFile(
        packageView,
        COMPARATOR_CONFIGURATION.withValidationStrategy(
  ...
  protected void doTestSourceJava(@NotNull String javaFileName) {
    try {
      ...
      Pair<PackageViewDescriptor, BindingContext> javaPackageAndContext =
          loadTestPackageAndBindingContextFromJavaRoot(
              tmpdir,
              getTestRootDisposable(),
              getJdkKind(),
              ConfigurationKind.JDK_ONLY,
      ...
      checkJavaPackage(
          expectedFile,
          javaPackageAndContext.first,
          javaPackageAndContext.second,
  ...
  @NotNull
  private Pair<PackageViewDescriptor, BindingContext>
      compileJavaAndLoadTestPackageAndBindingContextFromBinary(
  ...
  private static void checkJavaPackage(
    ...
    try {
      ExpectedLoadErrorsUtil.checkForLoadErrors(
          javaPackage, bindingContext, JUnit4Assertions.INSTANCE);
  ...
  private File getTxtFile(String javaFileName) {
    try {
      ...
      if (useJavacWrapper()
          && InTextDirectivesUtils.isDirectiveDefined(fileText, "// JAVAC_EXPECTED_FILE")) {
