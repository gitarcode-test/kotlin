...
public class TypeSubstitutor implements TypeSubstitutorMarker {
  ...
  public TypeSubstitutor replaceWithNonApproximatingSubstitution() {
    if (!(substitution instanceof IndexedParametersSubstitution)
        || !substitution.approximateContravariantCapturedTypes()) return this;
  ...
  public static TypeSubstitutor createChainedSubstitutor(
      @NotNull TypeSubstitution first, @NotNull TypeSubstitution second) {
    return create(DisjointKeysUnionTypeSubstitution.create(first, second));
  }

  @NotNull
  public static TypeSubstitutor create(
  ...
  public static TypeSubstitutor create(@NotNull KotlinType context) {
    return create(
        TypeConstructorSubstitution.create(context.getConstructor(), context.getArguments()));
  }
  ...
  public KotlinType substitute(@NotNull KotlinType type, @NotNull Variance howThisTypeIsUsed) {
    TypeProjection projection =
        substitute(
            new TypeProjectionImpl(
                howThisTypeIsUsed, getSubstitution().prepareTopLevelType(type, howThisTypeIsUsed)));
  ...
  private TypeProjection unsafeSubstitute(
    ...
    if (type instanceof TypeWithEnhancement) {
      ...
      KotlinType substitutedEnhancement =
          substitute(enhancement, originalProjection.getProjectionKind());
      KotlinType resultingType =
          TypeWithEnhancementKt.wrapEnhancement(
              substitution.getType().unwrap(), substitutedEnhancement);
    ...
    TypeProjection replacement =
        substituted != null
            ? projectedTypeForConflictedTypeWithUnsafeVariance(
                type, substituted, typeParameter, originalProjection)
            : null;

    Variance originalProjectionKind = originalProjection.getProjectionKind();
    if (replacement == null
        && FlexibleTypesKt.isFlexible(type)
        && !TypeCapabilitiesKt.isCustomTypeParameter(type)) {
      ...
      if (substitutedLower.getType() == flexibleType.getLowerBound()
          && substitutedUpper.getType() == flexibleType.getUpperBound()) return originalProjection;

      KotlinType substitutedFlexibleType =
          KotlinTypeFactory.flexibleType(
              TypeSubstitutionKt.asSimpleType(substitutedLower.getType()),
              TypeSubstitutionKt.asSimpleType(substitutedUpper.getType()));
    ...
    if (replacement != null) {
      ...
      if (!type.getAnnotations().isEmpty()) {
        Annotations typeAnnotations =
            filterOutUnsafeVariance(substitution.filterAnnotations(type.getAnnotations()));
        substitutedType =
  ...
  private static TypeProjection projectedTypeForConflictedTypeWithUnsafeVariance(
    ...
    VarianceConflictType conflictWithTopLevelType =
        conflictType(originalProjection.getProjectionKind(), varianceOfCapturedType);
    ...
    VarianceConflictType conflictTypeWithTypeParameter =
        conflictType(typeParameter.getVariance(), varianceOfCapturedType);
  ...
  private static Annotations filterOutUnsafeVariance(@NotNull Annotations annotations) {
    ...
    return new FilteredAnnotations(
        annotations,
  ...
  private TypeProjection substituteCompoundType(
    ...
    if (abbreviation != null) {
      // We shouldn't approximate abbreviation at the top-level as they can't be projected: below we
      // substitute this always as invariant
    ...
    KotlinType substitutedType =
        TypeSubstitutionKt.replace(
            type, substitutedArguments, substitution.filterAnnotations(type.getAnnotations()));
    if (substitutedType instanceof SimpleType && substitutedAbbreviation instanceof SimpleType) {
      substitutedType =
          SpecialTypesKt.withAbbreviation(
              (SimpleType) substitutedType, (SimpleType) substitutedAbbreviation);
    }
  ...
  private List<TypeProjection> substituteTypeArguments(
    ...
    for (int i = 0; i < typeParameters.size(); i++) {
      ...
      TypeProjection substitutedTypeArgument =
          unsafeSubstitute(typeArgument, typeParameter, recursionDepth + 1);
  ...
  @NotNull
  public static Variance combine(
      @NotNull Variance typeParameterVariance, @NotNull TypeProjection typeProjection) {
  ...
  @NotNull
  public static Variance combine(
      @NotNull Variance typeParameterVariance, @NotNull Variance projectionKind) {
  ...
  private static void assertRecursionDepth(
      int recursionDepth, TypeProjection projection, TypeSubstitution substitution) {
