...
public final class QualifiedExpressionTranslator {
  ...
  public static AccessTranslator getAccessTranslator(
    ...
    if (selector instanceof KtSimpleNameExpression) {
      return VariableAccessTranslator.newInstance(
          context, (KtSimpleNameExpression) selector, receiver);
    }
    if (selector instanceof KtCallExpression) {
      return new QualifiedExpressionWithCallSelectorAccessTranslator(
          (KtCallExpression) selector, receiver, context);
    }
  ...
  public static JsNode translateQualifiedExpression(
      @NotNull KtQualifiedExpression expression, @NotNull TranslationContext context) {
  ...
  private static JsNode dispatchToCorrectTranslator(
    ...
    if (ReferenceTranslator.canBePropertyAccess(selector, context)) {
      assert selector instanceof KtSimpleNameExpression
          : "Selectors for properties must be simple names.";
      return VariableAccessTranslator.newInstance(
              context, (KtSimpleNameExpression) selector, receiver)
          .translateAsGet();
    }
  ...
  @Nullable
  private static JsExpression translateReceiver(
      @NotNull KtQualifiedExpression expression, @NotNull TranslationContext context) {
  ...
  private static boolean isFullQualifierForExpression(
      @Nullable KtExpression receiverExpression, @NotNull TranslationContext context) {
