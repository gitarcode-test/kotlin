...
public class KtPsiUtil {
  ...
  public static KtExpression safeDeparenthesize(
      @NotNull KtExpression expression, boolean keepAnnotations) {
  ...
  @Nullable
  public static KtExpression deparenthesize(
  ...
  public static boolean isSelectorInQualified(@NotNull KtSimpleNameExpression nameExpression) {
    ...
    return qualifiedElement instanceof KtQualifiedExpression
        || ((qualifiedElement instanceof KtUserType)
            && ((KtUserType) qualifiedElement).getQualifier() != null);
  }
  ...
  public static PsiElement getTopmostParentOfTypes(
      @Nullable PsiElement element, @NotNull Class<? extends PsiElement>... parentTypes) {
  ...
  @Nullable
  public static KtExpression getExpressionOrLastStatementInBlock(
  ...
  private static IElementType getOperation(@NotNull KtExpression expression) {
    if (expression instanceof KtQualifiedExpression) {
      ...
      return ((KtOperationExpression) expression)
  ...
  private static int getPriority(@NotNull KtExpression expression) {
    ...
    for (KotlinExpressionParsing.Precedence precedence :
        KotlinExpressionParsing.Precedence.values()) {
    ...
    if (parentElement instanceof KtCallExpression
      ...
      if (targetInnerExpression instanceof KtDotQualifiedExpression) {
        KtExpression selector =
            ((KtDotQualifiedExpression) targetInnerExpression).getSelectorExpression();
        if (selector != null) {
      ...
      if (targetInnerExpression instanceof KtCallExpression
    ...
    if (innerExpression instanceof KtBinaryExpression) {
      ...
      if (innerOperation != KtTokens.ELVIS
          ...
          && !(parentElement instanceof KtContainerNode
              &&
              // for `if` branch, `else` branch, loops body and `when` entry parentheses are
              // required
  ...
  private static boolean isKeepBinaryExpressionParenthesized(KtBinaryExpression expression) {
    ...
    return (expression.getRight() instanceof KtBinaryExpression
            && isKeepBinaryExpressionParenthesized((KtBinaryExpression) expression.getRight()))
        || (expression.getLeft() instanceof KtBinaryExpression
            && isKeepBinaryExpressionParenthesized((KtBinaryExpression) expression.getLeft()));
  }
  ...
  public static boolean checkVariableDeclarationInBlock(
      @NotNull KtBlockExpression block, @NotNull String varName) {
  ...
  public static PsiElement getPreviousWord(@NotNull PsiElement element, @NotNull String word) {
    ...
    if (prev != null
        && prev.getNode().getElementType() == KtTokens.IDENTIFIER
        && word.equals(prev.getText())) {
      return prev;
    }
   ...
   *
   * <p>Also, we can not add KDocTokens to COMMENTS TokenSet, because it is used in
   * KotlinParserDefinition.getCommentTokens(), and therefor all COMMENTS tokens will be ignored by
   * PsiBuilder.
   *
   * @param element
   * @return
  ...
  public static KtElement getOutermostDescendantElement(
      @Nullable PsiElement root, boolean first, @NotNull Predicate<KtElement> predicate) {
  ...
  @Nullable
  public static PsiElement findChildByType(
      @NotNull PsiElement element, @NotNull IElementType type) {
    ASTNode node = element.getNode().findChildByType(type);
    return node == null ? null : node.getPsi();
  }

  @Nullable
  public static PsiElement skipSiblingsBackwardByPredicate(
  ...
  private static boolean isMemberOfObjectExpression(
      @NotNull KtCallableDeclaration propertyOrFunction) {
    ...
    while (current != null) {
      ...
      if (current instanceof KtDelegatedSuperTypeEntry || current instanceof KtSuperTypeCallEntry) {
        ...
        if (grandParent instanceof KtClassOrObject
            && !(grandParent.getParent() instanceof KtObjectLiteralExpression)) {
  ...
  public static KtToken getOperationToken(@NotNull KtOperationExpression expression) {
    ...
    assert elementType == null || elementType instanceof KtToken
        : "KtOperationExpression should have operation token of type KtToken: " + expression;
  ...
  public static KtExpression getLastElementDeparenthesized(
      @Nullable KtExpression expression, @NotNull StatementFilter statementFilter) {
    ...
    if (deparenthesizedExpression instanceof KtBlockExpression) {
      ...
      // The right way to implement this logic is to interpret 'if' branches as function literals
      // with explicitly-typed signatures
      // (no arguments and no receiver) and therefore analyze them straight away (not in the
      // 'complete' phase).
      KtExpression lastStatementInABlock =
          StatementFilterKt.getLastStatementInABlock(statementFilter, blockExpression);
