...
public abstract class AbstractWriteFlagsTest extends CodegenTestCase {
    ...
    for (TestedObject testedObject : testedObjects) {
      ...
      for (OutputFile outputFile : classFileFactory.asList()) {
        ...
        if (testedObject.isFullContainingClassName
            ...
            || !testedObject.isFullContainingClassName
    ...
    for (String testData : testObjectData) {
      ...
      if (testedObjects.size() == 1) {
        ...
        throw new IllegalArgumentException(
            "TESTED_OBJECTS instruction must contain one (for class), two or three (for function"
                + " and property) values");
      }
  ...
  private static TestClassVisitor getClassVisitor(
      @NotNull TestedObject object, boolean allowSynthetic) {
    switch (object.kind) {
      case "class":
      ...
      default:
        throw new IllegalArgumentException(
            "Value of TESTED_OBJECT_KIND is incorrect: " + object.kind);
    }
  }

  protected abstract static class TestClassVisitor extends ClassVisitor {
  ...
  private static class FunctionFlagsVisitor extends TestClassVisitor {
    ...
    public FunctionFlagsVisitor(
        @NotNull String name, @Nullable String signature, boolean allowSynthetic) {
      this.funName = name;
  ...
  private static class PropertyFlagsVisitor extends TestClassVisitor {
    ...
    public FieldVisitor visitField(
      ...
      if (name.equals(propertyName)
          && (propertySignature == null || propertySignature.equals(desc))) {
        this.access = access;
        isExists = true;
      }
      return null;
  ...
  private static class InnerClassFlagsVisitor extends TestClassVisitor {
    ...
    public void visitInnerClass(
        @NotNull String innerClassInternalName,
