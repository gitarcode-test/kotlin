...
public class DFS {
  ...
  public static <N> Boolean ifAny(
    ...
    return dfs(
        nodes,
        neighbors,
        new AbstractNodeHandler<N, Boolean>() {
          @Override
  ...
  public static <N> void dfsFromNode(
      @NotNull N node, @NotNull Neighbors<N> neighbors, @NotNull Visited<N> visited) {
    dfsFromNode(
        node,
        neighbors,
        visited,
        new AbstractNodeHandler<N, Void>() {
          @Override
          public Void result() {
            return null;
          }
  ...
  public static <N> List<N> topologicalOrder(
      @NotNull Iterable<N> nodes, @NotNull Neighbors<N> neighbors) {
    return topologicalOrder(nodes, neighbors, new VisitedWithSet<N>());
  }

  public static <N> void doDfs(
      @NotNull N current,
      @NotNull Neighbors<N> neighbors,
      @NotNull Visited<N> visited,
  ...
  public abstract static class AbstractNodeHandler<N, R> implements NodeHandler<N, R> {
    @Override
    public boolean beforeChildren(N current) {
      return true;
    }
  ...
  public abstract static class CollectingNodeHandler<N, R, C extends Iterable<R>>
      extends AbstractNodeHandler<N, C> {
    @NotNull protected final C result;
  ...
  public abstract static class NodeHandlerWithListResult<N, R>
